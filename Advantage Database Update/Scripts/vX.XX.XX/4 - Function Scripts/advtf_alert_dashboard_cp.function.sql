IF EXISTS ( SELECT * FROM sysobjects WHERE id = OBJECT_ID( N'[dbo].[advtf_alert_dashboard_cp]' ) AND xtype IN ( N'FN', N'IF', N'TF' ))
BEGIN
	DROP FUNCTION [dbo].[advtf_alert_dashboard_cp]
END
GO
CREATE FUNCTION [dbo].[advtf_alert_dashboard_cp] (
...
BEGIN	
	...
	BEGIN
		DECLARE @CC TABLE(CDP_CONTACT_ID INT);

		INSERT INTO @CC SELECT LTRIM(RTRIM(items)) as CDP_CONTACT_ID FROM udf_split_list(@ClientContactID, ',');	
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				SELECT 
					...
					AR.READ_ALERT, 
					1, 
					NULL,--AR.CARD_SEQ_NBR,
					A.JOB_NUMBER,
					A.JOB_COMPONENT_NBR,
					A.TASK_SEQ_NBR					
				FROM 
					ALERT A WITH(NOLOCK)
					INNER JOIN CP_ALERT_RCPT AR WITH(NOLOCK) ON A.ALERT_ID = AR.ALERT_ID 
				WHERE 
					AR.CDP_CONTACT_ID IN (select CDP_CONTACT_ID FROM @CC)
		...
		BEGIN
			...
			FROM 
				ALERT A WITH(NOLOCK)
				INNER JOIN CP_ALERT_RCPT AR WITH(NOLOCK) ON A.ALERT_ID = AR.ALERT_ID --AND A.ALRT_NOTIFY_HDR_ID = AR.ALRT_NOTIFY_HDR_ID AND A.ALERT_STATE_ID = AR.ALERT_STATE_ID
				LEFT OUTER JOIN JOB_COMPONENT JC WITH(NOLOCK) ON A.JOB_NUMBER = JC.JOB_NUMBER AND A.JOB_COMPONENT_NBR = JC.JOB_COMPONENT_NBR
			WHERE 
				AR.CDP_CONTACT_ID IN (select CDP_CONTACT_ID FROM @CC)
				AND AR.CURRENT_NOTIFY = 1
		...
		BEGIN
			...
			WHERE 
				JTDE.CDP_CONTACT_ID IN (select CDP_CONTACT_ID FROM @CC)
		...
		BEGIN
            ...
            BEGIN
			    ...
			    FROM 
				    ALERT A WITH(NOLOCK)
				    INNER JOIN CP_ALERT_RCPT AR WITH(NOLOCK) ON A.ALERT_ID = AR.ALERT_ID 
				    INNER JOIN @CC E ON AR.CDP_CONTACT_ID = E.CDP_CONTACT_ID
			    WHERE 
           ...
           --	Proofing
			BEGIN
				...
				FROM
				(
					SELECT DISTINCT -- OPEN ROUTED
						A.ALERT_ID, 
						A.[START_DATE], 
						A.DUE_DATE, 
						AR.READ_ALERT, 
						0 AS IsAlertCC, 
						NULL AS CARD_SEQ_NBR,--AR.CARD_SEQ_NBR,
						A.JOB_NUMBER,
						A.JOB_COMPONENT_NBR,
						A.TASK_SEQ_NBR,
						1 AS IsProof
					...
					FROM 
						ALERT A WITH(NOLOCK)
						INNER JOIN CP_ALERT_RCPT_DISMISSED AR WITH(NOLOCK) ON A.ALERT_ID = AR.ALERT_ID --AND A.ALRT_NOTIFY_HDR_ID = AR.ALRT_NOTIFY_HDR_ID AND A.ALERT_STATE_ID = AR.ALERT_STATE_ID
						INNER JOIN @CC E ON AR.CDP_CONTACT_ID = E.CDP_CONTACT_ID
					WHERE 
						--AR.CURRENT_NOTIFY = 1
						--AND 
	...
	BEGIN -- DISMISSED!
		...
		BEGIN
			...
			BEGIN
				...
				FROM 
					ALERT A WITH(NOLOCK)
					INNER JOIN CP_ALERT_RCPT_DISMISSED AR WITH(NOLOCK) ON A.ALERT_ID = AR.ALERT_ID 
				WHERE 
					AR.CDP_CONTACT_ID IN (select CDP_CONTACT_ID FROM @CC)
					AND COALESCE(AR.CURRENT_RCPT, 0) = 0
					AND COALESCE(AR.CURRENT_NOTIFY, 0) = 0
