IF EXISTS ( SELECT * FROM sysobjects WHERE id = OBJECT_ID( N'[dbo].[advtf_alert_dashboard_jobjacket]' ) AND xtype IN ( N'FN', N'IF', N'TF' ))
BEGIN
	DROP FUNCTION [dbo].[advtf_alert_dashboard_jobjacket]
END
GO
CREATE FUNCTION [dbo].[advtf_alert_dashboard_jobjacket] (
	...
	BEGIN -- DISMISSED!
		...
		BEGIN
			...
			WHERE 
				...
				AND A.ALERT_CAT_ID <> 79
				AND COALESCE(A.IS_DRAFT, 0) = 0
				AND 1 = CASE WHEN @IncludeCompleted = 0 AND COALESCE(A.ASSIGN_COMPLETED, 0) = 1 AND JTD.JOB_COMPLETED_DATE IS NULL THEN 0
				             WHEN @IncludeCompleted = 1 AND COALESCE(A.ASSIGN_COMPLETED, 0) = 1 AND JTD.JOB_COMPLETED_DATE IS NOT NULL THEN 1
							 ELSE 1 END;
		END
		--  Reviews
		BEGIN
			...
			BEGIN
				...
				FROM 
					...
					INNER JOIN @EMPLOYEES E ON AR.EMP_CODE = E.EMP_CODE
				WHERE 
					(COALESCE(A.IS_CS_REVIEW, 0) = 1 OR A.ALERT_TYPE_ID = 15 OR A.ALERT_CAT_ID = 79)
					AND COALESCE(A.IS_DRAFT, 0) = 0
				;
			END
			--	Proofing
