			...
			BEGIN
				...
				BEGIN
					...
					WHERE
						...
						AND
						1 = CASE
								WHEN @ShowAssignmentType = 'myalerts' AND (X.IsAlertCC = 1 or X.IsProof = 1) THEN 1
	...
	BEGIN
        ...
        BEGIN
            DELETE AA 
				...
				WHERE IS_CC = 1 OR IS_STANDARD_ALERT = 1 AND AA.ALERT_CAT_ID = 79
        END

        IF @ShowAssignmentType IN ('myalertsandassignments', 'myalerts') AND @InboxType <> 'dismissed'
        BEGIN
            DELETE AA 
				FROM #ALERT AA
				INNER JOIN ALERT A WITH(NOLOCK) 
					ON A.ALERT_ID = AA.ALERT_ID
				WHERE IS_CC = 1 AND AA.ALERT_CAT_ID = 79 AND IS_MY_ALERT_COMPLETED = 1
        END
