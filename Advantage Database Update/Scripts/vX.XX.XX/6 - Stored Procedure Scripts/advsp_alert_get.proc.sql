IF EXISTS (SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[advsp_alert_get]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
...
BEGIN
	...
	SELECT
		...
		@IS_MY_ASSIGNMENT = 0,
		@IS_MY_ALERT_OPEN = 0,
		@IS_MY_ALERT_DISMISSED = 0,
		@IS_MY_ASSIGNMENT_OPEN = 0,
		@IS_MY_ASSIGNMENT_COMPLETED = 0,
		@IS_TASK = 0,
		@IS_MY_TASK = 0,
		@IS_MY_TASK_TEMP_COMPLETE = 0,
		@WAS_MARKED_READ = 0,
		@ALERT_CAT_ID = 0,
	...
	SELECT @SELECTED_DOCUMENT_ID = MAX(DOCUMENT_ID) FROM ALERT_ATTACHMENT AA WITH(NOLOCK) WHERE ALERT_ID = 1251;
...
FROM
	(SELECT TOP (1) 
		...
		[TaskStatusDescription] =	CASE
										...
										ELSE 'Projected'
									END,
		[SelectedDocumentID] = ISNULL(@SELECTED_DOCUMENT_ID, 0)
	FROM         
		CLIENT WITH (NOLOCK) 
		RIGHT OUTER JOIN
		...
		RIGHT OUTER JOIN
			SOFTWARE_VERSION WITH (NOLOCK) 
		RIGHT OUTER JOIN
			SOFTWARE_BUILD WITH (NOLOCK) 
		RIGHT OUTER JOIN
			ALERT WITH (NOLOCK) 
		LEFT OUTER JOIN
		...
		LEFT OUTER JOIN
			TRAFFIC_FNC WITH(NOLOCK) ON JOB_TRAFFIC_DET.FNC_CODE = TRAFFIC_FNC.TRF_CODE
		INNER JOIN
			ALERT_TYPE WITH (NOLOCK) ON ALERT.ALERT_TYPE_ID = ALERT_TYPE.ALERT_TYPE_ID 
		INNER JOIN
			ALERT_CATEGORY WITH (NOLOCK) ON ALERT.ALERT_CAT_ID = ALERT_CATEGORY.ALERT_CAT_ID ON SOFTWARE_BUILD.VERSION_ID = ALERT.VER AND 
												SOFTWARE_BUILD.BUILD_ID = ALERT.BUILD ON SOFTWARE_VERSION.VERSION_ID = ALERT.VER ON 
												SEC_USER.EMP_CODE = ALERT.EMP_CODE 
		LEFT OUTER JOIN
			ALERT_STATES WITH (NOLOCK) ON ALERT.ALERT_STATE_ID = ALERT_STATES.ALERT_STATE_ID ON 
		...
		LEFT OUTER JOIN
			PRODUCT WITH (NOLOCK) ON ALERT.CL_CODE = PRODUCT.CL_CODE AND ALERT.DIV_CODE = PRODUCT.DIV_CODE AND 
										 ALERT.PRD_CODE = PRODUCT.PRD_CODE 
		LEFT OUTER JOIN
			JOB_LOG WITH (NOLOCK) ON ALERT.JOB_NUMBER = JOB_LOG.JOB_NUMBER 
		LEFT OUTER JOIN
			CAMPAIGN WITH (NOLOCK) ON ALERT.CMP_IDENTIFIER = CAMPAIGN.CMP_IDENTIFIER 
		LEFT OUTER JOIN
			OFFICE WITH (NOLOCK) ON ALERT.OFFICE_CODE = OFFICE.OFFICE_CODE 
		LEFT OUTER JOIN
			DIVISION WITH (NOLOCK) ON ALERT.CL_CODE = DIVISION.CL_CODE AND 
										  ALERT.DIV_CODE = DIVISION.DIV_CODE ON CLIENT.CL_CODE = ALERT.CL_CODE 
		LEFT OUTER JOIN
			ATB_REV WITH(NOLOCK) ON ALERT.ATB_REV_ID = ATB_REV.ATB_REV_ID
		LEFT OUTER JOIN
		   SPRINT_DTL SD WITH (NOLOCK) ON SD.ALERT_ID = ALERT.ALERT_ID
		LEFT OUTER JOIN
			SPRINT_HDR WITH (NOLOCK) ON SD.SPRINT_HDR_ID = SPRINT_HDR.ID OR SPRINT_HDR.ID = @SPRINT_ID
		LEFT OUTER JOIN
			BOARD WITH (NOLOCK) ON SPRINT_HDR.BOARD_ID = BOARD.ID
		LEFT OUTER JOIN
			BOARD_HDR WITH (NOLOCK) ON BOARD.BOARD_HDR_ID = BOARD_HDR.ID
		LEFT OUTER JOIN
			ALERT_STATES BOARD_STATE WITH (NOLOCK) ON ALERT.BOARD_STATE_ID = BOARD_STATE.ALERT_STATE_ID
		LEFT OUTER JOIN
			SEC_USER SU2 WITH (NOLOCK) ON UPPER(ALERT.ALERT_USER) = UPPER(SU2.USER_CODE)
		LEFT OUTER JOIN
			EMPLOYEE WITH (NOLOCK) ON SU2.EMP_CODE = EMPLOYEE.EMP_CODE
		LEFT OUTER JOIN
			EMP_NON_TASKS WITH (NOLOCK) ON ALERT.NON_TASK_ID = EMP_NON_TASKS.NON_TASK_ID
	WHERE  
		(ALERT.ALERT_ID = @ALERT_ID) 
		AND
		1 = CASE 
				WHEN ISNULL(@SPRINT_ID, 0) = 0 THEN 1 
				WHEN SPRINT_HDR.ID = @SPRINT_ID THEN 1 ELSE 0 
			END) AS A
