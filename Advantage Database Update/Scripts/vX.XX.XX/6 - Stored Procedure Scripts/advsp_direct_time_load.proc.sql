IF EXISTS ( SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[advsp_direct_time_load]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1 )
	DROP PROCEDURE [dbo].[advsp_direct_time_load]
GO

CREATE PROCEDURE [dbo].[advsp_direct_time_load] 
	@DATE_TYPE AS int,
...
BEGIN
	...
	CREATE TABLE #EMP_WORK_DAYS(EMP_CODE varchar(6) COLLATE SQL_Latin1_General_CP1_CS_AS, 
	...
	( 
		SELECT @START_DATE 
		UNION ALL
		SELECT DateAdd(day,1,T.[DATE]) FROM T WHERE T.[DATE] < @END_DATE
	)
	...
	WHERE 
		NOT EXISTS(SELECT 
						* 
					FROM 
						dbo.EMP_NON_TASKS AS ENT 
					WHERE 
						ENT.[TYPE] = 'H' AND 
						ENT.ALL_DAY = 1 AND 
						AD.[DATE] >= ENT.[START_DATE] AND 
						AD.[DATE] <= CONVERT(DATETIME, ENT.END_DATE +' 23:59:00', 101)) AND
		DATEPART(dw, AD.[DATE]) NOT IN (1, 7)
	...
	SELECT 
		E.EMP_CODE, 
		D.DAYDATE,
		COALESCE(CASE DATEPART(dw, D.DAYDATE) WHEN 1 THEN E.SUN_HRS
											  WHEN 2 THEN E.MON_HRS
											  WHEN 3 THEN E.TUE_HRS
											  WHEN 4 THEN E.WED_HRS
											  WHEN 5 THEN E.THU_HRS
											  WHEN 6 THEN E.FRI_HRS
											  WHEN 7 THEN E.SAT_HRS END, 0)
	FROM 
		#DAYS D CROSS JOIN 
		dbo.EMPLOYEE E 
	WHERE 
		(E.EMP_TERM_DATE >= D.DAYDATE OR E.EMP_TERM_DATE IS NULL) AND 
		(E.EMP_DATE <= D.DAYDATE OR E.EMP_DATE IS NULL) 
    ...
    SELECT  
		...
		[UserID] = ET.[USER_ID],
		[AssignmentType] = ET.ALERT_DESC,
		[TaskCode] = CASE WHEN ET.TRF_CODE IS NOT NULL THEN ET.TRF_CODE ELSE CASE WHEN ALERT_LEVEL = 'BRD' THEN ET.JTD_FNC_CODE ELSE NULL END END,
		[Assignment] = CASE WHEN ET.TRF_DESC IS NOT NULL THEN ET.TRF_DESC ELSE ET.[SUBJECT] END,
	...
	FROM 
		...
		FROM 
			...
			[dbo].[ALERT] AS A ON A.ALERT_ID = ETD.ALERT_ID LEFT OUTER JOIN
			[dbo].[ALERT_CATEGORY] AS AC ON AC.ALERT_CAT_ID = A.ALERT_CAT_ID LEFT OUTER JOIN
			[dbo].[JOB_TRAFFIC_DET] AS JTD ON JTD.JOB_NUMBER = A.JOB_NUMBER AND JTD.JOB_COMPONENT_NBR = A.JOB_COMPONENT_NBR AND JTD.SEQ_NBR = A.TASK_SEQ_NBR LEFT OUTER JOIN
			(SELECT 
				ET_ID, 
		...
		WHERE 
			...
			1 = CASE WHEN @DATE_TYPE = 0 THEN CASE WHEN ET.EMP_DATE >= @START_DATE AND ET.EMP_DATE <= CONVERT(DATETIME, @END_DATE +' 23:59:00', 101) THEN 1 ELSE 0 END
					 WHEN @DATE_TYPE = 1 THEN CASE WHEN ETD.DATE_ENTERED >= @START_DATE AND ETD.DATE_ENTERED <= CONVERT(DATETIME, @END_DATE +' 23:59:00', 101) THEN 1 ELSE 0 END
					 ...
					 WHEN @DATE_TYPE = 3 THEN 1 END AND (ETD.EDIT_FLAG IS NULL OR ETD.EDIT_FLAG = 0)) AS ET LEFT OUTER JOIN
		(SELECT 
			DISTINCT 
			AR.AR_POST_PERIOD,
			AR.AR_INV_NBR, 
	...
	WHERE 
		...
		1 = CASE WHEN @PRODUCT_LIST IS NULL THEN 1 WHEN (J.CL_CODE + '|' + J.DIV_CODE + '|' + J.PRD_CODE) IN (SELECT * FROM dbo.udf_split_list(@PRODUCT_LIST, ',')) THEN 1 ELSE 0 END 
		AND	(@EmployeesList IS NULL OR (@EmployeesList IS NOT NULL AND ET.EMP_CODE IN (SELECT * from dbo.udf_split_list(@EmployeesList, ','))))
		...
		AND (J.JOB_NUMBER IN (SELECT * FROM dbo.udf_split_list(@JOB_LIST, ',')) OR @JOB_LIST IS NULL)
		AND	(@DepartmentsList IS NULL OR (@DepartmentsList IS NOT NULL AND ET.DP_TM_CODE IN (SELECT * from dbo.udf_split_list(@DepartmentsList, ','))))
		AND	(@FunctionsList IS NULL OR (@FunctionsList IS NOT NULL AND ET.FNC_CODE IN (SELECT * from dbo.udf_split_list(@FunctionsList, ','))))
        AND 1 = CASE WHEN @OnlyActiveEmployees = 1 AND (EMP.EMP_TERM_DATE IS NOT NULL ) THEN 0 ELSE 1 END
	GROUP BY 
		ET.EMP_CODE, 
		...
		ET.[USER_ID],
		ET.ALERT_DESC,
		CASE WHEN ET.TRF_CODE IS NOT NULL THEN ET.TRF_CODE ELSE CASE WHEN ALERT_LEVEL = 'BRD' THEN ET.JTD_FNC_CODE ELSE NULL END END,
		CASE WHEN ET.TRF_DESC IS NOT NULL THEN ET.TRF_DESC ELSE ET.[SUBJECT] END,
		EWD.STD_HOURS,
		CAMP.CMP_BEG_DATE,
		CAMP.CMP_END_DATE,
		F.FNC_CONSOLIDATION,
