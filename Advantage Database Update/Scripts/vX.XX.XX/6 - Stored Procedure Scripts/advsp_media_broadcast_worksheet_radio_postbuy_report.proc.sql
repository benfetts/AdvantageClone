CREATE PROCEDURE [dbo].[advsp_media_broadcast_worksheet_radio_postbuy_report]
	...
	FROM (
		SELECT
			...
			ActualSpots = (SELECT COUNT(DISTINCT ATBD.DTL_ID)
							FROM dbo.AP_RADIO_BROADCAST_DTL ATBD
			...
			DetailEstimate = CASE WHEN DetailID IS NULL THEN 
									CASE WHEN @USE_IMPRESSIONS = 1 THEN CASE WHEN @USE_PRIMARY_RATING = 1 THEN CAST(CAST(MBWMD.PRIMARY_AQH AS decimal(19,1)) / 100 AS decimal(19,1)) ELSE CAST(CAST(MBWMD.SECONDARY_AQH AS decimal(19,1)) / 100 AS decimal(19,1)) END
																   ELSE CASE WHEN @USE_PRIMARY_RATING = 1 THEN MBWMD.PRIMARY_AQH_RATING ELSE MBWMD.SECONDARY_AQH_RATING END END
								ELSE CASE WHEN @USE_IMPRESSIONS = 1 THEN CASE WHEN @USE_PRIMARY_RATING = 1 THEN CAST(CAST(MBWMD.PRIMARY_AQH AS decimal(19,1)) / 100 AS decimal(19,1)) ELSE CAST(CAST(MBWMD.SECONDARY_AQH AS decimal(19,1)) / 100 AS decimal(19,1)) END
																	ELSE CASE WHEN @USE_PRIMARY_RATING = 1 THEN MBWMD.PRIMARY_AQH_RATING ELSE MBWMD.SECONDARY_AQH_RATING END END
								END,
			DetailEstimateFull = CASE WHEN DetailID IS NULL THEN 
										CASE WHEN @USE_IMPRESSIONS = 1 THEN CASE WHEN @USE_PRIMARY_RATING = 1 THEN CAST(MBWMD.PRIMARY_AQH AS decimal(19,1)) ELSE CAST(MBWMD.SECONDARY_AQH AS decimal(19,1)) END
																			   ELSE CASE WHEN @USE_PRIMARY_RATING = 1 THEN MBWMD.PRIMARY_AQH_RATING ELSE MBWMD.SECONDARY_AQH_RATING END END
								 ELSE CASE WHEN @USE_IMPRESSIONS = 1 THEN CASE WHEN @USE_PRIMARY_RATING = 1 THEN CAST(MBWMD.PRIMARY_AQH AS decimal(19,1)) ELSE CAST(MBWMD.SECONDARY_AQH AS decimal(19,1)) END
																				ELSE CASE WHEN @USE_PRIMARY_RATING = 1 THEN MBWMD.PRIMARY_AQH_RATING ELSE MBWMD.SECONDARY_AQH_RATING END END
								 END,
			DetailEstimateImpressionFull = CASE WHEN @USE_PRIMARY_RATING = 1 THEN CAST(MBWMD.PRIMARY_AQH AS decimal(19,2)) ELSE CAST(MBWMD.SECONDARY_AQH AS decimal(19,2)) END,
			DetailEstimateRatingFull = CASE WHEN @USE_PRIMARY_RATING = 1 THEN MBWMD.PRIMARY_AQH_RATING ELSE MBWMD.SECONDARY_AQH_RATING END,
			DetailEstimateGrossImpressionFull = CASE WHEN @USE_PRIMARY_RATING = 1 THEN CAST(MBWMD.PRIMARY_AQH AS decimal(19,2)) ELSE CAST(MBWMD.SECONDARY_AQH AS decimal(19,2)) END,
			OverridePost = MBWMD.OVERRIDE_POST, --ratings
			OverridePostAQH = MBWMD.OVERRIDE_POST_AQH, --impressions
			DetailActual = CASE WHEN @USE_IMPRESSIONS = 1 THEN 
									CASE WHEN MBWMD.OVERRIDE_POST_AQH = 1 THEN CASE WHEN @USE_PRIMARY_RATING = 1 THEN CAST(CAST(MBWMD.PRIMARY_AQH AS decimal(19,2)) / 100 AS decimal(19,2)) ELSE CAST(CAST(MBWMD.SECONDARY_AQH AS decimal(19,2)) / 100 AS decimal(19,2)) END
										 ELSE NULL END
								ELSE 
									CASE WHEN MBWMD.OVERRIDE_POST = 1 THEN CASE WHEN @USE_PRIMARY_RATING = 1 THEN MBWMD.PRIMARY_AQH_RATING ELSE MBWMD.SECONDARY_AQH_RATING END 
										 ELSE NULL END
								END,
			DetailActualImpression = CASE WHEN MBWMD.OVERRIDE_POST_AQH = 1 THEN CASE WHEN @USE_PRIMARY_RATING = 1 THEN CAST(CAST(MBWMD.PRIMARY_AQH AS decimal(19,2)) / 100 AS decimal(19,2)) ELSE CAST(CAST(MBWMD.SECONDARY_AQH AS decimal(19,2)) / 100 AS decimal(19,2)) END
										 ELSE NULL END,
			DetailActualRating = CASE WHEN MBWMD.OVERRIDE_POST = 1 THEN CASE WHEN @USE_PRIMARY_RATING = 1 THEN MBWMD.PRIMARY_AQH_RATING ELSE MBWMD.SECONDARY_AQH_RATING END 
										 ELSE NULL END,
			DetailActualGrossImpression = CASE WHEN MBWMD.OVERRIDE_POST_AQH = 1 THEN CASE WHEN @USE_PRIMARY_RATING = 1 THEN CAST(CAST(MBWMD.PRIMARY_AQH AS decimal(19,2)) / 1000 AS decimal(19,2)) ELSE CAST(CAST(MBWMD.SECONDARY_AQH AS decimal(19,2)) / 1000 AS decimal(19,2)) END
										  ELSE NULL END,
			...
			ReportedDateEnd =	CASE WHEN @DATE_START IS NOT NULL AND @DATE_END IS NOT NULL THEN 
								...
								ELSE MBW.[END_DATE] 
								END,
			ExternalRadioSource = MBWM.EXTERNAL_RADIO_SOURCE,
			Bookend = MBWMD.BOOKEND,
			AddedValue = MBWMD.VALUE_ADDED,
			DetailEstimateRating = CASE WHEN DetailID IS NULL THEN 
										CASE WHEN @USE_PRIMARY_RATING = 1 THEN MBWMD.PRIMARY_AQH_RATING ELSE MBWMD.SECONDARY_AQH_RATING END * 
									...
									ELSE CASE WHEN @USE_PRIMARY_RATING = 1 THEN MBWMD.PRIMARY_AQH_RATING ELSE MBWMD.SECONDARY_AQH_RATING END
									END,
			DetailEstimateImpression = CASE WHEN DetailID IS NULL THEN 
											CASE WHEN @USE_PRIMARY_RATING = 1 THEN CAST(CAST(MBWMD.PRIMARY_AQH AS decimal(19,1)) / 100 AS decimal(19,1)) ELSE CAST(CAST(MBWMD.SECONDARY_AQH AS decimal(19,1)) / 100 AS decimal(19,1)) END * 
											(SELECT SUM(SPOTS) as SPOTS
											 FROM dbo.MEDIA_BROADCAST_WORKSHEET_MARKET_DETAIL_DATE MBWMDD
											 WHERE MEDIA_BROADCAST_WORKSHEET_MARKET_DETAIL_ID = MBWMD.MEDIA_BROADCAST_WORKSHEET_MARKET_DETAIL_ID
												   AND ((@HAS_DATES = 0)
												   OR(@HAS_DATES = 1 AND MBWMDD.[DATE] between @DATE_START AND @DATE_END)))
										ELSE CASE WHEN @USE_PRIMARY_RATING = 1 THEN CAST(CAST(MBWMD.PRIMARY_AQH AS decimal(19,1)) / 100 AS decimal(19,1)) ELSE CAST(CAST(MBWMD.SECONDARY_AQH AS decimal(19,1)) / 100 AS decimal(19,1)) END
										END,
