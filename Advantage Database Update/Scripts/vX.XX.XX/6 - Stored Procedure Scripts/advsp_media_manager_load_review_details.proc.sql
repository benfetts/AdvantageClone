...
DECLARE @media_summary TABLE (
    ...
    APStatus                varchar(50),
	ImportedFrom			varchar(2)
)

DECLARE @AP_ORDER_LINES TABLE (
	AP_ID int NOT NULL,
...
BEGIN
	INSERT INTO @media_summary ( CL_CODE, DIV_CODE, PRD_CODE, ORDER_NBR, LINE_NBR,
		...
		JobOrMediaDateToBill, BuyerEmployeeCode, BillCoopCode, OrderAccepted, RevisedBy, WorksheetLineNumber, MatCompDate, ImportedFrom )
	SELECT h.CL_CODE, h.DIV_CODE, h.PRD_CODE, d.ORDER_NBR, d.LINE_NBR, 
		p.OFFICE_CODE, h.CMP_IDENTIFIER, c.CMP_NAME, h.MARKET_CODE, h.VN_CODE, v.VN_NAME,
		h.LINK_ID, d.LINK_DETID, h.CLIENT_PO, h.BUYER, d.REV_NBR, h.ORDER_DESC, h.MEDIA_TYPE, sc.SC_DESCRIPTION,
	...
	FROM dbo.RADIO_HDR h
		...
		LEFT OUTER JOIN dbo.BRD_IMPORT_XREF x ON h.ORDER_NBR = x.ORDER_NBR
	WHERE
		(
			COALESCE(h.[STATUS], 0) = 0 AND @IncludeOrder = 1 
		OR
...
BEGIN
	...
	SELECT h.CL_CODE, h.DIV_CODE, h.PRD_CODE, d.ORDER_NBR, d.LINE_NBR, 
		...
		EC.EMP_FNAME + ' ' + LTRIM(' ' + COALESCE(EC.EMP_MI, '') + ' ') + EC.EMP_LNAME, COALESCE(h.[STATUS], 0), h.NET_GROSS, jc.MEDIA_BILL_DATE,
		JL.JOB_DESC, jc.JOB_COMP_DESC, d.EXT_CLOSE_DATE, d.EXT_MATL_DATE,
		d.START_TIME, d.END_TIME, d.PROGRAMMING, COALESCE(NULLIF(d.NETWORK_ID,''), d.CABLE_NETWORK_STATION_CODE), d.[LENGTH], d.REMARKS,
		BroadcastDatesbyWeek = (CASE WHEN d.MONDAY = 1 THEN 'M ' ELSE '' END +
								 CASE WHEN d.TUESDAY = 1 THEN 'T ' ELSE '' END +
								 CASE WHEN d.WEDNESDAY = 1 THEN 'W ' ELSE '' END +
								 CASE WHEN d.THURSDAY = 1 THEN 'TH ' ELSE '' END +
...
DELETE @AP_ORDER_LINES

INSERT INTO @AP_ORDER_LINES (AP_ID, ORDER_NBR, ORDER_LINE_NBR)
SELECT DISTINCT AP_ID, d.ORDER_NBR, d.ORDER_LINE_NBR
FROM dbo.AP_MAGAZINE d
	INNER JOIN @media_summary ms ON d.ORDER_NBR = ms.ORDER_NBR AND d.ORDER_LINE_NBR = ms.LINE_NBR 
WHERE 
	(d.DELETE_FLAG IS NULL OR d.DELETE_FLAG = 0)
AND (d.MODIFY_DELETE IS NULL OR d.MODIFY_DELETE = 0)

UPDATE aol SET APPROVED = 1
FROM @AP_ORDER_LINES aol
	INNER JOIN dbo.AP_MEDIA_APPROVAL ama ON aol.AP_ID = ama.AP_ID AND aol.ORDER_NBR = ama.ORDER_NBR AND aol.ORDER_LINE_NBR = ama.LINE_NBR AND ama.ACTIVE_REV = 1 AND [STATUS] IN (2,3,0)

UPDATE @media_summary SET 
	AP_EXISTS = CASE WHEN EXISTS(SELECT 1 FROM @AP_ORDER_LINES WHERE ORDER_NBR = ms.ORDER_NBR AND ORDER_LINE_NBR = ms.LINE_NBR) THEN 1 ELSE 0 END,
	APStatus = CASE 
				WHEN NOT EXISTS(SELECT 1 FROM dbo.AP_MEDIA_APPROVAL WHERE ORDER_NBR = ms.ORDER_NBR AND LINE_NBR = ms.LINE_NBR AND ACTIVE_REV = 1) THEN 'Needs Review'
				WHEN EXISTS(SELECT 1 FROM @AP_ORDER_LINES WHERE ORDER_NBR = ms.ORDER_NBR AND LINE_NBR = ms.LINE_NBR AND APPROVED = 0) THEN 'Needs Review'
				ELSE 'Approved'
				END
FROM @media_summary ms
WHERE ms.MEDIA_FROM = 'Magazine'
...
if @debug = 1 
	select getdate() AS 'after last MS update'
	 ...
	 SELECT DISTINCT
        ...
        APStatus = CASE WHEN COALESCE(ms.AP_EXISTS, 0) = 1 THEN ms.APStatus ELSE '' END,
		ms.ImportedFrom 
	FROM @media_summary ms
