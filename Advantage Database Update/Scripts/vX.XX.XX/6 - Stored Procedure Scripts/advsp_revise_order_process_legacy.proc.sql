IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[advsp_revise_order_process_legacy]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[advsp_revise_order_process_legacy]
GO

CREATE PROCEDURE [dbo].[advsp_revise_order_process_legacy] 
...
SELECT @user_id, @action, @order_type, @batch_id 
...
PJH 08/02/19 - No split processing
PJH 05/12/20 - Added 'CT' as @imported_from / @media_interface
PJH 10/15/20 - Allow vedor codes with leading zeros per EC
PJH 01/26/21 - Expanded Rate and Pct precision
PJH 11/18/21 - Delete rows from BRDCAST_IMPORT with DELETE_FLAG = 99 and there is no matching Advantage order/line
************************************************************************************************************/
...
DECLARE @sql varchar(500), @param_def varchar(500), @error_msg_w varchar(MAX), @error_msg_u varchar(200)
...
IF @DEBUG = 1 BEGIN
	...
	WHERE (MEDIA_INTERFACE IN ('AB','AM','AP','ST','OT','CT','SN') AND MEDIA_TYPE = @media_type_in AND (POST_FLAG = 1 OR DELETE_ORDER = 1))

	SELECT 'BRDCAST_ORDER (Posted)' 'DESC', MEDIA_TYPE, MEDIA_INTERFACE, POST_FLAG, * FROM BRDCAST_IMPORT
	WHERE (MEDIA_INTERFACE IN ('AB','AM','AP','AW','ST','OT','CT','SN') AND MEDIA_TYPE = @media_type_in AND (POST_FLAG = 1 OR DELETE_ORDER = 1))
END
ELSE BEGIN
	...
	INSERT INTO @orders
	SELECT A.LINK_ID, CAST(A.LINE_NBR AS INT)  FROM BRDCAST_IMPORT A
	LEFT JOIN BRD_IMPORT_XREF B ON A.LINK_ID = B.IMPORT_ORDER_NBR AND A.LINE_NBR = CAST(B.IMPORT_LINE_NBR AS INT) 
	WHERE B.IMPORT_ORDER_NBR IS NULL AND A.DELETE_FLAG = 99 AND A.POST_FLAG IN (0,2)
	ORDER BY LINK_ID, CAST(A.LINE_NBR AS INT)

	DELETE A FROM BRDCAST_IMPORT A JOIN @orders B ON A.LINK_ID = B.IMPORT_ORDER_NBR AND CAST(A.LINE_NBR AS INT) = B.IMPORT_LINE_NBR
	WHERE (A.MEDIA_INTERFACE IN ('AB','AM','AP','AW','ST','OT','CT','SN') AND A.MEDIA_TYPE = @media_type_in)
END
...
IF @order_type = 'TV' OR @order_type = 'RA' BEGIN
	...
	(MEDIA_INTERFACE IN ('OT','ST','SN','CT') AND MEDIA_TYPE = @media_type_in AND POST_FLAG IN (0) AND USER_ID = @user_id) AND (CAL_TYPE = 'BM' OR (COALESCE(CAL_TYPE, '') = '' AND FLIGHT_TYPE = 'L'))
...
IF @order_type IN ('NP','MA','IN','OD') BEGIN
	...
	WHERE (MEDIA_INTERFACE IN ('AB','AM','AP','AW') AND MEDIA_TYPE = @media_type_in AND POST_FLAG = 0 AND USER_ID = @user_id) OR
			(MEDIA_INTERFACE IN ('OT','ST','SN','CT') AND MEDIA_TYPE = @media_type_in AND POST_FLAG = 0 AND USER_ID = @user_id)  --@@@@@@@@@@
...
IF @order_type IN ('NP','MA','IN','OD') BEGIN
	SET @media_interface_search = 'AB,AM,AP,OT,ST,SN,CT'
END ELSE BEGIN
	SET @media_interface_search = 'AB,AM,AP,AW,OT,ST,SN,CT'
END
...
WHILE @ROW_ID <= @ROW_COUNT BEGIN
	...
	IF @order_type IN ('NP','MA','IN','OD') BEGIN
		IF @media_interface_in = 'AM' OR @media_interface_in = 'AB' OR @media_interface_in IN ('OT', 'CT') OR @media_interface_in = 'AP' OR @media_interface_in = 'ST' OR @media_interface_in = 'SN' BEGIN    --@@@@@@@@@@
	...
	END	
	ELSE BEGIN
		IF @media_interface_in = 'ST' OR @media_interface_in IN ('OT', 'CT') OR @media_interface_in = 'SN'  BEGIN --@@@@@@@@@@
				/* PJH 08/13/18 - fixed for radio */
				IF LEFT(@media_type,1) = 'T' BEGIN
					SELECT  @order_nbr = COALESCE(A.ORDER_NBR, 0), @line_nbr = COALESCE(A.LINE_NBR, 0)
					FROM   BRD_IMPORT_XREF A 
						JOIN TV_DETAIL B 	ON A.ORDER_NBR = B.ORDER_NBR AND A.LINE_NBR = B.LINE_NBR 
					WHERE  IMPORT_ORDER_NBR = @link_id AND CAST(IMPORT_LINE_NBR AS INT) = @link_detid AND (IMPORTED_FROM = @media_interface_in) AND (LEFT(MEDIA_TYPE,1) = LEFT(@media_type,1))
					--AND B.START_DATE = @start_date /* PJH 11/15/18 */ /* PJH 08/02/19 -  No splits */
				END
				ELSE
				IF LEFT(@media_type,1) = 'R' BEGIN
					SELECT  @order_nbr = COALESCE(A.ORDER_NBR, 0), @line_nbr = COALESCE(A.LINE_NBR, 0)
					FROM   BRD_IMPORT_XREF A 
						JOIN RADIO_DETAIL B 	ON A.ORDER_NBR = B.ORDER_NBR AND A.LINE_NBR = B.LINE_NBR 
					WHERE  IMPORT_ORDER_NBR = @link_id AND CAST(IMPORT_LINE_NBR AS INT) = @link_detid AND (IMPORTED_FROM = @media_interface_in) AND (LEFT(MEDIA_TYPE,1) = LEFT(@media_type,1))
					--AND B.START_DATE = @start_date  /* PJH 11/15/18 */ /* PJH 08/02/19 -  No splits */
				END

				--SELECT @link_id '@link_id', @link_detid '@link_detid', @order_nbr '@order_nbr', @line_nbr '@line_nbr'

		END
		ELSE	IF @media_interface_in = 'AM' OR @media_interface_in = 'AB' /*OR @media_interface_in = 'OT'*/ OR @media_interface_in = 'AP' OR @media_interface_in = 'AW' BEGIN  --@@@@@@@@@@
				SELECT @order_nbr = COALESCE(ORDER_NBR, 0), @line_nbr = COALESCE(LINE_NBR, 0)
	...
	IF @dtl_exists = 0 BEGIN
		...
		ELSE BEGIN
			IF @media_interface_in = 'ST' OR @media_interface_in IN ('OT', 'CT') OR @media_interface_in = 'SN'  BEGIN--@@@@@@@@@@
					SELECT @order_nbr = COALESCE(ORDER_NBR, 0), @media_type = COALESCE(MEDIA_TYPE, @media_type) --@@@@@@@@@@ /* Keep the MEDIA_TYPE from existing rows */
					FROM   BRD_IMPORT_XREF
					WHERE  IMPORT_ORDER_NBR = @link_id AND (IMPORTED_FROM = @media_interface_in) AND (LEFT(MEDIA_TYPE,1) = LEFT(@media_type,1));
			...
			END
			ELSE IF @media_interface_in = 'AM' OR @media_interface_in = 'AB' OR @media_interface_in = 'AP' OR @media_interface_in = 'AW'  /*OR @media_interface_in = 'OT' OR @media_interface_in = 'ST'*/ BEGIN  --@@@@@@@@@@
				SELECT @order_nbr = COALESCE(ORDER_NBR, 0)
				FROM   BRD_IMPORT_XREF
				WHERE  IMPORT_ORDER_NBR = @link_id AND (IMPORTED_FROM = @media_interface_in) AND (MEDIA_TYPE = @media_type); 
			END	
	...
	IF @media_type_in = 'N' BEGIN
		...
		SELECT
			...
			,@net_gross  = CASE @hdr_exists 
									WHEN 1 THEN @net_gross
									ELSE			
										CASE WHEN [NET_GROSS] IS NULL THEN /* [NET_GROSS] */ /* PJH 11/27/18 - Use new NET_GROSS flag */
											CASE [AGENCY_NET_RATE] 
											WHEN 1 THEN 0 
											ELSE 1 
											END 
									ELSE
			...
			,@comm_pct_or = CASE WHEN [COMM_PCT] IS NULL THEN 
											CASE WHEN (1 - COALESCE(AGENCY_NET_RATE,0)) BETWEEN .01 AND .99 THEN
												((1 - COALESCE(AGENCY_NET_RATE,0)) * 100)
											ELSE 
												0
											END
										ELSE
	...
	IF @media_type_in = 'T' BEGIN
		...
		--IF @media_interface_in = 'ST' OR @media_interface_in IN ('OT', 'CT') OR @media_interface_in = 'SN' --@@@@@@@@@@
			--WHILE LEFT(@vn_code, 1) = '0' BEGIN
			--	SET @vn_code = RIGHT(@vn_code, LEN(@vn_code) - 1)		
			--END
	...
	*/
	...
	IF @media_interface_in = 'ST' OR @media_interface_in = 'SN' BEGIN   --@@@@@@@@@@@
		SET @modified_comments = 'Revised by Strata SP'
...
UPDATE A SET A.POST_FLAG = B.POST_FLAG, A.BATCH_ID = @batch_id  --@@@@@@@@@@
...
UPDATE A SET A.POST_FLAG = B.POST_FLAG, A.BATCH_ID = @batch_id, A.DELETE_FLAG = B.DELETE_FLAG, A.TOTAL_SPOTS = B.TOTAL_SPOTS  --@@@@@@@@@@
...
SELECT 
	...
	,NULL --[ADV_LINE_NBR]
FROM BRDCAST_IMPORT
WHERE (MEDIA_INTERFACE IN ('OT','ST','SN','CT') AND MEDIA_TYPE = @media_type_in AND POST_FLAG IN (0,88) AND USER_ID = @user_id) AND (CAL_TYPE = 'BM' OR (COALESCE(CAL_TYPE, '') = '' AND FLIGHT_TYPE = 'L')) 
...
GRANT EXECUTE ON [advsp_revise_order_process_legacy] TO PUBLIC AS dbo
GO