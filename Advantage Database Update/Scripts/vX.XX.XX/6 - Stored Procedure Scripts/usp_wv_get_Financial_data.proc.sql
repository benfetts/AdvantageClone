...
BEGIN   
    ...
    UPDATE #FIN
    SET MONTH_B = ISNULL(((SELECT SUM(ISNULL(AMT,0)) * -1 FROM #TMP WHERE DESCR ='S' AND PP_PRIOR = POST_PERIOD AND PRIOR_MONTH = 1 GROUP BY POST_PERIOD) 
                    - ISNULL((SELECT SUM(ISNULL(AMT,0)) FROM #TMP WHERE DESCR = 'COS' AND PP_PRIOR = POST_PERIOD AND PRIOR_MONTH = 1 GROUP BY POST_PERIOD),0)) - ISNULL((SELECT SUM(ISNULL(AMT,0)) FROM #TMP WHERE DESCR ='E' AND PP_PRIOR = POST_PERIOD AND PRIOR_MONTH = 1 GROUP BY POST_PERIOD),0),0) 
    WHERE DESCR = 'Net Income'
