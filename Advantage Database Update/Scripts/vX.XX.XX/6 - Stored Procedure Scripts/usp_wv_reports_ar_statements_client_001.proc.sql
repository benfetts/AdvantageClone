IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[usp_wv_reports_ar_statements_client_001]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[usp_wv_reports_ar_statements_client_001]
GO
...
CREATE PROCEDURE [dbo].[usp_wv_reports_ar_statements_client_001] 
@ID VarChar(4000),
...
Declare @CR_InvoiceSeq varchar(3) 
...
Select   CLIENT_AR_STMT.CL_CODE as ClientCode, 
	...
	AR.AR_INV_NBR as InvoiceNumber, 
...
from CLIENT_AR_STMT	
    ...
    --LEFT OUTER JOIN
				--(SELECT BC.BILL_COMMENT FROM [dbo].[BILL_COMMENT] AS BC WHERE BC.BC_ID = (SELECT MAX(BCM.BC_ID) FROM [dbo].[BILL_COMMENT] AS BCM WHERE BC.AR_INV_NBR = BCM.AR_INV_NBR AND BC.AR_INV_SEQ = BCM.AR_INV_SEQ)) AS BC ON BC.AR_INV_NBR = AR.AR_INV_NBR	
...
AR.REC_TYPE,
AR.MANUAL_INV,
ARINV_MEDIA.ORDER_TYPE, C.CL_P_PAYDAYS, C.CL_M_PAYDAYS	
...
@CR_CCity, @CR_CState, @CR_CZip, @CR_InvoiceDate, @CR_InvoiceNumber, @CR_InvoiceSeq,
...
Select
ClientCode, (SELECT PRD_CODE FROM ACCT_REC WHERE AR_INV_NBR = InvoiceNumber AND AR_INV_SEQ = InvoiceSeq AND AR_TYPE = 'IN') AS ProductCode, CDP_CONTACT_ID, ContactCode, UseAddress, IncludeOnAccount,
ClientName, FirstName, LastName, SAddress1, isnull(SAddress2, '@@@') as SAddress2, 
SCity, SState, SZip, CAddress1, isnull(CAddress2, '@@@') as CAddress2, CCity, CState, CZip, InvoiceDate, CASE WHEN LEN(InvoiceNumber) > 6 OR InvoiceNumber = '' THEN InvoiceNumber ELSE dbo.fn_LZ(6,InvoiceNumber) END as InvoiceNumber, InvoiceSeq, Reference, 
Description, InvoiceAmount, OriginalAmount, PostingPeriod, DaysAged, Comments, OnAccountAmount, OnAccount, 
