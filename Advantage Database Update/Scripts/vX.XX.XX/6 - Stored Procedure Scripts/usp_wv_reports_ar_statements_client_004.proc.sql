IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[usp_wv_reports_ar_statements_client_004]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[usp_wv_reports_ar_statements_client_004]
GO
...
CREATE PROCEDURE [dbo].[usp_wv_reports_ar_statements_client_004] 
@ID VarChar(4000),
...
Declare @CR_OriginalAmount as Money
...
Declare @CR_InvoiceComment varchar(8000)
...
@AgencyPhone = isnull(PHONE, ''), @AgencyFax = isnull(FAX, ''), @AgencyEmail = isnull(EMAIL, ''), @PrtHeader = isnull(PRT_HEADER, 0), @PrtAddr1 = isnull(PRT_ADDR1, 0), @PrtAddr2 = isnull(PRT_ADDR2, 0),
@PrtCity = isnull(PRT_CITY, 0), @PrtState = isnull(PRT_STATE, 0), @PrtZip = isnull(PRT_ZIP, 0), @PrtPhone = isnull(PRT_PHONE, 0), @PrtFax = isnull(PRT_FAX, 0), @PrtEmail = isnull(PRT_EMAIL, 0),
@PrtFooter = isnull(PRT_FOOTER, 0), @PrtFtrAddr1 = isnull(PRT_ADDR1_FTR, 0), @PrtFtrAddr2 = isnull(PRT_ADDR2_FTR, 0), @PrtFtrCity = isnull(PRT_CITY_FTR, 0), @PrtFtrState = isnull(PRT_STATE_FTR, 0),
@PrtFtrZip = isnull(PRT_ZIP_FTR, 0), @PrtFtrPhone = isnull(PRT_PHONE_FTR, 0), @PrtFtrFax = isnull(PRT_FAX_FTR, 0), @PrtFtrEmail = isnull(PRT_EMAIL_FTR, 0), @PrtName = isnull(PRT_NAME, 0),
@PrtFtrName = isnull(PRT_NAME_FTR, 0), @LogoPath = isnull(LOGO_PATH, '') From LOCATIONS where ID = @Location and LOGO_LOCATION <> 'N'
...
Description varchar(200), InvoiceAmount Money, OriginalAmount Money, PostingPeriod varchar(6), DaysAged int, Comments varchar(2000), OnAccountAmount Money, OnAccount varchar(1), 
...
CurrentAR Money, OverThirtyAR Money, OverSixtyAR Money, TotalAR Money, CDP_CONTACT_ID INT, InvoiceComment varchar(8000))
...
declare @T_AR306090PAY table (tr_CL_CODE varchar(6), tr_INV_NBR int, tr_INV_SEQ smallint, CR_AMT Money)
...
END
CREATE TABLE #T_AR306090AMT (t_CL_CODE varchar(6) COLLATE SQL_Latin1_General_Cp1_CI_AS, t_INV_NBR int, t_INV_SEQ smallint, t_PostPeriod varchar(6) COLLATE SQL_Latin1_General_Cp1_CI_AS , t_Bucket varchar(4) COLLATE SQL_Latin1_General_Cp1_CI_AS , AR_AMOUNT Money)
declare @T_AR306090 table (t_CL_CODE varchar(6), t_INV_NBR int, t_INV_SEQ smallint, t_PostPeriod varchar(6), t_Bucket varchar(4), AR_AMOUNT Money, CR_AMT Money)
...
Select   CLIENT_AR_STMT.CL_CODE as ClientCode, 
    ...
    (SELECT BC.BILL_COMMENT FROM [dbo].[BILL_COMMENT] AS BC WHERE BC.BC_ID = (SELECT MAX(BCM.BC_ID) FROM [dbo].[BILL_COMMENT] AS BCM WHERE BC.AR_INV_NBR = BCM.AR_INV_NBR AND BC.AR_INV_SEQ = BCM.AR_INV_SEQ) AND BC.AR_INV_NBR = AR.AR_INV_NBR) as InvoiceComment,
...
from CLIENT_AR_STMT	
	...
	Inner Join CDP_CONTACT_HDR CON
		...
		and CON.CDP_CONTACT_ID = CLIENT_AR_STMT.CDP_CONTACT_ID	
...
Begin
	...
	Select TOP 1 CLIENT_AR_STMT.CL_CODE as ClientCode, 
	CLIENT_AR_STMT.CONT_CODE as ContactCode,
	CLIENT_AR_STMT.USE_ADDRESS as UseAddress,
	CLIENT_AR_STMT.INCL_ON_ACCT as IncludeOnAccount,
	C.CL_NAME AS ClientName, 
...
from CLIENT_AR_STMT
	...
	Inner Join CDP_CONTACT_HDR CON
		ON CON.CL_CODE = CLIENT_AR_STMT.CL_CODE
		and CON.CDP_CONTACT_ID = CLIENT_AR_STMT.CDP_CONTACT_ID	
Where CLIENT_AR_STMT.CL_CODE = @pair_value1 and 
	CLIENT_AR_STMT.CONT_CODE = @pair_value2  
Group By CLIENT_AR_STMT.CL_CODE,
	CLIENT_AR_STMT.CDP_CONTACT_ID,
	CLIENT_AR_STMT.CONT_CODE,
	CLIENT_AR_STMT.USE_ADDRESS,
...
SELECT @rowCount = COUNT(*) FROM ARINV_MEDIA INNER JOIN ACCT_REC ON ACCT_REC.AR_INV_NBR = ARINV_MEDIA.AR_INV_NBR and ACCT_REC.AR_INV_SEQ = ARINV_MEDIA.AR_INV_SEQ
...
Group by ARINV_MEDIA.ORDER_TYPE, ACCT_REC.AR_DESCRIPTION, ARINV_MEDIA.AR_INV_NBR, ARINV_MEDIA.AR_INV_SEQ

Declare INTERNET_LINE CURSOR FOR
SELECT case When @rowCount = 1 Then (convert(varchar(6),dbo.fn_LZ(6,max(ARINV_MEDIA.ORDER_NBR)))) else ' ' end as Reference, case When @rowCount > 1 Then 'Various Internet Orders' else INTERNET_HEADER.ORDER_DESC + ' ' + max(VENDOR.VN_NAME) end as DESCRIPTION
...
SELECT case When @rowCount = 1 Then (convert(varchar(6),dbo.fn_LZ(6,min(ARINV_MEDIA.ORDER_NBR)))) else ' ' end as Reference, case When @rowCount > 1 Then 'Various Magazine Orders' else MAG_HEADER.ORDER_DESC + ' ' + max(VENDOR.VN_NAME) end as DESCRIPTION
...
SELECT case When @rowCount = 1 Then (convert(varchar(6),dbo.fn_LZ(6,min(ARINV_MEDIA.ORDER_NBR)))) else ' ' end as Reference, case When @rowCount > 1 Then 'Various Magazine Orders' else MAGAZINE_HEADER.ORDER_DESC + ' ' + max(VENDOR.VN_NAME) end as DESCRIPTION
...
SELECT case When @rowCount = 1 Then (convert(varchar(6),dbo.fn_LZ(6,min(ARINV_MEDIA.ORDER_NBR)))) else ' ' end as Reference, case When @rowCount > 1 Then 'Various Outdoor Orders' else OUTDOOR_HEADER.ORDER_DESC + ' ' + max(VENDOR.VN_NAME) end as DESCRIPTION
...
SELECT case When @rowCount = 1 Then (convert(varchar(6),dbo.fn_LZ(6,min(ARINV_MEDIA.ORDER_NBR)))) else ' ' end as Reference, case When @rowCount > 1 Then 'Various News Orders' else NEWS_HEADER.ORDER_DESC + ' ' + max(VENDOR.VN_NAME) end as DESCRIPTION
...
SELECT case When @rowCount = 1 Then (convert(varchar(6),dbo.fn_LZ(6,min(ARINV_MEDIA.ORDER_NBR)))) else ' ' end as Reference, case When @rowCount > 1 Then 'Various News Orders' else NEWSPAPER_HEADER.ORDER_DESC + ' ' + max(VENDOR.VN_NAME) end as DESCRIPTION
...
SELECT @rowCount = COUNT(A.ORDERS) FROM (
SELECT COUNT(*) AS ORDERS FROM ARINV_MEDIA INNER JOIN ACCT_REC ON ACCT_REC.AR_INV_NBR = ARINV_MEDIA.AR_INV_NBR and ACCT_REC.AR_INV_SEQ = ARINV_MEDIA.AR_INV_SEQ
...
AND (ACCT_REC.OFFICE_CODE COLLATE SQL_Latin1_General_Cp1_CI_AS IN (SELECT OFFICE_CODE COLLATE SQL_Latin1_General_Cp1_CI_AS FROM #TEMP_AR_STATEMENT_LINE_OFFICES))
Group by ARINV_MEDIA.ORDER_NBR, ARINV_MEDIA.ORDER_TYPE, ACCT_REC.AR_DESCRIPTION, ARINV_MEDIA.AR_INV_NBR, ARINV_MEDIA.AR_INV_SEQ) A

Declare RADIO_LINE CURSOR FOR
SELECT case When @rowCount = 1 Then (convert(varchar(6),dbo.fn_LZ(6,min(ARINV_MEDIA.ORDER_NBR)))) else ' ' end as Reference, case When @rowCount > 1 Then 'Various Radio Orders' else RH.ORDER_DESC end as DESCRIPTION
...
if @CR_JobDescription IS NULL
	Begin
		...
		Declare RADIO_LINE2 CURSOR FOR
		SELECT case When @rowCount = 1 Then (convert(varchar(6),dbo.fn_LZ(6,min(ARINV_MEDIA.ORDER_NBR)))) else ' ' end as Reference, case When @rowCount > 1 Then 'Various Radio Orders' else RH.ORDER_DESC end as DESCRIPTION
		FROM ARINV_MEDIA INNER JOIN ACCT_REC ON ACCT_REC.AR_INV_NBR = ARINV_MEDIA.AR_INV_NBR and ACCT_REC.AR_INV_SEQ = ARINV_MEDIA.AR_INV_SEQ  Left Outer JOIN (SELECT RADIO_HDR.ORDER_DESC + ' ' + isnull(VENDOR.VN_NAME, '') AS ORDER_DESC, RADIO_HDR.ORDER_NBR FROM RADIO_HDR INNER JOIN VENDOR ON RADIO_HDR.VN_CODE = VENDOR.VN_CODE) RH on RH.ORDER_NBR = ARINV_MEDIA.ORDER_NBR
		where ORDER_TYPE = 'R' and ACCT_REC.AR_INV_NBR = @CR_InvoiceNumber and ARINV_MEDIA.LINE_NBR = (select max(ARINV_MEDIA.LINE_NBR) From ARINV_MEDIA where ARINV_MEDIA.AR_INV_NBR = @CR_InvoiceNumber) 
		AND (ACCT_REC.OFFICE_CODE COLLATE SQL_Latin1_General_Cp1_CI_AS IN (SELECT OFFICE_CODE COLLATE SQL_Latin1_General_Cp1_CI_AS FROM #TEMP_AR_STATEMENT_LINE_OFFICES))
		Group by ARINV_MEDIA.ORDER_TYPE, ACCT_REC.AR_DESCRIPTION, ARINV_MEDIA.AR_INV_NBR, ARINV_MEDIA.AR_INV_SEQ, RH.ORDER_DESC
		...
		OPEN RADIO_LINE2
			...
			FETCH NEXT FROM RADIO_LINE2 into @CR_JobNumber, @CR_JobDescription
			WHILE @@FETCH_STATUS=0
			Begin
...
SELECT case When @rowCount = 1 Then (convert(varchar(6),dbo.fn_LZ(6,min(ARINV_MEDIA.ORDER_NBR)))) else ' ' end as Reference, case When @rowCount > 1 Then 'Various Television Orders' else TV.ORDER_DESC end as DESCRIPTION
...
if @CR_JobDescription IS NULL
	Begin
		...
		Declare TV_LINE2 CURSOR FOR
		SELECT case When @rowCount = 1 Then (convert(varchar(6),dbo.fn_LZ(6,min(ARINV_MEDIA.ORDER_NBR)))) else ' ' end as Reference, case When @rowCount > 1 Then 'Various Television Orders' else TV.ORDER_DESC end as DESCRIPTION
		FROM ARINV_MEDIA INNER JOIN ACCT_REC ON ACCT_REC.AR_INV_NBR = ARINV_MEDIA.AR_INV_NBR and ACCT_REC.AR_INV_SEQ = ARINV_MEDIA.AR_INV_SEQ Left Outer JOIN (SELECT TV_HDR.ORDER_DESC + ' ' + isnull(VENDOR.VN_NAME, '') AS ORDER_DESC, TV_HDR.ORDER_NBR FROM TV_HDR INNER JOIN VENDOR ON TV_HDR.VN_CODE = VENDOR.VN_CODE) TV on TV.ORDER_NBR = ARINV_MEDIA.ORDER_NBR
		where ORDER_TYPE = 'T' and ACCT_REC.AR_INV_NBR = @CR_InvoiceNumber and ARINV_MEDIA.LINE_NBR = (select max(ARINV_MEDIA.LINE_NBR) From ARINV_MEDIA where ARINV_MEDIA.AR_INV_NBR = @CR_InvoiceNumber) 
		AND (ACCT_REC.OFFICE_CODE COLLATE SQL_Latin1_General_Cp1_CI_AS IN (SELECT OFFICE_CODE COLLATE SQL_Latin1_General_Cp1_CI_AS FROM #TEMP_AR_STATEMENT_LINE_OFFICES))
		Group by ARINV_MEDIA.ORDER_TYPE, ACCT_REC.AR_DESCRIPTION, ARINV_MEDIA.AR_INV_NBR, ARINV_MEDIA.AR_INV_SEQ, TV.ORDER_DESC
		...
		OPEN TV_LINE2
			...
			FETCH NEXT FROM TV_LINE2 into @CR_JobNumber, @CR_JobDescription
			WHILE @@FETCH_STATUS=0
			Begin
...
INSERT INTO @AR_LINE_ITEM (ClientCode, CDP_CONTACT_ID, ContactCode, UseAddress, IncludeOnAccount,
ClientName, FirstName, LastName, SAddress1, SAddress2, 
SCity, SState, SZip, CAddress1, CAddress2, CCity, CState, CZip, InvoiceDate, InvoiceNumber, InvoiceSeq, Reference, 
Description, InvoiceAmount, OriginalAmount, PostingPeriod, DaysAged, Comments, OnAccountAmount, OnAccount, 
...
CurrentAR, OverThirtyAR, OverSixtyAR, TotalAR, InvoiceComment)
Values (@CR_ClientCode, @CDP_CONTACT_ID, @CR_ContactCode,
@CR_UseAddress, @CR_IncludeOnAccount, @CR_ClientName, @CR_FirstName, @CR_LastName, @CR_SAddress1, 
...
@CR_JobNumber, @CR_JobDescription, @CR_InvoiceAmount, @CR_OriginalAmount, @CR_PostingPeriod, 
...
@CR_OverThirtyAR, @CR_OverSixtyAR, @CR_TotalAR, @CR_InvoiceComment)
...
Select
ClientCode, CDP_CONTACT_ID, ContactCode, UseAddress, IncludeOnAccount,
...
,ISNULL(ClientCode,'')+CAST(ISNULL(CDP_CONTACT_ID,0) AS VARCHAR)+ISNULL(ContactCode,'') AS GROUPING_KEY, InvoiceComment
from @AR_LINE_ITEM
GROUP BY 
CDP_CONTACT_ID,ContactCode, ClientCode, UseAddress, IncludeOnAccount,
ClientName, FirstName, LastName, SAddress1,SAddress2, 
SCity, SState, SZip, CAddress1, CAddress2, CCity, CState, CZip, InvoiceDate, InvoiceNumber, InvoiceSeq, Reference, 
Description, InvoiceAmount, OriginalAmount, PostingPeriod, DaysAged, Comments, OnAccountAmount, OnAccount, 
AddressUse, Template, AgencyName, AgencyAddress1,AgencyAddress2,AgencyCity,AgencyState,AgencyZip,AgencyPhone, 
AgencyFax, AgencyEmail,PrtHeader,PrtAddr1,PrtAddr2,PrtCity,PrtState,PrtZip,PrtPhone,PrtFax,
PrtEmail,PrtFooter,PrtFtrAddr1,PrtFtrAddr2,PrtFtrCity,PrtFtrState,PrtFtrZip,PrtFtrPhone,
PrtFtrFax,PrtFtrEmail,PrtName,PrtFtrName,LogoPath,CurrentAR,OverThirtyAR,OverSixtyAR,TotalAR, InvoiceComment
Order by ClientCode, ContactCode, Convert(Datetime, InvoiceDate, 101), InvoiceNumber, InvoiceSeq
