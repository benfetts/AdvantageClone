IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[usp_wv_reports_ar_statements_product_004]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[usp_wv_reports_ar_statements_product_004]
GO
...
CREATE PROCEDURE [dbo].[usp_wv_reports_ar_statements_product_004] 
@ID VarChar(4000),
...
--ST:  Added for Silk WebvPostQA#129
--IF REC_TYPE<>P  AND ORDER_TYPE = M PRINT ORDER DESCRIPTION
...
Select
	ClientCode, DivisionCode, ProductCode, CDP_CONTACT_ID, ContactCode, UseAddress, IncludeOnAccount,
	...
	,ISNULL(ClientCode,'')+ISNULL(DivisionCode,'')+ISNULL(ProductCode,'')+CAST(ISNULL(CDP_CONTACT_ID,0) AS VARCHAR)+ISNULL(ContactCode,'') AS GROUPING_KEY, InvoiceComment
	from @AR_LINE_ITEM
	GROUP BY
	...
	PrtFtrFax,PrtFtrEmail,PrtName,PrtFtrName,LogoPath,CurrentAR,OverThirtyAR,OverSixtyAR,TotalAR,
	 DivisionCode, ProductCode,DivisionName,ProductDescription, InvoiceComment
