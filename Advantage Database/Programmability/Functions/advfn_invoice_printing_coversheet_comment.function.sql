CREATE FUNCTION [dbo].[advfn_invoice_printing_coversheet_comment](
...
AS
BEGIN
	DECLARE @std_comment varchar(MAX)
				
	--Comment for OCDP (when a comment exists for the specified, office, client, division and product)
	DECLARE @comment_ocdp varchar(MAX)
	SELECT @comment_ocdp = (SELECT HTML_COMMENT FROM dbo.STD_COMMENT 
	...
	DECLARE @comment_ocdp2 varchar(MAX)
	SELECT @comment_ocdp2 = (SELECT HTML_COMMENT FROM dbo.STD_COMMENT 
		WHERE APP_CODE = @app_code
		AND COMMENT_TYPE = @comment_type
	...
	--Comment for OCD	
	DECLARE @comment_ocd varchar(MAX)
	SELECT @comment_ocd = (SELECT HTML_COMMENT FROM dbo.STD_COMMENT 
		WHERE APP_CODE = @app_code
		AND COMMENT_TYPE = @comment_type
		AND OFFICE_CODE = @office_code
		AND CLIENT_CODE = @client_code
	...
	DECLARE @comment_ocd2 varchar(MAX)
	SELECT @comment_ocd2 = (SELECT HTML_COMMENT FROM dbo.STD_COMMENT 
		WHERE APP_CODE = @app_code
		AND COMMENT_TYPE = @comment_type
		AND OFFICE_CODE IS NULL
		AND CLIENT_CODE = @client_code
	...
	--Comment for OC	
	DECLARE @comment_oc varchar(MAX)
	SELECT @comment_oc = (SELECT HTML_COMMENT FROM dbo.STD_COMMENT 
		WHERE APP_CODE = @app_code
		AND COMMENT_TYPE = @comment_type
		AND OFFICE_CODE = @office_code
		AND CLIENT_CODE = @client_code
	...
	DECLARE @comment_oc2 varchar(MAX)
	SELECT @comment_oc2 = (SELECT HTML_COMMENT FROM dbo.STD_COMMENT 
		WHERE APP_CODE = @app_code
		AND COMMENT_TYPE = @comment_type
		AND OFFICE_CODE IS NULL
		AND CLIENT_CODE = @client_code
	...
	--Comment for O	
	DECLARE @comment_o varchar(MAX)
	SELECT @comment_o = (SELECT HTML_COMMENT FROM dbo.STD_COMMENT 
		WHERE APP_CODE = @app_code
		AND COMMENT_TYPE = @comment_type
		AND OFFICE_CODE = @office_code
	...
	DECLARE @comment_o2 varchar(MAX)
	SELECT @comment_o2 = (SELECT HTML_COMMENT FROM dbo.STD_COMMENT 
		WHERE APP_CODE = @app_code
		AND COMMENT_TYPE = @comment_type
		AND OFFICE_CODE IS NULL
		AND CLIENT_CODE IS NULL
