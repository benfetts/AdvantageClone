CREATE FUNCTION [dbo].[advtf_broadcast_order_dtl_verification] (
...
SELECT DISTINCT O.BRDCAST_DTL_VERIFICATION_SETTING_ID, O.MEDIA_BROADCAST_WORKSHEET_SPOT_MATCHING_SETTING_ID, TIME_LENGTH, TIME_SEPARATION
FROM @Orders O
 					...
 					(SELECT
    					...
    					[GrossRate] = DTL.GROSS_RATE,
 						...
 						[TotalSpots] = Spots.TOTAL_SPOTS,
  						...
  						[OrderEndDate] = DATEADD(day, 6, COALESCE(DATE7, COALESCE(DATE6, COALESCE(DATE5, COALESCE(DATE4, COALESCE(DATE3, COALESCE(DATE2, DATE1))))))), --ORD.END_DATE,
   										...
   										dbo.AP_TV_BROADCAST_DTL D
   											...
   											(SELECT DISTINCT
												...
												FROM
													dbo.AP_TV AP
												WHERE
													MODIFY_DELETE IS NULL
                                                    AND AP.ORDER_NBR IN (SELECT items FROM dbo.udf_split_list(@OrderNumbers, ','))) H ON D.AP_ID = H.AP_ID AND
