...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[advtf_med_import_brdcast_weeks_csv]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].[advtf_med_import_brdcast_weeks_csv]
GO
...
CREATE FUNCTION [dbo].advtf_med_import_brdcast_weeks_csv (
...
BEGIN
	...
	*/
	...
	IF @spots > '' BEGIN
		...
		SELECT * FROM dbo.udf_split_list(@spots, ',')

		SET @bc_spot_cnt = @@ROWCOUNT
	END

	SET @bc_index = 1
	SET @bc_date = @start_date /* First Spot Date 1 */
	WHILE @bc_index <= @bc_spot_cnt BEGIN /* This is enough for a 6 week buy. Max for Strata is 13 weeks or 14 days & Generic is 52 weeks. */
		--SET @bc_date = @start_date /* First Spot Date 1 */
		SET @tmp_date = @bc_date /* Previous spot date */
		SELECT @bc_spots = [SPOTS] FROM @spots_table WHERE RECID = @bc_index
