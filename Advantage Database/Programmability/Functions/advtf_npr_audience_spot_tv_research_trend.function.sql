CREATE FUNCTION [dbo].[advtf_npr_audience_spot_tv_research_trend](
...
BEGIN
	...
	FROM (
		...
		GROUP BY aud.NPR_STATION_ID, aud.MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_DAYTIME_ID, aud.NUMBER_DAYS, AdjustedHour, MINUTEPART, [AdjustedDateTime]

		) suma
	GROUP BY [npr_station_id], [media_spot_tv_research_daytime_id], [number_days], [AdjustedDateTime]

	INSERT INTO @RETURN_TABLE
	SELECT
			u.nielsen_demo_code, u.audience, u.npr_station_id, u.media_spot_tv_research_daytime_id, u.number_days, [AdjustedDateTime]
	FROM @tt
	UNPIVOT
	(
		audience
