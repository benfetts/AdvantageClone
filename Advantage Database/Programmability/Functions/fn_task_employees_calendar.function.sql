CREATE FUNCTION [dbo].[fn_task_employees_calendar] ( @job_number integer, @job_component_nbr smallint, @seq_nbr smallint, @list_type tinyint, @depts varchar(8000), @roles varchar(8000) ) RETURNS varchar(4000)  	
AS  	
BEGIN  		
	...
	Begin
		DECLARE emp_cursor CURSOR FOR 
		 SELECT jtde.EMP_CODE, EMP_FNAME, EMP_MI, EMP_LNAME, HOURS_ALLOWED   		   
	...
	Begin
		DECLARE emp_cursor CURSOR FOR 
		 SELECT jtde.EMP_CODE, EMP_FNAME, EMP_MI, EMP_LNAME, HOURS_ALLOWED   		   
		   FROM dbo.JOB_TRAFFIC_DET_EMPS jtde LEFT OUTER JOIN dbo.EMPLOYEE e 	     
			 ON  (jtde.EMP_CODE = e.EMP_CODE) LEFT OUTER JOIN EMP_TRAFFIC_ROLE ON jtde.EMP_CODE = EMP_TRAFFIC_ROLE.EMP_CODE  
