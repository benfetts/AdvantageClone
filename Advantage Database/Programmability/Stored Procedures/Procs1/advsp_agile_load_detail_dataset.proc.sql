IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[advsp_agile_load_detail_dataset]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
    DROP PROCEDURE [dbo].[advsp_agile_load_detail_dataset]
GO

CREATE PROCEDURE [dbo].[advsp_agile_load_detail_dataset] 
...
BEGIN
    ...
    --WHERE WEEK_START BETWEEN @START_DATE AND @END_DATE --ORDER BY SE.EMP_CODE, SE.WEEK_START


	INSERT INTO #WEEKLY_HRS (SprintEmployeeID, EmployeeCode, EmployeeOfficeCode, WeekNumber, WeekStart, WeekEnd, HoursAllocated, SprintDetailID, AlertID, AlertLevel, RecordType)
    SELECT 
	   ...
	   NULL, ALERT.ALERT_ID, ALERT.ALERT_LEVEL, 'AS'
	...
	SELECT SPRINT_EMPLOYEE.ID,
			...
			JOB_LOG.OFFICE_CODE,NULL,NULL,NULL,NULL,
			V_JOB_TRAFFIC_DET.JOB_NUMBER,
			JOB_LOG.JOB_DESC,
			V_JOB_TRAFFIC_DET.JOB_COMPONENT_NBR,
			JOB_COMPONENT.JOB_COMP_DESC,
				 V_JOB_TRAFFIC_DET.SEQ_NBR,V_JOB_TRAFFIC_DET.FNC_CODE, F.FNC_DESCRIPTION,
				 ...
				 CASE WHEN WEEK_END IS NOT NULL THEN WEEK_END ELSE  CONVERT(
					 ...
					 ALERT.ALERT_ID,
					 ALERT.ALERT_LEVEL,0,0,0,0, 'T'				 
					-- (SELECT COUNT(*) FROM #WORK_DAY
					--										WHERE EMP_CODE = V_JOB_TRAFFIC_DET.EMP_CODE
					--										 AND WORK_DATE >= CASE WHEN WEEK_START IS NOT NULL THEN WEEK_START ELSE CONVERT(Date,V_JOB_TRAFFIC_DET.TASK_START_DATE) END
					--										 AND WORK_DATE <= CASE WHEN WEEK_END IS NOT NULL THEN WEEK_END ELSE CONVERT(Date,V_JOB_TRAFFIC_DET.JOB_REVISED_DATE) END
					--										 AND NON_TASK_DAY <> 1
					--										),
					...
					FROM         
						...
						LEFT OUTER JOIN FUNCTIONS F WITH(NOLOCK) ON F.FNC_CODE = V_JOB_TRAFFIC_DET.FNC_CODE
						WHERE (JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6, 12)) AND (JOB_TRAFFIC.COMPLETED_DATE IS NULL)
								AND (V_JOB_TRAFFIC_DET.JOB_COMPLETED_DATE IS NULL AND V_JOB_TRAFFIC_DET.TEMP_COMP_DATE IS NULL) AND V_JOB_TRAFFIC_DET.EMP_CODE IS NOT NULL
	...
	INSERT INTO #WEEKLY_HRS (SprintEmployeeID, EmployeeCode, EmployeeName, EmployeeFirstName, EmployeeLastName, EmployeeOfficeCode, EmployeeDepartmentCode, WeekNumber, WeekStart, WeekEnd, HoursAllocated, SprintDetailID, AlertID, AlertLevel)
	SELECT NULL, #EMPLOYEES.EMP_CODE, ISNULL(E.EMP_FNAME+' ', '')+ISNULL(E.EMP_MI+'. ', '')+ISNULL(E.EMP_LNAME, ''), E.EMP_FNAME, E.EMP_LNAME, E.OFFICE_CODE, E.DP_TM_CODE, NULL, NULL, NULL, 0, NULL, NULL, NULL
	...
	UPDATE #WEEKLY_HRS
	SET EmployeeFirstName = (SELECT EMP_FNAME FROM EMPLOYEE E WITH(NOLOCK) WHERE E.EMP_CODE = #WEEKLY_HRS.EmployeeCode)
	WHERE EmployeeFirstName IS NULL

	UPDATE #WEEKLY_HRS
	SET EmployeeLastName = (SELECT EMP_LNAME FROM EMPLOYEE E WITH(NOLOCK) WHERE E.EMP_CODE = #WEEKLY_HRS.EmployeeCode)
	WHERE EmployeeLastName IS NULL

	UPDATE #WEEKLY_HRS
	SET EmployeeDepartmentCode = (SELECT DP_TM_CODE FROM EMPLOYEE E WITH(NOLOCK) WHERE E.EMP_CODE = #WEEKLY_HRS.EmployeeCode)
	WHERE EmployeeDepartmentCode IS NULL

	UPDATE #WEEKLY_HRS
	SET EmployeeOfficeCode = (SELECT OFFICE_CODE FROM EMPLOYEE E WITH(NOLOCK) WHERE E.EMP_CODE = #WEEKLY_HRS.EmployeeCode)
	...
	BEGIN
		...
		BEGIN
			SET @day_count2 = 0
			SET @year_start_date = @START_DATE
			SET @year_end_date = @END_DATE
	...
	SET TOTAL_HOURS = ISNULL((SELECT CASE WHEN ALL_DAY = 1 THEN CASE DATEPART(dw,WorkDay)
					  ...
					  WHERE EMP_NON_TASKS_EMPS.EMP_CODE = #NON_TASK_TIME.EMP_CODE
					    AND WorkDay BETWEEN [START_DATE] AND [END_DATE]),0)
	WHERE Holiday = 0

	--SELECT * FROM #NON_TASK_TIME WHERE EMP_CODE = 'ama' AND WorkDay BETWEEN '7/22/2018' AND '7/28/2018'
	...
	BEGIN
		...
		SET AssignmentHoursAllowed = (SELECT ISNULL(SUM(HOURS_ALLOWED),SUM(JTD.JOB_HRS))
										FROM ALERT A WITH(NOLOCK) LEFT OUTER JOIN JOB_TRAFFIC_DET JTD WITH(NOLOCK) ON A.JOB_NUMBER = JTD.JOB_NUMBER AND A.JOB_COMPONENT_NBR = JTD.JOB_COMPONENT_NBR AND A.TASK_SEQ_NBR = JTD.SEQ_NBR
										...
										WHERE #TOTALS.AlertID = A.ALERT_ID AND JTDE.EMP_CODE = #TOTALS.EmployeeCode)
	...
	UPDATE #WEEKLY_HRS
	SET AssignmentDescription = (SELECT [SUBJECT] FROM ALERT WITH(NOLOCK) WHERE ALERT.ALERT_ID = #WEEKLY_HRS.AlertID)

	UPDATE #WEEKLY_HRS
	SET JobNumber = (SELECT JOB_NUMBER FROM ALERT WITH(NOLOCK) WHERE ALERT.ALERT_ID = #WEEKLY_HRS.AlertID)

	UPDATE #WEEKLY_HRS
	SET JobDescription = (SELECT JOB_DESC FROM JOB_LOG WITH(NOLOCK) WHERE JOB_LOG.JOB_NUMBER = #WEEKLY_HRS.JobNumber)

	UPDATE #WEEKLY_HRS
	SET JobComponentNumber = (SELECT JOB_COMPONENT_NBR FROM ALERT WITH(NOLOCK) WHERE ALERT.ALERT_ID = #WEEKLY_HRS.AlertID)

	UPDATE #WEEKLY_HRS
	SET JobComponentDescription = (SELECT JOB_COMP_DESC FROM JOB_COMPONENT WITH(NOLOCK) WHERE JOB_COMPONENT.JOB_NUMBER = #WEEKLY_HRS.JobNumber AND JOB_COMPONENT.JOB_COMPONENT_NBR = #WEEKLY_HRS.JobComponentNumber)

	UPDATE #WEEKLY_HRS
	SET TaskSequenceNumber = (SELECT TASK_SEQ_NBR FROM ALERT WITH(NOLOCK) WHERE ALERT.ALERT_ID = #WEEKLY_HRS.AlertID)

	UPDATE #WEEKLY_HRS
	SET FunctionCode = (SELECT FNC_CODE FROM JOB_TRAFFIC_DET JTD WITH(NOLOCK) WHERE JTD.JOB_NUMBER = #WEEKLY_HRS.JobNumber AND JTD.JOB_COMPONENT_NBR = #WEEKLY_HRS.JobComponentNumber AND JTD.SEQ_NBR = #WEEKLY_HRS.TaskSequenceNumber)

	UPDATE #WEEKLY_HRS
	SET FunctionDescription = (SELECT TASK_DESCRIPTION FROM JOB_TRAFFIC_DET JTD WITH(NOLOCK) WHERE JTD.JOB_NUMBER = #WEEKLY_HRS.JobNumber AND JTD.JOB_COMPONENT_NBR = #WEEKLY_HRS.JobComponentNumber AND JTD.SEQ_NBR = #WEEKLY_HRS.TaskSequenceNumber)
	--(SELECT FNC_DESCRIPTION FROM FUNCTIONS F WHERE F.FNC_CODE = #WEEKLY_HRS.FunctionCode)


	---SELECT * FROM #WEEKLY_HRS WHERE RecordType = 'AS' AND EmployeeCode = 'ama'
	--SELECT * FROM #WEEKLY_HRS WHERE RecordType = 'T' AND EmployeeCode = 'ama' --and JobNumber = 270
	--SELECT * FROM #TOTALS WHERE EmployeeCode = 'ama'
	...
	DECLARE @label_text varchar(40), @advan_dtype varchar(20), @dtype_prec smallint, @dtype_scale smallint, @dtype_id smallint,@pos int, @pos2 int, @ct int,
			 ...
			 @cat varchar(10), @select_sql varchar(4000), @wkst varchar(40), @wked varchar(40), @CumulativeHourAvailable decimal(18,3), @CumulativeHourAllocated decimal(18,3), @CumulativeHourPosted decimal(18,3),@prevwkst varchar(40), @prev_label_text varchar(40), @no_of_days int
	...
	BEGIN	
		...
		--SELECT @CumulativeHourAvailable,@CumulativeHourAllocated,@CumulativeHourPosted
		...
		SELECT @update_sql2 = @update_sql2 + '	 WHERE #WEEKLY_HRS.WeekStart = ''' + @wkst + ''' AND #WEEKLY_HRS.EmployeeCode = #WEEKLY_HRS_TOTAL.EmployeeCode AND #WEEKLY_HRS.JobNumber = #WEEKLY_HRS_TOTAL.JobNumber AND #WEEKLY_HRS.JobComponentNumber = #WEEKLY_HRS_TOTAL.JobComponentNumber'
		SELECT @update_sql2 = @update_sql2 + '	 GROUP BY WeekStart,EmployeeCode,JobNumber,JobComponentNumber),0)'
		...
		SELECT @update_sql3 = @update_sql3 + '	 WHERE #WEEKLY_HRS.WeekStart = ''' + @wkst + ''' AND #WEEKLY_HRS.EmployeeCode = #WEEKLY_HRS_TOTAL.EmployeeCode AND #WEEKLY_HRS.JobNumber = #WEEKLY_HRS_TOTAL.JobNumber AND #WEEKLY_HRS.JobComponentNumber = #WEEKLY_HRS_TOTAL.JobComponentNumber'
		SELECT @update_sql3 = @update_sql3 + '	 GROUP BY WeekStart,EmployeeCode,JobNumber,JobComponentNumber),0)'
		...
		SELECT @update_sql4 = @update_sql4 + '	 WHERE #WEEKLY_HRS.WeekStart = ''' + @wkst + ''' AND #WEEKLY_HRS.EmployeeCode = #WEEKLY_HRS_TOTAL.EmployeeCode AND #WEEKLY_HRS.JobNumber = #WEEKLY_HRS_TOTAL.JobNumber AND #WEEKLY_HRS.JobComponentNumber = #WEEKLY_HRS_TOTAL.JobComponentNumber'
		SELECT @update_sql4 = @update_sql4 + '	 GROUP BY WeekStart,EmployeeCode,JobNumber,JobComponentNumber),0)'	
		...
		SELECT @update_sql6 = @update_sql6 + '	 WHERE #WEEKLY_HRS.WeekStart = ''' + @wkst + ''' AND #WEEKLY_HRS.EmployeeCode = #WEEKLY_HRS_TOTAL.EmployeeCode AND #WEEKLY_HRS.JobNumber = #WEEKLY_HRS_TOTAL.JobNumber AND #WEEKLY_HRS.JobComponentNumber = #WEEKLY_HRS_TOTAL.JobComponentNumber'
		SELECT @update_sql6 = @update_sql6 + '	 ),0) - '
		SELECT @update_sql6 = @update_sql6 + 'ISNULL((SELECT ISNULL(SUM(HoursPosted),0) ' 
		SELECT @update_sql6 = @update_sql6 + '	  FROM #WEEKLY_HRS '	
		SELECT @update_sql6 = @update_sql6 + '	 WHERE #WEEKLY_HRS.WeekStart = ''' + @wkst + ''' AND #WEEKLY_HRS.EmployeeCode = #WEEKLY_HRS_TOTAL.EmployeeCode AND #WEEKLY_HRS.JobNumber = #WEEKLY_HRS_TOTAL.JobNumber AND #WEEKLY_HRS.JobComponentNumber = #WEEKLY_HRS_TOTAL.JobComponentNumber'
		SELECT @update_sql6 = @update_sql6 + '	 GROUP BY WeekStart,EmployeeCode,JobNumber,JobComponentNumber),0)'
	...
	UPDATE #WEEKLY_HRS_TOTAL
	SET EmployeeOfficeName = (SELECT OFFICE_NAME FROM OFFICE O WITH(NOLOCK) WHERE O.OFFICE_CODE = EmployeeOfficeCode)

	UPDATE #WEEKLY_HRS_TOTAL
	SET EmployeeDepartmentName = (SELECT DP_TM_DESC FROM DEPT_TEAM D WITH(NOLOCK) WHERE D.DP_TM_CODE = EmployeeDepartmentCode)
	...
	UPDATE #WEEKLY_HRS_TOTAL SET BurnRate = CASE WHEN AssignmentHoursAllowed > 0 THEN HoursPosted / AssignmentHoursAllowed + HoursAllowedTask ELSE 0 END
 ...
 --   FROM
	...
	SELECT @sql = 'SELECT ID, EmployeeCode,EmployeeName as Employee,EmployeeFirstName,EmployeeLastName,EmployeeOfficeCode AS OfficeCode,
		EmployeeOfficeName AS Office,EmployeeDepartmentCode AS DepartmentCode,EmployeeDepartmentName AS Department,	AssignmentDescription,
		JobNumber,
		JobDescription,
		JobComponentNumber,
		JobComponentDescription,
