...
BEGIN
	...
	BEGIN
		...
		FROM (
			SELECT 
				...
				CASE
					WHEN SEC_USER.EMP_CODE = @EMP_CODE AND (AC.IS_SYSTEM IS NULL OR AC.IS_SYSTEM = 0) THEN 1
					ELSE 0
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					END
				END
				--  CLEANUP APPROVED/DEFERRED/REJECTED COMMENTS NOT MARKED AS SYSTEM
				BEGIN
					...
					WHERE
						IS_MY_COMMENT = 1
						AND (	C.COMMENT LIKE 'APPROVED |%'
								OR C.COMMENT LIKE 'REJECTED |%'
								OR C.COMMENT LIKE 'DEFERRED |%'
								OR C.COMMENT LIKE 'NEW FILE |%'
								OR C.COMMENT LIKE 'NEW VERSION |%'
							)
				END
			END
		END
	...
	BEGIN
		SELECT
			...
			[IsMyComment] = ISNULL(C.IS_MY_COMMENT, 0)
		FROM 
			@COMMENTS C
		ORDER BY 
			[GENERATED_DATE] DESC,
