CREATE PROCEDURE [dbo].[advsp_ap_available_outofhome_orders] @vn_code varchar(6) = NULL, @office_code varchar(4) = NULL,
...
BEGIN
	...
	SELECT DISTINCT
        ...
        [Type] = O.OUTDOOR_TYPE
	FROM
		[dbo].OUTDOOR_HEADER A
		INNER JOIN [dbo].VENDOR B ON A.VN_CODE = B.VN_CODE
		INNER JOIN [dbo].OUTDOOR_DETAIL O ON A.ORDER_NBR = O.ORDER_NBR 
