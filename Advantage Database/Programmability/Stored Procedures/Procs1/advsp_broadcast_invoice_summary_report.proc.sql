...
BEGIN
	...
	IF @IncludeTV = 1 BEGIN
		...
		SELECT
			...
			SC.SC_DESCRIPTION,
			CASE WHEN 
						ISNULL(MBWM.MEDIA_BROADCAST_WORKSHEET_MARKET_ID, 0) = 0 THEN ''
				 ELSE
		...
		FROM
			...
			LEFT OUTER JOIN dbo.MEDIA_BROADCAST_WORKSHEET_MARKET MBWM ON MBWM.MEDIA_BROADCAST_WORKSHEET_MARKET_ID = TV.CLIENT_PO
			LEFT OUTER JOIN dbo.MEDIA_BROADCAST_WORKSHEET MBW ON MBW.MEDIA_BROADCAST_WORKSHEET_ID = MBWM.MEDIA_BROADCAST_WORKSHEET_ID
	...
	IF @IncludeRadio = 1 BEGIN
		...
		FROM
			...
			LEFT OUTER JOIN dbo.MEDIA_BROADCAST_WORKSHEET_MARKET MBWM ON MBWM.MEDIA_BROADCAST_WORKSHEET_MARKET_ID = R.CLIENT_PO
			LEFT OUTER JOIN dbo.MEDIA_BROADCAST_WORKSHEET MBW ON MBW.MEDIA_BROADCAST_WORKSHEET_ID = MBWM.MEDIA_BROADCAST_WORKSHEET_ID
			LEFT OUTER JOIN (SELECT 
									APMA.AP_ID,
									APMA.ORDER_NBR,
