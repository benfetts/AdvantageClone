...
--set @OrderNumberLineNumbers = '1164|1'
--set	@OrderYearMonths = '1164|2020|5'
--set	@OrderNumbers = '1164' 
--set	@ShowWeekOf = 0
--set @debug = 1
...
IF @ShowWeekOf = 1 BEGIN
	...
	SELECT DISTINCT
		...
		WeekOf = CASE WHEN @ShowWeekOf = 1 THEN
				 ...
				 ELSE NULL
		  		 END,
		Spots = SPOTS   --CASE WHEN @ShowWeekOf = 1 THEN SPOTS
				--ELSE NULL
				--END
	FROM (
...
END ELSE BEGIN
	INSERT INTO @weekof (OrderNumber, OrderLineNumber, Spots)
	SELECT DISTINCT
		OrderNumber = ORDER_NBR,
		OrderLineNumber = LINE_NBR,
		Spots = SUM(SPOTS)
...
IF @ShowWeekOf = 1 BEGIN
    INSERT INTO @ActualSpots (OrderNumber, OrderLineNumber, WeekOf, Spots)
    SELECT OrderNumber, OrderLineNumber, WeekOf, COUNT(1) 
    FROM @view
    WHERE OrderLineNumber IS NOT NULL
    GROUP BY OrderNumber, OrderLineNumber, WeekOf

    --insert actual spots weeks that do not belong in a week that was ordered to the min ordered week
    INSERT INTO @ActualSpots (OrderNumber, OrderLineNumber, WeekOf, Spots)
    SELECT a.OrderNumber, a.OrderLineNumber, (SELECT MIN(WeekOf) FROM @weekof WHERE OrderNumber = a.OrderNumber AND OrderLineNumber = a.OrderLineNumber), SUM(a.Spots) 
    FROM @ActualSpots a
        LEFT OUTER JOIN @weekof w ON w.OrderNumber = a.OrderNumber AND w.OrderLineNumber = a.OrderLineNumber AND w.WeekOf = a.WeekOf
    WHERE w.WeekOf IS NULL
    AND a.OrderLineNumber IS NOT NULL
    GROUP BY a.OrderNumber, a.OrderLineNumber
END ELSE
    INSERT INTO @ActualSpots (OrderNumber, OrderLineNumber, Spots)
    SELECT OrderNumber, OrderLineNumber, COUNT(1) 
    FROM @view
    WHERE OrderLineNumber IS NOT NULL
...
FROM
	(SELECT
        ...
        [ActualSpots] = ISNULL( (SELECT SUM(Spots) FROM @ActualSpots v WHERE Ord.OrderNumber = v.OrderNumber AND Ord.OrderLineNumber = v.OrderLineNumber AND (@ShowWeekOf = 0 OR Ord.WeekOf = v.WeekOf AND @ShowWeekOf = 1)), 0),
 		[VariantCodes] = CASE CAST(C.[IsRateVerified] as bit)
							...
							--END,
                            CASE
								WHEN (SELECT COUNT(1) FROM @bookend_table bt WHERE bt.OrderNumber = Ord.OrderNumber AND bt.OrderLineNumber = Ord.OrderLineNumber) = 0 THEN 
									''
								WHEN (SELECT COUNT(1) FROM @bookend_table bt WHERE bt.OrderNumber = Ord.OrderNumber AND bt.OrderLineNumber = Ord.OrderLineNumber) = 1 THEN
									(SELECT [IsBookendVerified] FROM @bookend_table bt WHERE bt.OrderNumber = Ord.OrderNumber AND bt.OrderLineNumber = Ord.OrderLineNumber)
								WHEN (SELECT COUNT(1) FROM @bookend_table bt WHERE bt.OrderNumber = Ord.OrderNumber AND bt.OrderLineNumber = Ord.OrderLineNumber) = 2 THEN
									'Bookend Sep | Time Sep | '
							END,
		[OrderMonthNumber] = [OrderMonthNumber],
		[OrderYearNumber] = [OrderYearNumber],
