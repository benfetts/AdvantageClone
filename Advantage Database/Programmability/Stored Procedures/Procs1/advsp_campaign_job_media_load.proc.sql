IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[advsp_campaign_job_media_load]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[advsp_campaign_job_media_load]
GO

CREATE PROCEDURE [dbo].[advsp_campaign_job_media_load](
...
END
	...
	Begin
		...
		WHERE CMP_IDENTIFIER BETWEEN @CampaignIDFrom AND @CampaignIDTo AND CL_CODE = @ClientCode
	End
	Else
	Begin
		INSERT INTO #CMPIDS
		SELECT CMP_IDENTIFIER, JOB_NUMBER, JOB_COMPONENT_NBR, CAST(JOB_NUMBER AS VARCHAR(10)) + '|' + CAST(JOB_COMPONENT_NBR AS VARCHAR(10)) FROM CAMPAIGN 
		WHERE CMP_CLOSED = 0 AND CMP_IDENTIFIER BETWEEN @CampaignIDFrom AND @CampaignIDTo AND CL_CODE = @ClientCode
	End
...
SELECT [OfficeCode],
		...
		[JobStartDate],
		[JobDueDate],
		[CampaignComments] FROM #CAMP_TOTAL --WHERE CampaignID = 1
...
GRANT EXECUTE ON [advsp_campaign_job_media_load] TO PUBLIC
GO