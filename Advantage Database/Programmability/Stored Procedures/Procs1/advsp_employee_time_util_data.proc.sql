IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[advsp_employee_time_util_data]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[advsp_employee_time_util_data] 
GO
-- Parameters
...
CREATE PROCEDURE [dbo].[advsp_employee_time_util_data] (@sel_dp_tm_opt tinyint, @dp_tm_opt tinyint, @dp_tm_list varchar(4000), 
	...
	@cost_opt tinyint = 1, @wip_opt tinyint = 1, @incl_rtax tinyint = 0, @limit_wip tinyint = 0)
AS
BEGIN
	SET NOCOUNT ON;
...
--	SELECT * FROM #emp_util_emp_time_direct WHERE CLIENT = 'veri' AND EMP_CODE = 'ama'
	...
	IF @wip_opt = 1
	BEGIN
		DELETE FROM #emp_util_emp_time_direct
		INSERT INTO #emp_util_emp_time_direct		
			SELECT
				EMP_CODE = e.EMP_CODE,
				PERIOD = (SELECT PPPERIOD FROM dbo.POSTPERIOD WHERE e.EMP_DATE BETWEEN PPSRTDATE AND PPENDDATE
			...
			LEFT JOIN dbo.CLIENT AS c
				ON j.CL_CODE = c.CL_CODE			
			WHERE (ISNULL(ar.AR_INV_SEQ, 0) = 0 OR ar.AR_INV_SEQ = 99)
				AND (ed.EMP_HOURS <> 0)			--Added 11/11/08 JP
				AND ((@sel_dp_tm_opt = 1)		--Added 11/11/08 JP
				OR (@sel_dp_tm_opt = 2 AND dpc.DP_TM_CODE IS NOT NULL))	AND (ar.AR_POST_PERIOD IS NULL AND ISNULL(ed.EMP_NON_BILL_FLAG, 0) = 0 AND agy.CL_CODE IS NULL AND ISNULL(c.NEW_BUSINESS,0) <> 1 AND ISNULL(ed.FEE_TIME, 0) = 0)
		--SELECT * FROM #emp_util_emp_time_direct ORDER BY JOB_NUMBER
		if @limit_wip = 1
		BEGIN
			INSERT INTO #emp_util_time_data
