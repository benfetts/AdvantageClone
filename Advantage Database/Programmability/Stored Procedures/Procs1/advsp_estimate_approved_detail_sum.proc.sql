if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[advsp_estimate_approved_detail_sum]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[advsp_estimate_approved_detail_sum]
GO
...
CREATE PROCEDURE [dbo].[advsp_estimate_approved_detail_sum] @from_date AS datetime = '1/1/1900', 
	@to_date AS datetime = '12/31/2199'

-- advsp_estimate_approved_detail_sum (summarizes by function)============================
-- #00 01/27/14 - initial
...
-- =======================================================================================	
	SELECT 
		...
		CAST(erd.EST_FNC_COMMENT AS varchar(4000)) AS DetailComments, 
		...
		CAST(erd.EST_REV_SUP_BY_NTE AS varchar(4000)) AS SuppliedByNotes, 
		...
		CASE erd.EST_FNC_TYPE
			WHEN 'E' THEN SUM(ISNULL(erd.EST_REV_QUANTITY,0))
			ELSE 0
		END AS EstimateHours,	
		--(IIf([EST_FNC_TYPE]<>"E",Nz([EST_REV_QUANTITY],0),0)) AS EstimateQuantity, 
		CASE erd.EST_FNC_TYPE
			WHEN 'E' THEN 0
			ELSE SUM(ISNULL(erd.EST_REV_QUANTITY,0))
