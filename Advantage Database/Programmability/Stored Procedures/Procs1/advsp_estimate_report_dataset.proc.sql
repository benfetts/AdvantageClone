if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[advsp_estimate_report_dataset]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[advsp_estimate_report_dataset]
GO



CREATE PROCEDURE [dbo].[advsp_estimate_report_dataset] 
@EstNo Int,
@EstCompNo Int,
@UserID varchar(100),
@Quotes varchar(100)
...
SELECT @sqlfrom = ' FROM #estPrint EP INNER JOIN PRODUCT P ON EP.ClientCode = P.CL_CODE AND EP.DivisionCode = P.DIV_CODE AND EP.ProductCode = P.PRD_CODE
				  ...
				  LEFT OUTER JOIN V_ESTIMATEAPPR AS EA ON EA.ESTIMATE_NUMBER = EP.EstimateNumber AND EA.EST_COMPONENT_NBR = EP.EstimateComponentNumber
					AND EA.EST_QUOTE_NBR = EP.QuoteNumber AND EA.EST_REVISION_NBR = EP.RevisionNumber LEFT OUTER JOIN
