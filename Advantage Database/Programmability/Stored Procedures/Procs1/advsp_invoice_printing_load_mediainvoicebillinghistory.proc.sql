CREATE PROCEDURE [dbo].[advsp_invoice_printing_load_mediainvoicebillinghistory] 
...
BEGIN
	...
	END ELSE BEGIN
		...
		SELECT 
			[InvoiceNumber] = RIGHT('000000' + CAST(ARS.AR_INV_NBR AS varchar(6)), 6) + '-' + RIGHT('0000' + CAST(ARS.AR_INV_SEQ AS varchar(4)), 4),
			[InvoiceDate] = ARID.AR_INV_DATE,
			[Amount] = SUM(ARS.TOTAL_BILL)
		FROM 
			[dbo].[AR_SUMMARY] AS ARS INNER JOIN 
