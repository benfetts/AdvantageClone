if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[advsp_job_detail_analysis_category_load]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[advsp_job_detail_analysis_category_load]
GO

CREATE PROCEDURE [dbo].[advsp_job_detail_analysis_category_load]
...
BEGIN
	...
	SELECT
		...
		[ItemCategory] = SJDA.ITEM_CATEGORY,
	...
	FROM
		(SELECT
			...
			[ITEM_CATEGORY] = JDA.ITEM_CATEGORY,
		...
		FROM
			(SELECT
				...
				[ITEM_CODE] = CASE JD.[ResourceType] WHEN 'AB' THEN JD.[ItemExtra] WHEN 'E' THEN NULL WHEN 'I' THEN NULL WHEN 'V' THEN JD.[ItemExtra] WHEN 'PO' THEN 'PO #: ' + CAST(JD.[PurchaseOrderNumber] AS varchar) END,
				...
				[ITEM_CATEGORY] = CASE WHEN JD.[ResourceType] = 'E' AND JD.[TransferAdjustmentUserCode] IS NOT NULL THEN 'Adjust/Transfer'
			...
			FROM
				...
				WHERE
					...
					((C.ClientCode + '|' + C.DivisionCode) IN (SELECT * FROM dbo.udf_split_list(@DIVISION_LIST, ',')) OR @DIVISION_LIST IS NULL) AND
					((C.ClientCode + '|' + C.DivisionCode + '|' + C.ProductCode) IN (SELECT * FROM dbo.udf_split_list(@PRODUCT_LIST, ',')) OR @PRODUCT_LIST IS NULL) AND
