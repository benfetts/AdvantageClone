...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[advsp_job_detail_analysis_category_load_report]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[advsp_job_detail_analysis_category_load_report]
GO


CREATE PROCEDURE [dbo].[advsp_job_detail_analysis_category_load_report](
...
BEGIN
	...
	(
		...
		[TransferAdjustmentUserCode] varchar(100)
	);	
	...
	SELECT
		...
		[TransferAdjustmentUserCode] = null
	FROM 
	...
	SELECT
		...
		[ProductCategoryCode] = null,
		[ProductCategoryDescription] = null,
		[TransferAdjustmentUserCode] = [IO].XFER_ADJ_USER
	FROM 
	...
	SELECT
		...
		[ProductCategoryCode] = null,
		[ProductCategoryDescription] = null,
		[TransferAdjustmentUserCode] = APP.XFER_ADJ_USER
	FROM 
 ...
 --   WHERE J.JobNumber = 265
	...
	FROM
		...
		[ItemCategory] = CASE WHEN JD.[ResourceType] = 'E' AND JD.[TransferAdjustmentUserCode] IS NOT NULL THEN 'Adjust/Transfer'
								WHEN JD.[ResourceType] = 'E' AND JD.[TransferAdjustmentUserCode] IS NULL THEN 'Employee Time'	
								WHEN JD.[ResourceType] = 'V' AND JD.[TransferAdjustmentUserCode] IS NOT NULL THEN 'Adjust/Transfer'
								WHEN JD.[ResourceType] = 'V' AND JD.[TransferAdjustmentUserCode] IS NULL THEN 'Accounts Payable'	
								WHEN JD.[ResourceType] = 'I' AND JD.[TransferAdjustmentUserCode] IS NOT NULL THEN 'Adjust/Transfer'
								WHEN JD.[ResourceType] = 'I' AND JD.[TransferAdjustmentUserCode] IS NULL THEN 'Income Only'
								WHEN JD.[ResourceType] = 'ES' THEN 'Estimate Amount'
								WHEN JD.[ResourceType] = 'EI' THEN 'Estimate Amount'
								WHEN JD.[ResourceType] = 'AB' THEN 'Advance Billing'
								WHEN JD.[ResourceType] = 'PO' THEN 'Purchase Order' ELSE NULL END,
		[ClientPO] = J.[ClientPO],
	...
	FROM 
		#JOB AS J INNER JOIN 
		#JOB_DETAIL AS JD ON J.[JobNumber] = JD.[JobNumber] AND
							 J.[JobComponentNumber] = JD.[JobComponentNumber] INNER JOIN
		(SELECT 
			JC.JOB_NUMBER, 
		 ...
		 FROM [dbo].[JOB_COMPONENT] JC) AS JC ON JC.JOB_NUMBER = J.[JobNumber] AND
												 JC.JOB_COMPONENT_NBR = J.[JobComponentNumber] ) AS JDA 
