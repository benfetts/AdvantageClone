if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[advsp_job_detail_analysis_employeetime_load]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[advsp_job_detail_analysis_employeetime_load]
GO

CREATE PROCEDURE [dbo].[advsp_job_detail_analysis_employeetime_load]
...
BEGIN
	...
	FROM
		...
		GROUP BY 
			...
			JDA.[EstimateComponentNumber],
			JDA.[JobComponent],
			JDA.[FunctionDescription]) AS SJDA INNER JOIN
		(SELECT 
			JC.JOB_NUMBER, 
