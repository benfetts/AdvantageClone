...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[advsp_job_detail_analysis_employeetime_load_report]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[advsp_job_detail_analysis_employeetime_load_report]
GO


CREATE PROCEDURE [dbo].[advsp_job_detail_analysis_employeetime_load_report](
...
--	SELECT GETDATE() 'BEFORE MAIN'
 ...
 --   WHERE J.JobNumber = 89653
	...
	SELECT
		...
		[SumOfNonBillableMarkupAmount] = SUM(JDA.[SumOfNonBillableMarkupAmount]),
	...
	FROM
		...
		[SumOfNonBillableMarkupAmount] = CASE WHEN JD.[ResourceType] IN('E','I','V') AND JD.[IsNonBillable] = 1 THEN JD.[ExtMarkupAmount] ELSE 0 END,
	...
	GROUP BY 
			...
			JDA.[ProcessDesc],
			JDA.[IsAdvanceBill]  OPTION (RECOMPILE)--) AS J
