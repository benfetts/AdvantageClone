if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[advsp_job_detail_analysis_qva_load]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[advsp_job_detail_analysis_qva_load]
GO

CREATE PROCEDURE [dbo].[advsp_job_detail_analysis_qva_load]
...
BEGIN
	...
	SELECT
		...
		[FunctionHeading] = SJDA.[FunctionHeading],
		...
		[SumOfNonBillableMarkup] = SJDA.SumOfNB_MARKUP,
		...
		[EstimateDescription] = SJDA.[EstimateDescription],
		[EstimateComponentNumber] = SJDA.[EstimateComponentNumber],
		[EstimateComponentDescription] = SJDA.[EstimateComponentDescription],
		[EstimateQuoteNumber] = CAST(SJDA.[EstimateQuoteNumber] AS SmallInt),
		[EstimateRevisionNumber] = CAST(SJDA.[EstimateRevisionNumber] AS SmallInt),
		[SumOfEstimateNetEmployeeTime] = CASE SJDA.FNC_TYPE WHEN 'E' THEN SJDA.SumOfESTIMATE - SJDA.SumOfESTCONT - SJDA.SumOfEST_RESALE_TAX - SJDA.SumOfEST_EXT_MARKUP ELSE 0 END,
		[SumOfActualNetEmployeeTime] = CASE SJDA.FNC_TYPE WHEN 'E' THEN SJDA.SumOfLINE_TOTAL -  SJDA.SumOfEXT_MARKUP_AMT - SJDA.SumOfRESALE_TAX + SJDA.SumOfNB_AMOUNT - SJDA.SumOfNB_MARKUP ELSE 0 END,
		[SumOfEstimateNet] = SJDA.SumOfESTIMATE - SJDA.SumOfESTCONT - SJDA.SumOfEST_RESALE_TAX - SJDA.SumOfEST_EXT_MARKUP,
		[SumOfActualNet] = SJDA.SumOfLINE_TOTAL -  SJDA.SumOfEXT_MARKUP_AMT - SJDA.SumOfRESALE_TAX + SJDA.SumOfNB_AMOUNT - SJDA.SumOfNB_MARKUP
	FROM
		(SELECT
			...
			[SumOfNB_MARKUP] = SUM(JDA.NB_MARKUP_AMT),
		...
		FROM
			(SELECT
				...
				[BILL_EMP_HOURS] = CASE WHEN JD.[ResourceType] = 'E' AND JD.[IsNonBillable] = 0 THEN JD.[Hours]
										WHEN JD.[ResourceType] = 'I' AND JD.[IsNonBillable] = 0 THEN JD.[Quantity] ELSE 0 END,
				...
				[AP_NET_COST] = CASE WHEN JD.[ResourceType] = 'V' AND (JD.[AccountsReceivableInvoiceNumber] IS NOT NULL OR JD.[AdvanceBillFlagDetail] = 3) THEN JD.[CostAmount] ELSE 0 END,
				...
				[ESTIMATE_HRS_QTY] = CASE WHEN (JD.[ResourceType] IN('ES','EI') AND JD.[FunctionType] IN('V','I')) THEN JD.[EstimateQuantity] ELSE JD.[EstimateHours] END,
				...
				JD.[JobTypeCode],
				JD.[EstimateNumber],
				JD.[EstimateDescription],
				JD.[EstimateComponentNumber],
				JD.[EstimateComponentDescription],
				JD.[EstimateQuoteNumber],
				JD.[EstimateRevisionNumber],
				JD.[JobComponent],
			...
			FROM
				(SELECT	
					...
					C.[JobTypeCode],
					C.[EstimateNumber],
					C.[EstimateDescription],
					C.[EstimateComponentNumber],
					C.[EstimateComponentDescription],
					C.[EstimateQuoteNumber],
					C.[EstimateRevisionNumber],
					C.[JobComponent]
				FROM 
		...
		GROUP BY 
			...
			JDA.[JobComponent],
			JDA.[FunctionDescription],
			JDA.[FunctionHeading]) AS SJDA INNER JOIN
		(SELECT 
			JC.JOB_NUMBER, 
