CREATE PROCEDURE [dbo].[advsp_job_detail_analysis_qva_service_load]
AS
BEGIN

	DECLARE @OPEN_JOB_ONLY AS bit
	DECLARE @EXCLUDE_NONBILL_HRS AS bit
	DECLARE @OFFICE_LIST AS varchar(MAX)
	DECLARE @CLIENT_LIST AS varchar(MAX)
	DECLARE @DIVISION_LIST AS varchar(MAX)
	DECLARE @PRODUCT_LIST AS varchar(MAX)
	DECLARE @JOB_LIST AS varchar(MAX)
	DECLARE @ACCT_EXEC_LIST AS varchar(MAX)
	DECLARE @SALES_CLASS_LIST AS varchar(MAX)

	DECLARE @CALCULATION_INCLUDE_NON_BILLABLE_TIME BIT,
			@CALCULATION_INCLUDE_TIME_MARKED_FEE_TIME BIT,
			@ADV_SERVICE_ID INT
	...
	FROM
		dbo.ADV_SERVICE
	WHERE
		ADV_SERVICE.CODE = 'AdvantageQvAAlertWindowsService'
	
	SELECT
		@CALCULATION_INCLUDE_NON_BILLABLE_TIME = CONVERT(BIT, VALUE)
	FROM
		dbo.ADV_SERVICE_SETTING 
	WHERE
		ADV_SERVICE_ID = @ADV_SERVICE_ID AND
		CODE = 'Calculation_IncludeNonBillableTime'

	SELECT
		@CALCULATION_INCLUDE_TIME_MARKED_FEE_TIME = CONVERT(BIT, VALUE)
	FROM
		dbo.ADV_SERVICE_SETTING 
	WHERE
		ADV_SERVICE_ID = @ADV_SERVICE_ID AND
		CODE = 'Calculation_IncludeTimeMarkedFeeTime'
		
	SET @CALCULATION_INCLUDE_NON_BILLABLE_TIME = ISNULL(@CALCULATION_INCLUDE_NON_BILLABLE_TIME, 0)
	SET @CALCULATION_INCLUDE_TIME_MARKED_FEE_TIME = ISNULL(@CALCULATION_INCLUDE_TIME_MARKED_FEE_TIME, 0)

	SET @OPEN_JOB_ONLY = 1
	SET @EXCLUDE_NONBILL_HRS = 0
	...
	FROM
		...
		FROM
			(SELECT
				...
				[LINE_TOTAL] = CASE 
									WHEN JD.[ResourceType] IN ('E', 'I','V') AND JD.[IsNonBillable] = 0 THEN JD.[Total] 
									WHEN JD.[ResourceType] = 'E' AND @CALCULATION_INCLUDE_NON_BILLABLE_TIME = 1 AND JD.FeeTime = 0 THEN JD.[Total]
									WHEN JD.[ResourceType] = 'E' AND @CALCULATION_INCLUDE_TIME_MARKED_FEE_TIME = 1 AND JD.FeeTime = 1 THEN JD.[Total] 
								ELSE 0 END,
				[CLIENT_OOP_AMT] = NULL,
			...
			FROM
				(SELECT	
					...
					C.[NonBillable]
				FROM 
					[dbo].[V_DRPT_JOB_DETAILS_ITEM_C] AS C 
				WHERE
				...
				SELECT	
					...
					C.[NonBillable]
				FROM 
					[dbo].[V_DRPT_JOB_DETAILS_ITEM_E] AS C 
				WHERE
					1 = CASE WHEN @OPEN_JOB_ONLY = 1 AND (C.[JobProcessControl] = 'Closed' OR C.[JobProcessControl] = 'Archive') THEN 0 ELSE 1 END AND
					1 = CASE WHEN @EXCLUDE_NONBILL_HRS = 1 AND C.[IsNonBillable] = 1 THEN 0 ELSE 1 END AND
					(C.OfficeCode IN (SELECT * FROM dbo.udf_split_list(@OFFICE_LIST, ',')) OR @OFFICE_LIST IS NULL) AND
				...
				SELECT	
					...
					C.[NonBillable]
				FROM 
					[dbo].[V_DRPT_JOB_DETAILS_ITEM_V] AS C 
				WHERE
					1 = CASE WHEN @OPEN_JOB_ONLY = 1 AND (C.[JobProcessControl] = 'Closed' OR C.[JobProcessControl] = 'Archive') THEN 0 ELSE 1 END AND
					1 = CASE WHEN @EXCLUDE_NONBILL_HRS = 1 AND C.[IsNonBillable] = 1 THEN 0 ELSE 1 END AND
					(C.OfficeCode IN (SELECT * FROM dbo.udf_split_list(@OFFICE_LIST, ',')) OR @OFFICE_LIST IS NULL) AND
				...
				SELECT	
					...
					C.[NonBillable]
				FROM 
					[dbo].[V_DRPT_JOB_DETAILS_ITEM_I] AS C
				WHERE
					1 = CASE WHEN @OPEN_JOB_ONLY = 1 AND (C.[JobProcessControl] = 'Closed' OR C.[JobProcessControl] = 'Archive') THEN 0 ELSE 1 END AND
					1 = CASE WHEN @EXCLUDE_NONBILL_HRS = 1 AND C.[IsNonBillable] = 1 THEN 0 ELSE 1 END AND
					(C.OfficeCode IN (SELECT * FROM dbo.udf_split_list(@OFFICE_LIST, ',')) OR @OFFICE_LIST IS NULL) AND
				...
				SELECT	
					...
					C.[NonBillable]
				FROM 
					[dbo].[V_DRPT_JOB_DETAILS_ITEM_AB] AS C 
				WHERE
					1 = CASE WHEN @OPEN_JOB_ONLY = 1 AND (C.[JobProcessControl] = 'Closed' OR C.[JobProcessControl] = 'Archive') THEN 0 ELSE 1 END AND
					1 = CASE WHEN @EXCLUDE_NONBILL_HRS = 1 AND C.[IsNonBillable] = 1 THEN 0 ELSE 1 END AND
					(C.OfficeCode IN (SELECT * FROM dbo.udf_split_list(@OFFICE_LIST, ',')) OR @OFFICE_LIST IS NULL) AND
				...
				SELECT	
					...
					C.[NonBillable]
				FROM 
					[dbo].[V_DRPT_JOB_DETAILS_ITEM_ES] AS C 
				WHERE
					1 = CASE WHEN @OPEN_JOB_ONLY = 1 AND (C.[JobProcessControl] = 'Closed' OR C.[JobProcessControl] = 'Archive') THEN 0 ELSE 1 END AND
					1 = CASE WHEN @EXCLUDE_NONBILL_HRS = 1 AND C.[IsNonBillable] = 1 THEN 0 ELSE 1 END AND
					(C.OfficeCode IN (SELECT * FROM dbo.udf_split_list(@OFFICE_LIST, ',')) OR @OFFICE_LIST IS NULL) AND
				...
				SELECT	
					...
					C.[NonBillable]
				FROM 
					[dbo].[V_DRPT_JOB_DETAILS_ITEM_EI] AS C 
				WHERE
					1 = CASE WHEN @OPEN_JOB_ONLY = 1 AND (C.[JobProcessControl] = 'Closed' OR C.[JobProcessControl] = 'Archive') THEN 0 ELSE 1 END AND
					1 = CASE WHEN @EXCLUDE_NONBILL_HRS = 1 AND C.[IsNonBillable] = 1 THEN 0 ELSE 1 END AND
					(C.OfficeCode IN (SELECT * FROM dbo.udf_split_list(@OFFICE_LIST, ',')) OR @OFFICE_LIST IS NULL) AND
				...
				SELECT	
					...
					C.[NonBillable]
				FROM 
					[dbo].[V_DRPT_JOB_DETAILS_ITEM_PO] AS C 
				WHERE
					1 = CASE WHEN @OPEN_JOB_ONLY = 1 AND (C.[JobProcessControl] = 'Closed' OR C.[JobProcessControl] = 'Archive') THEN 0 ELSE 1 END AND
					1 = CASE WHEN @EXCLUDE_NONBILL_HRS = 1 AND C.[IsNonBillable] = 1 THEN 0 ELSE 1 END AND
					(C.OfficeCode IN (SELECT * FROM dbo.udf_split_list(@OFFICE_LIST, ',')) OR @OFFICE_LIST IS NULL) AND
