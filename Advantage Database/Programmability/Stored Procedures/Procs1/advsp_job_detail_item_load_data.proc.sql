CREATE PROCEDURE [dbo].[advsp_job_detail_item_load_data]
AS
BEGIN
	...
	(
		...
		[AdvanceBillingBalance] decimal(15,2)
	);
...
IF @debug = 1 SELECT getdate() 'after JOB'
	...
	SELECT
		...
		[AdvanceBillingBalance] = 0 - CASE WHEN IR.AR_INV_NBR IS NOT NULL THEN COALESCE(IR.REC_AMT, 0) ELSE 0 END
	FROM 
...
IF @debug = 1 SELECT getdate() 'after IR'
	...
	SELECT
		...
		[BilledAmount] = ISNULL(AB.LINE_TOTAL, 0) - ISNULL(AB.EXT_CITY_RESALE, 0) - ISNULL(AB.EXT_STATE_RESALE, 0) - ISNULL(AB.EXT_COUNTY_RESALE, 0),
		[BilledWithResale] = ISNULL(AB.LINE_TOTAL, 0),
		[BilledHours] = 0,
		[BilledQuantity] = 0,
		[FeeTimeAmount] = 0,
		[FeeTimeHours] = 0,
		...
		[AdvanceBillingBalance] = CASE WHEN AB.AR_INV_NBR IS NOT NULL AND AB.AB_FLAG <> 5 THEN COALESCE(ADV_BILL_NET_AMT, 0) + COALESCE(EXT_MARKUP_AMT, 0) + COALESCE(EXT_NONRESALE_TAX, 0) ELSE 0 END
	FROM 
...
IF @debug = 1 SELECT getdate() 'after ND'
	...
	DELETE FROM DS_JOB_DETAIL_ITEM

	INSERT INTO DS_JOB_DETAIL_ITEM
	SELECT 
		JD.ResourceType,
		J.JobNumber,
		...
		JD.[AdvanceBillingBalance],
		[PercentCompleteHours] = 0,
