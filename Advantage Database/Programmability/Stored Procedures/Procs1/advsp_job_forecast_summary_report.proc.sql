IF EXISTS (SELECT
    *
  FROM dbo.sysobjects
  WHERE id = OBJECT_ID(N'[dbo].[advsp_job_forecast_summary_report]')
  AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
  DROP PROCEDURE [dbo].[advsp_job_forecast_summary_report]
GO

CREATE PROCEDURE [dbo].[advsp_job_forecast_summary_report]
...
BEGIN
    ...
    SELECT A.PostPeriod, SUM(A.Forecast) as Revenue, SUM(A.Actual) as Expenses, SUM(A.Variance) AS Variance 
    FROM (
	SELECT
		[ID] = NEWID(),
		...
		(SELECT
		 ...
		 GROUP BY
	 		...
	 		PURCHASE_ORDER_DET.JOB_COMPONENT_NBR) POINFO ON JOB_COMPONENT.JOB_NUMBER = POINFO.JOB_NUMBER AND
															JOB_COMPONENT.JOB_COMPONENT_NBR = POINFO.JOB_COMPONENT_NBR ) A
        GROUP BY A.PostPeriod



    SELECT [ItemPostPeriodCode],
           SUM([ActualBillableNetAmount]) AS [ActualBillableNetAmount],
		   SUM([ActualBillableMarkupAmount]) AS [ActualBillableMarkupAmount],
		   SUM([ActualBillableAmount]) AS [ActualBillableAmount],
		   SUM([ActualRevenueAmount]) AS [ActualRevenueAmount]
    FROM V_JOB_COMP_ACTUALS
    GROUP BY [ItemPostPeriodCode]
    ORDER BY [ItemPostPeriodCode]
