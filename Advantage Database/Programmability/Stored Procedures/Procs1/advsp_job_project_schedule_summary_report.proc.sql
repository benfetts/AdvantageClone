if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[advsp_job_project_schedule_summary_report]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[advsp_job_project_schedule_summary_report]
GO


CREATE PROCEDURE [dbo].[advsp_job_project_schedule_summary_report]
...
SELECT
		...
		[JobProcessControlNumber] = JC.JOB_PROCESS_CONTRL,
	...
	FROM
		...
		[dbo].[EMPLOYEE_CLOAK] AS MEMP ON MEMP.EMP_CODE = JOBTMGR.MGR_EMP_CODE LEFT OUTER JOIN
		(SELECT 
			JOB_NUMBER, 
			JOB_COMPONENT_NBR, 
...
GRANT EXECUTE ON [advsp_job_project_schedule_summary_report] TO PUBLIC
GO