CREATE PROCEDURE [dbo].[advsp_load_drpt_job_details_bill_month]

AS
BEGIN

	DELETE FROM [dbo].[DRPT_JOB_DETAILS_BILL_MONTH]
	
	DBCC CHECKIDENT (DRPT_JOB_DETAILS_BILL_MONTH, reseed, 0)
	
	INSERT INTO [dbo].[DRPT_JOB_DETAILS_BILL_MONTH]
	SELECT 
		...
		[AccountsReceivablePostPeriodCode] = ISNULL(JDI.[AccountsReceivablePostPeriodCode], ''),
	...
	GROUP BY
		ISNULL(JDI.[AccountsReceivablePostPeriodCode], ''),
		JDI.[JobNumber],
