CREATE PROCEDURE [dbo].[advsp_load_drpt_project_hours_used]

AS
BEGIN

	DELETE FROM [dbo].[DRPT_PROJECT_HOURS_USED]
	
	DBCC CHECKIDENT (DRPT_PROJECT_HOURS_USED, reseed, 0)
	
	INSERT INTO [dbo].[DRPT_PROJECT_HOURS_USED]
	SELECT 
		...
		[Employee] = JOBTDEMP.EmployeeName,
	...
	FROM
		...
		[dbo].[advtf_emp_names]() AS MEMP ON MEMP.EmployeeCode = JOBT.MGR_EMP_CODE LEFT OUTER JOIN
		...
		[dbo].[advtf_emp_names]() AS JOBTDEMP ON JOBTDEMP.EmployeeCode = JOBTDE.EMP_CODE LEFT OUTER JOIN
