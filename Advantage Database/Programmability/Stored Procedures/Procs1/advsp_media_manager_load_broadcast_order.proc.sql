...
IF @MediaType = 'T' BEGIN
	...
	IF (SELECT UNITS FROM dbo.TV_HDR WHERE ORDER_NBR = @OrderNumber) <> 'CM'
		SELECT 
	...
	ELSE
		SELECT 
				@MinWeekdate = MIN(MinWeekdate),
				@MaxRevision = MAX(REV_NBR),
				@MaxWeekdate = DATEADD(d,6,MAX(MaxWeekdate)), --MAX(WeekdateEnd),
				@DailyMaxWeekdate = MAX(MaxWeekdate)
		FROM (
			SELECT 
				--[Weekdate] = weekdate,
	...
	FROM (
		SELECT
			...
			MaxWeekdate = CAST('12/31/2020' as smalldatetime), --@MaxWeekdate,
...
END ELSE IF @MediaType = 'R' BEGIN
    ...
    IF (SELECT UNITS FROM dbo.RADIO_HDR WHERE ORDER_NBR = @OrderNumber) <> 'CM'
	    SELECT
			    @MinWeekdate = MIN(MinWeekdate),
			    @MaxRevision = MAX(REV_NBR),
			    @MaxWeekdate = MAX(WeekdateEnd),
