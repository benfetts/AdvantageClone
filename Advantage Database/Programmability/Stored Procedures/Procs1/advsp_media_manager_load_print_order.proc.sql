...
END ELSE IF @MediaType = 'I' BEGIN
	...
	SELECT
		...
		[Impressions] = CASE
							...
							WHEN d.GUARANTEED_IMPRESS <> 0 AND mpd.GUARANTEED_FLAG = 1 AND d.COST_RATE <> 0 AND mpd.COST_PER_FLAG = 1 THEN 
								CASE WHEN d.COST_TYPE = 'NA' THEN
									REPLACE(CONVERT(VARCHAR(50), (CAST(d.GUARANTEED_IMPRESS AS money)), 1), '.00', '')
								ELSE
									REPLACE(CONVERT(VARCHAR(50), (CAST(d.GUARANTEED_IMPRESS AS money)), 1), '.00', '') + ' @ ' + CAST(COST_RATE as VARCHAR(20)) + ' ' + d.COST_TYPE
								END
							...
							WHEN d.COST_RATE <> 0 AND mpd.COST_PER_FLAG = 1 THEN CAST(COST_RATE as VARCHAR(20)) + ' ' + d.COST_TYPE
						END,
		[BillTypeFlag] = d.BILL_TYPE_FLAG,
		[ShippingAddressFlag] = mpd.SHIP_ADDR_FLAG,
		[IncludeRep1] = mpd.INCLUDE_REP1,
