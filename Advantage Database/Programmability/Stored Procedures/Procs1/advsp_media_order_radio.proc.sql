...
CREATE PROCEDURE [dbo].[advsp_media_order_radio] ( @user_code varchar(100) )
AS
BEGIN
	SET NOCOUNT ON;
...
-- ==========================================================
-- Temp table RadioOrderDtlFilterMaxSeq
CREATE TABLE #RadioOrderDtlFilterMaxSeq(
	[ORDER_NBR] int NULL,
...
INSERT INTO #RadioOrderDtlFilterMaxSeq
SELECT d.ORDER_NBR,
	d.LINE_NBR,
	d.REV_NBR,
	MAX(d.SEQ_NBR),
...
-- SELECT * FROM #RadioOrderDtlFilterMaxSeq
...
SELECT h.ORDER_DATE AS OrderDate, 
	...
	CASE s.SUNDAY
		...
		ELSE '-'
	END AS DAYS	
FROM #RadioOrderDtlFilterMaxSeq AS f
INNER JOIN dbo.RADIO_DETAIL1 AS d
...
WHERE ISNULL(d.LINE_CANCELLED,0) = 0 
	...
	AND h.REV_NBR = (SELECT MAX(h2.REV_NBR) FROM dbo.RADIO_HEADER AS h2 WHERE h.ORDER_NBR = h2.ORDER_NBR)
