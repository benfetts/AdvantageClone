CREATE PROCEDURE [dbo].[advsp_npr_spot_tv_puerto_rico_research_results_trend]
...
--declare @MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID int,
--	    @NPR_STATION_IDs varchar(max),
--        @debug bit

--        set @MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID = 3
--        set @NPR_STATION_IDs = '3'
--        set @debug=1
	...
	DECLARE @ADJUST_MINUTES smallint,
            ...
            @UniverseIntabEndDate smalldatetime
	...
	SELECT @ReportType = REPORT_TYPE, @ShowProgramName = SHOW_PROGRAM_NAME, @SubtotalByWeekdayWeekend  = SUBTOTAL_BY_WEEKDAY_WEEKEND,
        @StartDate = SHARE_START_DATE, @EndDate = DATEADD(d,1,SHARE_END_DATE), @UniverseIntabEndDate = SHARE_END_DATE
	...
	DECLARE @AUD_METRICS TABLE (
        ...
        [trend_date] smalldatetime NOT NULL
	)
	...
	SELECT rd.MEDIA_DEMO_ID, SUM(aud.audience), aud.npr_station_id, aud.media_spot_tv_research_daytime_id, aud.number_days, 0, 0, 0, 0, 0, 0, 0, 0, 0, aud.[AdjustedDateTime]
	FROM 
		...
		INNER JOIN (
					SELECT * 
					FROM [dbo].[advtf_npr_audience_spot_tv_research_trend](@MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID, @NPR_STATION_IDs, @StartDate, @EndDate)
					) aud ON nd.CODE = aud.nielsen_demo_code
    WHERE rd.MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID = @MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID
	GROUP BY rd.MEDIA_DEMO_ID, aud.npr_station_id, aud.media_spot_tv_research_daytime_id, aud.number_days, aud.[AdjustedDateTime]
	
IF @debug = 1
	select * from @AUDIENCE

	INSERT @AUD_METRICS (media_demo_id, ue, [media_spot_tv_research_daytime_id], [trend_date])
	SELECT rd.MEDIA_DEMO_ID, SUM(universe.ue), [media_spot_tv_research_daytime_id], [trend_date]
	FROM 
		dbo.MEDIA_DEMO_DETAIL mdd
		INNER JOIN dbo.NIELSEN_DEMO nd ON mdd.NIELSEN_DEMO_ID = nd.NIELSEN_DEMO_ID 
		INNER JOIN dbo.MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_DEMO rd ON rd.MEDIA_DEMO_ID = mdd.MEDIA_DEMO_ID
		...
		INNER JOIN (
					SELECT *
					FROM dbo.advtf_npr_universe_spot_tv_research_trend(@MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID, @StartDate, @UniverseIntabEndDate)
					) universe ON nd.CODE = universe.nielsen_demo_code
    WHERE rd.MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID = @MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID
	GROUP BY rd.MEDIA_DEMO_ID, universe.[media_spot_tv_research_daytime_id], [trend_date]
...
IF @debug = 1
	...
	FROM @AUDIENCE a
		INNER JOIN @AUD_METRICS h ON a.media_demo_id = h.media_demo_id AND a.[media_spot_tv_research_daytime_id] = h.[media_spot_tv_research_daytime_id] AND a.[AdjustedDateTime] = h.trend_date 
	...
	SELECT rd.MEDIA_DEMO_ID, SUM(hutput.avg_hutput), hutput.media_spot_tv_research_daytime_id, hutput.[AdjustedDateTime]
	FROM 
		dbo.MEDIA_DEMO_DETAIL mdd
		INNER JOIN dbo.NIELSEN_DEMO nd ON mdd.NIELSEN_DEMO_ID = nd.NIELSEN_DEMO_ID 
		INNER JOIN dbo.MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_DEMO rd ON rd.MEDIA_DEMO_ID = mdd.MEDIA_DEMO_ID
		...
		INNER JOIN (
					SELECT * 
					FROM dbo.advtf_npr_hutput_spot_tv_research_trend(@MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID, @StartDate, @EndDate)
					) hutput ON nd.CODE = hutput.nielsen_demo_code 
    WHERE rd.MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID = @MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID
	GROUP BY rd.MEDIA_DEMO_ID, hutput.media_spot_tv_research_daytime_id, hutput.[AdjustedDateTime]
...
IF @debug = 1
	...
	FROM @AUDIENCE a
		INNER JOIN @HUTPUT_AVG h ON a.media_demo_id = h.media_demo_id AND a.media_spot_tv_research_daytime_id = h.media_spot_tv_research_daytime_id AND a.[AdjustedDateTime] = h.[AdjustedDateTime] 
...
END
	...
	SELECT rd.MEDIA_DEMO_ID, SUM(intab.avg_intab), [media_spot_tv_research_daytime_id], intab.trend_date
	FROM 
		dbo.MEDIA_DEMO_DETAIL mdd
		INNER JOIN dbo.NIELSEN_DEMO nd ON mdd.NIELSEN_DEMO_ID = nd.NIELSEN_DEMO_ID 
		INNER JOIN dbo.MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_DEMO rd ON rd.MEDIA_DEMO_ID = mdd.MEDIA_DEMO_ID
		...
		INNER JOIN (
					SELECT * 
					FROM dbo.advtf_npr_intab_spot_tv_research_trend(@MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID, @StartDate, @UniverseIntabEndDate)
					) intab ON nd.CODE = intab.nielsen_demo_code 
    WHERE rd.MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID = @MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID
	GROUP BY rd.MEDIA_DEMO_ID, intab.[media_spot_tv_research_daytime_id], intab.trend_date
...
IF @debug = 1
	...
	UPDATE a SET
		...
		show_intab_warning = 
			...
			END
	FROM @AUDIENCE a
		INNER JOIN @INTAB_AVG h ON a.media_demo_id = h.media_demo_id AND a.[media_spot_tv_research_daytime_id] = h.[media_spot_tv_research_daytime_id] AND a.[AdjustedDateTime] = h.trend_date 
	...
	SELECT
		...
		ProgramName = CASE WHEN @ShowProgramName = 1 THEN
								dbo.advfn_npr_program_get(npr_station_id, [AdjustedDateTime], [AdjustedDateTime], d.START_HOUR, d.END_HOUR,
    ...
    WHERE rd.MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID = @MEDIA_SPOT_TV_PUERTO_RICO_RESEARCH_ID
    AND [AdjustedDateTime] between @StartDate AND @UniverseIntabEndDate
END
GO
