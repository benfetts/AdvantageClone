IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[advsp_pmt_mgr_acom]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[advsp_pmt_mgr_acom]
GO
...
CREATE PROCEDURE [dbo].[advsp_pmt_mgr_acom] 
...
DECLARE @is_pm_dir varchar(100), @is_pm_ftp varchar(254)
DECLARE @is_office_code varchar(4), @is_office_desc varchar(60)
DECLARE @is_export_file varchar(max)
...
BEGIN TRY
	...
	SELECT @is_pm_type=COALESCE(PYMT_MGR_TYPE, ''),	@is_pm_acct=COALESCE(BK_ACCOUNT_NO, ''), @is_bank_desc=COALESCE(BK_DESCRIPTION, ''), @is_pm_id=COALESCE(PYMT_MGR_ID, ''), 
			 ...
			 @is_pm_dir=COALESCE(PYMT_MGR_DIR, ''), @is_pm_ftp=COALESCE(PYMT_MGR_FTP, ''), @is_office_code=COALESCE(OFFICE_CODE, '')
	FROM   BANK
	WHERE  BK_CODE = @as_bank;

	SELECT @is_office_desc = OFFICE_NAME 
	FROM OFFICE
	WHERE OFFICE_CODE = @is_office_code;
			...
			BEGIN
				...
				IF @is_pm_type = 'ACOM' BEGIN
					...
					IF @ap_chk_nbr_last <> @ap_chk_nbr BEGIN
						...
						SET @ls_output =  '"' + 'H' + '"'
						SET @ls_output =  @ls_output  + ',"' + UPPER(@is_office_desc) + '"'
						
						SET @ls_output =  @ls_output  + ',"' + '' + '"' /* Blank */

						SET @ls_output =  @ls_output  + ',"' + @is_pm_acct + '"'

						SET @ls_output =  @ls_output  + ',"' + @pay_to_code + '"' /** Vendor Code **/
						SET @ls_output =  @ls_output  + ',"' + @vn_name + '"' /** Vendor Name **/
						SET @ls_output =  @ls_output  + ',"' + @ap_chk_nbr + '"' /**  Check Nbr **/
						...
						SET @ls_output =  @ls_output  + ',"' + @chk_date + '"' /** Check Date **/

						SELECT @inv_cnt = COUNT(1)
						FROM @pmt_mgr_invoices
						WHERE AP_CHK_NBR = @ap_chk_nbr_last

						SET @ls_output =  @ls_output  + ',' + CAST(@inv_cnt AS varchar(10)) + '' /** Inv Cnt **/

						BEGIN
							IF @vn_pay_to_address1 IS NULL SET @vn_pay_to_address1 = ''
							IF @vn_pay_to_address2 IS NULL SET @vn_pay_to_address2 = ''

							SET @vn_pay_to_address3 =  RTRIM(UPPER(LEFT((@vn_pay_to_city), 21))) /** Vendor City **/
							SET @vn_pay_to_address3 =  @vn_pay_to_address3 + ',' + RTRIM(UPPER(LEFT((@vn_pay_to_state), 2)))  /** Vendor State **/

							IF @vn_pay_to_zip > ''
								SELECT @vn_pay_to_zip
							ELSE
								SET @vn_pay_to_zip = '00000'

							SET @vn_pay_to_address3 =  @vn_pay_to_address3 + ',' + RTRIM(UPPER(LEFT((@vn_pay_to_zip), 5))) /** Vendor Zip **/	
						...
						BEGIN
							SET @ls_output =  @ls_output  + ',"' + UPPER((@vn_pay_to_address1 )) + '"' /** Vendor Addr 1 **/
							SET @ls_output =  @ls_output  + ',"' + UPPER((@vn_pay_to_address2 )) + '"' /** Vendor Addr 2 **/
							SET @ls_output =  @ls_output  + ',"' + UPPER((@vn_pay_to_address3 )) + '"' /** Vendor Addr 3 **/

							SET @ls_output =  @ls_output  + ',"' + '' + '"' /* Blank */  /** Vendor Addr 4 **/
							SET @ls_output =  @ls_output  + ',"' + '' + '"' /* Blank */  /** Vendor Addr 5 **/
							SET @ls_output =  @ls_output  + ',"' + '' + '"' /* Blank */  /** Filler **/
							SET @ls_output =  @ls_output  + ',"' + '' + '"' /* Blank */  /** Filler **/
							SET @ls_output =  @ls_output  + ',"' + '' + '"' /* Blank */  /** Filler **/
							...
							SET @ls_output =  @ls_output  + ',"' + COALESCE(@emp_code,'N/A') + '"'  /** Emp Code **/	
				...
				--**************************
					...
					SET @ls_output =  @ls_output  + ',"' + UPPER(@ap_inv_vchr) + '"' /** Invoice Nbr */
					...
					SET @ls_output =  @ls_output  + ',"' + @str_amt + '"' /** Net Amt **/
					...
					SET @ls_output =  @ls_output  + ',"' + @str_amt + '"' /** Gross Amt **/
					...
					SET @ls_output =  @ls_output  + ',"' + @str_amt + '"' /** Disc Amt **/

					SET @ls_output =  @ls_output  + ',"' + '0.00' + '"' /** Future Use -Inv Pmts to date **/

					SET @ls_output =  @ls_output  + ',"' + @ap_desc + '"' /** AP Desc - CHk Stub Notes **/
