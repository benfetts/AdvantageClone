...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[advsp_pmt_mgr_citidirect]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[advsp_pmt_mgr_citidirect]
GO
...
CREATE PROCEDURE [dbo].[advsp_pmt_mgr_citidirect] 
...
BEGIN TRY
	...
	IF LEN(@is_pm_acct) = 0
	BEGIN
		SET @is_log = @is_log + 'Bank Account Number is missing.  ' + CHAR(13)
		SET @success = 0
	END

	IF LEN(@is_pm_acct) > 10
	BEGIN
		SET @is_log = @is_log + 'Bank Account Number must be 10 digits or less.  ' + CHAR(13)
		SET @success = 0
	END
	...
	SET @is_output_file = 'CITD.' + @is_pm_acct + '.' + REPLACE(CONVERT(VARCHAR(10), GETDATE(), 101), '/', '') +  '.' + REPLACE(CONVERT(VARCHAR(12), GETDATE(), 114), ':','') + '0' + '.txt'
	...
	DECLARE @rule_off_date smalldatetime, @rule_off_date_str varchar(30), @check_date_raw smalldatetime
		...
		BEGIN
			...
			IF @success = 1
				...
				BEGIN  
					...
					SET @ls_output = '#' /** Place Holder **/

					SET @ls_output = @ls_output + 'NAD' /** Pmt Method **/
					...
					SET @ls_output = @ls_output + '#' + RIGHT(@is_pm_acct, 10) /** Bank Account **/

					SET @ls_output = @ls_output  + '#' + 'SINGLE' /** Range **/

					SET @ls_output = @ls_output  + '#' + '' /** Currency - Not required **/

					SET @ls_output = @ls_output  + '#' + '' /** Acct Name - Not required **/

					SET @ls_output =  @ls_output + '#' + RIGHT((@check_date), 4) + '' + LEFT((@check_date), 2) + '' + SUBSTRING((@check_date), 3, 2) /** Check Date **/

					SET @str_amt = @check_amt

					IF LEN(@str_amt) > 11 	
					BEGIN
						SET @is_log = 'Check amount exceeds maximum amount for Check No: ' + @check_nbr + ', Vendor: ' + @pay_to + '.' + CHAR(13)
						SET @success = 0
					END

					SET @ls_output =  @ls_output + '#' + RIGHT(@str_amt, 11) /* Dec 8,2 */

					IF LEN(@check_nbr) > 12
					BEGIN
						SET @is_log = 'Check number exceeds maximum length for Vendor: ' + @pay_to + '.' + CHAR(13)
						SET @success = 0
					END
				
					SET @ls_output =  @ls_output + '#' + LEFT(@check_nbr, 12) /** Check Nbr */

					SET @ls_output = @ls_output  + '#' + '' /** Plan # - Optional **/

					SET @ls_output = @ls_output  + '#' + '' /** Addl Data - Optional **/
				
					SET @ls_output =  @ls_output + '#' + (LEFT((dbo.advfn_remove_char(@pay_to, '#')), 60)) /** Vendor Name **/	
					/* alternate method  ->  dbo.advfn_clear_special_characters(@pay_to, '') */
					
					SET @ls_output = @ls_output  + '#' + '' /** Vendor Name 2 = Optional **/		

					SET @ls_output = @ls_output  + '#' + '' /** FSI = Optional **/

					SET @rule_off_date =  DATEADD(m, 1, @check_date_raw)

					SET @rule_off_date_str = RIGHT(CONVERT(VARCHAR(8), @rule_off_date, 112), 8)


					SET @rule_off_date_str = SUBSTRING((@rule_off_date_str), 5, 2) + '/' + '01' + '/' + LEFT((@rule_off_date_str), 4) /** Rule Off Date **/

					SET @rule_off_date =  @rule_off_date_str

					SET @rule_off_date =  DATEADD(d, -1, @rule_off_date)

					SET @rule_off_date_str = REPLACE(CONVERT(VARCHAR(10), @rule_off_date, 101), '/', '')

					SET @rule_off_date_str =  RIGHT((@rule_off_date_str), 4) + '' + LEFT((@rule_off_date_str), 2) + '' + SUBSTRING((@rule_off_date_str), 3, 2)
					--SUBSTRING((@rule_off_date_str), 5, 2) + '/' + LEFT((@rule_off_date_str), 2) + '/' + LEFT((@rule_off_date_str), 4) /** Rule Off Date **/
			
					SET @ls_output =  @ls_output + '#' + @rule_off_date_str 

					SET @ls_output = @ls_output  + '#' + '' /** Placeholder 17 **/

					SET @ls_output = @ls_output  + '#' + '' /** Placeholder 18 **/

					SET @ls_output = @ls_output  + '#' + '' /** Placeholder 19 **/

					SET @ls_output = @ls_output  + '#' + '' /** Placeholder 20 **/

					IF @void_flag = 'VOID'
						SET @ls_output = @ls_output  + '#' + 'Y' /** Void Issuance **/
					ELSE
						SET @ls_output = @ls_output  + '#' + 'N' /** Void Issuance **/

					SET @ls_output = @ls_output  + '#' + '' /** Placeholder 22 **/

					SET @ls_output = @ls_output  + '#' + '' /** Placeholder 23 **/

					SET @ls_output = @ls_output  + '#' + '' /** Placeholder 24 **/

					SET @ls_output = @ls_output  + '#' + '' /** Placeholder 25 **/

					SET @ls_output = @ls_output  + '#' + '' /** Placeholder 26 **/

					SET @ls_output = @ls_output  + '#' + '' /** Placeholder 27 **/

					SET @ls_output = @ls_output  + '#' + '' /** Placeholder 28 **/

					SET @ls_output = @ls_output  + '#' /** Ends Field 28 **/
...
GRANT EXECUTE ON [advsp_pmt_mgr_citidirect] TO PUBLIC
GO