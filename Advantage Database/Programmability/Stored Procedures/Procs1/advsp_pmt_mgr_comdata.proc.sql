...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[advsp_pmt_mgr_comdata]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[advsp_pmt_mgr_comdata]
GO
...
CREATE PROCEDURE [dbo].[advsp_pmt_mgr_comdata] 
...
BEGIN TRY
	...
	SET @is_output_file = @is_pm_acct + '.' + @is_pm_id + '.PS00020.' + REPLACE(CONVERT(VARCHAR(10), GETDATE(), 101), '/', '') +  '.' + REPLACE(CONVERT(VARCHAR(12), GETDATE(), 114), ':','') + '0'
	...
	IF @success = 1 AND @rec_cnt > 0
		BEGIN
			...
			--SET @ls_hdr = @ls_hdr + Char(9) + UPPER(LEFT((@is_pm_id + SPACE(0)), 10)) /** Customer ID **/
			SET @ls_hdr = @ls_hdr + Char(9) + UPPER(LEFT((@is_pm_acct + SPACE(0)), 5)) /** Account Code **/
			SET @ls_hdr = @ls_hdr + Char(9) + UPPER(LEFT((CAST((@rec_cnt) AS varchar(8)) + SPACE(0)), 8)) /** Record Count **/					
			...
			BEGIN  
				...
				SET @ls_hdr = UPPER(LEFT((@is_pm_acct + SPACE(0)), 5)) /** Account Code **/
				SET @ls_hdr = @ls_hdr + Char(9) + UPPER(LEFT((@is_pm_id + SPACE(0)), 10)) /** Customer ID **/
				SET @ls_hdr = @ls_hdr + Char(9) + UPPER(LEFT((@is_pm_word + SPACE(0)), 20)) /** Word **/	
				
				SET @ls_output = @ls_hdr		
			
				SET @ls_mail = @pymt_mgr_email
				IF (@ls_mail = '') SET @ls_mail = @vn_email
			
				--SET @ls_output = @ls_hdr /** from above */
				SET @ls_output =  @ls_output + Char(9)  + UPPER(LEFT((@pay_to_code), 25)) + ''/** Vendor Code **/
				
				SET @ls_output =  @ls_output + Char(9) /** Location Code **/
				
				SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@vn_name), 50)) + ''/** Vendor Name **/
				
				SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@ls_mail), 50)) + ''/** Vendor Email **/
				SET @ls_output =  @ls_output + Char(9) /** Email 2 **/
				...
				BEGIN
					SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@vn_pay_to_address1), 50)) + '' /** Vendor Addr 1 **/
					SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@vn_pay_to_address2), 50)) + '' /** Vendor Addr 2 **/
					--SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@vn_pay_to_address3), 60)) + '' /** Vendor Addr 3 **/
					--SET @ls_output =  @ls_output + Char(9) + '' /** Vendor Addr 4 **/
					SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@vn_pay_to_city), 30)) + ''/** Vendor City **/
					SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@vn_pay_to_state), 2)) + '' /** Vendor State **/
					SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@vn_pay_to_zip), 10)) + '' /** Vendor Zip **/		
					--SET @vn_pay_to_country = 'US'
					--SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@vn_pay_to_country), 2)) + '' /** Country **/	
					SET @temp = REPLACE(@vn_fax_number,'(', '')
					SET @temp = REPLACE(@temp,')', '')
					SET @temp = REPLACE(@temp,'-', '')
					SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT(@temp, 10)) + '' /** Vendor PHONE **/	
				END				
				
				BEGIN
					SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((SPACE(0)), 50)) + '' /** Name **/
					SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((SPACE(0)), 50)) + '' /** Addr  **/		
					SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((SPACE(0)), 50)) + '' /** Addr  **/
					SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((SPACE(0)), 30)) + '' /** City **/	
					SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((SPACE(0)), 2)) + '' /** State**/
					SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((SPACE(0)), 10)) + '' /** Zip **/	
					SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((SPACE(0)), 10)) + '' /** Phone **/
					SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((SPACE(0)), 50)) + '' /** Account **/																		
				END
				
				SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@ap_inv_vchr), 25)) + '' /** Invoice Nbr */
				SET @ls_output =  @ls_output + Char(9) + LEFT((@ap_inv_date), 2) + SUBSTRING((@ap_inv_date), 3, 2) + RIGHT((@ap_inv_date), 4) + '' /** Invoice Date **/
				SET @ls_output =  @ls_output + Char(9) + LEFT((@ap_inv_date), 2) + SUBSTRING((@ap_inv_date), 3, 2) + RIGHT((@ap_inv_date), 4) + '' /** Due Date **/
				SET @ls_output =  @ls_output + Char(9) + LEFT((@payment_date), 2) + SUBSTRING((@payment_date), 3, 2) + RIGHT((@payment_date), 4) + '' /** Pmt Date - Today **/
				SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@ap_chk_nbr), 10)) + '' /** Check Nbr-Pmt Nbr */

				SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@ap_desc ), 60)) + '' /** AP Desc **/
				
				SET @ls_output =  @ls_output + Char(9)  /** Purchase Order Number - 15 **/				
								
				SET @inv_gross_amt = @ap_chk_amt + @ap_disc_amt /* Gross Amt */

				--SET @ls_output =  @ls_output + Char(9) /** Gross Amt **/
			
				SET @str_amt = @inv_gross_amt
				SET @str_amt = REPLACE(@str_amt,'.', '') /* Implied Decimal */
				IF LEN(@str_amt) > 10
				BEGIN
				...
				SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@str_amt), 10)) + '' /** Gross **/
				...
				SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@str_amt), 10)) + '' /** Net **/
				...
				SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@str_amt), 10)) + '' /**Discount**/
		
				--SET @ls_output =  @ls_output + Char(9) + '' /** Virtual Card Inv No **/ 
				/* PJH 07/26/18 - changed to use Chk Nbr */
				SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@ap_chk_nbr), 25)) + '' /** Check Nbr as Virtual Card Inv No **/ 
				--SET @ls_output =  @ls_output + Char(9) + '' /** GL Code **/		
				SET @ls_output =  @ls_output + Char(9) + '' /** Vendor Fax **/		
				SET @ls_output =  @ls_output + Char(9) + '' /** N/A - 1 **/		
				SET @ls_output =  @ls_output + Char(9) + '' /** N/A - 2 **/		
				SET @ls_output =  @ls_output + Char(9) + '' /** N/A - 3 **/		
				
				SELECT @vn_tax_id = ISNULL(@vn_tax_id, 'N/A')
				IF @vn_tax_id = 'N/A' 
					SET @ls_output =  @ls_output + Char(9) + '' /** Vendor Tax Id **/
				ELSE
					SET @ls_output =  @ls_output + Char(9) + UPPER(LEFT((@vn_tax_id), 20)) + '' /** Vendor Tax Id **/
				
				
				SET @ls_output =  @ls_output + Char(9) + '' /** Filler **/
				SET @ls_output =  @ls_output + Char(9) + '' /** Filler **/
				SET @ls_output =  @ls_output + Char(9) + '' /** Filler **/
				SET @ls_output =  @ls_output + Char(9) + '' /** Filler **/
				SET @ls_output =  @ls_output + Char(9) + '' /** Filler **/
...
GRANT EXECUTE ON [advsp_pmt_mgr_comdata] TO PUBLIC AS dbo
