...
ELSE BEGIN
	...
	ELSE BEGIN
		...
		SELECT @found = 1 FROM [dbo].[CDP_CONTACT_HDR] WHERE [CL_CODE] = @cl_code AND [CDP_CONTACT_ID] = @cdp_contact_id
...
ELSE BEGIN
	...
	ELSE BEGIN
		...
		SELECT @found = 1 FROM [CDP_CONTACT_HDR] A JOIN 
						[CDP_CONTACT_DTL] B ON A.CDP_CONTACT_ID = B.CDP_CONTACT_ID
		WHERE CL_CODE = @cl_code AND B.DIV_CODE = @div_code AND A.CDP_CONTACT_ID = @cdp_contact_id
...
ELSE BEGIN
	...
	ELSE BEGIN
		...
		SELECT @found = 1 FROM [CDP_CONTACT_HDR] A JOIN 
						[CDP_CONTACT_DTL] B ON A.CDP_CONTACT_ID = B.CDP_CONTACT_ID
		WHERE CL_CODE = @cl_code AND B.DIV_CODE = @div_code AND B.PRD_CODE = @prd_code AND A.CDP_CONTACT_ID = @cdp_contact_id
