IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[advsp_zip_by_radius]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[advsp_zip_by_radius]
GO
...
CREATE PROCEDURE [dbo].[advsp_zip_by_radius] 
	@zip_in nvarchar(5),
...
--SELECT DISTINCT (DISTANCE - .75) MILES, ZIP_CODE, STATE_ABBR FROM (
--SELECT [dbo].[advfn_calculate_distance] (@Longitude1, @Latitude1, LONGITUDE, LATITUDE ) + 1 DISTANCE, ZIP_CODE, STATE_ABBR 
--FROM ZIP_CODE_RADIUS
--WHERE ([dbo].[advfn_calculate_distance] (
--	@Longitude1,
--	@Latitude1,
--	LONGITUDE,
--	LATITUDE ) - 1) < @max_distance
--) B
--ORDER BY ZIP_CODE
