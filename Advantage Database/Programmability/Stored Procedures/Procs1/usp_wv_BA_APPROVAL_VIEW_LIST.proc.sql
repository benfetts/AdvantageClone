--usp_wv_BA_APPROVAL_VIEW_LIST
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_BA_APPROVAL_VIEW_LIST]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_BA_APPROVAL_VIEW_LIST]
GO
...
CREATE PROCEDURE [dbo].[usp_wv_BA_APPROVAL_VIEW_LIST]
	@MONTH SMALLINT,
	@YEAR SMALLINT,
	@BA_EMP_CODE VARCHAR(6)
AS
	...
	FROM         
		BILL_APPR_BATCH WITH(NOLOCK) LEFT OUTER JOIN
		EMPLOYEE WITH(NOLOCK) ON BILL_APPR_BATCH.EMP_CODE = EMPLOYEE.EMP_CODE
	...
	WHERE
		...
		AND BILL_APPR_BATCH.EMP_CODE = @BA_EMP_CODE;
