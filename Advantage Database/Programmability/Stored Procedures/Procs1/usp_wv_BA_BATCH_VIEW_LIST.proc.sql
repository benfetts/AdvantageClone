if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_BA_BATCH_VIEW_LIST]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_BA_BATCH_VIEW_LIST]
GO
...
CREATE PROCEDURE [dbo].[usp_wv_BA_BATCH_VIEW_LIST]
@MONTH SMALLINT,
@YEAR SMALLINT,
...
AS
	...
	IF @UNFINISHED IS NULL OR @UNFINISHED = 0
	BEGIN
	--MY BATCHES, INCLUDES UNFINISHED FOR SELECTED MONTH/YEAR
		SELECT     
	...
	ELSE
	BEGIN
		--MY UNFINISHED BATCHES, EXCLUDE BATCHES THAT ARE IN PREVIOUS DATATABLE
		SELECT     
			BILL_APPR_BATCH.BA_BATCH_ID, 
			BILL_APPR_BATCH.BA_BATCH_DESC, 
			BILL_APPR_BATCH.BATCH_DATE, 
		...
		FROM BILL_APPR_BATCH WITH(NOLOCK) 
			LEFT OUTER JOIN EMPLOYEE WITH(NOLOCK) ON BILL_APPR_BATCH.EMP_CODE = EMPLOYEE.EMP_CODE
			LEFT OUTER JOIN SEC_USER WITH(NOLOCK) ON UPPER(BILL_APPR_BATCH.CREATE_USER) = UPPER(SEC_USER.USER_CODE)
		WHERE (BILL_APPR_BATCH.FINISHED IS NULL OR BILL_APPR_BATCH.FINISHED = 0)
				AND UPPER(BILL_APPR_BATCH.CREATE_USER) = UPPER(@USER_CODE)
		AND BILL_APPR_BATCH.BA_BATCH_ID NOT IN
		(
			SELECT     
				BILL_APPR_BATCH.BA_BATCH_ID
			FROM BILL_APPR_BATCH WITH(NOLOCK) 
		...
		ORDER BY BILL_APPR_BATCH.BATCH_DATE DESC;
	END
