if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_Dashboard_GetAvgHourlyRateByProduct]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_Dashboard_GetAvgHourlyRateByProduct]
GO

CREATE PROCEDURE [dbo].[usp_wv_Dashboard_GetAvgHourlyRateByProduct]
(
	@EmpCode varchar(6),
...
SELECT @sql = 'SELECT DASH_DATA_EMP_TIME.CLIENT, CLIENT.CL_NAME, DASH_DATA_EMP_TIME.DIVISION, DIVISION.DIV_NAME, DASH_DATA_EMP_TIME.PRODUCT, PRODUCT.PRD_DESCRIPTION,
			...
			SELECT @sql = @sql + ' FROM DASH_DATA_EMP_TIME INNER JOIN
				 CLIENT ON DASH_DATA_EMP_TIME.CLIENT = CLIENT.CL_CODE LEFT OUTER JOIN
                      ...
                      DIVISION ON DASH_DATA_EMP_TIME.CLIENT = DIVISION.CL_CODE AND DASH_DATA_EMP_TIME.DIVISION = DIVISION.DIV_CODE LEFT OUTER JOIN EMPLOYEE ON EMPLOYEE.EMP_CODE = DASH_DATA_EMP_TIME.EMP_CODE'
			If @Period = 1
					Begin
...
SELECT @sql = @sql + ' GROUP BY DASH_DATA_EMP_TIME.CLIENT, CLIENT.CL_NAME, DASH_DATA_EMP_TIME.DIVISION, DIVISION.DIV_NAME, DASH_DATA_EMP_TIME.PRODUCT, PRODUCT.PRD_DESCRIPTION'
			If @Period = 1
				Begin
					SELECT @sql = @sql + ', #client.BILLED, #client.BILLED_AMT'
				End
