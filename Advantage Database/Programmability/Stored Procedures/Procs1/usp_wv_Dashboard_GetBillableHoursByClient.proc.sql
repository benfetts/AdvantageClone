if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_Dashboard_GetBillableHoursByClient]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_Dashboard_GetBillableHoursByClient]
GO


CREATE PROCEDURE [dbo].[usp_wv_Dashboard_GetBillableHoursByClient]
(
	@EmpCode varchar(6),
...
Begin
	if @StartDate <> '' and @EndDate <> ''
		Begin
			SELECT DASH_DATA_EMP_TIME.CLIENT, CLIENT.CL_NAME, SUM(BILLABLE_HOURS) AS BILLABLE, SUM(DIRECT_NONBILL_HOURS) AS NON_BILLABLE
...
Else
Begin
	SELECT @sql = 'SELECT DASH_DATA_EMP_TIME.CLIENT, CLIENT.CL_NAME, SUM(BILLABLE_HOURS) AS BILLABLE, SUM(DIRECT_NONBILL_HOURS) AS NON_BILLABLE
				FROM DASH_DATA_EMP_TIME INNER JOIN CLIENT ON DASH_DATA_EMP_TIME.CLIENT = CLIENT.CL_CODE 
