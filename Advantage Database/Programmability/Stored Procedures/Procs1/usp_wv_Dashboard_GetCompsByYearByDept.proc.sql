if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_Dashboard_GetCompsByYearByDept]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_Dashboard_GetCompsByYearByDept]
GO
...
CREATE PROCEDURE [dbo].[usp_wv_Dashboard_GetCompsByYearByDept]
(
	@EmpCode varchar(6),
...
if @Level = 'C'
Begin
	SELECT @sql = '	SELECT DISTINCT CAST(DATEPART(yy,DASH_DATA_PROJ_HDR.JOB_COMP_DATE) AS varchar(4)) AS YEAR_OPENED, DATEADD(yy, DATEDIFF(yy, 0, JOB_COMP_DATE), 0) AS YEAR_START,
					DATEADD(dd,-1,DATEADD(yy,0,DATEADD(yy,DATEDIFF(yy,0,JOB_COMP_DATE)+1,0))) AS YEAR_END 
					...
					IF @Restrictions > 0
						...
						End						
					SELECT @sql = @sql + ' ORDER BY DATEADD(yy, DATEDIFF(yy, 0, JOB_COMP_DATE), 0)'
	...
	SELECT @sql = 'SELECT DISTINCT DASH_DATA_PROJ_HDR.CL_CODE, DASH_DATA_PROJ_HDR.CL_NAME
					FROM DASH_DATA_PROJ_HDR'
					if @Project = 'Due' OR @Project = 'Pending' OR @Project = 'Cancelled' OR @Project = 'Cancelled'
						Begin
							SELECT @sql = @sql + ' INNER JOIN JOB_TRAFFIC ON DASH_DATA_PROJ_HDR.JOB = JOB_TRAFFIC.JOB_NUMBER AND DASH_DATA_PROJ_HDR.JOB_COMPONENT_NBR = JOB_TRAFFIC.JOB_COMPONENT_NBR'
						End
					if @Office <> ''
	...
	SET @sql = ''
					...
					if @Project = 'Created'
						Begin
							SELECT @sql = 'SELECT CAST(DATEPART(yy,DASH_DATA_PROJ_HDR.JOB_COMP_DATE) AS varchar(4)) AS YEAR_OPENED,'
						End
					if @Project = 'Completed' or @Project = 'Pending' or @Project = 'Cancelled'
						Begin
							SELECT @sql = 'SELECT CAST(DATEPART(yy,DASH_DATA_PROJ_HDR.COMPLETED_DATE) AS varchar(4)) AS YEAR_OPENED,'
						End
					if @Project = 'Due'
						Begin
							SELECT @sql = 'SELECT CAST(DATEPART(yy,DASH_DATA_PROJ_HDR.JOB_FIRST_USE_DATE) AS varchar(4)) AS YEAR_OPENED,'
						End	
	SELECT @sql = @sql + ' DASH_DATA_PROJ_HDR.CL_CODE, DASH_DATA_PROJ_HDR.CL_NAME, COUNT(*) AS COMPS
					...
					if @Project = 'Created'
						Begin
							SELECT @sql = @sql + ' , CAST(DATEPART(yy,DASH_DATA_PROJ_HDR.JOB_COMP_DATE) AS varchar(4))'
						End
					if @Project = 'Completed' or @Project = 'Pending' or @Project = 'Cancelled'
						Begin
							SELECT @sql = @sql + ' , CAST(DATEPART(yy,DASH_DATA_PROJ_HDR.COMPLETED_DATE) AS varchar(4))'
						End
					if @Project = 'Due'
						Begin
							SELECT @sql = @sql + ' , CAST(DATEPART(yy,DASH_DATA_PROJ_HDR.JOB_FIRST_USE_DATE) AS varchar(4))'
						End	
				SELECT @paramlist = '@EmpCode varchar(6), @StartDate SmallDatetime, @EndDate SmallDatetime, @Office varchar(4000), @Dept varchar(4000), @AE varchar(4000), @Client varchar(4000), @SalesClass varchar(4000), @JobType varchar(4000), @CancelledCode VARCHAR(100), @EMP_CODE AS VARCHAR(6), @UserID varchar(100)'
				print @sql
				EXEC sp_executesql @sql, @paramlist, @EmpCode, @StartDate, @EndDate, @Office, @Dept, @AE, @Client, @SalesClass, @JobType, @CancelledCode, @EMP_CODE, @UserID
