...
CREATE PROCEDURE [dbo].[usp_wv_Dashboard_GetDirectHoursWithAgency]
(
	@EmpCode varchar(6),
	@IsGauge tinyint,
...
Begin
	if @StartDate <> '' and @EndDate <> ''
		Begin
			SELECT DASH_DATA_EMP_TIME.EMP_CODE, isnull(EMP_FNAME,'') + ' ' + isnull(EMP_MI,'') + case when EMP_MI is not null then '. ' else '' end +  isnull(EMP_LNAME,'') AS EMP_DESC, SUM(DIRECT_HOURS) AS DIRECT, SUM(DIRECT_AGENCY_HOURS) AS AGENCY
...
Begin
	...
	Begin
		if @StartDate <> '' and @EndDate <> ''
			Begin
				SELECT SUM(DIRECT_HOURS) AS DIRECT, SUM(DIRECT_AGENCY_HOURS) AS AGENCY
	...
	Begin
		if @StartDate <> '' and @EndDate <> ''
			Begin
				SELECT '' AS TOTAL, '' AS TOTAL, SUM(DIRECT_HOURS) AS DIRECT, SUM(DIRECT_AGENCY_HOURS) AS AGENCY
				FROM DASH_DATA_EMP_TIME
				WHERE DASH_DATA_EMP_TIME.EMP_DATE >= @StartDate AND DASH_DATA_EMP_TIME.EMP_DATE <= @EndDate
			End
		Else
