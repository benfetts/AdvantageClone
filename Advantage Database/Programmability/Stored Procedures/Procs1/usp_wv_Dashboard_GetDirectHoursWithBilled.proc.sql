if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_Dashboard_GetDirectHoursWithBilled]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_Dashboard_GetDirectHoursWithBilled]
GO

CREATE PROCEDURE [dbo].[usp_wv_Dashboard_GetDirectHoursWithBilled]
(
	@EmpCode varchar(6),
	@IsGauge tinyint,
		...
		Begin
			If @Period = 1
				Begin
					...
					SELECT DASH_DATA_EMP_TIME.EMP_CODE, isnull(EMP_FNAME,'') + ' ' + isnull(EMP_MI,'') + case when EMP_MI is not null then '. ' else '' end +  isnull(EMP_LNAME,'') AS EMP_DESC, SUM(DIRECT_HOURS) AS DIRECT, @TotalBilled AS BILLED
			...
			Else
				Begin
					SELECT DASH_DATA_EMP_TIME.EMP_CODE, isnull(EMP_FNAME,'') + ' ' + isnull(EMP_MI,'') + case when EMP_MI is not null then '. ' else '' end +  isnull(EMP_LNAME,'') AS EMP_DESC, SUM(DIRECT_HOURS) AS DIRECT, ISNULL(SUM(BILLED_HOURS), 0) AS BILLED
					FROM DASH_DATA_EMP_TIME INNER JOIN
						EMPLOYEE ON DASH_DATA_EMP_TIME.EMP_CODE = EMPLOYEE.EMP_CODE
					WHERE DASH_DATA_EMP_TIME.EMP_CODE = @EmpCode AND DASH_DATA_EMP_TIME.EMP_DATE >= @StartDate AND DASH_DATA_EMP_TIME.EMP_DATE <= @EndDate
					GROUP BY DASH_DATA_EMP_TIME.EMP_CODE, EMP_FNAME, EMP_MI, EMP_LNAME
...
Begin
	...
	if @IsGauge = 1
	Begin		
		SELECT @sql = 'SELECT SUM(DIRECT_HOURS) AS DIRECT,'
				If @Period = 1
					Begin
	...
	Else
	Begin
		SELECT @sql = 'SELECT '''' AS TOTAL, '''' AS TOTAL, SUM(DIRECT_HOURS) AS DIRECT, '
				If @Period = 1
					Begin
						SELECT @sql = @sql + ' @TotalBilled AS BILLED'
					End
