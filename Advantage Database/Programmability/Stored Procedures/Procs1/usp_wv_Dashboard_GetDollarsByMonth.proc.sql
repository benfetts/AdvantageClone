if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_Dashboard_GetDollarsByMonth]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_Dashboard_GetDollarsByMonth]
GO
...
CREATE PROCEDURE [dbo].[usp_wv_Dashboard_GetDollarsByMonth]
(
	@EmpCode varchar(6),
...
Declare @Restrictions Int,
		@sql nvarchar(4000),
		@paramlist nvarchar(4000), @PPMonth int, @no_of_days int, @RowCount int, @OfficeRestrictions int, @EMP_CODE AS VARCHAR(6), @TotalDollars decimal(15,2)
...
Begin
	...
	SELECT @sql = 'SELECT @TotalDollars ='
				IF @Option = 'Estimated'
					Begin
						SELECT @sql = @sql + ' ISNULL(SUM(DASH_DATA_PROJ_DTL.EST_TOTAL),0)'
					End
				IF @Option = 'Actual'
					Begin
						SELECT @sql = @sql + ' ISNULL(SUM(DASH_DATA_PROJ_DTL.TOTAL_BILL),0)'
					End
				IF @Option = 'Billable'
					Begin
						SELECT @sql = @sql + ' ISNULL(SUM(DASH_DATA_PROJ_DTL.TOTAL_BILL),0) - SUM(DASH_DATA_PROJ_DTL.NB_AMT)'
					End
				IF @Option = 'Fee'
					Begin
						SELECT @sql = @sql + ' ISNULL(SUM(DASH_DATA_PROJ_DTL.FEE_TIME_AMT),0)'
					End
				IF @Option = 'Non Billable'
					Begin
						SELECT @sql = @sql + ' ISNULL(SUM(DASH_DATA_PROJ_DTL.NB_AMT),0)'
					End	 
				SELECT @sql = @sql + ' FROM JOB_COMPONENT INNER JOIN
						 DASH_DATA_PROJ_DTL ON JOB_COMPONENT.JOB_NUMBER = DASH_DATA_PROJ_DTL.JOB_NUMBER AND 
						 JOB_COMPONENT.JOB_COMPONENT_NBR = DASH_DATA_PROJ_DTL.JOB_COMPONENT_NBR INNER JOIN
					...
					IF @Restrictions > 0
						Begin
							SELECT @sql = @sql + ' AND (UPPER(SEC_CLIENT.USER_ID) = UPPER(@UserID)) AND (SEC_CLIENT.TIME_ENTRY = 0 OR SEC_CLIENT.TIME_ENTRY IS NULL)'
						End	
				SELECT @paramlist = '@EmpCode varchar(6), @StartDate SmallDatetime, @EndDate SmallDatetime, @Office varchar(4000), @Dept varchar(4000), @AE varchar(4000), @Client varchar(4000), @SalesClass varchar(4000), @JobType varchar(4000), @CancelledCode VARCHAR(100), @EMP_CODE varchar(6), @UserID varchar(100), @TotalDollars decimal(15,2) OUTPUT'
				print @sql
				EXEC sp_executesql @sql, @paramlist, @EmpCode, @StartDate, @EndDate, @Office, @Dept, @AE, @Client, @SalesClass, @JobType, @CancelledCode, @EMP_CODE, @UserID, @TotalDollars OUTPUT
	...
	SELECT @sql = 'SELECT JOB_LOG.CL_CODE, CLIENT.CL_NAME,'
				IF @Option = 'Estimated'
					Begin
						SELECT @sql = @sql + ' ISNULL(SUM(DASH_DATA_PROJ_DTL.EST_TOTAL),0) AS DOLLARS, CASE WHEN @TotalDollars > 0 THEN (ISNULL(SUM(DASH_DATA_PROJ_DTL.EST_TOTAL),0) / @TotalDollars) * 100 ELSE 0 END AS PERCENT_DOLLARS'
					End
				IF @Option = 'Actual'
					Begin
						SELECT @sql = @sql + ' ISNULL(SUM(DASH_DATA_PROJ_DTL.TOTAL_BILL),0) AS DOLLARS, CASE WHEN @TotalDollars > 0 THEN (ISNULL(SUM(DASH_DATA_PROJ_DTL.TOTAL_BILL),0) / @TotalDollars) * 100 ELSE 0 END AS PERCENT_DOLLARS'
					End
				IF @Option = 'Billable'
					Begin
						SELECT @sql = @sql + ' ISNULL(SUM(DASH_DATA_PROJ_DTL.TOTAL_BILL),0) - ISNULL(SUM(DASH_DATA_PROJ_DTL.NB_AMT),0) AS DOLLARS, CASE WHEN @TotalDollars > 0 THEN ((ISNULL(SUM(DASH_DATA_PROJ_DTL.TOTAL_BILL),0) - ISNULL(SUM(DASH_DATA_PROJ_DTL.NB_AMT),0)) / @TotalDollars) * 100 ELSE 0 END AS PERCENT_DOLLARS'
					End
				IF @Option = 'Fee'
					Begin
						SELECT @sql = @sql + ' ISNULL(SUM(DASH_DATA_PROJ_DTL.FEE_TIME_AMT),0) AS DOLLARS, CASE WHEN @TotalDollars > 0 THEN (ISNULL(SUM(DASH_DATA_PROJ_DTL.FEE_TIME_AMT),0) / @TotalDollars) * 100 ELSE 0 END AS PERCENT_DOLLARS'
					End
				IF @Option = 'Non Billable'
					Begin
						SELECT @sql = @sql + ' ISNULL(SUM(DASH_DATA_PROJ_DTL.NB_AMT),0) AS DOLLARS, CASE WHEN @TotalDollars > 0 THEN (ISNULL(SUM(DASH_DATA_PROJ_DTL.NB_AMT),0) / @TotalDollars) * 100 ELSE 0 END AS PERCENT_DOLLARS'
					End	 
				SELECT @sql = @sql + ' FROM JOB_COMPONENT INNER JOIN
						 DASH_DATA_PROJ_DTL ON JOB_COMPONENT.JOB_NUMBER = DASH_DATA_PROJ_DTL.JOB_NUMBER AND 
						 JOB_COMPONENT.JOB_COMPONENT_NBR = DASH_DATA_PROJ_DTL.JOB_COMPONENT_NBR INNER JOIN
				...
				SELECT @paramlist = '@EmpCode varchar(6), @StartDate SmallDatetime, @EndDate SmallDatetime, @Office varchar(4000), @Dept varchar(4000), @AE varchar(4000), @Client varchar(4000), @SalesClass varchar(4000), @JobType varchar(4000), @CancelledCode VARCHAR(100), @EMP_CODE varchar(6), @UserID varchar(100), @TotalDollars decimal(15,2)'
				print @sql
				EXEC sp_executesql @sql, @paramlist, @EmpCode, @StartDate, @EndDate, @Office, @Dept, @AE, @Client, @SalesClass, @JobType, @CancelledCode, @EMP_CODE, @UserID, @TotalDollars
...
Begin
	...
	SELECT @sql = 'SELECT JOB_LOG.CL_CODE, CLIENT.CL_NAME, JOB_LOG.DIV_CODE, DIVISION.DIV_NAME, JOB_LOG.PRD_CODE, PRODUCT.PRD_DESCRIPTION,'
				...
				SELECT @paramlist = '@EmpCode varchar(6), @StartDate SmallDatetime, @EndDate SmallDatetime, @Office varchar(4000), @Dept varchar(4000), @AE varchar(4000), @Client varchar(4000), @SalesClass varchar(4000), @JobType varchar(4000), @CancelledCode VARCHAR(100), @EMP_CODE varchar(6), @Division varchar(6), @Product varchar(6), @UserID varchar(100), @TotalDollars decimal(15,2)'
				print @sql
				EXEC sp_executesql @sql, @paramlist, @EmpCode, @StartDate, @EndDate, @Office, @Dept, @AE, @Client, @SalesClass, @JobType, @CancelledCode, @EMP_CODE, @Division, @Product, @UserID, @TotalDollars
