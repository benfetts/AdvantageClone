if exists (select * from sysobjects where id = object_id(N'[dbo].[usp_wv_Dashboard_GetHours]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_Dashboard_GetHours]
GO
...
CREATE PROCEDURE [dbo].[usp_wv_Dashboard_GetHours]
(
	@EmpCode varchar(6),
...
if @Level = 'C'
Begin
	SELECT @sql = 'SELECT DASH_DATA_PROJ_HDR.CL_CODE, DASH_DATA_PROJ_HDR.CL_NAME, ISNULL(SUM(DASH_DATA_PROJ_DTL.HOURS_QTY),0) AS HOURS
					FROM DASH_DATA_PROJ_HDR INNER JOIN
                      DASH_DATA_PROJ_DTL ON DASH_DATA_PROJ_HDR.JOB = DASH_DATA_PROJ_DTL.JOB_NUMBER AND 
                      DASH_DATA_PROJ_HDR.JOB_COMPONENT_NBR = DASH_DATA_PROJ_DTL.JOB_COMPONENT_NBR'
					...
					if @Project = 'Cancelled'
						Begin
							...
							Else
								Begin
									SELECT @sql = @sql + ' AND (NOT (DASH_DATA_PROJ_HDR.JOB_PROCESS_CONTRL IN (6, 12))) AND (((DASH_DATA_PROJ_HDR.COMPLETED_DATE >= @StartDate) AND 
									(DASH_DATA_PROJ_HDR.COMPLETED_DATE <= @EndDate)) OR DASH_DATA_PROJ_HDR.COMPLETED_DATE IS NULL) AND (JOB_TRAFFIC.TRF_CODE = @CancelledCode)'
								End							
						End			
					IF @Restrictions > 0
						Begin
	...
	SELECT @paramlist = '@EmpCode varchar(6), @StartDate varchar(12), @EndDate varchar(12), @Office varchar(4000), @Dept varchar(4000), @AE varchar(4000), @Client varchar(4000), @SalesClass varchar(4000), @JobType varchar(4000), @CancelledCode VARCHAR(100), @EMP_CODE varchar(6)'
	print @sql
	EXEC sp_executesql @sql, @paramlist, @EmpCode, @StartDate, @EndDate, @Office, @Dept, @AE, @Client, @SalesClass, @JobType, @CancelledCode, @EMP_CODE		
	SET @sql = ''
...
if @Level = 'CD'
Begin
	SELECT @sql = 'SELECT DASH_DATA_PROJ_HDR.CL_CODE, DASH_DATA_PROJ_HDR.CL_NAME, DASH_DATA_PROJ_HDR.DIV_CODE, DASH_DATA_PROJ_HDR.DIV_NAME, ISNULL(SUM(DASH_DATA_PROJ_DTL.HOURS_QTY),0) AS HOURS
					FROM DASH_DATA_PROJ_HDR INNER JOIN
                      DASH_DATA_PROJ_DTL ON DASH_DATA_PROJ_HDR.JOB = DASH_DATA_PROJ_DTL.JOB_NUMBER AND 
                      DASH_DATA_PROJ_HDR.JOB_COMPONENT_NBR = DASH_DATA_PROJ_DTL.JOB_COMPONENT_NBR'
					if @Project = 'Due' OR @Project = 'Pending' OR @Project = 'Cancelled'
...
if @Level = 'CDP'
Begin
	SELECT @sql = 'SELECT DASH_DATA_PROJ_HDR.CL_CODE, DASH_DATA_PROJ_HDR.CL_NAME, DASH_DATA_PROJ_HDR.DIV_CODE, DASH_DATA_PROJ_HDR.DIV_NAME, DASH_DATA_PROJ_HDR.PRD_CODE, DASH_DATA_PROJ_HDR.PRD_DESCRIPTION, ISNULL(SUM(DASH_DATA_PROJ_DTL.HOURS_QTY),0) AS HOURS
					FROM DASH_DATA_PROJ_HDR INNER JOIN
                      DASH_DATA_PROJ_DTL ON DASH_DATA_PROJ_HDR.JOB = DASH_DATA_PROJ_DTL.JOB_NUMBER AND 
                      DASH_DATA_PROJ_HDR.JOB_COMPONENT_NBR = DASH_DATA_PROJ_DTL.JOB_COMPONENT_NBR'
					if @Project = 'Due' OR @Project = 'Pending' OR @Project = 'Cancelled'
...
Begin
	...
	SELECT @sql = 'SELECT DASH_DATA_PROJ_HDR.CL_CODE, DASH_DATA_PROJ_HDR.CL_NAME, DASH_DATA_PROJ_HDR.DIV_CODE, DASH_DATA_PROJ_HDR.DIV_NAME, DASH_DATA_PROJ_HDR.PRD_CODE, DASH_DATA_PROJ_HDR.PRD_DESCRIPTION, ACCT_EXEC, dbo.udf_get_empl_name(DASH_DATA_PROJ_HDR.ACCT_EXEC, ''FML'') AS ACCT_NAME, ISNULL(SUM(DASH_DATA_PROJ_DTL.HOURS_QTY),0) AS HOURS
					FROM DASH_DATA_PROJ_HDR INNER JOIN
                      DASH_DATA_PROJ_DTL ON DASH_DATA_PROJ_HDR.JOB = DASH_DATA_PROJ_DTL.JOB_NUMBER AND 
                      DASH_DATA_PROJ_HDR.JOB_COMPONENT_NBR = DASH_DATA_PROJ_DTL.JOB_COMPONENT_NBR'
					if @Project = 'Due' OR @Project = 'Pending' OR @Project = 'Cancelled'
	...
	SELECT @sql = 'SELECT ACCT_EXEC, dbo.udf_get_empl_name(DASH_DATA_PROJ_HDR.ACCT_EXEC, ''FML'') AS ACCT_NAME, ISNULL(SUM(DASH_DATA_PROJ_DTL.HOURS_QTY),0) AS HOURS
					FROM DASH_DATA_PROJ_HDR INNER JOIN
                      DASH_DATA_PROJ_DTL ON DASH_DATA_PROJ_HDR.JOB = DASH_DATA_PROJ_DTL.JOB_NUMBER AND 
                      DASH_DATA_PROJ_HDR.JOB_COMPONENT_NBR = DASH_DATA_PROJ_DTL.JOB_COMPONENT_NBR'
					if @Project = 'Due' OR @Project = 'Pending' OR @Project = 'Cancelled'
...
Begin
	...
	SELECT @sql = 'SELECT DASH_DATA_PROJ_HDR.CL_CODE, DASH_DATA_PROJ_HDR.CL_NAME, DASH_DATA_PROJ_HDR.DIV_CODE, DASH_DATA_PROJ_HDR.DIV_NAME, DASH_DATA_PROJ_HDR.PRD_CODE, DASH_DATA_PROJ_HDR.PRD_DESCRIPTION, DASH_DATA_PROJ_HDR.DP_TM_CODE, DP_TM_DESC, ISNULL(SUM(DASH_DATA_PROJ_DTL.HOURS_QTY),0) AS HOURS
					FROM DASH_DATA_PROJ_HDR INNER JOIN
                      DASH_DATA_PROJ_DTL ON DASH_DATA_PROJ_HDR.JOB = DASH_DATA_PROJ_DTL.JOB_NUMBER AND 
                      DASH_DATA_PROJ_HDR.JOB_COMPONENT_NBR = DASH_DATA_PROJ_DTL.JOB_COMPONENT_NBR'
					if @Project = 'Due' OR @Project = 'Pending' OR @Project = 'Cancelled'
	...
	SELECT @sql = 'SELECT DASH_DATA_PROJ_HDR.DP_TM_CODE, DP_TM_DESC, ISNULL(SUM(DASH_DATA_PROJ_DTL.HOURS_QTY),0) AS HOURS
					FROM DASH_DATA_PROJ_HDR INNER JOIN
                      DASH_DATA_PROJ_DTL ON DASH_DATA_PROJ_HDR.JOB = DASH_DATA_PROJ_DTL.JOB_NUMBER AND 
                      DASH_DATA_PROJ_HDR.JOB_COMPONENT_NBR = DASH_DATA_PROJ_DTL.JOB_COMPONENT_NBR'
					if @Project = 'Due' OR @Project = 'Pending' OR @Project = 'Cancelled'
...
Begin
	...
	SELECT @sql = 'SELECT DASH_DATA_PROJ_HDR.CL_CODE, DASH_DATA_PROJ_HDR.CL_NAME, DASH_DATA_PROJ_HDR.DIV_CODE, DASH_DATA_PROJ_HDR.DIV_NAME, DASH_DATA_PROJ_HDR.PRD_CODE, DASH_DATA_PROJ_HDR.PRD_DESCRIPTION, DASH_DATA_PROJ_HDR.SC_CODE, SC_DESCRIPTION, ISNULL(SUM(DASH_DATA_PROJ_DTL.HOURS_QTY),0) AS HOURS
					FROM DASH_DATA_PROJ_HDR INNER JOIN
                      DASH_DATA_PROJ_DTL ON DASH_DATA_PROJ_HDR.JOB = DASH_DATA_PROJ_DTL.JOB_NUMBER AND 
                      DASH_DATA_PROJ_HDR.JOB_COMPONENT_NBR = DASH_DATA_PROJ_DTL.JOB_COMPONENT_NBR'
					if @Project = 'Due' OR @Project = 'Pending' OR @Project = 'Cancelled'
	...
	SELECT @sql = 'SELECT DASH_DATA_PROJ_HDR.SC_CODE, SC_DESCRIPTION, ISNULL(SUM(DASH_DATA_PROJ_DTL.HOURS_QTY),0) AS HOURS
					FROM DASH_DATA_PROJ_HDR INNER JOIN
                      DASH_DATA_PROJ_DTL ON DASH_DATA_PROJ_HDR.JOB = DASH_DATA_PROJ_DTL.JOB_NUMBER AND 
                      DASH_DATA_PROJ_HDR.JOB_COMPONENT_NBR = DASH_DATA_PROJ_DTL.JOB_COMPONENT_NBR'
					if @Project = 'Due' OR @Project = 'Pending' OR @Project = 'Cancelled'
...
if @Level = 'JT'
Begin
	SELECT @sql = 'SELECT DISTINCT DASH_DATA_PROJ_HDR.JOB_TYPE, ISNULL(JT_DESC,'''') AS JT_DESC
					FROM DASH_DATA_PROJ_HDR INNER JOIN
                      DASH_DATA_PROJ_DTL ON DASH_DATA_PROJ_HDR.JOB = DASH_DATA_PROJ_DTL.JOB_NUMBER AND 
                      DASH_DATA_PROJ_HDR.JOB_COMPONENT_NBR = DASH_DATA_PROJ_DTL.JOB_COMPONENT_NBR'
					if @Project = 'Due' OR @Project = 'Pending' OR @Project = 'Cancelled'
	...
	SELECT @sql = 'SELECT DASH_DATA_PROJ_HDR.CL_CODE, DASH_DATA_PROJ_HDR.CL_NAME, DASH_DATA_PROJ_HDR.DIV_CODE, DASH_DATA_PROJ_HDR.DIV_NAME, DASH_DATA_PROJ_HDR.PRD_CODE, DASH_DATA_PROJ_HDR.PRD_DESCRIPTION, DASH_DATA_PROJ_HDR.JOB_TYPE, ISNULL(JT_DESC,'''') AS JT_DESC, ISNULL(SUM(DASH_DATA_PROJ_DTL.HOURS_QTY),0) AS HOURS
					FROM DASH_DATA_PROJ_HDR INNER JOIN
                      DASH_DATA_PROJ_DTL ON DASH_DATA_PROJ_HDR.JOB = DASH_DATA_PROJ_DTL.JOB_NUMBER AND 
                      DASH_DATA_PROJ_HDR.JOB_COMPONENT_NBR = DASH_DATA_PROJ_DTL.JOB_COMPONENT_NBR'
					if @Project = 'Due' OR @Project = 'Pending' OR @Project = 'Cancelled'
	...
	SELECT @sql = 'SELECT DASH_DATA_PROJ_HDR.JOB_TYPE, ISNULL(JT_DESC,'''') AS JT_DESC, ISNULL(SUM(DASH_DATA_PROJ_DTL.HOURS_QTY),0) AS HOURS
					FROM DASH_DATA_PROJ_HDR INNER JOIN
                      DASH_DATA_PROJ_DTL ON DASH_DATA_PROJ_HDR.JOB = DASH_DATA_PROJ_DTL.JOB_NUMBER AND 
                      DASH_DATA_PROJ_HDR.JOB_COMPONENT_NBR = DASH_DATA_PROJ_DTL.JOB_COMPONENT_NBR'
					if @Project = 'Due' OR @Project = 'Pending' OR @Project = 'Cancelled'
