if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_Dashboard_GetHoursByWeek]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_Dashboard_GetHoursByWeek]
GO
...
CREATE PROCEDURE [dbo].[usp_wv_Dashboard_GetHoursByWeek]
(
	@EmpCode varchar(6),
...
Begin
	...
	SELECT @sql = 'SELECT DATEPART(week,DASH_DATA_PROJ_DTL.DATE) AS DATE_OPENED, JOB_LOG.CL_CODE, CLIENT.CL_NAME,'
				...
				IF @Option = 'Billable'
					Begin
						SELECT @sql = @sql + ' ISNULL(SUM(DASH_DATA_PROJ_DTL.HOURS_QTY),0) - ISNULL(SUM(DASH_DATA_PROJ_DTL.NB_QTY),0) AS HOURS'
					End
				IF @Option = 'Fee'
					Begin
						SELECT @sql = @sql + ' ISNULL(SUM(DASH_DATA_PROJ_DTL.FEE_TIME_HRS),0) AS HOURS'
					End
				IF @Option = 'Non Billable'
					Begin
						SELECT @sql = @sql + ' ISNULL(SUM(DASH_DATA_PROJ_DTL.NB_QTY),0) AS HOURS'
					End	 
