...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_campaign_search_jobnew]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_campaign_search_jobnew]
GO


CREATE PROCEDURE [dbo].[usp_wv_campaign_search_jobnew] 
    @UserID			VARCHAR(100),
...
SET @sql_where = ' WHERE CMP_TYPE = 2 '
...
If @DivisionCode <> ''
 Begin
	SET @sql_where = @sql_where + ' AND (CAMPAIGN.DIV_CODE = ''' + @DivisionCode + ''' OR CAMPAIGN.DIV_CODE IS NULL)'
...
If @ProductCode  <> ''
 Begin
	SET @sql_where = @sql_where + ' AND (CAMPAIGN.PRD_CODE = ''' + @ProductCode + ''' OR CAMPAIGN.PRD_CODE IS NULL)'
...
If @InclClosedStr = '0' 
	SET @sql_where = @sql_where + ' AND (ISNULL(CMP_CLOSED,0) = 0) '
