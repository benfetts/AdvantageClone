/****** Object:  StoredProcedure [dbo].[usp_wv_campaign_search_popup_v2]    Script Date: 3/11/2019 2:32:18 PM ******/
SET ANSI_NULLS OFF
GO
...
22-Mar-2010 - jtg - Created
	exec usp_wv_campaign_search_popup_v2 'ama','','','','',0

****************************************************************************************/
CREATE PROCEDURE [dbo].[usp_wv_campaign_search_popup_v2] 
    @UserID			VARCHAR(100),
...
SET @sql = 'SELECT DISTINCT CAMPAIGN.CMP_CODE AS Code, '
...
SET @sql = @sql + ',CAMPAIGN.CMP_NAME Name'
SET @sql = @sql + ',CAMPAIGN.CMP_IDENTIFIER ID'
SET @sql = @sql + ',CAMPAIGN.DIV_CODE DivisionCode'
SET @sql = @sql + ',CAMPAIGN.PRD_CODE ProductCode'
SET @sql = @sql + ',CAMPAIGN.CL_CODE ClientCode'
...
set @sql = @sql + ' ORDER BY CAMPAIGN.CMP_CODE DESC'
