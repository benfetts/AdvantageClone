/****** Object:  StoredProcedure [dbo].[usp_wv_dd_GetAllJobComps]    Script Date: 5/10/2019 9:33:56 AM ******/
SET ANSI_NULLS OFF
GO
...
/*
	exec usp_wv_dd_GetAllJobComps 'ama','abc' , '', '', '', '', 0, '',0,'','', 1
*/

CREATE PROCEDURE [dbo].[usp_wv_dd_GetAllJobComps] (
	@UserID VarChar(100),
	@Text varchar(100),
...
AS
	...
	IF @ClientCode is null AND @DivisionCode is null AND @ProductCode is null and @JobCode is null and @Text is null
	BEGIN
		select @recordCount = count(*) FROM JOB_LOG 
				INNER JOIN JOB_COMPONENT ON JOB_LOG.JOB_NUMBER = JOB_COMPONENT.JOB_NUMBER
				 where JOB_LOG.COMP_OPEN = 1  OR @ShowClosed = 1
	END
	ELSE
	BEGIN
		set @recordCount = 0
	...
	BEGIN
		...
		IF @SprintID = 0
		BEGIN
		set @sql = @sql + ' ltrim(str(JOB_LOG.JOB_NUMBER)) + ''-'' +   ltrim(STR(JOB_COMPONENT.JOB_COMPONENT_NBR)) AS Code,
		...
		BEGIN
		 ...
		 set @sql = @sql + ' AND (JOB_LOG.JOB_NUMBER = ISNULL(@JobCode,JOB_LOG.JOB_NUMBER) or (@JobCode is null and JOB_LOG.JOB_NUMBER is null))
					AND (JOB_COMPONENT.EMP_CODE = ISNULL(@AccountExecutive,JOB_COMPONENT.EMP_CODE) or (@SalesClass is null and JOB_LOG.SC_CODE is null))
					AND (JOB_LOG.SC_CODE = ISNULL(@SalesClass,JOB_LOG.SC_CODE) or (@SalesClass is null and JOB_LOG.SC_CODE is null))
					AND (JOB_COMPONENT.JT_CODE = ISNULL(@JobType,JOB_COMPONENT.JT_CODE) or (@JobType is null and JOB_COMPONENT.JT_CODE is null))
					AND (JOB_LOG.CMP_IDENTIFIER = ISNULL(@CampaignID,JOB_LOG.CMP_IDENTIFIER) or (@CampaignID is null and JOB_LOG.CMP_IDENTIFIER is null))
		...
		set @params = '@OfficeCode VARCHAR(6),@ClientCode VARCHAR(6),@DivisionCode VARCHAR(6),@ProductCode VARCHAR(6),@JobCode int,@AccountExecutive varchar(6),@CampaignID int,@SalesClass varchar(6),@JobType varchar(10),@ShowClosed bit,@EMP_CODE AS VARCHAR(6),@UserID VarChar(100), @SprintID int,@Text varchar(100)'
		exec sp_executesql @sql,@params,@OfficeCode,@ClientCode,@DivisionCode,@ProductCode,@JobCode,@AccountExecutive,@CampaignID,@SalesClass,@JobType,@ShowClosed,@EMP_CODE,@UserID,@SprintID,@Text
	END
	ELSE
	BEGIN
		set @Short = 0
		SELECT '0000' + '-' +   '0000' AS Code,
						CAST(@recordCount as varchar(10)) + ' jobs. Please use client filters first.' AS Description
						,'' CL_CODE
						,CAST(@recordCount as varchar(10)) + ' jobs. Please use client filters first.' Name
						 ,0 JobCode
						 ,'' ClientCode
						 ,'' DivisionCode
						 ,'' ProductCode
