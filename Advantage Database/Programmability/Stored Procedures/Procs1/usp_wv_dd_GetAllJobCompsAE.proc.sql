...
SET ANSI_NULLS OFF 
GO
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[usp_wv_dd_GetAllJobCompsAE]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[usp_wv_dd_GetAllJobCompsAE]
GO
CREATE PROCEDURE [dbo].[usp_wv_dd_GetAllJobCompsAE] /*WITH ENCRYPTION*/
...
/*=========== QUERY ===========*/
	...
	SET @SQL = '
			...
			SELECT     
				LTRIM(STR(JOB_LOG.JOB_NUMBER)) + ''-'' +   LTRIM(STR(JOB_COMPONENT.JOB_COMPONENT_NBR)) AS Code, 
