IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[usp_wv_dd_GetAllJobComps_Estimate_Index]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
    DROP PROCEDURE [dbo].[usp_wv_dd_GetAllJobComps_Estimate_Index]
GO


/*
	declare @index int
	exec usp_wv_dd_GetAllJobComps_Estimate_Index 'ama','','','','',0,426,'',0,'','',0,0,'',@Index=@index OUTPUT
	print @index
*/
CREATE procedure [dbo].[usp_wv_dd_GetAllJobComps_Estimate_Index](
			@UserID VarChar(100),
...
GRANT EXECUTE ON [usp_wv_dd_GetAllJobComps_Estimate_Index] TO PUBLIC AS dbo
GO
