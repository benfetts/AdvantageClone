IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[usp_wv_dd_GetAllJobComps_PS_Index]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
    DROP PROCEDURE [dbo].[usp_wv_dd_GetAllJobComps_PS_Index]
GO
...
/*
	...
	print @index
*/
CREATE procedure [dbo].[usp_wv_dd_GetAllJobComps_PS_Index](
			@UserID VarChar(100),
...
AS
	...
	EXEC sp_executesql @sql, @params, @UserID, @OfficeCode, @ClientCode,@DivisionCode,@ProductCode,@JobCode,@ComponenetID,@AccountExecutive,@CampaignID,@SalesClass,@JobType,@ShowClosed,@EMP_CODE,@Index=@Index OUTPUT

GO

GRANT EXECUTE ON [usp_wv_dd_GetAllJobComps_PS_Index] TO PUBLIC AS dbo
GO
