IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[usp_wv_dd_GetAllJobComps_PS_Paging]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
    DROP PROCEDURE [dbo].[usp_wv_dd_GetAllJobComps_PS_Paging]
GO

/*
	declare @totalRows int
	exec usp_wv_dd_GetAllJobComps_PS_Paging 'EML04','' , '', '', '', 0, '',0,'','',0,'',0,100,@TotalRows=@totalRows OUTPUT
	print @totalRows
*/
CREATE PROCEDURE [dbo].[usp_wv_dd_GetAllJobComps_PS_Paging] (
			@UserID VarChar(100),
...
AS
	...
	set @sql = N'	;WITH CTE AS (SELECT a.*,ROW_NUMBER() OVER(ORDER BY  a.JobCode DESC, a.JobCompCode ASC) ROW_ID FROM (SELECT DISTINCT ltrim(str(JOB_LOG.JOB_NUMBER)) + ''-'' +   ltrim(STR(JOB_COMPONENT.JOB_COMPONENT_NBR)) AS Code,
	...
	set @where = 'WHERE (((NOT (JOB_COMPONENT.JOB_PROCESS_CONTRL IN (6, 12))) AND (JOB_TRAFFIC.COMPLETED_DATE IS NULL)  ) OR @ShowClosed = 1)
	'
	...
	set @join = N'FROM JOB_LOG 
				...
				JOIN JOB_TRAFFIC ON JOB_COMPONENT.JOB_NUMBER = JOB_TRAFFIC.JOB_NUMBER AND JOB_COMPONENT.JOB_COMPONENT_NBR = JOB_TRAFFIC.JOB_COMPONENT_NBR
...
GRANT EXECUTE ON [usp_wv_dd_GetAllJobComps_PS_Paging] TO PUBLIC AS dbo
GO
