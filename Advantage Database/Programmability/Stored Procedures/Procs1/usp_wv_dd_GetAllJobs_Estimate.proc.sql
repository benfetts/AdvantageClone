if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_dd_GetAllJobs_Estimate]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_dd_GetAllJobs_Estimate]
GO
...
CREATE PROCEDURE [dbo].[usp_wv_dd_GetAllJobs_Estimate] 
@UserID VarChar(100)
AS
...
ELSE
	...
	Begin
		...
		WHERE     (JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6, 12)) AND JOB_COMPONENT.ESTIMATE_NUMBER IS NOT NULL
		--And JOB_TRAFFIC.COMPLETED_DATE IS NULL
