...
CREATE PROCEDURE [dbo].[usp_wv_dd_GetJobComp] 
@UserID VARCHAR(100),
...
IF @Restrictions > 0 
    ...
    IF @Job = 0
        BEGIN
             SELECT     
                JOB_COMPONENT.JOB_COMPONENT_NBR AS Code,  RTRIM(LTRIM(STR(JOB_LOG.JOB_NUMBER)))+'-'+RTRIM(LTRIM(STR(JOB_COMPONENT.JOB_COMPONENT_NBR)))+ ' | ' + JOB_COMPONENT.JOB_COMP_DESC +' | '+ @ClientCode+' - '+@DivisionCode+' - '+@ProductCode  AS Description
            FROM         
                JOB_LOG INNER JOIN
    ...
    ELSE
        BEGIN
            SELECT     
                JOB_COMPONENT.JOB_COMPONENT_NBR AS Code,  RTRIM(LTRIM(STR(JOB_COMPONENT.JOB_COMPONENT_NBR))) + ' - ' + JOB_COMPONENT.JOB_COMP_DESC AS Description
            FROM         
                JOB_LOG INNER JOIN
                JOB_COMPONENT ON JOB_LOG.JOB_NUMBER = JOB_COMPONENT.JOB_NUMBER INNER JOIN
                SEC_CLIENT ON JOB_LOG.CL_CODE = SEC_CLIENT.CL_CODE AND JOB_LOG.DIV_CODE = SEC_CLIENT.DIV_CODE AND 
