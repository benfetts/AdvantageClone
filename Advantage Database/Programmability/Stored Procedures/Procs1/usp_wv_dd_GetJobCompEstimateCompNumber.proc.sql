...
CREATE PROCEDURE [dbo].[usp_wv_dd_GetJobCompEstimateCompNumber] 
@UserID VARCHAR(100),
@EstNum INT,
@EstComp INT
AS
        ...
        BEGIN
                SELECT     RTRIM(LTRIM(STR(JOB_LOG.JOB_NUMBER))) + '-' + RTRIM(LTRIM(STR(JOB_COMPONENT.JOB_COMPONENT_NBR))) AS Code, 
                        ...
                        WHERE     (JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6, 12)) AND (JOB_COMPONENT.ESTIMATE_NUMBER = '' OR JOB_COMPONENT.ESTIMATE_NUMBER IS NULL) 
									AND JOB_LOG.JOB_NUMBER = (SELECT JOB_NUMBER FROM JOB_COMPONENT WHERE ESTIMATE_NUMBER = @EstNum AND EST_COMPONENT_NBR = @EstComp)
