...
CREATE PROCEDURE [dbo].[usp_wv_dd_GetJobCompEstimate_withClosed] 
@UserID VARCHAR(100),
                ...
                SELECT     RTRIM(LTRIM(STR(JOB_LOG.JOB_NUMBER))) + '-' + RTRIM(LTRIM(STR(JOB_COMPONENT.JOB_COMPONENT_NBR))) AS Code, 
                        ...
                        WHERE     --(JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6, 12)) AND 
										(JOB_LOG.CL_CODE LIKE @ClientCode + '%') AND (JOB_LOG.DIV_CODE LIKE @DivisionCode + '%') AND 
                                              (JOB_LOG.PRD_CODE LIKE @ProductCode + '%') AND (JOB_COMPONENT.JOB_NUMBER = @Job) AND (UPPER(SEC_CLIENT.USER_ID) = UPPER(@UserID)) AND (SEC_CLIENT.TIME_ENTRY = 0 OR SEC_CLIENT.TIME_ENTRY IS NULL) AND JOB_COMPONENT.ESTIMATE_NUMBER IS NOT NULL 
                                              --AND 
    ...
    ELSE
        BEGIN
                SELECT     RTRIM(LTRIM(STR(JOB_LOG.JOB_NUMBER))) + '-' + RTRIM(LTRIM(STR(JOB_COMPONENT.JOB_COMPONENT_NBR))) AS Code, 
                        ...
                        WHERE     --(JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6, 12)) AND 
									(JOB_LOG.CL_CODE LIKE @ClientCode + '%') AND (JOB_LOG.DIV_CODE LIKE @DivisionCode + '%') AND 
                                              (JOB_LOG.PRD_CODE LIKE @ProductCode + '%') AND (JOB_COMPONENT.JOB_NUMBER = @Job) AND JOB_COMPONENT.ESTIMATE_NUMBER IS NOT NULL --AND (JOB_TRAFFIC.COMPLETED_DATE IS NULL)
                        ORDER BY JOB_LOG.JOB_NUMBER DESC, JOB_COMPONENT.JOB_COMPONENT_NBR ASC
        END
END
        ...
        BEGIN
                SELECT     RTRIM(LTRIM(STR(JOB_LOG.JOB_NUMBER))) + '-' + RTRIM(LTRIM(STR(JOB_COMPONENT.JOB_COMPONENT_NBR))) AS Code, 
                        ...
                        FROM         JOB_LOG INNER JOIN
                                              ...
                                              JOB_LOG.PRD_CODE = SEC_CLIENT.PRD_CODE
                        WHERE     --(JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6, 12)) AND 
									(JOB_COMPONENT.JOB_NUMBER = @Job) AND (UPPER(SEC_CLIENT.USER_ID) = UPPER(@UserID)) AND (SEC_CLIENT.TIME_ENTRY = 0 OR SEC_CLIENT.TIME_ENTRY IS NULL) AND JOB_COMPONENT.ESTIMATE_NUMBER IS NOT NULL 
												--AND 
                                              --(JOB_TRAFFIC.COMPLETED_DATE IS NULL)  
        ...
        BEGIN
                SELECT     RTRIM(LTRIM(STR(JOB_LOG.JOB_NUMBER))) + '-' + RTRIM(LTRIM(STR(JOB_COMPONENT.JOB_COMPONENT_NBR))) AS Code, 
                        ...
                        FROM         JOB_LOG INNER JOIN
                                              JOB_COMPONENT ON JOB_LOG.JOB_NUMBER = JOB_COMPONENT.JOB_NUMBER
                        WHERE     --(JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6, 12)) AND 
									(JOB_COMPONENT.JOB_NUMBER = @Job) AND JOB_COMPONENT.ESTIMATE_NUMBER IS NOT NULL 
