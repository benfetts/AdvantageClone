if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_dd_GetJobCompJJ]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_dd_GetJobCompJJ]
GO
...
CREATE PROCEDURE [dbo].[usp_wv_dd_GetJobCompJJ] 
@UserID VARCHAR(100),
        ...
        ELSE
            BEGIN	
				...
				Else
					Begin
						SELECT     
							STR(JOB_LOG.JOB_NUMBER) + '-' + ltrim(STR(JOB_COMPONENT.JOB_COMPONENT_NBR)) AS Code,  STR(JOB_LOG.JOB_NUMBER) + '-' + ltrim(STR(JOB_COMPONENT.JOB_COMPONENT_NBR)) + ' | ' + JOB_COMPONENT.JOB_COMP_DESC AS Description, JOB_LOG.JOB_NUMBER, JOB_COMPONENT.JOB_COMPONENT_NBR
						FROM         
							JOB_LOG INNER JOIN
							JOB_COMPONENT ON JOB_LOG.JOB_NUMBER = JOB_COMPONENT.JOB_NUMBER INNER JOIN
							SEC_CLIENT ON JOB_LOG.CL_CODE = SEC_CLIENT.CL_CODE AND JOB_LOG.DIV_CODE = SEC_CLIENT.DIV_CODE AND 
