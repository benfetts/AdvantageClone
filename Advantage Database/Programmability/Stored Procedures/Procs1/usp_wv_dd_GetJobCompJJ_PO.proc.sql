if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_dd_GetJobCompJJ_PO]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_dd_GetJobCompJJ_PO]
GO
/* CHANGE LOG:
==========================================================
JTG, 20090529:
...
CREATE PROCEDURE [dbo].[usp_wv_dd_GetJobCompJJ_PO] 
@UserID VARCHAR(100),
    ...
    BEGIN
        IF @Job <> 0
            BEGIN
				...
				Begin
					SELECT     
						ltrim(STR(JOB_LOG.JOB_NUMBER)) + '-' + ltrim(STR(JOB_COMPONENT.JOB_COMPONENT_NBR)) AS Code,  STR(JOB_COMPONENT.JOB_COMPONENT_NBR) + ' - ' + JOB_COMPONENT.JOB_COMP_DESC AS Description
					FROM         
						JOB_LOG INNER JOIN
        ...
        ELSE
            BEGIN
				...
				Begin
					SELECT     
						ltrim(STR(JOB_LOG.JOB_NUMBER)) + '-' + ltrim(STR(JOB_COMPONENT.JOB_COMPONENT_NBR)) AS Code,  STR(JOB_LOG.JOB_NUMBER) + '-' + ltrim(STR(JOB_COMPONENT.JOB_COMPONENT_NBR)) + ' | ' + JOB_COMPONENT.JOB_COMP_DESC AS Description
					FROM         
						JOB_LOG INNER JOIN
						JOB_COMPONENT ON JOB_LOG.JOB_NUMBER = JOB_COMPONENT.JOB_NUMBER INNER JOIN
						SEC_CLIENT ON JOB_LOG.CL_CODE = SEC_CLIENT.CL_CODE AND JOB_LOG.DIV_CODE = SEC_CLIENT.DIV_CODE AND 
