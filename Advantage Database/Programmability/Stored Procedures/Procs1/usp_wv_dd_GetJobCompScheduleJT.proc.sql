...
CREATE PROCEDURE [dbo].[usp_wv_dd_GetJobCompScheduleJT] 
@UserID VARCHAR(100),
@JobType VarChar(6)
AS
DECLARE 
...
SELECT     RTRIM(LTRIM(STR(JOB_LOG.JOB_NUMBER))) + '-' + RTRIM(LTRIM(STR(JOB_COMPONENT.JOB_COMPONENT_NBR))) AS Code, 
        ...
        WHERE     (JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6, 12))  AND (UPPER(SEC_CLIENT.USER_ID) = UPPER(@UserID)) AND (SEC_CLIENT.TIME_ENTRY = 0 OR SEC_CLIENT.TIME_ENTRY IS NULL) AND (JOB_COMPONENT.JT_CODE = @JobType)
...
SELECT     RTRIM(LTRIM(STR(JOB_LOG.JOB_NUMBER))) + '-' + RTRIM(LTRIM(STR(JOB_COMPONENT.JOB_COMPONENT_NBR))) AS Code, 
        ...
        FROM         JOB_LOG INNER JOIN
                              ...
                              JOB_TRAFFIC ON JOB_COMPONENT.JOB_NUMBER = JOB_TRAFFIC.JOB_NUMBER AND 
                              JOB_COMPONENT.JOB_COMPONENT_NBR = JOB_TRAFFIC.JOB_COMPONENT_NBR
        WHERE     (JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6, 12)) AND (JOB_COMPONENT.JT_CODE = @JobType)
        ORDER BY JOB_LOG.JOB_NUMBER DESC, JOB_COMPONENT.JOB_COMPONENT_NBR ASC
    END
