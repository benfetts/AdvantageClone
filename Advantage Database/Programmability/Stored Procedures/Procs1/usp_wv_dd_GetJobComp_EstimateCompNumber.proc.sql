...
CREATE PROCEDURE [dbo].[usp_wv_dd_GetJobComp_EstimateCompNumber] 
@UserID VARCHAR(100),
@EstNum INT,
@EstComp INT
AS
    ...
    IF @Restrictions > 0 
        BEGIN
                SELECT     RTRIM(LTRIM(STR(JOB_LOG.JOB_NUMBER))) + '-' + RTRIM(LTRIM(STR(JOB_COMPONENT.JOB_COMPONENT_NBR))) AS Code, 
                        ...
                        WHERE     (JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6, 12)) AND (JOB_COMPONENT.ESTIMATE_NUMBER = @EstNum) AND (JOB_COMPONENT.EST_COMPONENT_NBR = @EstComp)
