...
CREATE PROCEDURE [dbo].[usp_wv_dd_GetJobComp_EstimateNumber_withClosed] 
@UserID VARCHAR(100),
@EstNum INT
AS
DECLARE 
    ...
    IF @Restrictions > 0 
        BEGIN
                SELECT     RTRIM(LTRIM(STR(JOB_LOG.JOB_NUMBER))) + '-' + RTRIM(LTRIM(STR(JOB_COMPONENT.JOB_COMPONENT_NBR))) AS Code, 
                        ...
                        WHERE     (JOB_COMPONENT.ESTIMATE_NUMBER = @EstNum)--(JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6, 12))  AND 
