...
*/
CREATE PROCEDURE [dbo].[usp_wv_dd_GetJob_ForEstimateCompNumber]
@UserID VarChar(100),
@EstNum INT,
@EstComp INT
...
If @Rescrictions > 0
     ...
     INNER JOIN SEC_CLIENT 
	  ...
	  WHERE ( JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6, 12)) 
	    AND (UPPER(SEC_CLIENT.USER_ID) = UPPER(@UserID)) AND (SEC_CLIENT.TIME_ENTRY = 0 OR SEC_CLIENT.TIME_ENTRY IS NULL) AND JOB_COMPONENT.ESTIMATE_NUMBER = @EstNum AND JOB_COMPONENT.EST_COMPONENT_NBR = @EstComp	    
