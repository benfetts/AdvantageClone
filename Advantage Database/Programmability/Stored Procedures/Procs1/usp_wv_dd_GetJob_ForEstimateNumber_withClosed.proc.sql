...
*/
CREATE PROCEDURE [dbo].[usp_wv_dd_GetJob_ForEstimateNumber_withClosed]
@UserID VarChar(100),
@EstNum INT
...
If @Rescrictions > 0
     ...
     INNER JOIN SEC_CLIENT 
	  ...
	  WHERE --( JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6, 12)) AND 
		(UPPER(SEC_CLIENT.USER_ID) = UPPER(@UserID)) AND (SEC_CLIENT.TIME_ENTRY = 0 OR SEC_CLIENT.TIME_ENTRY IS NULL) AND JOB_COMPONENT.ESTIMATE_NUMBER = @EstNum	    
...
ELSE
     ...
     INNER JOIN JOB_COMPONENT 
	     ON JOB_LOG.JOB_NUMBER = JOB_COMPONENT.JOB_NUMBER
	  WHERE --( JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6, 12) ) AND 
		JOB_COMPONENT.ESTIMATE_NUMBER = @EstNum		   
       Order By JOB_LOG.JOB_NUMBER DESC
