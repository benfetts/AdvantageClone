if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_dd_GetJob_JobJacket]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_dd_GetJob_JobJacket]
GO
/* CHANGE LOG:
==========================================================
ST, 20060613:
...
*/
CREATE PROCEDURE [dbo].[usp_wv_dd_GetJob_JobJacket]
@UserID VarChar(100),
...
If @RestrictionsCP > 0
	...
	WHERE     (JOB_LOG.COMP_OPEN = 1) AND CP_SEC_CLIENT.CDP_CONTACT_ID = UPPER(@CPID)
...
If @Restrictions > 0	
	IF @OfficeCount = 0
		Begin
			...
			WHERE     (JOB_LOG.COMP_OPEN = 1) AND (UPPER(SEC_CLIENT.USER_ID) = UPPER(@UserID)) AND (SEC_CLIENT.TIME_ENTRY = 0 OR SEC_CLIENT.TIME_ENTRY IS NULL)
			--And JOB_TRAFFIC.COMPLETED_DATE IS NULL
			AND  (JOB_LOG.CL_CODE Like @ClientCode) 
			AND (JOB_LOG.DIV_CODE Like @DivisionCode) 
			AND (JOB_LOG.PRD_CODE Like @ProductCode)
	...
	Else
		Begin
			SELECT DISTINCT 
				...
				STR(JOB_LOG.JOB_NUMBER) + ' - ' +isnull(JOB_LOG.JOB_DESC, '') + '(' + JOB_LOG.CL_CODE + ', ' 
				+ JOB_LOG.DIV_CODE + ',' + JOB_LOG.PRD_CODE + ')' AS Description
			FROM         JOB_LOG INNER JOIN EMP_OFFICE ON JOB_LOG.OFFICE_CODE = EMP_OFFICE.OFFICE_CODE
									AND EMP_OFFICE.EMP_CODE = @EMP_CODE
			WHERE     (JOB_LOG.COMP_OPEN = 1) 
			--And JOB_TRAFFIC.COMPLETED_DATE IS NULL
			AND(JOB_LOG.CL_CODE Like @ClientCode) 
