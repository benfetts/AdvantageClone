...
*/
CREATE PROCEDURE [dbo].[usp_wv_dd_GetJobs_ForEstimateSave_withClosed] 
@UserID VarChar(100),
...
If @Rescrictions > 0
 ...
 (UPPER(SEC_CLIENT.USER_ID) = UPPER(@UserID)) AND (SEC_CLIENT.TIME_ENTRY = 0 OR SEC_CLIENT.TIME_ENTRY IS NULL) AND JOB_COMPONENT.ESTIMATE_NUMBER IS NULL
...
ELSE
	...
	AND (JOB_LOG.DIV_CODE Like @DivisionCode) 
	AND (JOB_LOG.PRD_CODE Like @ProductCode) AND JOB_COMPONENT.ESTIMATE_NUMBER IS NULL
	Order By JOB_LOG.JOB_NUMBER DESC
