IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[usp_wv_dd_JobJacket_Estimates_Paging]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
    DROP PROCEDURE [dbo].[usp_wv_dd_JobJacket_Estimates_Paging]
GO

/*
	declare @totalRows int
	exec usp_wv_dd_JobJacket_Estimates_Paging 'SYSADM', '', '', '', '', '', 0,'','',0,0,'',0,31,@TotalRows=@totalRows OUTPUT
	print @totalRows
*/
CREATE PROCEDURE [dbo].[usp_wv_dd_JobJacket_Estimates_Paging] (
			@UserID VarChar(100),
...
GRANT EXECUTE ON [usp_wv_dd_JobJacket_Estimates_Paging] TO PUBLIC AS dbo
GO
