if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_Estimating_GetComments]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_Estimating_GetComments]
GO


CREATE PROCEDURE [dbo].[usp_wv_Estimating_GetComments]
(
	@EST_NBR INT,
...
BEGIN
	SELECT @sql = 'SELECT EQ.ESTIMATE_NUMBER, EQ.EST_COMPONENT_NBR, EQ.EST_QUOTE_NBR, ESTIMATE_REV.EST_REV_NBR, E.FNC_CODE,
			ISNULL(E.EST_FNC_COMMENT,'''') AS EST_FNC_COMMENT, ISNULL(E.EST_FNC_COMMENT_HTML,'''') AS EST_FNC_COMMENT_HTML, ISNULL(E.EST_REV_SUP_BY_NTE,'''') AS EST_REV_SUP_BY_NTE,
			ISNULL(E.EST_REV_SUP_BY_HTML,'''') AS EST_REV_SUP_BY_HTML, ISNULL(EL.EST_LOG_COMMENT,'''') AS EST_LOG_COMMENT, ISNULL(EL.EST_LOG_COMMENT_HTML,'''') AS EST_LOG_COMMENT_HTML, 
			ISNULL(EC.EST_COMP_COMMENT,'''') AS EST_COMP_COMMENT, ISNULL(EC.EST_COMP_COMMENT_HTML,'''') AS EST_COMP_COMMENT_HTML, ISNULL(EQ.EST_QTE_COMMENT,'''') AS EST_QTE_COMMENT,
			ISNULL(EQ.EST_QTE_COMMENT_HTML,'''') AS EST_QTE_COMMENT_HTML, ISNULL(ESTIMATE_REV.EST_REV_COMMENT,'''') AS EST_REV_COMMENT, ISNULL(ESTIMATE_REV.EST_REV_COMMENT_HTML,'''') AS EST_REV_COMMENT_HTML,
			E.EST_REV_RATE, ISNULL(ESTIMATE_REV.JOB_QTY,0) AS JOB_QTY
	...
	WHERE     (EQ.ESTIMATE_NUMBER = @EST_NBR) AND (EQ.EST_COMPONENT_NBR = @EST_COMPONENT_NBR) AND 
						  (EQ.EST_QUOTE_NBR = @EST_QUOTE_NBR) AND (ESTIMATE_REV.EST_REV_NBR = @EST_REV_NBR)'
	...
	--if @FunctionOption = 'N'
	--Begin
	--    SELECT @sql = @sql + '   AND E.SEQ_NBR = @SEQ_NBR'
	--End
	if @EST_REV_RATE <> 0
	Begin
		SELECT @sql = @sql + ' AND E.EST_REV_RATE = @EST_REV_RATE'
	End

SELECT @paramlist = '@EST_NBR INT, @EST_COMPONENT_NBR INT, @EST_QUOTE_NBR INT, @EST_REV_NBR INT, @FNC_CODE VARCHAR(6), @EST_REV_RATE decimal(15,4)'
print @sql
EXEC sp_executesql @sql, @paramlist, @EST_NBR, @EST_COMPONENT_NBR, @EST_QUOTE_NBR, @EST_REV_NBR, @FNC_CODE, @EST_REV_RATE--, @SEQ_NBR
