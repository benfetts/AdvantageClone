if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_Estimating_Print_Details]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_Estimating_Print_Details]
GO



CREATE PROCEDURE [dbo].[usp_wv_Estimating_Print_Details] 
@EstNo Int,
@EstCompNo Int,
@UserID varchar(100),
@Quotes varchar(100)
...
SELECT     @DateToPrint = ISNULL(DATE_TO_PRINT, 0), @TaxSeparate = ISNULL(TAX_SEPARATE, 0), @TaxIndicator = ISNULL(TAX_INDICATOR, 0), @CommMUSeparate = ISNULL(COMM_MU_SEPARATE, 0), @CommMUIndicator = ISNULL(COMM_MU_INDICATOR, 0), @FunctionOption = ISNULL(FUNCTION_OPTION, ''), 
                      ...
                      @ConsoleOverride = ISNULL(CONSOL_OVERRIDE, 0), @SubTotOnly = ISNULL(SUBTOT_ONLY, 0), @ExclEmpTime = ISNULL(EXCL_EMP_TIME, 0), @ExclVendor = ISNULL(EXCL_VENDOR, 0), @ExclIO = ISNULL(EXCL_IO, 0), @ExclNonBill = ISNULL(EXCL_NONBILL,0), @PrtAdNumber = ISNULL(PRT_AD_NBR,0)
...
BEGIN
 ...
 SELECT @sql2 = 'INSERT INTO #estPrint SELECT EQ.ESTIMATE_NUMBER, EL.EST_LOG_DESC, EQ.EST_COMPONENT_NBR, EC.EST_COMP_DESC, EQ.EST_QUOTE_NBR, EQ.EST_QUOTE_DESC, JOB_LOG.JOB_NUMBER,
			JOB_LOG.JOB_DESC, JOB_COMPONENT.JOB_COMPONENT_NBR, JOB_COMPONENT.JOB_COMP_DESC, ESTIMATE_REV.EST_REV_NBR,'
				...
				if @FunctionOption = 'N'
					Begin
						SELECT @sql2 = @sql2 + ' E.SEQ_NBR, E.EST_FNC_COMMENT, E.EST_REV_SUP_BY_CDE, E.EST_REV_SUP_BY_NTE,'
					End	
				Else
					Begin
						SELECT @sql2 = @sql2 + ' 0,'''', '''', '''', '
				...
				Else
					...
					End				
				if @FunctionOption = 'R' or @FunctionOption = 'N'
					Begin
