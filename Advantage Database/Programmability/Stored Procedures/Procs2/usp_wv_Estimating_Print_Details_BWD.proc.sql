IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[usp_wv_Estimating_Print_Details_BWD]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[usp_wv_Estimating_Print_Details_BWD]
GO


CREATE PROCEDURE [dbo].[usp_wv_Estimating_Print_Details_BWD] 
@EstNo Int,
@EstCompNo Int,
@UserID varchar(100),
@Comps varchar(100),
...
SELECT @sql2 = @sql2 + ' ISNULL(ESTIMATE_REV.JOB_QTY,0) AS JOB_QTY, ISNULL(CDP_CONTACT_HDR.CDP_CONTACT_ID,0), ISNULL(CDP_CONTACT_HDR.CONT_CODE,''''), ISNULL(CDP_CONTACT_HDR.CONT_FML,''''), ISNULL(CDP_CONTACT_HDR.CONT_ADDRESS1,''''), 
                    ...
                    Else
                        ...
                        End      
SELECT @sql2 = @sql2 + ', JOB_LOG.CMP_CODE, CAMPAIGN.CMP_NAME, JOB_COMPONENT.JOB_CL_PO_NBR, EMPLOYEE.EMP_TITLE, ISNULL(CDP_CONTACT_HDR.CONT_TITLE,'''')'
...
SELECT @sqlgroupby = @sqlgroupby + ' JOB_LOG.JOB_CLI_REF, EL.SC_CODE, SALES_CLASS.SC_DESCRIPTION, EMPLOYEE.EMP_FNAME, EMPLOYEE.EMP_MI, EMPLOYEE.EMP_LNAME,
                      ...
                      CDP_CONTACT_HDR.CONT_ADDRESS2, CDP_CONTACT_HDR.CONT_CITY, CDP_CONTACT_HDR.CONT_STATE, CDP_CONTACT_HDR.CONT_ZIP, CDP_CONTACT_HDR.CONT_COUNTRY, JOB_LOG.CMP_CODE, CAMPAIGN.CMP_NAME, JOB_COMPONENT.JOB_CL_PO_NBR, EMPLOYEE.EMP_TITLE, CDP_CONTACT_HDR.CONT_TITLE'
                      if @SortOption = 'O' AND ((@ProdConsFunc = 1 AND @ConsoleOverride = 0) OR @FunctionOption = 'C')
                        Begin
...
if @Combine = 0
Begin
	SELECT @sql = 'SELECT *, (Cast(ESTIMATE_NUMBER AS VARCHAR(7))+''/''+Cast(EST_COMPONENT_NBR AS VARCHAR(3))) AS ESTCOMP FROM #estPrint WHERE 1=1'
	if @SuppressZero = 1
	Begin
...
Begin
	...
	SELECT DISTINCT @cdpid = CDP_CONTACT_ID, @contcode = ISNULL(CONT_CODE,''), @contfml = ISNULL(CONT_FML,''), @conta1 = ISNULL(CONT_ADDRESS1,''), @conta2 = ISNULL(CONT_ADDRESS2,''), @contcity = ISNULL(CONT_CITY,''), @contstate = ISNULL(CONT_STATE,''), @contzip = ISNULL(CONT_ZIP,''), @contcountry = ISNULL(CONT_COUNTRY,'')
	FROM #estPrint
	...
	SELECT @sql = @sql + ' SUM(TAX) AS TAX, SUM(JOB_QTY) AS JOB_QTY, ''' + Cast(@cdpid AS Varchar(5)) + ''' AS CDP_CONTACT_ID, ''' + @contcode + ''' AS CONT_CODE, ''' + @contfml + ''' AS CONT_FML, ''' + @conta1 + ''' AS CONT_ADDRESS1,
					 ...
					 ESTCOMPQUOTE, INOUT, FNC_ORDER, CMP_CODE, CMP_NAME, JOB_CL_PO_NBR, EMP_TITLE, CONT_TITLE
	...
	SELECT @sql = @sql + ' SC_CODE, SC_DESCRIPTION, CL_CODE, CL_NAME, DIV_CODE, 
						  DIV_NAME, PRD_CODE, PRD_DESCRIPTION, ESTCOMPQUOTE, INOUT, FNC_ORDER, CMP_CODE, CMP_NAME, JOB_CL_PO_NBR, EMP_TITLE, CONT_TITLE'
