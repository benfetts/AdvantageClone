IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[usp_wv_Estimating_Print_Details_Campaign]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[usp_wv_Estimating_Print_Details_Campaign]
GO
...
CREATE PROCEDURE [dbo].[usp_wv_Estimating_Print_Details_Campaign] 
@EstNo Int,
@EstCompNo Int,
@UserID varchar(100),
@Comps varchar(100),
...
SELECT @sql2 = @sql2 + ' ISNULL(ESTIMATE_REV.JOB_QTY,0) AS JOB_QTY, ISNULL(CDP_CONTACT_HDR.CDP_CONTACT_ID,0), ISNULL(CDP_CONTACT_HDR.CONT_CODE,''''), ISNULL(CDP_CONTACT_HDR.CONT_FML,''''), ISNULL(CDP_CONTACT_HDR.CONT_ADDRESS1,''''), 
                    ...
                    Else
                        ...
                        End      
SELECT @sql2 = @sql2 + ' JOB_COMPONENT.JOB_FIRST_USE_DATE, JOB_LOG.CMP_IDENTIFIER, JOB_LOG.CMP_CODE, CAMPAIGN.CMP_NAME, JOB_COMPONENT.JOB_CL_PO_NBR, CAMPAIGN.CMP_BILL_BUDGET, JOB_LOG.CMP_CODE+''/''+Cast(EQ.EST_QUOTE_NBR AS VARCHAR(3)), CASE WHEN E.EST_FNC_TYPE = ''E'' THEN ''EI'' WHEN E.EST_FNC_TYPE = ''I'' THEN ''EI'' ELSE E.EST_FNC_TYPE END AS FNC_GROUP, ''''' 
...
SELECT @sql = @sql + ' ORDER BY CMPQUOTE, ESTIMATE_NUMBER, EST_COMPONENT_NBR'
