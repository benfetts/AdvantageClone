if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_Estimating_Print_Details_Combined]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_Estimating_Print_Details_Combined]
GO



CREATE PROCEDURE [dbo].[usp_wv_Estimating_Print_Details_Combined] 
@EstNo Int,
@EstCompNo Int,
@UserID varchar(100),
@Comps varchar(100),
...
Begin
	...
	SELECT @sql = @sql + ' SUM(TAX) AS TAX, SUM(JOB_QTY) AS JOB_QTY, ''' + Cast(@cdpid AS Varchar(5)) + ''' AS CDP_CONTACT_ID, ''' + @contcode + ''' AS CONT_CODE, ''' + @contfml + ''' AS CONT_FML, ''' + @conta1 + ''' AS CONT_ADDRESS1,
					 ...
					 ESTCOMPQUOTE, INOUT, FNC_ORDER,CMP_IDENTIFIER, CMP_CODE, CMP_NAME
	...
	SELECT @sql = @sql + ' SC_CODE, SC_DESCRIPTION, CL_CODE, CL_NAME, DIV_CODE, 
						  DIV_NAME, PRD_CODE, PRD_DESCRIPTION, ESTCOMPQUOTE, INOUT, FNC_ORDER,CMP_IDENTIFIER, CMP_CODE, CMP_NAME'
