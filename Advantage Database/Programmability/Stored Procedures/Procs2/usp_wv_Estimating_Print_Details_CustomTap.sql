...
/*
	estimate print details script for Quarry and version 008 and 009
*/


IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[usp_wv_Estimating_Print_Details_CustomTap]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[usp_wv_Estimating_Print_Details_CustomTap]
GO


CREATE PROCEDURE [dbo].[usp_wv_Estimating_Print_Details_CustomTap] 
@EstNo Int,
@EstCompNo Int,
@UserID varchar(100),
@Comps varchar(100),
 ...
 SELECT @sql2 = 'INSERT INTO #estPrint SELECT EQ.ESTIMATE_NUMBER, EL.EST_LOG_DESC, EQ.EST_COMPONENT_NBR, EC.EST_COMP_DESC, EQ.EST_QUOTE_NBR, EQ.EST_QUOTE_DESC, JOB_LOG.JOB_NUMBER,
			JOB_LOG.JOB_DESC, JOB_COMPONENT.JOB_COMPONENT_NBR, JOB_COMPONENT.JOB_COMP_DESC, ESTIMATE_REV.EST_REV_NBR,'
			    if (@ProdConsFunc = 1 AND @ConsoleOverride = 0) OR @FunctionOption = 'C'
			        Begin
			            SELECT @sql2 = @sql2 + ' CASE WHEN FUNCTIONS.FNC_CONSOLIDATION IS NULL OR E.EST_FNC_TYPE <> ''E'' THEN E.FNC_CODE ELSE FUNCTIONS.FNC_CONSOLIDATION END, '''','
			        End
			    Else
			        Begin
			   ...
			   -- if @GroupOption = 'H'
					 ...
					 --else
					 --   Begin
					 --       SELECT @sql2 = @sql2 + ' ISNULL(FNC_HEADING.FNC_HEADING_ID, 0) AS FNC_HEADING_ID, ISNULL(FNC_HEADING.FNC_HEADING_DESC, FUNCTIONS.FNC_DESCRIPTION), FNC_HEADING.FNC_HEADING_SEQ,'
					 --   End 
...
SELECT @sql2 = @sql2 + ' ISNULL(ESTIMATE_REV.JOB_QTY,0) AS JOB_QTY, ISNULL(CDP_CONTACT_HDR.CDP_CONTACT_ID,0), ISNULL(CDP_CONTACT_HDR.CONT_CODE,''''), ISNULL(CDP_CONTACT_HDR.CONT_FML,''''), ISNULL(CDP_CONTACT_HDR.CONT_ADDRESS1,''''), 
                      ...
                      ISNULL(CDP_CONTACT_HDR.CONT_ZIP,''''), ISNULL(CDP_CONTACT_HDR.CONT_COUNTRY,''''), 0, 0, (Cast(EQ.ESTIMATE_NUMBER AS VARCHAR(7))+''/''+Cast(EQ.EST_COMPONENT_NBR AS VARCHAR(2))+''/''+Cast(EQ.EST_QUOTE_NBR AS VARCHAR(3))), (Cast(JOB_LOG.JOB_NUMBER AS VARCHAR(7))+''/''+Cast(JOB_COMPONENT.JOB_COMPONENT_NBR AS VARCHAR(3))+''/''+Cast(EQ.EST_QUOTE_NBR AS VARCHAR(3))),'                      
				    if @GroupOption = 'I'
					    Begin
                    ...
                    Else
                        ...
                        End      
SELECT @sql2 = @sql2 + ' JOB_COMPONENT.JOB_FIRST_USE_DATE, JOB_LOG.CMP_IDENTIFIER, JOB_LOG.CMP_CODE, CAMPAIGN.CMP_NAME, JOB_COMPONENT.JOB_CL_PO_NBR, CAMPAIGN.CMP_BILL_BUDGET, Cast(JOB_LOG.CMP_IDENTIFIER AS VARCHAR(5))+''/''+Cast(EQ.EST_QUOTE_NBR AS VARCHAR(3)), CASE WHEN E.EST_FNC_TYPE = ''E'' THEN ''EI'' WHEN E.EST_FNC_TYPE = ''I'' THEN ''EI'' ELSE E.EST_FNC_TYPE END AS FNC_GROUP, ''''' 
SELECT @sqlfrom = '	FROM ESTIMATE_QUOTE AS EQ INNER JOIN
                      ...
                      CDP_CONTACT_HDR ON JOB_COMPONENT.CDP_CONTACT_ID = CDP_CONTACT_HDR.CDP_CONTACT_ID'

SELECT @sqlwhere = ' WHERE (EQ.ESTIMATE_NUMBER = ''' + Cast(@EstNum AS Varchar(5)) + ''') AND (EQ.EST_COMPONENT_NBR = ''' + Cast(@EstCompNum AS Varchar(5)) + ''') AND 
...
SELECT @sqlgroupby = ' GROUP BY EQ.ESTIMATE_NUMBER, EL.EST_LOG_DESC, EQ.EST_COMPONENT_NBR, EC.EST_COMP_DESC,
                      ...
                      if (@ProdConsFunc = 1 AND @ConsoleOverride = 0) OR @FunctionOption = 'C'
			                Begin
			                    SELECT @sqlgroupby = @sqlgroupby + ' CASE WHEN FUNCTIONS.FNC_CONSOLIDATION IS NULL OR E.EST_FNC_TYPE <> ''E'' THEN E.FNC_CODE ELSE FUNCTIONS.FNC_CONSOLIDATION END,'
			                End
			          Else
			                Begin
					  ...
					  --if @GroupOption = 'H'
					  --  Begin
					  --      SELECT @sqlgroupby = @sqlgroupby + ' FNC_HEADING.FNC_HEADING_ID, ISNULL(FNC_HEADING.FNC_HEADING_DESC, FUNCTIONS.FNC_DESCRIPTION), FNC_HEADING.FNC_HEADING_SEQ, E.EST_FNC_TYPE, FUNCTIONS.FNC_TYPE,'
					  --  End	  
SELECT @sqlgroupby = @sqlgroupby + ' JOB_LOG.JOB_CLI_REF, EL.SC_CODE, SALES_CLASS.SC_DESCRIPTION, EMPLOYEE.EMP_FNAME, EMPLOYEE.EMP_MI, EMPLOYEE.EMP_LNAME,
					  ...
					  JOB_COMPONENT.JOB_FIRST_USE_DATE,JOB_LOG.CMP_IDENTIFIER, JOB_LOG.CMP_CODE, CAMPAIGN.CMP_NAME, JOB_COMPONENT.JOB_CL_PO_NBR, CAMPAIGN.CMP_BILL_BUDGET'
                      if @SortOption = 'O' AND ((@ProdConsFunc = 1 AND @ConsoleOverride = 0) OR @FunctionOption = 'C')
                        Begin
...
BEGIN
 ...
 SELECT @budget = CMP_BILL_BUDGET
 FROM #estPrint
 WHERE CMP_BILL_BUDGET IS NOT NULL

 UPDATE #estPrint
 SET CMP_BILL_BUDGET = @budget
 WHERE CMP_BILL_BUDGET IS NULL
 
 if @func = 'V' or @func = 'I'
 Begin
	UPDATE #estPrint 
	SET EST_REV_QUANTITY = 0
	WHERE FNC_TYPE = @func
 End
UPDATE #estPrint 
...
if @rptType = 'EstimatingTAP'
Begin
	UPDATE #estPrint 
	SET JOB_NUMBER = 0, JOB_COMPONENT_NBR = 0, JOBCOMP = '0/0/0', ESTIMATE_NUMBER = 0, EST_COMPONENT_NBR = 0, ESTCOMPQUOTE = '0/0/0',
		 JOB_DESC = '', JOB_COMP_DESC = '', FNC_CODE='', FNC_DESCRIPTION='', EST_QUOTE_NBR = 0, EST_REV_NBR = 0 
	WHERE FNC_TYPE = 'E' OR FNC_TYPE = 'I'

	SELECT @sql = 'SELECT ESTIMATE_NUMBER,EST_COMPONENT_NBR,EST_QUOTE_NBR,JOB_NUMBER,JOB_DESC,JOB_COMPONENT_NBR,JOB_COMP_DESC,EST_REV_NBR,
					FNC_CODE,FNC_DESCRIPTION,SUM(EST_REV_QUANTITY) AS EST_REV_QUANTITY,SUM(EST_REV_EXT_AMT) AS EST_REV_EXT_AMT,SUM(EXT_MARKUP_AMT) AS EXT_MARKUP_AMT,
					SUM(LINE_TOTAL) AS LINE_TOTAL,SUM(EXT_CONTINGENCY) AS EXT_CONTINGENCY,SUM(EXT_MU_CONT) AS EXT_MU_CONT,
					EST_FNC_TYPE,FNC_TYPE,CL_CODE,CL_NAME,DIV_CODE,DIV_NAME,PRD_CODE,PRD_DESCRIPTION,
					SUM(TAX) AS TAX,ESTCOMPQUOTE,JOBCOMP,CMP_ID,CMPQUOTE,'''' AS CMP_COMMENTS, AE, CMP_NAME,CDP_CONTACT_ID, CONT_CODE, CONT_FML,
					CONT_ADDRESS1, CONT_ADDRESS2, CONT_CITY, CONT_STATE, CONT_ZIP, '''' AS EST_FNC_COMMENT
	...
	End
	SELECT @sql = @sql + ' GROUP BY CMPQUOTE,FNC_TYPE, EST_FNC_TYPE,JOBCOMP,JOB_NUMBER,JOB_COMPONENT_NBR, ESTIMATE_NUMBER,EST_COMPONENT_NBR,EST_QUOTE_NBR,JOB_NUMBER,JOB_DESC,JOB_COMPONENT_NBR,JOB_COMP_DESC,EST_REV_NBR,
					FNC_CODE,FNC_DESCRIPTION,CL_CODE,CL_NAME,DIV_CODE,DIV_NAME,PRD_CODE,PRD_DESCRIPTION,
					ESTCOMPQUOTE,JOBCOMP,CMP_ID, AE, CMP_NAME,CDP_CONTACT_ID, CONT_CODE, CONT_FML,
					CONT_ADDRESS1, CONT_ADDRESS2, CONT_CITY, CONT_STATE, CONT_ZIP'
	SELECT @sql = @sql + ' ORDER BY CMPQUOTE, JOB_NUMBER,JOB_COMPONENT_NBR'
	print @sql
	EXEC (@sql)
End
Else
Begin
	UPDATE #estPrint 
	SET FNC_CODE='', FNC_DESCRIPTION='', FNC_TYPE = '', EST_FNC_TYPE = ''
	WHERE FNC_TYPE = 'E' OR FNC_TYPE = 'I'
