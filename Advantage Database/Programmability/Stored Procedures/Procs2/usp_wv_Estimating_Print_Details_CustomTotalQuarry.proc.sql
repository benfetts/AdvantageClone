...
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[usp_wv_Estimating_Print_Details_CustomTotalQuarry]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[usp_wv_Estimating_Print_Details_CustomTotalQuarry]
GO

CREATE PROCEDURE [dbo].[usp_wv_Estimating_Print_Details_CustomTotalQuarry] 
@EstNo Int,
@EstCompNo Int,
@UserID varchar(100),
@Comps varchar(100),
 ...
 SELECT @sql2 = 'INSERT INTO #estPrint SELECT EQ.ESTIMATE_NUMBER, EL.EST_LOG_DESC, EQ.EST_COMPONENT_NBR, EC.EST_COMP_DESC, EQ.EST_QUOTE_NBR, EQ.EST_QUOTE_DESC, JOB_LOG.JOB_NUMBER,
			JOB_LOG.JOB_DESC, JOB_COMPONENT.JOB_COMPONENT_NBR, JOB_COMPONENT.JOB_COMP_DESC, ESTIMATE_REV.EST_REV_NBR,'
			   ...
			   -- if @GroupOption = 'H'
					 ...
					 --else
					 --   Begin
					        SELECT @sql2 = @sql2 + ' FNC_HEADING.FNC_HEADING_ID, ISNULL(FNC_HEADING.FNC_HEADING_DESC, FUNCTIONS.FNC_DESCRIPTION), FNC_HEADING.FNC_HEADING_SEQ,'
					 --   End 
...
if @rptType <> 'EstimatingTAP' AND @rptType <> 'EstimatingTAP2'
Begin
	if @ExclEmpTime = 1
	Begin
...
SELECT @sql = 'SELECT CMPQUOTE, JOB_NUMBER, JOB_COMPONENT_NBR, JOB_DESC, SUM(EST_REV_EXT_AMT) AS EST_REV_EXT_AMT, SUM(EXT_MARKUP_AMT) AS EXT_MARKUP_AMT, SUM(TAX) AS TAX FROM #estPrint WHERE 1=1'
if @SuppressZero = 1
Begin
	SELECT @sql = @sql + ' AND EST_REV_EXT_AMT <> 0'
End

SELECT @sql = @sql + ' GROUP BY CMPQUOTE, JOB_NUMBER, JOB_COMPONENT_NBR, JOB_DESC'
