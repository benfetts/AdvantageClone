if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_Estimating_Print_Details_Report300]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_Estimating_Print_Details_Report300]
GO
...
CREATE PROCEDURE [dbo].[usp_wv_Estimating_Print_Details_Report300] 
@EstNo Int,
@EstCompNo Int,
@UserID varchar(100),
@Comps varchar(100),
...
SELECT @sql2 = @sql2 + ' ISNULL(ESTIMATE_REV.JOB_QTY,0) AS JOB_QTY, ISNULL(CDP_CONTACT_HDR.CDP_CONTACT_ID,0), ISNULL(CDP_CONTACT_HDR.CONT_CODE,''''), ISNULL(CDP_CONTACT_HDR.CONT_FML,''''), ISNULL(CDP_CONTACT_HDR.CONT_ADDRESS1,''''), 
                      ...
                      ISNULL(CDP_CONTACT_HDR.CONT_ZIP,''''), ISNULL(CDP_CONTACT_HDR.CONT_COUNTRY,''''), 0, 0, (Cast(EQ.ESTIMATE_NUMBER AS VARCHAR(7))+''/''+Cast(EQ.EST_QUOTE_NBR AS VARCHAR(3))), (Cast(JOB_LOG.JOB_NUMBER AS VARCHAR(7))+''/''+Cast(JOB_COMPONENT.JOB_COMPONENT_NBR AS VARCHAR(3))),'                      
				    if @GroupOption = 'I'
					    Begin
                    ...
                    Else
                        ...
                        End      
SELECT @sql2 = @sql2 + ' JOB_COMPONENT.JOB_FIRST_USE_DATE, JOB_LOG.CMP_CODE, CAMPAIGN.CMP_NAME, JOB_COMPONENT.JOB_CL_PO_NBR, CAMPAIGN.CMP_BILL_BUDGET, JOB_LOG.CMP_CODE+''/''+Cast(EQ.EST_QUOTE_NBR AS VARCHAR(3))' 
...
SELECT @sqlgroupby = @sqlgroupby + ' JOB_LOG.JOB_CLI_REF, EL.SC_CODE, SALES_CLASS.SC_DESCRIPTION, EMPLOYEE.EMP_FNAME, EMPLOYEE.EMP_MI, EMPLOYEE.EMP_LNAME,
					  ...
					  JOB_COMPONENT.JOB_FIRST_USE_DATE, JOB_LOG.CMP_CODE, CAMPAIGN.CMP_NAME, JOB_COMPONENT.JOB_CL_PO_NBR, CAMPAIGN.CMP_BILL_BUDGET'
                      if @SortOption = 'O' AND ((@ProdConsFunc = 1 AND @ConsoleOverride = 0) OR @FunctionOption = 'C')
                        Begin
...
SELECT @sql = @sql + ' ORDER BY ESTIMATE_NUMBER, EST_QUOTE_NBR, EST_COMPONENT_NBR'
