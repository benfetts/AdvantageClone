if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_Job_Template_GetAllJobs]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_Job_Template_GetAllJobs]
GO

CREATE PROCEDURE [dbo].[usp_wv_Job_Template_GetAllJobs] 
@UserID VarChar(100),
@Client VarChar(6),
@Division VarChar(6),
@Product Varchar(6),
...
If @RestrictionsCP > 0
	SELECT @sql = 'SELECT JOB_LOG.JOB_NUMBER, isnull(JOB_LOG.JOB_DESC, '''') AS JOB_DESC, 
					...
					WHERE (JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6,12)) AND (CP_SEC_CLIENT.CDP_CONTACT_ID = @CPID)'    
	if @Client <> ''
...
Begin
	...
	IF @OfficeCount > 0
		...
		END
	SELECT @sql = @sql + ' WHERE (JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6,12)) AND (UPPER(SEC_CLIENT.USER_ID) = UPPER(@UserID)) AND (SEC_CLIENT.TIME_ENTRY = 0 OR SEC_CLIENT.TIME_ENTRY IS NULL)'    
	if @Client <> ''
		SELECT @sql = @sql + ' AND (JOB_LOG.CL_CODE = @Client)'
	if @Division <> ''
		SELECT @sql = @sql + ' AND (JOB_LOG.DIV_CODE = @Division)'
...
Begin
	...
	IF @OfficeCount > 0
		BEGIN
			SELECT @sql = @sql + ' INNER JOIN EMP_OFFICE ON JOB_LOG.OFFICE_CODE = EMP_OFFICE.OFFICE_CODE AND EMP_OFFICE.EMP_CODE = @EMP_CODE'
		END
	SELECT @sql = @sql + ' WHERE (JOB_COMPONENT.JOB_PROCESS_CONTRL NOT IN (6,12))'    
	if @Client <> ''
		SELECT @sql = @sql + ' AND (JOB_LOG.CL_CODE = @Client)'
	if @Division <> ''
		SELECT @sql = @sql + ' AND (JOB_LOG.DIV_CODE = @Division)'
