if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_Revenue_Forecast_Month_Report]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_Revenue_Forecast_Month_Report]
GO

CREATE PROCEDURE [dbo].[usp_wv_Revenue_Forecast_Month_Report] 
	@DATE_TYPE AS int,
    ...
    SET ARITHIGNORE ON
       ...
       CREATE TABLE #PROJECT_TASK_ADJ 
		...
		SELECT EMP_TIME_DTL.JOB_NUMBER, EMP_TIME_DTL.JOB_COMPONENT_NBR,EMP_TIME_DTL.FNC_CODE,FUNCTIONS.FNC_DESCRIPTION, EMP_TIME.EMP_CODE,
				...
				DATEPART(m,EMP_TIME.EMP_DATE), DATEPART(yy,EMP_TIME.EMP_DATE),
				CASE WHEN EMP_TIME.EMP_DATE <= @CURRENT_END_DATE THEN ISNULL(SUM(EMP_TIME_DTL.EMP_HOURS),0) ELSE 0 END, CASE WHEN EMP_TIME.EMP_DATE <= @CURRENT_END_DATE THEN SUM(EMP_TIME_DTL.TOTAL_BILL + EMP_TIME_DTL.EXT_MARKUP_AMT) ELSE 0 END
		...
		GROUP BY EMP_TIME_DTL.JOB_NUMBER, EMP_TIME_DTL.JOB_COMPONENT_NBR,EMP_TIME_DTL.FNC_CODE,FUNCTIONS.FNC_DESCRIPTION,EMP_TIME.EMP_CODE, DATEPART(m,EMP_TIME.EMP_DATE), DATEPART(yy,EMP_TIME.EMP_DATE),EMP_TIME.EMP_DATE
		...
		SELECT JOB_NUMBER, JOB_COMPONENT_NBR, '', '', '', NULL, NULL, EST_FUNC, FUNC_DESC, EMP_CODE, NULL, NULL, NULL, 0 AS QUOTED_HOURS, 0 AS QUOTED_HRS_AMT, 0 AS FORECAST_HOURS, 0 AS FORECAST_HOURS_AMT, 0 AS HOURS_ALLOWED, 0,
				0 AS ADJ_HOURS_ALLOWED, 0 AS ADJ_HOURS_ALLOWED_AMT, SUM(ACTUAL_HOURS) AS ACTUAL_HOURS, SUM(ACTUAL_AMT) AS ACTUAL_HOURS_AMT,0,0, 0, '',0, [MONTH], [YEAR], SUM(PRIOR_ACTUAL_HOURS) AS PRIOR_ACTUAL_HOURS, SUM(PRIOR_ACTUAL_HOURS_AMT) AS PRIOR_ACTUAL_HOURS_AMT
		FROM #PROJECT_ACTUAL
		GROUP BY JOB_NUMBER, JOB_COMPONENT_NBR, EST_FUNC, FUNC_DESC, EMP_CODE, [MONTH], [YEAR]
		...
		SELECT JOB_NUMBER, JOB_COMPONENT_NBR, '', '', '', NULL, NULL, EST_FUNC, NULL, EMP_CODE, NULL, NULL, NULL, 0 AS QUOTED_HOURS, 0 AS QUOTED_HRS_AMT, 0 AS FORECAST_HOURS, 0 AS FORECAST_HOURS_AMT,
				SUM(ADJ_HOURS_ALLOWED) AS HOURS_ALLOWED, SUM(ADJ_HOURS_ALLOWED * BILL_RATE) AS HOURS_ALLOWED_AMT,0 AS ADJ_HOURS_ALLOWED, 0 AS ADJ_HOURS_ALLOWED_AMT, 0 AS ACTUAL_HOURS, 0 AS ACTUAL_HOURS_AMT,0,0, 0, '',0, ADJ_MONTH, ADJ_YEAR, 0,0
		FROM #PROJECT_TASK_ADJ
		WHERE IS_WORK_DAY = 1
		GROUP BY JOB_NUMBER, JOB_COMPONENT_NBR, EST_FUNC, EMP_CODE, ADJ_MONTH, ADJ_YEAR
		...
		--BEGIN
			...
			SELECT JOB_NUMBER, JOB_COMPONENT_NBR, '', '', '', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 0 AS QUOTED_HOURS, 0 AS QUOTED_HRS_AMT, SUM(FORECAST_HOURS), SUM(FORECAST_AMT) AS FORECAST_HOURS_AMT, 0, 0,
					0 AS ADJ_HOURS_ALLOWED, 0 AS ADJ_HOURS_ALLOWED_AMT, 0 AS ACTUAL_HOURS, 0 AS ACTUAL_HOURS_AMT,0,0, 0, '',0, NULL, NULL, 0,0
		...
		--BEGIN
			...
			SELECT JOB_NUMBER, JOB_COMPONENT_NBR, '', '', '', NULL, NULL, EST_FUNC, FUNC_DESC, NULL, NULL, NULL, NULL, ISNULL(SUM(QUOTED_HOURS),0) AS QUOTED_HOURS, SUM(QUOTED_AMT) AS QUOTED_HRS_AMT, 0 AS FORECAST_HOURS, 0 AS FORECAST_HOURS_AMT, 0, 0,
					0 AS ADJ_HOURS_ALLOWED, 0 AS ADJ_HOURS_ALLOWED_AMT, 0 AS ACTUAL_HOURS, 0 AS ACTUAL_HOURS_AMT,0,0, 0, '',0, 0, NULL, 0,0
			FROM #PROJECT_QUOTED
		...
		GROUP BY PM.JOB_NUMBER, PM.JOB_COMPONENT_NBR, [MONTH], JL.JOB_DESC,PROJECT_DESC,JOB_PROCESS_DESC
