if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_TrafficScheduleVersion_Copy]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_TrafficScheduleVersion_Copy]
GO

CREATE PROCEDURE [dbo].[usp_wv_TrafficScheduleVersion_Copy] /*WITH ENCRYPTION*/
...
/*=========== QUERY ===========*/
	...
	SELECT @MAX_SEQ_NBR = MAX(VER_SEQ_NBR) 
	FROM JOB_TRAFFIC_VER WITH(NOLOCK) 
...
--	1. Get current schedule and save to version table
	...
	SELECT        
		...
		JOB_COMPONENT_JOB_FIRST_USE_DATE
	FROM 
		JOB_TRAFFIC_VER AS JT WITH (NOLOCK)
	WHERE
		JT.ID = @SOURCE_VERSION_ID;
	...
	BEGIN
		...
		FROM JOB_TRAFFIC_VER_PREDS WITH(NOLOCK)
		WHERE
			JOB_TRAFFIC_VER_PREDS.VERSION_ID = @SOURCE_VERSION_ID;


		--	3. Get detail records
		DECLARE 
			@CURR_NEW_DETAIL_ID INT, 
			@CURR_SOURCE_DETAIL_ID INT
		...
		SET @TASK_CURSOR = CURSOR FOR
			SELECT 
				ID
			FROM 
				JOB_TRAFFIC_VER_DET WITH (NOLOCK)
			WHERE    
				JOB_TRAFFIC_VER_DET.VERSION_ID = @SOURCE_VERSION_ID;  

		OPEN @TASK_CURSOR;

		FETCH NEXT
		FROM @TASK_CURSOR INTO @CURR_SOURCE_DETAIL_ID;
		...
		BEGIN
			...
			FROM            
				JOB_TRAFFIC_VER_DET WITH(NOLOCK)
			WHERE
				JOB_TRAFFIC_VER_DET.ID = @CURR_SOURCE_DETAIL_ID;
			...
			-- 5. get related detail records and insert in version tables
				...
				FROM JOB_TRAFFIC_VER_DET_CNTS WITH(NOLOCK)
				WHERE
					JOB_TRAFFIC_VER_DET_CNTS.DETAIL_ID = @CURR_SOURCE_DETAIL_ID;
				...
				FROM JOB_TRAFFIC_VER_DET_EMPS WITH(NOLOCK)
				WHERE
					JOB_TRAFFIC_VER_DET_EMPS.DETAIL_ID = @CURR_SOURCE_DETAIL_ID;
				...
				FROM JOB_TRAFFIC_VER_DET_PREDS WITH(NOLOCK)
				WHERE
					JOB_TRAFFIC_VER_DET_PREDS.DETAIL_ID = @CURR_SOURCE_DETAIL_ID;

				-- 5d.  Detail Documents
				INSERT INTO JOB_TRAFFIC_VER_DET_DOCS (DOCUMENT_ID, DETAIL_ID)
				SELECT DOCUMENT_ID, @CURR_NEW_DETAIL_ID
				FROM JOB_TRAFFIC_VER_DET_DOCS WITH(NOLOCK)
				WHERE
					JOB_TRAFFIC_VER_DET_DOCS.DETAIL_ID = @CURR_SOURCE_DETAIL_ID;
	...
	--  6. Add to log
		...
		BEGIN
			...
			SET @VER_CREATED_COMMENT = 'CREATED FROM COPY';
	...
	SELECT ISNULL(@NEW_VERSION_ID, 0) AS VERSION_ID;
