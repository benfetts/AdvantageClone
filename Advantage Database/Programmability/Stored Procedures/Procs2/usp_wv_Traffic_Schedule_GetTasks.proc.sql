IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[usp_wv_Traffic_Schedule_GetTasks]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[usp_wv_Traffic_Schedule_GetTasks]
GO

CREATE PROCEDURE [dbo].[usp_wv_Traffic_Schedule_GetTasks]
	@JobNum AS INT,
	@JobCompNum AS INT,
	@Sort VARCHAR(10) = '',
	@USER_ID VARCHAR(100),
...
AS
/*=========== QUERY ===========*/
/*IMPORTANT NOTE:  IF YOU MODIFY THE INPUT OR OUTPUT OF THIS PROC, MAKE SURE IT WORKS WITH advsp_agile_get_sprint_details !!!!!*/
DECLARE
...
SELECT 
	...
	[TASK_CODE] = JOB_TRAFFIC_DET.FNC_CODE, 
	[TASK_DESCRIPTION] = JOB_TRAFFIC_DET.TASK_DESCRIPTION, 
	[TASK_STATUS] = JOB_TRAFFIC_DET.TASK_STATUS, 
	[EMP_CODE] = JOB_TRAFFIC_DET.EMP_CODE, 
	[FNC_EST] = JOB_TRAFFIC_DET.FNC_EST, 
	[TASK_START_DATE] = HT.TASK_START_DATE, 
	[JOB_REVISED_DATE] = HT.JOB_REVISED_DATE, 
	[JOB_COMPLETED_DATE] = HT.JOB_COMPLETED_DATE,
	[JOB_DUE_DATE] = HT.JOB_DUE_DATE,
	[REVISED_DUE_TIME] = JOB_TRAFFIC_DET.REVISED_DUE_TIME, 
	[TEMP_COMP_DATE] = HT.TEMP_COMP_DATE,
	[TRAFFIC_PHASE_ID] = ISNULL(JOB_TRAFFIC_DET.TRAFFIC_PHASE_ID, 0), 
	[PHASE_ORDER] = ISNULL(TRAFFIC_PHASE.PHASE_ORDER,0), 
	[PHASE_DESC] = ISNULL(TRAFFIC_PHASE.PHASE_DESC,''''), 
	[JOB_ORDER] = JOB_TRAFFIC_DET.JOB_ORDER, 
	[JOB_DAYS] = HT.JOB_DAYS, 
	[JOB_HRS] = HT.JOB_HRS, 
	[DUE_TIME] = JOB_TRAFFIC_DET.DUE_TIME, 
	[MILESTONE] = JOB_TRAFFIC_DET.MILESTONE, 
	[PREDECESSOR] = NULL, --JOB_TRAFFIC_DET_PREDS.PREDECESSOR_SEQ_NBR, 
	[DUE_DATE_LOCK] = ISNULL(JOB_TRAFFIC_DET.DUE_DATE_LOCK, 0), 
	[FNC_COMMENTS] = CAST(JOB_TRAFFIC_DET.FNC_COMMENTS AS VARCHAR(4000)), 
	[DUE_DATE_COMMENTS] = CAST(JOB_TRAFFIC_DET.DUE_DATE_COMMENTS AS VARCHAR(4000)), 
	[REV_DATE_COMMENTS] = CAST(JOB_TRAFFIC_DET.REV_DATE_COMMENTS AS VARCHAR(4000)), 
	[DISPERSED_HOURS] = ISNULL(SUM(JOB_TRAFFIC_DET_EMPS.HOURS_ALLOWED), 0), 
	[TRF_ROLE] = JOB_TRAFFIC_DET.TRF_ROLE, 
	[FNC_DESCRIPTION] = FUNCTIONS.FNC_DESCRIPTION, 
	[HAS_ASSIGNMENT] =	CASE 
							WHEN (SELECT 
									COUNT(1) 
	...
	--					EMP_TIME_DTL ETD 
	--				  WHERE 
	--					ETD.JOB_NUMBER = ' + CAST(@JobNum AS VARCHAR)+' AND 
	--					ETD.JOB_COMPONENT_NBR = '+ CAST(@JobCompNum AS VARCHAR) +' AND 
	--					ETD.FNC_CODE = JOB_TRAFFIC_DET.FNC_EST),
	[POSTED_HOURS_ASSN] = (SELECT 
						ISNULL(SUM(ETD.EMP_HOURS),0) 
					  FROM 
	...
	--																							ETD.JOB_NUMBER = ' + CAST(@JobNum AS VARCHAR)+' AND 
	--																							ETD.JOB_COMPONENT_NBR = '+ CAST(@JobCompNum AS VARCHAR) +' AND 
	--																							ETD.FNC_CODE = JOB_TRAFFIC_DET.FNC_EST) / (ISNULL(SUM(JOB_TRAFFIC_DET_EMPS.HOURS_ALLOWED), 0))) * 100 
	--						ELSE 0 END,
	[PERC_COMPLETE_ASSN] =	CASE 
							WHEN ISNULL(SUM(JOB_TRAFFIC_DET_EMPS.HOURS_ALLOWED), 0) > 0 THEN ((SELECT 
																								ISNULL(SUM(ETD.EMP_HOURS),0) 
																							  FROM 
																								EMP_TIME_DTL ETD INNER JOIN 
...
SET @SELECT_SQL = @SELECT_SQL + '
	[GRID_ORDER] = JOB_TRAFFIC_DET.GRID_ORDER,
	[LEVEL] = HT.[LEVEL],
	...
	[HAS_PREDECESSORS] = CONVERT(BIT, CASE WHEN JOB_TRAFFIC_DET_PREDS.JOB_NUMBER IS NOT NULL THEN 1 ELSE 0 END) '
	...
	(SELECT
	 ...
	 WHERE
		...
		JOIN ALERT ON JOB_TRAFFIC_DET.JOB_NUMBER = ALERT.JOB_NUMBER AND
					  ...
					  JOB_TRAFFIC_DET.SEQ_NBR = ALERT.TASK_SEQ_NBR
'
...
--ORDER BY 
--	JOB_TRAFFIC_DET.JOB_ORDER ASC,
--	ISNULL(JOB_TRAFFIC_DET.GRID_ORDER, 0) ASC,
--	CASE WHEN JOB_TRAFFIC_DET.GRID_ORDER IS NULL THEN HT.TASK_START_DATE END, 
--	CASE WHEN JOB_TRAFFIC_DET.GRID_ORDER IS NULL THEN HT.JOB_REVISED_DATE END ASC, 
--	ISNULL(JOB_TRAFFIC_DET.GRID_ORDER, JOB_TRAFFIC_DET.SEQ_NBR) ASC
--		'
--	END
--IF @Sort = ''
...
--	ISNULL(JOB_TRAFFIC_DET.GRID_ORDER, 0) ASC,
--	CASE WHEN JOB_TRAFFIC_DET.GRID_ORDER IS NULL THEN HT.TASK_START_DATE END, 
--	CASE WHEN JOB_TRAFFIC_DET.GRID_ORDER IS NULL THEN HT.JOB_REVISED_DATE END, 
--	CASE WHEN JOB_TRAFFIC_DET.GRID_ORDER IS NULL THEN HT.JOB_DUE_DATE END, 
