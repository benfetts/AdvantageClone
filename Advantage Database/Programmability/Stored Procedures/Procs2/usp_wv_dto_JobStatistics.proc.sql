...
SET ANSI_NULLS OFF 
GO
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_dto_JobStatistics]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_dto_JobStatistics]
GO


CREATE PROCEDURE [dbo].[usp_wv_dto_JobStatistics]
@UserID VARCHAR(100),
@StartDate DATETIME,
@EndDate DATETIME,
...
--				(JOB_COMPONENT.JOB_COMP_DATE <= @EndDate) AND (UPPER(SEC_CLIENT.USER_ID) = UPPER(@UserID)) AND (SEC_CLIENT.TIME_ENTRY = 0 OR SEC_CLIENT.TIME_ENTRY IS NULL)
...
--				(JOB_TRAFFIC.COMPLETED_DATE <= @EndDate) AND (UPPER(SEC_CLIENT.USER_ID) = UPPER(@UserID)) AND (SEC_CLIENT.TIME_ENTRY = 0 OR SEC_CLIENT.TIME_ENTRY IS NULL)
--			GROUP BY JOB_LOG.CL_CODE
--			) T1
--			INNER JOIN #JOB_STATS T2 ON (T1.CL_CODE COLLATE DATABASE_DEFAULT = T2.CL_CODE COLLATE DATABASE_DEFAULT)
--		END
...
--							(JOB_TRAFFIC.TRF_CODE = @CancelledCode) AND (UPPER(SEC_CLIENT.USER_ID) = UPPER(@UserID)) AND (SEC_CLIENT.TIME_ENTRY = 0 OR SEC_CLIENT.TIME_ENTRY IS NULL)
--						GROUP BY JOB_LOG.CL_CODE
--					) T1
--					INNER JOIN #JOB_STATS T2 ON (T1.CL_CODE COLLATE DATABASE_DEFAULT = T2.CL_CODE COLLATE DATABASE_DEFAULT)
--				END
...
--				(NOT (JOB_COMPONENT.JOB_PROCESS_CONTRL IN (6, 12))) AND (JOB_TRAFFIC.COMPLETED_DATE IS NULL) AND (UPPER(SEC_CLIENT.USER_ID) = UPPER(@UserID)) AND (SEC_CLIENT.TIME_ENTRY = 0 OR SEC_CLIENT.TIME_ENTRY IS NULL)
--			GROUP BY JOB_LOG.CL_CODE
--			) T1
--			INNER JOIN #JOB_STATS T2 ON (T1.CL_CODE COLLATE DATABASE_DEFAULT = T2.CL_CODE COLLATE DATABASE_DEFAULT)
--		END
...
IF @Restrictions > 0
	BEGIN
		...
		FROM         
			...
			SEC_CLIENT ON #JOB_STATS.CL_CODE COLLATE DATABASE_DEFAULT= SEC_CLIENT.CL_CODE COLLATE DATABASE_DEFAULT
		WHERE     
