if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_jobspecs_GetJobSpecs_ByJobandComp]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_jobspecs_GetJobSpecs_ByJobandComp]
GO
...
CREATE PROCEDURE [dbo].[usp_wv_jobspecs_GetJobSpecs_ByJobandComp] 
	@JOB_NUMBER INT,
...
----Get Job client ref:
--SELECT @JOB_CLI_REF = ISNULL(JOB_CLI_REF,'') FROM JOB_LOG WHERE JOB_NUMBER = @JOB_NUMBER
--
----Get CDP with their descriptions
--SELECT @JOB_CLI_DESCRIPT = (SELECT CL_CODE + '&nbsp;-&nbsp;' + ISNULL(CL_NAME, '') FROM  CLIENT WHERE CL_CODE= @JOB_LOG_CL_CODE)
--SELECT @JOB_DIV_DESCRIPT = (SELECT DIV_CODE+'&nbsp;-&nbsp;'+ISNULL(DIV_NAME,'') FROM DIVISION WHERE CL_CODE=@JOB_LOG_CL_CODE AND DIV_CODE=@JOB_LOG_DIV_CODE)
--SELECT @JOB_PRD_DESCRIPT = (SELECT PRD_CODE+'&nbsp;-&nbsp;'+ISNULL(PRD_DESCRIPTION,'') FROM PRODUCT WHERE CL_CODE=@JOB_LOG_CL_CODE AND DIV_CODE=@JOB_LOG_DIV_CODE AND PRD_CODE=@JOB_LOG_PRD_CODE)


--MAIN SELECT RETURN:
SELECT     
	JOB_SPEC_FIELDS.FIELD_NAME
	--, JOB_TMPLT_ITEMS.ITEM_TYPE
	, JOB_SPEC_FIELDS.FIELD_DESC AS SHORT_DESC
	--, JOB_TMPLT_ITEMS.ITEM_DESC AS LONG_DESC	
...
FROM  JOB_SPECS 
   ...
   INNER JOIN JOB_SPEC_CATEGORY ON JOB_SPEC_CATEGORY.SPEC_TYPE_CODE = JOB_SPEC_FIELDS.SPEC_TYPE_CODE
			AND JOB_SPEC_CATEGORY.CATEGORY_ID = JOB_SPEC_FIELDS.CATEGORY_ID
	
WHERE (JOB_SPECS.JOB_NUMBER = @JOB_NUMBER) AND (JOB_SPECS.JOB_COMPONENT_NBR = @JOB_COMPONENT_NBR)
		 AND (JOB_SPECS.SPEC_VER = @VERSION) AND (JOB_SPECS.SPEC_REV = @REVISION) AND JOB_SPEC_FIELDS.INACTIVE_FLAG = 0
		  AND (JOB_SPEC_CATEGORY.INACTIVE_FLAG = 0 OR JOB_SPEC_CATEGORY.INACTIVE_FLAG IS NULL)
ORDER BY 
	JOB_SPEC_CATEGORY.CATEGORY_SEQ, JOB_SPEC_FIELDS.CATEGORY_ID, JOB_SPEC_FIELDS.FIELD_SEQ
