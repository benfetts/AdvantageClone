...
CREATE PROCEDURE [dbo].[usp_wv_reports_ar_statements_product] 
@ID VarChar(4000),
...
declare @T_AR306090PAY table (tr_CL_CODE varchar(6), tr_DIV_CODE varchar(6), tr_PRD_CODE varchar(6), tr_INV_NBR int, tr_INV_SEQ smallint, CR_AMT Decimal(15,2))

declare @T_AR306090AMT table (t_CL_CODE varchar(6), t_DIV_CODE varchar(6), t_PRD_CODE varchar(6), t_INV_NBR int, t_INV_SEQ smallint, t_PostPeriod varchar(6), t_Bucket varchar(4), AR_AMOUNT Decimal(15,2))
declare @T_AR306090 table (t_CL_CODE varchar(6), t_DIV_CODE varchar(6), t_PRD_CODE varchar(6), t_INV_NBR int, t_INV_SEQ smallint, t_PostPeriod varchar(6), t_Bucket varchar(4), AR_AMOUNT Decimal(15,2), CR_AMT Decimal(15,2))
...
SELECT ACCT_REC.CL_CODE, ACCT_REC.DIV_CODE, ACCT_REC.PRD_CODE, ACCT_REC.AR_INV_NBR,  ACCT_REC.AR_INV_SEQ,  ACCT_REC.AR_POST_PERIOD, '00', SUM(ACCT_REC.AR_INV_AMOUNT)
FROM         ACCT_REC 
Where ACCT_REC.AR_POST_PERIOD <= @PostPeriod and (ACCT_REC.CL_CODE = @pair_value1) and (ACCT_REC.DIV_CODE = @pair_value2) and (ACCT_REC.PRD_CODE = @pair_value3) and  (ACCT_REC.AR_INV_DATE >= DateAdd(Day, -30, cast(@AgedDate as Datetime))) AND (ACCT_REC.VOID_FLAG = 0 OR
	                      ACCT_REC.VOID_FLAG IS NULL) AND (ACCT_REC.AR_INV_SEQ <> 99 OR ACCT_REC.AR_INV_SEQ IS NULL)
group by  ACCT_REC.CL_CODE, ACCT_REC.DIV_CODE, ACCT_REC.PRD_CODE, ACCT_REC.AR_INV_NBR,  ACCT_REC.AR_INV_SEQ,  ACCT_REC.AR_POST_PERIOD

insert into @T_AR306090AMT	
SELECT ACCT_REC.CL_CODE, ACCT_REC.DIV_CODE, ACCT_REC.PRD_CODE, ACCT_REC.AR_INV_NBR,  ACCT_REC.AR_INV_SEQ,  ACCT_REC.AR_POST_PERIOD, '30', SUM(ACCT_REC.AR_INV_AMOUNT)
FROM         ACCT_REC 
Where ACCT_REC.AR_POST_PERIOD <= @PostPeriod and (ACCT_REC.CL_CODE = @pair_value1) and (ACCT_REC.DIV_CODE = @pair_value2) and (ACCT_REC.PRD_CODE = @pair_value3) and  (ACCT_REC.AR_INV_DATE >= DateAdd(Day, -60, cast(@AgedDate as datetime)) and ACCT_REC.AR_INV_DATE <= DateAdd(Day, -31, cast(@AgedDate as datetime))) AND (ACCT_REC.VOID_FLAG = 0 OR
	                      ACCT_REC.VOID_FLAG IS NULL) AND (ACCT_REC.AR_INV_SEQ <> 99 OR ACCT_REC.AR_INV_SEQ IS NULL)
group by  ACCT_REC.CL_CODE, ACCT_REC.DIV_CODE, ACCT_REC.PRD_CODE, ACCT_REC.AR_INV_NBR,  ACCT_REC.AR_INV_SEQ,  ACCT_REC.AR_POST_PERIOD
insert into @T_AR306090AMT	
SELECT ACCT_REC.CL_CODE, ACCT_REC.DIV_CODE, ACCT_REC.PRD_CODE, ACCT_REC.AR_INV_NBR,  ACCT_REC.AR_INV_SEQ,  ACCT_REC.AR_POST_PERIOD, '60', SUM(ACCT_REC.AR_INV_AMOUNT)
FROM         ACCT_REC 
Where ACCT_REC.AR_POST_PERIOD <= @PostPeriod and (ACCT_REC.CL_CODE = @pair_value1) and (ACCT_REC.DIV_CODE = @pair_value2) and (ACCT_REC.PRD_CODE = @pair_value3) and  (ACCT_REC.AR_INV_DATE <= DateAdd(Day, -61, cast(@AgedDate as Datetime)) ) AND (ACCT_REC.VOID_FLAG = 0 OR
	                      ACCT_REC.VOID_FLAG IS NULL) AND (ACCT_REC.AR_INV_SEQ <> 99 OR ACCT_REC.AR_INV_SEQ IS NULL)
group by  ACCT_REC.CL_CODE, ACCT_REC.DIV_CODE, ACCT_REC.PRD_CODE, ACCT_REC.AR_INV_NBR,  ACCT_REC.AR_INV_SEQ,  ACCT_REC.AR_POST_PERIOD
...
Select   PRODUCT_AR_STMT.CL_CODE as ClientCode, 
	...
	P.PRD_BILL_ADDRESS1 as SAddress1,
	P.PRD_BILL_ADDRESS2 as SAddress2,
	P.PRD_BILL_CITY as SCity, 
	P.PRD_BILL_STATE as SState,
	P.PRD_BILL_ZIP as SZip, 
...
from PRODUCT_AR_STMT	
	...
	Inner Join PRD_CONTACT CON
		ON CON.CL_CODE = PRODUCT_AR_STMT.CL_CODE
 		and CON.DIV_CODE = PRODUCT_AR_STMT.DIV_CODE
		and CON.PRD_CODE = PRODUCT_AR_STMT.PRD_CODE
		and CON.CONT_CODE = PRODUCT_AR_STMT.CONT_CODE
...
select
ClientCode, DivisionCode, ProductCode, ContactCode, UseAddress, IncludeOnAccount,
