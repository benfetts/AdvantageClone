if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_reports_getCDPInfo]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_reports_getCDPInfo]
GO

CREATE PROCEDURE [dbo].[usp_wv_reports_getCDPInfo]
@LEVEL varchar(25),
@CLIENT varchar(6),
@DIVISION varchar(6),
@PRODUCT varchar(6),
...
Begin
	SELECT     CL_CODE, CL_NAME, ISNULL(CL_BADDRESS1,'') AS CL_BADDRESS1, ISNULL(CL_BADDRESS2,'') AS CL_BADDRESS2, ISNULL(CL_BCITY,'') AS CL_BCITY, ISNULL(CL_BSTATE,'') AS CL_BSTATE, ISNULL(CL_BZIP,'') AS CL_BZIP, ISNULL(CL_FOOTER,'') AS CL_FOOTER, ISNULL(CL_ATTENTION,'') AS CL_ATTENTION,
				ISNULL(CL_BCOUNTRY,'') AS CL_BCOUNTRY
	FROM         CLIENT
...
if @LEVEL = 'Division'
Begin
	SELECT     DIVISION.CL_CODE, CLIENT.CL_NAME, DIVISION.DIV_NAME, ISNULL(DIVISION.DIV_BADDRESS1,'') AS DIV_BADDRESS1, ISNULL(DIVISION.DIV_BADDRESS2,'') AS DIV_BADDRESS2, 
