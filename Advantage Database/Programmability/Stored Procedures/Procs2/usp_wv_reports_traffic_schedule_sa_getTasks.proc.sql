if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_reports_traffic_schedule_sa_getTasks]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_reports_traffic_schedule_sa_getTasks]
GO




CREATE PROCEDURE [dbo].[usp_wv_reports_traffic_schedule_sa_getTasks]
			...
			-- ************ Include Closed Jobs ****************
				...
				SELECT @sql2 = @sql2 + ' WHERE (NOT (JOB_COMPONENT.JOB_PROCESS_CONTRL IN (6, 12)))'
				 ...
				 SELECT @sql2 = @sql2 + ' AND ((JOB_TRAFFIC.COMPLETED_DATE IS NULL)'
					...
					SELECT @paramlist2 = '@ForJobDueDate char(1), @ForJobStartDate DateTime, @ForJobEndDate DateTime, @IncludeClosedStartDate DateTime, @IncludeClosedEndDate DateTime, @ClientCode varchar(4000), @OfficeCode varchar(4000), @Manager Varchar(6), @AEs VarChar(4000), @CPID int'

					EXEC sp_executesql @sql2, @paramlist2, @ForJobDueDate, @ForJobStartDate, @ForJobEndDate, @IncludeClosedStartDate, @IncludeClosedEndDate, @ClientCode, @OfficeCode, @Manager, @AEs, @CPID
		...
		Else
			Begin		
				...
				AND JOB_TRAFFIC.COMPLETED_DATE IS NULL'
					...
					SELECT @paramlist2 = '@ForJobDueDate char(1), @ForJobStartDate DateTime, @ForJobEndDate DateTime, @ClientCode varchar(4000), @OfficeCode varchar(4000), @Manager Varchar(6), @AEs VarChar(4000), @CPID int'

					EXEC sp_executesql @sql2, @paramlist2, @ForJobDueDate, @ForJobStartDate, @ForJobEndDate, @ClientCode, @OfficeCode, @Manager, @AEs, @CPID
			...
			-- ************ Include Closed Jobs ****************
				...
				SELECT @sql2 = @sql2 + ' WHERE (NOT (JOB_COMPONENT.JOB_PROCESS_CONTRL IN (6, 12)))'
				 ...
				 SELECT @sql2 = @sql2 + ' AND ((JOB_TRAFFIC.COMPLETED_DATE IS NULL)'
					...
					SELECT @paramlist2 = '@ForJobDueDate char(1), @ForJobStartDate DateTime, @ForJobEndDate DateTime, @IncludeClosedStartDate DateTime, @IncludeClosedEndDate DateTime, @ClientCode varchar(4000), @OfficeCode varchar(4000), @Manager Varchar(6), @AEs VarChar(4000), @UserID varchar(100)'

					EXEC sp_executesql @sql2, @paramlist2, @ForJobDueDate, @ForJobStartDate, @ForJobEndDate, @IncludeClosedStartDate, @IncludeClosedEndDate, @ClientCode, @OfficeCode, @Manager, @AEs, @UserID
		...
		Else
			Begin		
				...
				AND JOB_TRAFFIC.COMPLETED_DATE IS NULL'
					...
					SELECT @paramlist2 = '@ForJobDueDate char(1), @ForJobStartDate DateTime, @ForJobEndDate DateTime, @ClientCode varchar(4000), @OfficeCode varchar(4000), @Manager Varchar(6), @AEs VarChar(4000), @UserID varchar(100)'

					EXEC sp_executesql @sql2, @paramlist2, @ForJobDueDate, @ForJobStartDate, @ForJobEndDate, @ClientCode, @OfficeCode, @Manager, @AEs, @UserID
...
if @StatusCodes<>''
begin
 SELECT @sql = 'Select LEFT( JobStatus, 20 ) as [Job Status First],Manager,
...
If @ClosedJobs = 'Y'
	Begin 
		SELECT @sql3 = 'SELECT     JOB_TRAFFIC_DET.JOB_NUMBER, JOB_TRAFFIC_DET.JOB_COMPONENT_NBR, JOB_TRAFFIC_DET.SEQ_NBR, 
                      JOB_TRAFFIC_DET.FNC_CODE, JOB_TRAFFIC_DET.TASK_START_DATE, JOB_TRAFFIC_DET.JOB_DUE_DATE, 
                      JOB_TRAFFIC_DET.JOB_REVISED_DATE, JOB_TRAFFIC_DET.JOB_COMPLETED_DATE, JOB_TRAFFIC_DET.EMP_CODE, 
                      JOB_TRAFFIC_DET.TEMP_COMP_DATE, ISNULL(JOB_TRAFFIC_DET.REVISED_DUE_TIME,''NOTIME'') AS REVISED_DUE_TIME, JOB_TRAFFIC_DET.TRAFFIC_PHASE_ID, TP.PHASE_DESC
		FROM         JOB_TRAFFIC INNER JOIN
							  ...
							  LEFT OUTER JOIN TRAFFIC_PHASE TP ON TP.TRAFFIC_PHASE_ID = JOB_TRAFFIC_DET.TRAFFIC_PHASE_ID INNER JOIN
							  #report ON JOB_TRAFFIC_DET.JOB_NUMBER = #report.JobNumber AND
							  JOB_TRAFFIC_DET.JOB_COMPONENT_NBR = #report.JobCompNumber'
							  if @StatusCodes<>''
							  Begin
			    ...
			    SELECT @sql4 = @sql4 + ' UNION
                                        SELECT     JOB_TRAFFIC.JOB_NUMBER, JOB_TRAFFIC.JOB_COMPONENT_NBR, ISNULL(JOB_TRAFFIC_DET.SEQ_NBR,'''') AS SEQ_NBR, 
                                                              ...
                                                              ISNULL(JOB_TRAFFIC_DET.JOB_REVISED_DATE,'''') AS JOB_REVISED_DATE, ISNULL(JOB_TRAFFIC_DET.JOB_COMPLETED_DATE,'''') AS JOB_COMPLETED_DATE, ISNULL(JOB_TRAFFIC_DET.EMP_CODE,'''') AS EMP_CODE, 
                                                              ISNULL(JOB_TRAFFIC_DET.TEMP_COMP_DATE,'''') AS TEMP_COMP_DATE, ISNULL(JOB_TRAFFIC_DET.REVISED_DUE_TIME,''NOTIME'') AS REVISED_DUE_TIME, JOB_TRAFFIC_DET.TRAFFIC_PHASE_ID, TP.PHASE_DESC
				 ...
				 SELECT @sql4 = @sql4 + ' WHERE (NOT (JOB_COMPONENT.JOB_PROCESS_CONTRL IN (6, 12))) AND (JOB_TRAFFIC_DET.SEQ_NBR IS NULL) AND ((JOB_TRAFFIC.COMPLETED_DATE IS NULL) '
			  	...
			  	SELECT @sql4 = @sql4 + ' ORDER BY JOB_TRAFFIC_DET.JOB_NUMBER, JOB_TRAFFIC_DET.JOB_COMPONENT_NBR, JOB_TRAFFIC_DET.FNC_CODE, JOB_TRAFFIC_DET.SEQ_NBR	'	
			  		
			  	--SELECT @paramlist3 = '@ForJobDueDate char(1), @ForJobStartDate DateTime, @ForJobEndDate DateTime, @IncludeClosedStartDate DateTime, @IncludeClosedEndDate DateTime,
			  	...
			  	--					  @ClientCode varchar(4000), @OfficeCode varchar(4000), @Manager Varchar(6)'
					...
					--EXEC sp_executesql @sql3, @paramlist3, @ForJobDueDate, @ForJobStartDate, @ForJobEndDate, @IncludeClosedStartDate, @IncludeClosedEndDate,@Task1,@Task2,@Task3,@Task4,@Task5,
	...
	Begin 
		...
		FROM         JOB_TRAFFIC INNER JOIN
			    ...
			    SELECT @sql4 = @sql4 + ' UNION
					...
					If @RestrictionsOffice > 0
						Begin
							SELECT @sql4 = @sql4  + ' INNER JOIN EMP_OFFICE ON EMP_OFFICE.OFFICE_CODE = JOB_LOG.OFFICE_CODE AND EMP_OFFICE.EMP_CODE = ''' + @EMP_CDE + ''''
						End	    
				 SELECT @sql4 = @sql4 + ' WHERE (NOT (JOB_COMPONENT.JOB_PROCESS_CONTRL IN (6, 12))) AND (JOB_TRAFFIC_DET.SEQ_NBR IS NULL) AND ((JOB_TRAFFIC.COMPLETED_DATE IS NULL) '
						if @IncludeClosedStartDate <> '' and @IncludeClosedEndDate <> ''
							Begin
...
Select '' as [Job Status First],'' as [Manager], 
    '' as [Job Start Date],
	'' as [Job Due Date1],
