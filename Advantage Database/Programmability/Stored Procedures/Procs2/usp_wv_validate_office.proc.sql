 if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_validate_office]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_validate_office]
GO
...
CREATE PROCEDURE [dbo].[usp_wv_validate_office] 
    ...
    BEGIN
        IF EXISTS(
               ...
               WHERE  ((INACTIVE_FLAG IS NULL) OR (INACTIVE_FLAG = 0))
                      AND OFFICE.OFFICE_CODE = @OfficeCode
           )
            SET @ROffice = 1
        ELSE
            SET @ROffice = 0
