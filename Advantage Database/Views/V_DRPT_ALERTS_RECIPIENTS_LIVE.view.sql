CREATE VIEW [dbo].[V_DRPT_ALERTS_RECIPIENTS_LIVE]
...
AS
	...
	FROM
		...
		FROM 
			...
			[dbo].[EMPLOYEE_CLOAK] AS EMP ON AR.EMP_CODE = EMP.EMP_CODE LEFT OUTER JOIN 
		...
		FROM 
			...
			[dbo].[EMPLOYEE_CLOAK] AS EMP ON ARD.EMP_CODE = EMP.EMP_CODE LEFT OUTER JOIN 
			[dbo].[SOFTWARE_VERSION] AS SV ON SV.VERSION_ID = A.VER LEFT OUTER JOIN 
			[dbo].[SOFTWARE_BUILD] AS SB ON SB.VERSION_ID = A.VER AND
