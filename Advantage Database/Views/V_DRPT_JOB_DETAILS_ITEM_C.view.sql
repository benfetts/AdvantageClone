if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[V_DRPT_JOB_DETAILS_ITEM_C]') and OBJECTPROPERTY(id, N'IsView') = 1)
drop view [dbo].[V_DRPT_JOB_DETAILS_ITEM_C]
GO

CREATE VIEW [dbo].[V_DRPT_JOB_DETAILS_ITEM_C]
...
AS
	...
	FROM 
		...
		[dbo].[JOB_LOG] AS J ON J.JOB_NUMBER = COOP.JOB_NUMBER INNER JOIN
		[dbo].[JOB_COMPONENT] JC ON JC.JOB_NUMBER = COOP.JOB_NUMBER AND
									JC.JOB_COMPONENT_NBR = COOP.JOB_COMPONENT_NBR LEFT OUTER JOIN
