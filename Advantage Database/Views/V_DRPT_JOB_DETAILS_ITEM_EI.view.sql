if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[V_DRPT_JOB_DETAILS_ITEM_EI]') and OBJECTPROPERTY(id, N'IsView') = 1)
drop view [dbo].[V_DRPT_JOB_DETAILS_ITEM_EI]
GO

CREATE VIEW [dbo].[V_DRPT_JOB_DETAILS_ITEM_EI]
...
AS
	...
	FROM 
		...
		[dbo].[ESTIMATE_LOG] AS EL ON EL.ESTIMATE_NUMBER = EIA.ESTIMATE_NUMBER INNER JOIN
		[dbo].[ESTIMATE_COMPONENT] AS EC ON EC.ESTIMATE_NUMBER = EIA.ESTIMATE_NUMBER AND
									EC.EST_COMPONENT_NBR = EIA.EST_COMPONENT_NBR
