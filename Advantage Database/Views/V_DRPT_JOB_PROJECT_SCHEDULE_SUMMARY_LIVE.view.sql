if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[V_DRPT_JOB_PROJECT_SCHEDULE_SUMMARY_LIVE]') and OBJECTPROPERTY(id, N'IsView') = 1)
drop view [dbo].[V_DRPT_JOB_PROJECT_SCHEDULE_SUMMARY_LIVE]
GO

CREATE VIEW [dbo].[V_DRPT_JOB_PROJECT_SCHEDULE_SUMMARY_LIVE]
...
AS
	...
	SELECT
		...
		[Employee] = JOBTDEMP.EMP_CODE_LIST,
	...
	FROM
		...
		(SELECT	DISTINCT
		 ...
		 FROM
			dbo.JOB_COMPONENT
		 WHERE
			JOB_PROCESS_CONTRL NOT IN (6, 12)) COMPOPEN ON J.JOB_NUMBER = COMPOPEN.JOB_NUMBER INNER JOIN
