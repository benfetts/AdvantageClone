IF EXISTS ( SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[V_MEDIA_INV_DEF]') AND OBJECTPROPERTY( id, N'IsView') = 1 )
	DROP VIEW [dbo].[V_MEDIA_INV_DEF]
GO

CREATE VIEW dbo.V_MEDIA_INV_DEF 
AS
...
WHERE rs.APP_TYPE = 'MI'
	...
	AND rs.[USER_ID] = dbo.fn_AdvanUser()
GO

IF ( @@ERROR = 0 )
	GRANT SELECT ON [V_MEDIA_INV_DEF] TO PUBLIC AS dbo
GO	
