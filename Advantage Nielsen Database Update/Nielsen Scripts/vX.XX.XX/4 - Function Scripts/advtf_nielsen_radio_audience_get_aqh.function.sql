...
BEGIN
	...
	--20210902 match exactly A single NIELSEN_RADIO_DAYPART has AQH = 1 and matches all of the requested days and the exact start and end times	
	INSERT INTO @tt
	SELECT
		...
		FROM dbo.NIELSEN_RADIO_AUDIENCE a
			...
			INNER JOIN dbo.NIELSEN_RADIO_DAYPART dp ON c.NIELSEN_RADIO_DAYPART_ID = dp.NIELSEN_RADIO_DAYPART_ID AND dp.AQH = 1
			INNER JOIN (SELECT 
							ID,
							StartHour,
							EndHour, 
						...
						FROM 
							@LOCAL_MEDIA_SPOT_TV_RESEARCH_DAYTIME_TYPE) MRDT ON MRDT.ID > 0 AND MRDT.Monday = dp.MONDAY AND MRDT.Tuesday = dp.TUESDAY AND MRDT.Wednesday = dp.WEDNESDAY 
																	AND MRDT.Thursday = dp.THURSDAY AND MRDT.Friday = dp.FRIDAY AND MRDT.Saturday = dp.SATURDAY AND MRDT.Sunday = dp.SUNDAY 
																	AND MRDT.StartHour = dp.MIL_START_TIME AND MRDT.EndHour = dp.MIL_END_TIME 
