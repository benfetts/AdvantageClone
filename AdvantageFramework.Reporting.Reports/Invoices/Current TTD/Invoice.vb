...
#Region " Properties "
        ...
        Public ReadOnly Property InvoiceType As InvoicePrinting.InvoiceTypes Implements ICustomInvoice.InvoiceType
            Get
                InvoiceType = InvoicePrinting.InvoiceTypes.ProductionCurrentTTD
            End Get
        End Property
...
#Region " Methods "
        ...
        Private Sub XrSubreportBillingHistory_BeforePrint(sender As Object, e As System.Drawing.Printing.PrintEventArgs) Handles XrSubreportBillingHistory.BeforePrint
            ...
            If TypeOf XrSubreportBillingHistory.ReportSource Is BillingHistorySubReport AndAlso TypeOf Me.GetCurrentRow Is AdvantageFramework.InvoicePrinting.Classes.CurrentTTDInvoiceDetail Then
                ...
                BillingHistorySubReport.InvoiceNumber.Value = CurrentTTDInvoiceDetail.InvoiceNumber
                BillingHistorySubReport.JobNumber.Value = CurrentTTDInvoiceDetail.JobNumber
                BillingHistorySubReport.IsDraft = _IsDraft

                If HideComponentNumberAndDescription.Value = False Then

                    BillingHistorySubReport.ComponentNumber.Value = CurrentTTDInvoiceDetail.ComponentNumber
        ...
        Private Sub XrSubreportInvoiceTaxInformation_BeforePrint(sender As Object, e As System.Drawing.Printing.PrintEventArgs) Handles XrSubreportInvoiceTaxInformation.BeforePrint
            ...
            If TypeOf XrSubreportInvoiceTaxInformation.ReportSource Is TaxInformationSubReport Then
                ...
                CurrentTTDInvoiceDetails = TryCast(Me.DataSource, IEnumerable(Of AdvantageFramework.InvoicePrinting.Classes.CurrentTTDInvoiceDetail)).Where(Function(Entity) Entity.FullInvoiceNumber = FullInvoiceNumber).ToList

                TaxInformations = New Generic.List(Of AdvantageFramework.InvoicePrinting.Classes.TaxInformation)

                TTDTaxAmount = Nothing
                TaxAmount = Nothing
                TTDTaxAmount = CurrentTTDInvoiceDetails.Select(Function(Entity) Entity.TTDCityTax).Sum
                TaxAmount = CurrentTTDInvoiceDetails.Select(Function(Entity) Entity.CityTax).Sum

                If (TTDTaxAmount.HasValue AndAlso TTDTaxAmount.Value <> 0) OrElse (TaxAmount.HasValue AndAlso TaxAmount.Value <> 0) Then

                    TaxInformations.Add(New AdvantageFramework.InvoicePrinting.Classes.TaxInformation(FullInvoiceNumber, _InvoicePrintingSetting.CityTaxLabel, TaxAmount, TTDTaxAmount))

                End If

                TTDTaxAmount = Nothing
                TaxAmount = Nothing
                TTDTaxAmount = CurrentTTDInvoiceDetails.Select(Function(Entity) Entity.TTDCountyTax).Sum
                TaxAmount = CurrentTTDInvoiceDetails.Select(Function(Entity) Entity.CountyTax).Sum

                If (TTDTaxAmount.HasValue AndAlso TTDTaxAmount.Value <> 0) OrElse (TaxAmount.HasValue AndAlso TaxAmount.Value <> 0) Then

                    TaxInformations.Add(New AdvantageFramework.InvoicePrinting.Classes.TaxInformation(FullInvoiceNumber, _InvoicePrintingSetting.CountyTaxLabel, TaxAmount, TTDTaxAmount))

                End If

                TTDTaxAmount = Nothing
                TaxAmount = Nothing
                TTDTaxAmount = CurrentTTDInvoiceDetails.Select(Function(Entity) Entity.TTDStateTax).Sum
                TaxAmount = CurrentTTDInvoiceDetails.Select(Function(Entity) Entity.StateTax).Sum

                If (TTDTaxAmount.HasValue AndAlso TTDTaxAmount.Value <> 0) OrElse (TaxAmount.HasValue AndAlso TaxAmount.Value <> 0) Then

                    TaxInformations.Add(New AdvantageFramework.InvoicePrinting.Classes.TaxInformation(FullInvoiceNumber, _InvoicePrintingSetting.StateTaxLabel, TaxAmount, TTDTaxAmount))
        ...
        Private Sub XrSubreportJobTaxInformation_BeforePrint(sender As Object, e As System.Drawing.Printing.PrintEventArgs) Handles XrSubreportJobTaxInformation.BeforePrint
            ...
            If TypeOf XrSubreportJobTaxInformation.ReportSource Is TaxInformationSubReport Then
                ...
                CurrentTTDInvoiceDetails = TryCast(Me.DataSource, IEnumerable(Of AdvantageFramework.InvoicePrinting.Classes.CurrentTTDInvoiceDetail)).Where(Function(Entity) Entity.FullInvoiceNumber = FullInvoiceNumber AndAlso Entity.JobNumber = JobNumber AndAlso Entity.ComponentNumber = ComponentNumber).ToList
        ...
        Private Sub Invoice_DataSourceDemanded(sender As Object, e As EventArgs) Handles Me.DataSourceDemanded
            ...
            If _InvoicePrintingSetting IsNot Nothing Then
                ...
                Using DbContext = New AdvantageFramework.Database.DbContext(_Session.ConnectionString, _Session.UserCode)
                    ...
                    CurrentTTDInvoiceDetails = AdvantageFramework.InvoicePrinting.LoadCurrentTTDInvoiceDetails(DbContext, _UserCode, _InvoiceNumber, _SequenceNumber, _InvoicePrintingSetting.AddressBlockType, _InvoicePrintingSetting.PrintClientName, _InvoicePrintingSetting.PrintDivisionName,
					...
					If CurrentTTDInvoiceDetails IsNot Nothing AndAlso _InvoicePrintingSetting.ShowZeroFunctionAmounts.GetValueOrDefault(False) = False Then

                        Try

							CurrentTTDInvoiceDetails = CurrentTTDInvoiceDetails.Where(Function(Entity) Entity.TotalAmount.GetValueOrDefault(0) <> 0 OrElse Entity.TTDTotalAmount.GetValueOrDefault(0) <> 0).ToList

						Catch ex As Exception
                            CurrentTTDInvoiceDetails = Nothing
                        End Try
                    ...
                    For Each CDPDaysToPay In _CDPDaysToPay.ToList
                        ...
                        If CurrentTTDInvoiceDetails.Any(Function(Entity) Entity.ClientCode = CDPDaysToPay.ClientCode AndAlso
                    ...
                    Me.DataSource = CurrentTTDInvoiceDetails
        ...
        Private Sub GroupHeaderFunction_BeforePrint(sender As Object, e As System.Drawing.Printing.PrintEventArgs) Handles GroupHeaderFunction.BeforePrint
            ...
            If _InvoicePrintingSetting IsNot Nothing Then
                ...
                Try
                    ...
                    CampaignCode = Me.GetCurrentColumnValue(AdvantageFramework.InvoicePrinting.Classes.CurrentTTDInvoiceDetail.Properties.CampaignCode.ToString)
                ...
                Try
                    ...
                    SalesClassCode = Me.GetCurrentColumnValue(AdvantageFramework.InvoicePrinting.Classes.CurrentTTDInvoiceDetail.Properties.SalesClassCode.ToString)
                ...
                Try
                    ...
                    If _InvoicePrintingSetting.SummaryLevel = AdvantageFramework.InvoicePrinting.SummaryLevels.None Then
                        ...
                        CurrentTTDInvoiceDetails = TryCast(Me.DataSource, IEnumerable(Of AdvantageFramework.InvoicePrinting.Classes.CurrentTTDInvoiceDetail)).Where(Function(Entity) Entity.FullInvoiceNumber = FullInvoiceNumber AndAlso
                ...
                If CurrentTTDInvoiceDetails IsNot Nothing AndAlso _InvoicePrintingSetting.ShowZeroFunctionAmounts.GetValueOrDefault(False) = False Then
                    ...
                    Try
                        ...
                        CurrentTTDInvoiceDetails = CurrentTTDInvoiceDetails.Where(Function(Entity) Entity.NetAmount.GetValueOrDefault(0) <> 0 OrElse Entity.TTDNetAmount.GetValueOrDefault(0) <> 0).ToList
                ...
                If _InvoicePrintingSetting.SummaryLevel = AdvantageFramework.InvoicePrinting.SummaryLevels.Invoice Then
                    ...
                    If HeaderGroupBy.Value = 1 Then
                        ...
                        Try
                            ...
                            CurrentTTDInvoiceDetails = CurrentTTDInvoiceDetails.Where(Function(Entity) Entity.CampaignCode = CampaignCode).ToList
                    ...
                    ElseIf HeaderGroupBy.Value = 2 Then
                        ...
                        Try
                            ...
                            CurrentTTDInvoiceDetails = CurrentTTDInvoiceDetails.Where(Function(Entity) Entity.SalesClassCode = SalesClassCode).ToList
                    ...
                    ElseIf HeaderGroupBy.Value = 3 Then
                        ...
                        Try
                            ...
                            CurrentTTDInvoiceDetails = CurrentTTDInvoiceDetails.Where(Function(Entity) Entity.CampaignCode = CampaignCode AndAlso Entity.SalesClassCode = SalesClassCode).ToList
                    ...
                    ElseIf HeaderGroupBy.Value = 4 Then
                        ...
                        Try
                            ...
                            CurrentTTDInvoiceDetails = CurrentTTDInvoiceDetails.Where(Function(Entity) Entity.SalesClassCode = SalesClassCode AndAlso Entity.CampaignCode = CampaignCode).ToList
                ...
                If CurrentTTDInvoiceDetails IsNot Nothing Then

                    If CurrentTTDInvoiceDetails.Count > 0 Then
                        ...
                        If _InvoicePrintingSetting.GroupingOptionType.GetValueOrDefault(1) = 1 Then
                            ...
                            GroupByFunctionSubReport.DataSource = CurrentTTDInvoiceDetails
                        ...
                        ElseIf _InvoicePrintingSetting.GroupingOptionType.GetValueOrDefault(1) = 7 Then
                            ...
                            GroupByFunctionTypeFunctionSubReport.DataSource = CurrentTTDInvoiceDetails
                        ...
                        ElseIf _InvoicePrintingSetting.GroupingOptionType.GetValueOrDefault(1) = 2 Then
                            ...
                            GroupByFunctionHeadingFunctionSubReport.DataSource = CurrentTTDInvoiceDetails
                        ...
                        ElseIf _InvoicePrintingSetting.GroupingOptionType.GetValueOrDefault(1) = 8 Then
                            ...
                            GroupByFunctionTypeSubReport.DataSource = CurrentTTDInvoiceDetails
                        ...
                        ElseIf _InvoicePrintingSetting.GroupingOptionType.GetValueOrDefault(1) = 5 Then
                            ...
                            GroupByFunctionHeadingSubReport.DataSource = CurrentTTDInvoiceDetails
                        ...
                        ElseIf _InvoicePrintingSetting.GroupingOptionType.GetValueOrDefault(1) = 6 Then
                            ...
                            GroupByInsideOutsideSubReport.DataSource = CurrentTTDInvoiceDetails
                        ...
                        ElseIf _InvoicePrintingSetting.GroupingOptionType.GetValueOrDefault(1) = 3 Then
                            ...
                            GroupByInsideOutsideFunctionSubReport.DataSource = CurrentTTDInvoiceDetails
        ...
        Private Sub GroupFooterJobComponentComment_BeforePrint(sender As Object, e As System.Drawing.Printing.PrintEventArgs) Handles GroupFooterJobComponentComment.BeforePrint
            ...
            Else
				...
				If _InvoicePrintingSetting.SummaryLevel = AdvantageFramework.InvoicePrinting.SummaryLevels.None Then
					...
					CurrentTTDInvoiceDetails = TryCast(Me.DataSource, IEnumerable(Of AdvantageFramework.InvoicePrinting.Classes.CurrentTTDInvoiceDetail)).Where(Function(Entity) Entity.FullInvoiceNumber = Me.GetCurrentColumnValue("FullInvoiceNumber") AndAlso
				...
				ElseIf _InvoicePrintingSetting.SummaryLevel = AdvantageFramework.InvoicePrinting.SummaryLevels.Invoice Then
					...
					CurrentTTDInvoiceDetails = TryCast(Me.DataSource, IEnumerable(Of AdvantageFramework.InvoicePrinting.Classes.CurrentTTDInvoiceDetail)).Where(Function(Entity) Entity.FullInvoiceNumber = Me.GetCurrentColumnValue("FullInvoiceNumber")).ToList

				End If

				CurrentCommission = CurrentTTDInvoiceDetails.Sum(Function(EPCTTD) EPCTTD.CommissionAmount.GetValueOrDefault(0))
				TTDCommission = CurrentTTDInvoiceDetails.Sum(Function(EPCTTD) EPCTTD.TTDCommissionAmount.GetValueOrDefault(0))

				If CurrentCommission = 0 AndAlso TTDCommission = 0 Then
        ...
        Private Sub XrRichText1_BeforePrint(sender As Object, e As Drawing.Printing.PrintEventArgs) Handles XrRichText1.BeforePrint
            ...
            If TypeOf sender Is DevExpress.XtraReports.UI.XRRichText AndAlso _InvoicePrintingSetting IsNot Nothing AndAlso _InvoicePrintingSetting.SummaryLevel = AdvantageFramework.InvoicePrinting.SummaryLevels.None Then
                ...
                Comment = TryCast(Me.DataSource, IEnumerable(Of AdvantageFramework.InvoicePrinting.Classes.CurrentTTDInvoiceDetail)).Where(Function(Entity) Entity.FullInvoiceNumber = Me.GetCurrentColumnValue("FullInvoiceNumber") AndAlso
                                                                                                                                                            Entity.JobNumber = Me.GetCurrentColumnValue("JobNumber") AndAlso
        ...
        Private Sub XrTableCellClientPO_BeforePrint(sender As Object, e As Drawing.Printing.PrintEventArgs) Handles XrTableCellClientPO.BeforePrint
            ...
            CurrentTTDInvoiceDetail = TryCast(Me.DataSource, IEnumerable(Of AdvantageFramework.InvoicePrinting.Classes.CurrentTTDInvoiceDetail)).OrderBy(Function(Entity) Entity.JobNumber).ThenBy(Function(Entity) Entity.ComponentNumber).FirstOrDefault

            If CurrentTTDInvoiceDetail IsNot Nothing Then

                XrTableCellClientPO.Text = CurrentTTDInvoiceDetail.ClientPO
        ...
        Private Sub GroupFooterInvoice_BeforePrint(sender As Object, e As System.Drawing.Printing.PrintEventArgs) Handles GroupFooterInvoice.BeforePrint
            ...
            If TryCast(Me.DataSource, IEnumerable(Of AdvantageFramework.InvoicePrinting.Classes.CurrentTTDInvoiceDetail)).Any(Function(Entity) Entity.DiscountAmount.GetValueOrDefault(0) <> Entity.TotalAmount.GetValueOrDefault(0)) Then
