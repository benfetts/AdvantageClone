Namespace Reporting.Presentation
    ...
    Public Class EmployeeUtilizationReportEditForm
...
#Region " Methods "
        ...
        End Sub
        Private Sub LoadGrid(ByVal StartDate As Date, ByVal EndDate As Date, ByVal Groupby As String, ByVal LimitWIP As Boolean, ByVal UserID As String, ByVal UseType As Boolean)

            Try

                DataGridViewForm_JobVersionReport.DataSource = LoadEmployeeUtilizationReportDataTable(StartDate, EndDate, Groupby, LimitWIP, Me.Session.UserCode, UseType)

                Me.DataGridViewForm_JobVersionReport.CurrentView.ViewCaption = Me.DataGridViewForm_JobVersionReport.CurrentView.RowCount & " Employee Utilization Report(s)"
        ...
        End Sub
        Private Function LoadEmployeeUtilizationReportDataTable(ByVal StartDate As Date, ByVal EndDate As Date, ByVal Groupby As String, ByVal LimitWIP As Boolean, ByVal UserID As String, ByVal UseType As Boolean) As System.Data.DataTable
            ...
            Using DbContext = New AdvantageFramework.Database.DbContext(Me.Session.ConnectionString, Me.Session.UserCode)
                ...
                Try
                    ...
                    Using SqlCommand = DbContext.CreateCommand()
                        ...
                        If Me.ComboBox_ReportType.SelectedValue = 132 Then
                        ...
                        SqlCommand.Parameters.AddWithValue("StartDate", CDate(StartDate.ToShortDateString))
                        SqlCommand.Parameters.AddWithValue("EndDate", CDate(EndDate.ToShortDateString))
                        SqlCommand.Parameters.AddWithValue("UserID", UserID)
                        SqlCommand.Parameters.AddWithValue("Groupby", Groupby)
                        SqlCommand.Parameters.AddWithValue("LimitWIP", LimitWIP)
                        SqlCommand.Parameters.AddWithValue("ARW", 1)
                        SqlCommand.Parameters.AddWithValue("UseType", UseType)
            ...
            LoadEmployeeUtilizationReportDataTable = DataTable
        ...
        Private Sub LoadDynamicReportTemplate(ByVal DynamicReportID As Integer)
            ...
            Using ReportingDbContext = New AdvantageFramework.Reporting.Database.DbContext(Me.Session.ConnectionString, Me.Session.UserCode)
                ...
                If DynamicReport IsNot Nothing Then
                    ...
                    Me.ComboBox_ReportType.SelectedValue = DynamicReport.Type.ToString

                    Me.Text = "Employee Utilization Report - " & DynamicReport.Description

                    _Description = DynamicReport.Description
                    _UserDefinedReportCategoryID = DynamicReport.UserDefinedReportCategoryID

                    If Me.ComboBox_ReportType.SelectedValue = 105 Then
                        LoadGrid(CDate("01/01/1900"), CDate("01/01/1900"), "emp", 0, Session.UserCode, 0)
                    ElseIf Me.ComboBox_ReportType.SelectedValue = 106 Then
                        LoadGrid(CDate("01/01/1900"), CDate("01/01/1900"), "emp", 0, Session.UserCode, 1)
                    ElseIf Me.ComboBox_ReportType.SelectedValue = 132 Then
                        LoadGrid(CDate("01/01/1900"), CDate("01/01/1900"), "emp", 0, Session.UserCode, 0)
                    End If
                    ...
                    For Each DynamicReportUnboundColumn In AdvantageFramework.Reporting.Database.Procedures.DynamicReportUnboundColumn.LoadByDynamicReportID(ReportingDbContext, DynamicReport.ID).OrderBy(Function(Column) Column.VisibleIndex).ToList
                        ...
                        GridColumn = DataGridViewForm_JobVersionReport.Columns.AddField(DynamicReportUnboundColumn.FieldName)

                        GridColumn.FieldName = DynamicReportUnboundColumn.FieldName
                        GridColumn.ShowUnboundExpressionMenu = True

                        GridColumn.VisibleIndex = DataGridViewForm_JobVersionReport.Columns.Count
        ...
        Private Sub SaveDynamicReportTemplate()
            ...
            Using ReportingDbContext = New AdvantageFramework.Reporting.Database.DbContext(Me.Session.ConnectionString, Me.Session.UserCode)
                ...
                DynamicReport.Type = ComboBox_ReportType.SelectedValue
        ...
        Private Sub SaveAsDynamicReportTemplate()
            ...
            If AdvantageFramework.Reporting.Presentation.EmployeeUtilizationReportUpdateDialog.ShowFormDialog(0, Description, UserDefinedReportCategoryID, False) = System.Windows.Forms.DialogResult.OK Then

                Using ReportingDbContext = New AdvantageFramework.Reporting.Database.DbContext(Me.Session.ConnectionString, Me.Session.UserCode)

                    If AdvantageFramework.Reporting.Database.Procedures.DynamicReport.Insert(ReportingDbContext, ComboBox_ReportType.SelectedValue, Description, ReportingDbContext.UserCode, Now,
...
#Region "  Show Form Methods "
        ...
        Public Shared Sub ShowForm(ByVal DynamicReportID As Integer)
            ...
            'objects
            Dim EmployeeUtilizationReportEditForm As AdvantageFramework.Reporting.Presentation.EmployeeUtilizationReportEditForm = Nothing

            EmployeeUtilizationReportEditForm = New AdvantageFramework.Reporting.Presentation.EmployeeUtilizationReportEditForm(DynamicReportID)

            EmployeeUtilizationReportEditForm.Show()
...
#Region "  Form Event Handlers "
        ...
        Private Sub JobVersionReportEditForm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            ButtonItemUnbound_Columns.Image = AdvantageFramework.My.Resources.ColumnImage
            ...
            ComboBox_ReportType.DataSource = AdvantageFramework.EnumUtilities.GetEnumDataTable(GetType(AdvantageFramework.Reporting.EmployeeUtilizationReportTypes), False, True)
            ComboBoxTemplates_Templates.SelectedValue = 1

            DataGridViewForm_JobVersionReport.DataSource = New System.Data.DataTable

            Me.Checkbox_GroupByEmp.Checked = True
        ...
        Private Sub JobVersionReportEditForm_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
            ...
            Else
                ...
                If AdvantageFramework.Reporting.Presentation.EmployeeUtilizationReportUpdateDialog.ShowFormDialog(0, _Description, _UserDefinedReportCategoryID, True) = Windows.Forms.DialogResult.OK Then
                    ...
                    Try
                        ...
                        Me.Text = "Employee Utilization Report - " & _Description
...
#Region "  Control Event Handlers "
        ...
        Private Sub ButtonItemActions_View_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonItemActions_View.Click
            ...
            If Me.Validator Then
                ...
                If Me.Checkbox_GroupByEmp.Checked = True Then
                    GroupBy = "emp"
                End If
                If Me.Checkbox_GroupByEmpDate.Checked = True Then
                    GroupBy = "empdate"
                End If
                If Me.Checkbox_GroupByEmpPeriod.Checked = True Then
                    GroupBy = "empperiod"
                End If

                If Me.ComboBox_ReportType.SelectedValue = 105 Then
                    LoadGrid(CDate(DateTimePickerDateFrom_From.Value.ToShortDateString), CDate(DateTimePickerDateTo_To.Value.ToShortDateString),
                         GroupBy, Me.Checkbox_LimitWIP.Checked, Session.UserCode, 0)
                ElseIf Me.ComboBox_ReportType.SelectedValue = 106 Then
                    LoadGrid(CDate(DateTimePickerDateFrom_From.Value.ToShortDateString), CDate(DateTimePickerDateTo_To.Value.ToShortDateString),
                         GroupBy, Me.Checkbox_LimitWIP.Checked, Session.UserCode, 1)
                ElseIf Me.ComboBox_ReportType.SelectedValue = 132 Then
                    LoadGrid(CDate(DateTimePickerDateFrom_From.Value.ToShortDateString), CDate(DateTimePickerDateTo_To.Value.ToShortDateString),
                         GroupBy, Me.Checkbox_LimitWIP.Checked, Session.UserCode, 0)
                End If
        ...
        Private Sub ComboBox_ReportType_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBox_ReportType.SelectedIndexChanged
            Try
                For Each col As DevExpress.XtraGrid.Columns.GridColumn In DataGridViewForm_JobVersionReport.Columns
                    If col.GroupIndex > -1 Then
                        col.GroupIndex = -1
                    End If
                Next

                Me.DataGridViewForm_JobVersionReport.Columns.Clear()
        ...
        Private Sub ButtonItemUnboundColumns_Add_Click(sender As Object, e As EventArgs) Handles ButtonItemUnbound_Columns.Click

            'objects
            Dim GridColumn As DevExpress.XtraGrid.Columns.GridColumn = Nothing

            If AdvantageFramework.WinForm.Presentation.ComplexExpressionEditorDialog.ShowFormDialog("Add Unbound Column", DataGridViewForm_JobVersionReport.CurrentView, GridColumn, AdvantageFramework.Reporting.Database.Entities.DynamicReportColumn.Properties.HeaderText) = Windows.Forms.DialogResult.OK Then

                DataGridViewForm_JobVersionReport.CurrentView.BeginUpdate()

                DataGridViewForm_JobVersionReport.Columns.Add(GridColumn)

                GridColumn.VisibleIndex = DataGridViewForm_JobVersionReport.CurrentView.VisibleColumns.Count

                DataGridViewForm_JobVersionReport.CurrentView.EndUpdate()

                If TypeOf DataGridViewForm_JobVersionReport.DataSource Is System.Windows.Forms.BindingSource Then

                    DataGridViewForm_JobVersionReport.DataSource = DirectCast(DataGridViewForm_JobVersionReport.DataSource, System.Windows.Forms.BindingSource).List.OfType(Of Object).ToList
