CREATE PROCEDURE [dbo].[advsp_national_research_network_flowchart]
...
(
	Network ASC, ProgramType ASC
)
IF @DEBUG = 1 BEGIN
	SELECT GETDATE() as 'after create IDX #AUD'
	--SELECT * FROM #AUD
END
INSERT INTO #RESULT ( [Network], ProgramType, Airings, [MediaDemoID], [Demographic], DemographicOrder,
	tot_dur,
	tot_aud,
	tot_ue,
	tot_hut,
	tot_put,
	[yyyymm])
SELECT Network, ProgramType, (SELECT COUNT(1) FROM #AUD aa WHERE a.Network = aa.Network and a.ProgramType = aa.ProgramType), MSNRDT.MediaDemoID, MDT.[Description], MSNRDT.[Order],
	tot_dur = 0,
	tot_aud = SUM(AUD),
	tot_ue = 0,
	tot_hut = 0,
...
GROUP BY Network, ProgramType, MSNRDT.MediaDemoID, MDT.[Description], MSNRDT.[Order], [yyyymm]
...
(
	ProgramType ASC, Network ASC, MediaDemoID ASC, [yyyymm] ASC
)

UPDATE R SET tot_dur = T.tot_dur
FROM #RESULT R
	INNER JOIN (
				SELECT SUM(Duration) as tot_dur, Network COLLATE SQL_Latin1_General_CP1_CS_AS as Network, ProgramType COLLATE SQL_Latin1_General_CP1_CS_AS as ProgramType, [yyyymm] COLLATE SQL_Latin1_General_CP1_CS_AS as [yyyymm]
				FROM #TRPT
				GROUP BY Network, ProgramType, [yyyymm]) T ON R.Network = T.Network AND R.ProgramType = T.ProgramType AND R.[yyyymm] = T.[yyyymm]
...
FROM #RESULT R INNER JOIN 
	(
	SELECT SUM(PUT) AS TOT_PUT, Network COLLATE SQL_Latin1_General_CP1_CS_AS as Network, MDDT.MediaDemographicID, ProgramType COLLATE SQL_Latin1_General_CP1_CS_AS as ProgramType, [yyyymm] COLLATE SQL_Latin1_General_CP1_CS_AS as [yyyymm]
	FROM @PUT
	UNPIVOT
		(
			PUT
		...
		INNER JOIN dbo.NIELSEN_DEMO ND ON a.NIELSEN_DEMO_CODE = ND.CODE 
		INNER JOIN @LOCAL_MEDIA_DEMO_DETAIL_TYPE MDDT ON MDDT.NielsenDemographicID = ND.NIELSEN_DEMO_ID 
	GROUP BY Network, ProgramType, MDDT.MediaDemographicID, [yyyymm]
	) PUT ON R.Network = PUT.Network AND R.MediaDemoID = PUT.MediaDemographicID AND R.ProgramType = PUT.ProgramType AND PUT.[yyyymm] = R.[yyyymm]
...
FROM #RESULT R INNER JOIN 
	(
	SELECT SUM(UE) AS TOT_UE, Network COLLATE SQL_Latin1_General_CP1_CS_AS as Network, MDDT.MediaDemographicID, ProgramType COLLATE SQL_Latin1_General_CP1_CS_AS as ProgramType, [yyyymm] COLLATE SQL_Latin1_General_CP1_CS_AS as [yyyymm]
	FROM @UE
	UNPIVOT
		(
			UE
		...
		INNER JOIN @LOCAL_MEDIA_DEMO_DETAIL_TYPE MDDT ON MDDT.NielsenDemographicID = ND.NIELSEN_DEMO_ID 
	GROUP BY Network, ProgramType, MDDT.MediaDemographicID, [yyyymm]
	) UE  ON R.Network = UE.Network AND R.MediaDemoID = UE.MediaDemographicID AND R.ProgramType = UE.ProgramType AND R.[yyyymm] = UE.[yyyymm]
...
SELECT Network, ProgramType, MediaDemoID, Demographic, DemographicOrder, 
...
INTO #OUTPUT
FROM #RESULT
GROUP BY Network, ProgramType, MediaDemoID, Demographic, DemographicOrder, [yyyymm]
...
FROM #OUTPUT O
	INNER JOIN #OUTPUT OHH ON O.Network = OHH.Network AND OHH.MediaDemoID = 97 AND O.ProgramType = OHH.ProgramType AND OHH.[yyyymm] = O.[yyyymm]
...
select @cols_as = STUFF((SELECT ',SUM(' + QUOTENAME(YYYYMM) + ') as ' + QUOTENAME(YYYYMM)
        ...
        ,1,1,'')
--select @cols_as 
set @query = 'SELECT Network ' + CASE WHEN @SHOW_PROGRAM_TYPES = 1 THEN ', ProgramType' ELSE '' END + CASE WHEN @SHOW_AIRINGS = 1 THEN ', Airings' ELSE '' END + ',' + @cols_as + N' from 
             (
                select *
                from #COMPLETE
            ) x
			...
			group by Network ' + CASE WHEN @SHOW_PROGRAM_TYPES = 1 THEN ', ProgramType' ELSE '' END + CASE WHEN @SHOW_AIRINGS = 1 THEN ', Airings' ELSE '' END
