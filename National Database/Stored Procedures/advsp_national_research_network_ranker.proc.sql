CREATE PROCEDURE [dbo].[advsp_national_research_network_ranker]
...
GROUP BY Network, ProgramType, MSNRDT.MediaDemoID, MDT.[Description], MSNRDT.[Order]
...
(
	ProgramType ASC, Network ASC, MediaDemoID ASC
)

UPDATE R SET tot_dur = T.tot_dur
FROM #RESULT R
	INNER JOIN (
				SELECT SUM(Duration) as tot_dur, Network COLLATE SQL_Latin1_General_CP1_CS_AS as Network, ProgramType COLLATE SQL_Latin1_General_CP1_CS_AS as ProgramType
				FROM #TRPT
				GROUP BY Network, ProgramType) T ON R.Network = T.Network AND R.ProgramType = T.ProgramType
...
FROM #RESULT R INNER JOIN 
	(
	SELECT SUM(PUT) AS TOT_PUT, Network COLLATE SQL_Latin1_General_CP1_CS_AS as Network, MDDT.MediaDemographicID, ProgramType COLLATE SQL_Latin1_General_CP1_CS_AS as ProgramType
	FROM @PUT
	UNPIVOT
		(
			PUT
		...
		INNER JOIN dbo.NIELSEN_DEMO ND ON a.NIELSEN_DEMO_CODE = ND.CODE 
		INNER JOIN @LOCAL_MEDIA_DEMO_DETAIL_TYPE MDDT ON MDDT.NielsenDemographicID = ND.NIELSEN_DEMO_ID 
	GROUP BY Network, ProgramType, MDDT.MediaDemographicID 
	) PUT ON R.Network = PUT.Network AND R.MediaDemoID = PUT.MediaDemographicID AND R.ProgramType = PUT.ProgramType
...
FROM #RESULT R INNER JOIN 
	(
	SELECT SUM(UE) AS TOT_UE, Network COLLATE SQL_Latin1_General_CP1_CS_AS as Network, MDDT.MediaDemographicID, ProgramType COLLATE SQL_Latin1_General_CP1_CS_AS as ProgramType
	FROM @UE
	UNPIVOT
		(
			UE
		...
		INNER JOIN @LOCAL_MEDIA_DEMO_DETAIL_TYPE MDDT ON MDDT.NielsenDemographicID = ND.NIELSEN_DEMO_ID 
	GROUP BY Network, ProgramType, MDDT.MediaDemographicID 
	) UE  ON R.Network = UE.Network AND R.MediaDemoID = UE.MediaDemographicID AND R.ProgramType = UE.ProgramType
...
INTO #OUTPUT
FROM #RESULT
GROUP BY Network, ProgramType, MediaDemoID, Demographic, DemographicOrder
...
FROM #OUTPUT O
	INNER JOIN #OUTPUT OHH ON O.Network = OHH.Network AND OHH.MediaDemoID = 97 AND O.ProgramType = OHH.ProgramType
...
SET @SQL = 'INSERT INTO #FINAL (Network, ProgramType) 
						 SELECT DISTINCT Network, ProgramType FROM #COMPLETE'
exec sp_sqlexec @SQL;
...
SET @SQL = 'UPDATE F SET Airings = dtl.Airings, Demographic1 = dtl.Demographic, HPUT1 = dtl.HPUT, HPUTPercent1 = dtl.HPUTPercent, 
			...
			FROM #FINAL F 
				INNER JOIN (
						 SELECT Network, ProgramType, Airings, Demographic, HPUT, HPUTPercent, Impressions, Rating, Share, VPVH, Universe, 
...
SET @SQL = 'SELECT Network, ProgramType, Airings, ' + @COLS + 'FROM #FINAL ORDER BY Rank1'
exec sp_sqlexec @SQL;
GO
