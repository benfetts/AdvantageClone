CREATE FUNCTION [dbo].[advtf_ncc_tv_fusion_audience_book_weeks](
...
BEGIN
	...
	FROM (
		...
		FROM dbo.NCC_TV_FUSION_AUDIENCE a
			INNER JOIN dbo.NCC_TV_FUSION_UE u ON a.NCC_TV_FUSION_UE_ID = u.NCC_TV_FUSION_UE_ID AND u.VALIDATED = 1 AND a.STATION_CODE = @STATION_CODE
	...
	FROM (
		...
		FROM @RESULTS a
		WHERE [MATCH] > 0
		GROUP BY a.NIELSEN_TV_BOOK_ID, a.STATION_CODE, a.ID, a.NUMBER_DAYS, AdjustedHour, MINUTEPART, a.[week]
		HAVING SUM(a.HOUSEHOLD_AUD) IS NOT NULL
		) suma
	GROUP BY [station_code], [nielsen_tv_book_id], [week], [media_spot_tv_research_daytime_id], [number_days]

	INSERT INTO @RETURN_TABLE
	SELECT
			u.nielsen_demo_code, u.audience, u.station_code, u.nielsen_tv_book_id, u.[week], u.media_spot_tv_research_daytime_id, u.number_days
	FROM @tt
	UNPIVOT
	(
		audience
...
GRANT SELECT ON [advtf_ncc_tv_fusion_audience_book_weeks] TO PUBLIC
GO
