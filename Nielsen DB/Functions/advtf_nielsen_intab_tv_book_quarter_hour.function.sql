CREATE FUNCTION [dbo].[advtf_nielsen_intab_tv_book_quarter_hour](
...
BEGIN
	...
	FROM
		(SELECT 
			...
			[week] = CASE
					 ...
					 END,
			MINUTEPART = CASE WHEN DATEDIFF(mi,CAST(CONVERT(char(10), CASE WHEN DATEDIFF(mi, CONVERT(char(10), INTAB_DATE, 101), INTAB_DATE) < @ADJUST_MINUTES THEN CONVERT(char(10), DATEADD(dd, -1, INTAB_DATE), 101) ELSE CONVERT(char(10), INTAB_DATE, 101) END,110) + ' ' + d.StartTime AS smalldatetime),INTAB_DATE) BETWEEN -15 and -1 THEN
							   15 + DATEDIFF(mi,CAST(CONVERT(char(10),CASE WHEN DATEDIFF(mi, CONVERT(char(10), INTAB_DATE, 101), INTAB_DATE) < @ADJUST_MINUTES THEN CONVERT(char(10), DATEADD(dd, -1, INTAB_DATE), 101) ELSE CONVERT(char(10), INTAB_DATE, 101) END,110) + ' ' + d.StartTime AS smalldatetime),INTAB_DATE) 
						  WHEN DATEDIFF(mi,INTAB_DATE,CAST(CONVERT(char(10),a.INTAB_DATE,110) + ' ' + d.EndTime AS smalldatetime)) BETWEEN 1 and 14 THEN
								DATEDIFF(mi,INTAB_DATE,CAST(CONVERT(char(10),a.INTAB_DATE,110) + ' ' + d.EndTime AS smalldatetime))
						  ELSE 15 END,
			AdjustedDateTime = CASE WHEN DATEDIFF(mi, CONVERT(char(10), INTAB_DATE, 101), INTAB_DATE) < @ADJUST_MINUTES THEN 
										CONVERT(char(10), DATEADD(dd, -1, INTAB_DATE), 101)
									ELSE CONVERT(char(10), INTAB_DATE, 101)
									END,
			AdjustedHour = CASE WHEN DATEDIFF(mi, CONVERT(char(10), INTAB_DATE, 101), INTAB_DATE) < @ADJUST_MINUTES THEN 
								CAST(LEFT(CONVERT(char(5), INTAB_DATE, 108),2) as smallint) * 100 + CAST(RIGHT(CONVERT(char(5), INTAB_DATE, 108),2) as smallint) + 2400
							ELSE CAST(LEFT(CONVERT(char(5), INTAB_DATE, 108),2) as smallint) * 100 + CAST(RIGHT(CONVERT(char(5), INTAB_DATE, 108),2) as smallint)
							END
	...
	GROUP BY a.NIELSEN_TV_BOOK_ID, a.ID, [week], AdjustedHour, MINUTEPART, AdjustedDateTime
	HAVING AVG(a.HOUSEHOLD_INTAB) IS NOT NULL

	INSERT INTO @RETURN_TABLE
	SELECT
			u.nielsen_demo_code, u.avg_intab, u.nielsen_tv_book_id, u.media_spot_tv_research_daytime_id, [week], MINUTEPART, AdjustedDateTime, AdjustedHour
	FROM @tt
	UNPIVOT
	(
		avg_intab
