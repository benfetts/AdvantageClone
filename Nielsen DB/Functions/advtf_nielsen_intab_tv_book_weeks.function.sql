CREATE FUNCTION [dbo].[advtf_nielsen_intab_tv_book_weeks](
...
BEGIN
	...
	FROM
		(SELECT 
			...
			d.ID,
			[week] = CASE
						WHEN CAST(CONVERT(varchar(10), a.INTAB_DATE, 101) + ' ' + CONVERT(varchar(15), b.START_DATETIME, 114) AS smalldatetime) BETWEEN b.START_DATETIME AND DATEADD(wk,-3,b.END_DATETIME) THEN 1
						WHEN CAST(CONVERT(varchar(10), a.INTAB_DATE, 101) + ' ' + CONVERT(varchar(15), b.START_DATETIME, 114) AS smalldatetime) BETWEEN DATEADD(wk,1,b.START_DATETIME) AND DATEADD(wk,-2,b.END_DATETIME) THEN 2
						WHEN CAST(CONVERT(varchar(10), a.INTAB_DATE, 101) + ' ' + CONVERT(varchar(15), b.START_DATETIME, 114) AS smalldatetime) BETWEEN DATEADD(wk,2,b.START_DATETIME) AND DATEADD(wk,-1,b.END_DATETIME) THEN 3
						WHEN CAST(CONVERT(varchar(10), a.INTAB_DATE, 101) + ' ' + CONVERT(varchar(15), b.START_DATETIME, 114) AS smalldatetime) BETWEEN DATEADD(wk,3,b.START_DATETIME) AND b.END_DATETIME THEN 4
					 END
		FROM dbo.NIELSEN_TV_INTAB a
			INNER JOIN dbo.NIELSEN_TV_BOOK b ON a.NIELSEN_TV_BOOK_ID = b.NIELSEN_TV_BOOK_ID
											 ...
											 AND CAST(CONVERT(varchar(10), a.INTAB_DATE, 101) + ' ' + CONVERT(varchar(15), b.START_DATETIME, 114) AS smalldatetime) BETWEEN b.START_DATETIME AND b.END_DATETIME
	...
	GROUP BY a.NIELSEN_TV_BOOK_ID, a.ID, [week]
	HAVING AVG(a.HOUSEHOLD_INTAB) IS NOT NULL

	INSERT INTO @RETURN_TABLE
	SELECT
			u.nielsen_demo_code, u.avg_intab, u.nielsen_tv_book_id, u.media_spot_tv_research_daytime_id, [week]
	FROM @tt
	UNPIVOT
	(
		avg_intab
