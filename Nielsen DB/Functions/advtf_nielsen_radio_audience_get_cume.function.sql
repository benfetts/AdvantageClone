...
*/
	...
	SELECT
		...
		AND c.NIELSEN_RADIO_STATION_COMBO_ID IN (SELECT items FROM dbo.udf_split_list_int(@STATION_COMBO_IDs, ','))
	...
	SELECT
			...
			(dbo.advfn_calculate_num_quarter_hours(dp.MIL_START_TIME, dp.MIL_END_TIME) * MRDT.NumDays) - (CASE WHEN dp.MIL_START_TIME < MRDT.StartHour THEN dbo.advfn_calculate_num_quarter_hours(dp.MIL_START_TIME, MRDT.StartHour) * MRDT.NumDays ELSE 0 END) - (CASE WHEN dp.MIL_END_TIME > MRDT.EndHour THEN dbo.advfn_calculate_num_quarter_hours(MRDT.EndHour, dp.MIL_END_TIME) * MRDT.NumDays ELSE 0 END)
		FROM dbo.NIELSEN_RADIO_AUDIENCE a
			INNER JOIN dbo.NIELSEN_RADIO_SEGMENT_CHILD c ON a.NIELSEN_RADIO_SEGMENT_CHILD_ID = c.NIELSEN_RADIO_SEGMENT_CHILD_ID
														AND c.LISTENING_LOCATION = @LISTENING_LOCATION
														AND c.STATION_COMBO_TYPE = @STATION_COMBO_TYPE
		...
		AND	(
			...
			OR
				((MRDT.Saturday = 1 AND MRDT.MonFri = 1 AND MRDT.Sunday = 1 AND dp.CUME_SECTION = 5) OR
				 (MRDT.Saturday = 0 AND MRDT.MonFri = 1 AND MRDT.Sunday = 1 AND dp.CUME_SECTION = 5))
			OR
				(MRDT.Saturday = 1 AND MRDT.MonFri = 0 AND MRDT.Sunday = 1 AND dp.CUME_SECTION = 6)
			)
		AND (
	...
	DECLARE @CFACTOR TABLE (
		ID INT IDENTITY(1,1) NOT NULL,
		CUME_NUM_QHRS INT NOT NULL,
		REQUEST_NUM_QHRS INT NOT NULL,
		NIELSEN_RADIO_SEGMENT_PARENT_ID INT NULL,
		CUME_FACTOR decimal(5,4) NULL
	)

	INSERT INTO @CFACTOR (CUME_NUM_QHRS, REQUEST_NUM_QHRS, NIELSEN_RADIO_SEGMENT_PARENT_ID)
	SELECT CUME_NUM_QHRS = SUM(NUMBER_QUARTER_HOURS), REQUEST_NUM_QHRS = SUM([REQUEST_NUM_QHRS]), NIELSEN_RADIO_SEGMENT_PARENT_ID
	FROM @tt
	...
	WHILE  @ID <= (SELECT MAX(ID) FROM @CFACTOR)
	BEGIN
		SELECT @CUME_NUM_QHRS = CUME_NUM_QHRS, @REQUEST_NUM_QHRS = REQUEST_NUM_QHRS
		FROM @CFACTOR
		WHERE ID = @ID

		IF @CUME_NUM_QHRS = @REQUEST_NUM_QHRS
			UPDATE @CFACTOR SET CUME_FACTOR = 1 WHERE ID = @ID
		ELSE BEGIN
			...
			IF @CUME_NUM_QHRS > 0 BEGIN
				...
				END
				UPDATE @CFACTOR SET CUME_FACTOR = @CUME_FACTOR WHERE ID = @ID
			END
		END
		SET @ID = @ID + 1
	END
	...
	SELECT
		u.NIELSEN_DEMO_CODE, u.CUME * cf.CUME_FACTOR, mdd.MediaDemographicID, NIELSEN_RADIO_STATION_COMBO_ID, u.NIELSEN_RADIO_SEGMENT_PARENT_ID, u.ID, cf.CUME_FACTOR
	FROM (
		SELECT 
			[251] = SUM([251]),
			[42] = SUM([42]),
	...
	(
		...
		INNER JOIN @CFACTOR cf ON cf.NIELSEN_RADIO_SEGMENT_PARENT_ID = u.NIELSEN_RADIO_SEGMENT_PARENT_ID
