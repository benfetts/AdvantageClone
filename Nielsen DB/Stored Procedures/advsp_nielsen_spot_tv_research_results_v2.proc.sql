CREATE PROCEDURE [dbo].[advsp_nielsen_spot_tv_research_results_v2]
...
BEGIN
	...
	IF @USE_LATEST = 1 BEGIN
		...
		FROM dbo.NIELSEN_TV_BOOK nb
			INNER JOIN (
						SELECT TOP 1 [YEAR] maxyear, [MONTH] maxmonth, STREAM, NIELSEN_MARKET_NUM, REPORTING_SERVICE
						...
						ORDER BY [YEAR] DESC, [MONTH] DESC, REPORTING_SERVICE ASC
						) YM ON nb.[YEAR] = YM.maxyear AND nb.[MONTH] = YM.maxmonth AND nb.STREAM = YM.STREAM AND nb.NIELSEN_MARKET_NUM = YM.NIELSEN_MARKET_NUM AND nb.REPORTING_SERVICE = YM.REPORTING_SERVICE
...
IF @debug = 1
	...
	SELECT
		...
		Impressions = CAST(
							CASE WHEN hut_tv_book_id <> nielsen_tv_book_id THEN 
							CASE WHEN share_hut = 0 THEN CAST(0 as bigint) 
							ELSE						  	  CAST( ROUND ( CAST( CAST(impressions as decimal) / CAST(CAST(round(share_hut,0) as int) as decimal) * 100 * hut_hut_pct / 100 as decimal(21,2)) * share_ue / 100, 0) as bigint)
							END
						  ELSE impressions
						  END as decimal) / CAST(1000 as decimal),
		share_hut,
		hut_hut,
		hut_hut_pct,
		impressions,
		...
		DemographicStream = md.[Description] + ' / ' + SUBSTRING(DateName( month , DateAdd( month , b.[MONTH] , 0 ) - 1 ), 1, 3) + SUBSTRING(CAST(b.[YEAR] as varchar), 3, 2) + '-' +
							CASE 
								WHEN b.STREAM = 'LO' THEN 'L'
								WHEN b.STREAM = 'LS' THEN 'LS'
								WHEN b.STREAM = 'L1' THEN 'L1'
								WHEN b.STREAM = 'L3' THEN 'L3'
								WHEN b.STREAM = 'L7' THEN 'L7'
								ELSE ''
							END + 
							CASE
								WHEN b.REPORTING_SERVICE = '7' THEN '-IM'
								ELSE ''
							END +
							CASE WHEN rb.HPUTBookID IS NOT NULL AND @USE_LATEST = 0 THEN ' / ' + SUBSTRING(DateName( month , DateAdd( month , b2.[MONTH] , 0 ) - 1 ), 1, 3) + SUBSTRING(CAST(b2.[YEAR] as varchar), 3, 2) + '-' +
								CASE 
									WHEN b2.STREAM = 'LO' THEN 'L'
									WHEN b2.STREAM = 'LS' THEN 'LS'
									WHEN b2.STREAM = 'L1' THEN 'L1'
									WHEN b2.STREAM = 'L3' THEN 'L3'
									WHEN b2.STREAM = 'L7' THEN 'L7'
									ELSE ''
								END +
								CASE
									WHEN b2.REPORTING_SERVICE = '7' THEN '-IM'
									ELSE ''
								END
							ELSE ''
							END +
