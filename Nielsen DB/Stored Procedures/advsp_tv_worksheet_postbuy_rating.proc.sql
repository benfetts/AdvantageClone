CREATE PROCEDURE [dbo].[advsp_tv_worksheet_postbuy_rating]
...
SELECT
		...
		Rating = CASE WHEN share_ue = 0 THEN 0 ELSE ROUND(CAST(a.impressions as decimal) * 100 / CAST(share_ue as decimal), 2) END,
		...
		BookName = SUBSTRING(DATENAME(month, cast(b.[MONTH] as varchar) + '/01/' + cast(b.[YEAR] as varchar)),1,3) + SUBSTRING(cast(b.[YEAR] as varchar),3,2) + '-' + b.STREAM +
			CASE WHEN b.COLLECTION_METHOD IN ('5', '7', '8', '9', '10', '11', '12', '13') THEN '-IM' ELSE '' END +
			CASE
...
GRANT EXEC ON [advsp_tv_worksheet_postbuy_rating] TO PUBLIC
GO