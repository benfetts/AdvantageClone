CREATE PROCEDURE [dbo].[advsp_tv_worksheet_postbuy_rating_cable]
...
DECLARE
	...
	set @NIELSEN_MARKET_NUM = 133
	set @BookIDs  = '97'
	set @HPUTBookIDs = NULL
	set @StationCodes = '6218'
	set @NCCTVSyscodeID  = 1340
	set @HOSTED_CLIENT_CODE = NULL

	insert into @MEDIA_DEMO_TYPE (ID, Description) values(73	,'Adults 18-49')
	
		
	insert into @MEDIA_SPOT_TV_RESEARCH_DAYTIME_TYPE (ID, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday, StartTime, EndTime, StartHour, EndHour, [Days])
	values (1, 1, 1, 1, 1, 1, 0, 0, '9:00 am', '10:00 am', 900, 1000, 'M-F')
	
	insert into @MEDIA_DEMO_DETAIL_TYPE (MediaDemographicID, NielsenDemographicID) values(73	,15)
	insert into @MEDIA_DEMO_DETAIL_TYPE (MediaDemographicID, NielsenDemographicID) values(73	,6)
	insert into @MEDIA_DEMO_DETAIL_TYPE (MediaDemographicID, NielsenDemographicID) values(73	,16)
	insert into @MEDIA_DEMO_DETAIL_TYPE (MediaDemographicID, NielsenDemographicID) values(73	,7)
	insert into @MEDIA_DEMO_DETAIL_TYPE (MediaDemographicID, NielsenDemographicID) values(73	,17)
	insert into @MEDIA_DEMO_DETAIL_TYPE (MediaDemographicID, NielsenDemographicID) values(73	,8)
	insert into @MEDIA_DEMO_DETAIL_TYPE (MediaDemographicID, NielsenDemographicID) values(73	,18)
	insert into @MEDIA_DEMO_DETAIL_TYPE (MediaDemographicID, NielsenDemographicID) values(73	,9)
*/
...
--		INNER JOIN (
--					SELECT * 
--					FROM dbo.advtf_ncc_tv_fusion_audience_postbuy(@MEDIA_SPOT_TV_RESEARCH_DAYTIME_TYPE, @NIELSEN_MARKET_NUM, @StationCodes, @BookIDs)
...
--END ELSE BEGIN
	...
	FROM 
		...
		INNER JOIN (
					SELECT * 
					FROM dbo.advtf_nielsen_audience_spot_tv_postbuy(@MEDIA_SPOT_TV_RESEARCH_DAYTIME_TYPE, @NIELSEN_MARKET_NUM, @StationCodes, @BookIDs)
					) aud ON nd.CODE = aud.nielsen_demo_code
	GROUP BY md.ID, aud.station_code, aud.nielsen_tv_book_id, aud.media_spot_tv_research_daytime_id, aud.number_days
...
SELECT
	...
	Rating,
	Share = CASE WHEN HPUT = 0 THEN 0 ELSE ROUND(Rating, 2) / HPUT * 100 END,
	HPUT,
	Impressions,
	Universe,
	BookID,
...
FROM (
	...
	FROM (
		SELECT
			...
			BookName = SUBSTRING(DATENAME(month, cast(b.[MONTH] as varchar) + '/01/' + cast(b.[YEAR] as varchar)),1,3) + SUBSTRING(cast(b.[YEAR] as varchar),3,2) + '-' + b.STREAM,
			ProgramName = dbo.advfn_nielsen_program_get(@NIELSEN_MARKET_NUM, tablea.station_code, b.START_DATETIME, b.END_DATETIME, dt.StartHour, dt.EndHour,
