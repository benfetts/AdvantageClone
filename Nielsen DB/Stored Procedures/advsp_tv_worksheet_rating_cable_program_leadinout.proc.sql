CREATE PROCEDURE [dbo].[advsp_tv_worksheet_rating_cable_program_leadinout]
...
--		aud.media_spot_tv_research_daytime_id, aud.number_days, 0, 0, 0, 0, 0, 0, 0, 0, 0, 
--		CASE WHEN @hutbookid = 0 THEN aud.nielsen_tv_book_id ELSE @hutbookid END
--	FROM 
--		@MEDIA_DEMO_TYPE md
...
--		INNER JOIN (
--					SELECT * 
--					FROM dbo.advtf_ncc_tv_fusion_audience_book_quarter_hour(@MEDIA_SPOT_TV_RESEARCH_DAYTIME_TYPE, @NIELSEN_MARKET_NUM, @STATION_CODE, @SHARE_BOOK_ID)
--					) aud ON nd.CODE = aud.nielsen_demo_code
--	GROUP BY 
--		md.ID, aud.station_code, aud.nielsen_tv_book_id, aud.[week], aud.MINUTEPART, 
--		aud.AdjustedDateTime, aud.AdjustedHour, aud.media_spot_tv_research_daytime_id, aud.number_days
		
--	INSERT @AUD_METRICS (media_demo_id, ue, book_year, book_month, nielsen_tv_book_id)
--	SELECT 
--		md.ID, SUM(universe.ue), universe.book_year, universe.book_month, universe.nielsen_tv_book_id
--	FROM 
--		@MEDIA_DEMO_TYPE md
--		INNER JOIN @MEDIA_DEMO_DETAIL_TYPE mdd ON md.ID = mdd.MediaDemographicID 
--		INNER JOIN dbo.NIELSEN_DEMO nd ON mdd.NielsenDemographicID = nd.NIELSEN_DEMO_ID 
...
--	GROUP BY 
--		md.ID, universe.book_year, universe.book_month, universe.nielsen_tv_book_id
...
--		INNER JOIN (
--					SELECT * 
--					FROM dbo.advtf_ncc_tv_fusion_hutput_book_quarter_hour(@MEDIA_SPOT_TV_RESEARCH_DAYTIME_TYPE, @NIELSEN_MARKET_NUM, @hutbookid)
...
--		INNER JOIN (
--					SELECT * 
--					FROM dbo.advtf_ncc_tv_fusion_hutput_book_quarter_hour(@MEDIA_SPOT_TV_RESEARCH_DAYTIME_TYPE, @NIELSEN_MARKET_NUM, @SHARE_BOOK_ID)
--					) hutput ON nd.CODE = hutput.nielsen_demo_code 
--	GROUP BY 
--		md.ID, hutput.nielsen_tv_book_id, hutput.media_spot_tv_research_daytime_id, 
--		hutput.[week], hutput.MINUTEPART, hutput.AdjustedDateTime, hutput.AdjustedHour
...
WHERE u.STATION_CODE = @STATION_CODE
...
WHERE
	(u.STATION_CODE = 0 OR u.STATION_CODE = @STATION_CODE)
AND b.NIELSEN_TV_BOOK_ID = @SHARE_BOOK_ID
AND b.REPORTING_SERVICE = '1' AND b.ETHNICITY = '' AND b.EXCLUSION = ''
