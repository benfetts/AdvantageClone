CREATE PROCEDURE [dbo].[advsp_tv_worksheet_rating_program_leadinout]
...
AS
	...
	SELECT 
		md.ID, SUM(aud.audience), aud.station_code, aud.nielsen_tv_book_id, aud.[week], 
		aud.MINUTEPART, aud.AdjustedDateTime, aud.AdjustedHour,
		aud.media_spot_tv_research_daytime_id, aud.number_days, 0, 0, 0, 0, 0, 0, 0, 0, 0, @hutbookid
	FROM 
		@MEDIA_DEMO_TYPE md
		...
		INNER JOIN (
					SELECT * 
					FROM dbo.advtf_nielsen_audience_tv_book_quarter_hour(@MEDIA_SPOT_TV_RESEARCH_DAYTIME_TYPE, @NIELSEN_MARKET_NUM, @STATION_CODE, @SHARE_BOOK_ID)
					) aud ON nd.CODE = aud.nielsen_demo_code
	GROUP BY 
		md.ID, aud.station_code, aud.nielsen_tv_book_id, aud.[week], aud.MINUTEPART, aud.AdjustedDateTime, aud.AdjustedHour, aud.media_spot_tv_research_daytime_id, aud.number_days

	INSERT @AUD_METRICS (media_demo_id, ue, book_year, book_month, nielsen_tv_book_id, is_metered_market)
	SELECT 
		md.ID, SUM(universe.ue), universe.book_year, universe.book_month, universe.nielsen_tv_book_id, universe.is_metered_market
	FROM 
		@MEDIA_DEMO_TYPE md
		INNER JOIN @MEDIA_DEMO_DETAIL_TYPE mdd ON md.ID = mdd.MediaDemographicID 
		INNER JOIN dbo.NIELSEN_DEMO nd ON mdd.NielsenDemographicID = nd.NIELSEN_DEMO_ID 
	...
	GROUP BY 
		md.ID, universe.book_year, universe.book_month, universe.nielsen_tv_book_id, universe.is_metered_market

	UPDATE a 
	SET 
		a.share_ue = h.ue, a.is_metered_market = h.is_metered_market
	FROM 
		@AUDIENCE a
	...
	SELECT 
		md.ID, SUM(hutput.avg_hutput), hutput.nielsen_tv_book_id, hutput.media_spot_tv_research_daytime_id, 
		hutput.[week], hutput.MINUTEPART, hutput.AdjustedDateTime, hutput.AdjustedHour
	FROM 
		@MEDIA_DEMO_TYPE md
		INNER JOIN @MEDIA_DEMO_DETAIL_TYPE mdd ON md.ID = mdd.MediaDemographicID 
		INNER JOIN dbo.NIELSEN_DEMO nd ON mdd.NielsenDemographicID = nd.NIELSEN_DEMO_ID 
		INNER JOIN (
					SELECT * 
					FROM dbo.advtf_nielsen_hutput_tv_book_quarter_hour(@MEDIA_SPOT_TV_RESEARCH_DAYTIME_TYPE, @NIELSEN_MARKET_NUM, @hutbookid, @STATION_CODE)
					) hutput ON nd.CODE = hutput.nielsen_demo_code 
	GROUP BY 
		md.ID, hutput.nielsen_tv_book_id, hutput.media_spot_tv_research_daytime_id, 
		hutput.[week], hutput.MINUTEPART, hutput.AdjustedDateTime, hutput.AdjustedHour
	...
	FROM 
		...
		INNER JOIN (
					SELECT * 
					FROM dbo.advtf_nielsen_hutput_tv_book_quarter_hour(@MEDIA_SPOT_TV_RESEARCH_DAYTIME_TYPE, @NIELSEN_MARKET_NUM, @SHARE_BOOK_ID, @STATION_CODE)
					) hutput ON nd.CODE = hutput.nielsen_demo_code 
	GROUP BY 
		md.ID, hutput.nielsen_tv_book_id, hutput.media_spot_tv_research_daytime_id, hutput.[week], 
		hutput.[week], hutput.MINUTEPART, hutput.AdjustedDateTime, hutput.AdjustedHour

	UPDATE a 
	SET 
		a.share_hut = h.avg_hutput 
	FROM 
		@AUDIENCE a
		INNER JOIN @HUTPUT_AVG h ON a.media_demo_id = h.media_demo_id AND h.nielsen_tv_book_id = @SHARE_BOOK_ID  
									...
									AND a.[week] = h.[week]
									AND a.AdjustedDateTime = h.AdjustedDateTime
									AND a.AdjustedHour = h.AdjustedHour
		
	UPDATE a 
	SET 
		a.hut_hut = h.avg_hutput 
	FROM 
		@AUDIENCE a
	...
	UPDATE @AUDIENCE 
	SET 
		share_hut_pct = CAST(share_hut as decimal) / CAST(share_ue as decimal) * 100
	WHERE 
		share_ue <> 0 
	
	UPDATE @AUDIENCE 
	SET 
		hut_hut_pct = CAST(hut_hut as decimal) / CAST(hut_ue as decimal) * 100
	WHERE 
		hut_ue <> 0 

	INSERT INTO @INTAB_AVG
	SELECT 
		md.ID, SUM(intab.avg_intab), intab.nielsen_tv_book_id, intab.media_spot_tv_research_daytime_id, 
		intab.[week]
	FROM 
		@MEDIA_DEMO_TYPE md
		INNER JOIN @MEDIA_DEMO_DETAIL_TYPE mdd ON md.ID = mdd.MediaDemographicID 
	...
	GROUP BY 
		md.ID, intab.nielsen_tv_book_id, intab.media_spot_tv_research_daytime_id, 
		intab.[week]
	...
	FROM 
		@AUDIENCE a
		INNER JOIN @INTAB_AVG h ON a.media_demo_id = h.media_demo_id 
								   AND a.nielsen_tv_book_id = h.nielsen_tv_book_id 
								   AND a.media_spot_tv_research_daytime_id = h.media_spot_tv_research_daytime_id
	...
	SELECT 
		TOP 1 @START_TIME = StartHour, @END_TIME = EndHour, 
		@SUN = Sunday, @SAT = Saturday, @MON = Monday, @TUE = Tuesday, 
	...
	SELECT
		[Book] = CONVERT(char(3), CAST(CAST(b.MONTH AS char(2)) + '/1/' + 
				 ...
				 CASE WHEN b.STREAM = 'LO' THEN 'L' 
						...
						WHEN b.STREAM = 'L7' THEN 'L7' END + ' - Week ' + CAST(a.[week] AS char(1)), 
		[Program] = dbo.advfn_nielsen_program_get(@NIELSEN_MARKET_NUM, station_code, b.START_DATETIME, b.END_DATETIME, d.StartHour, d.EndHour,
