...
CREATE FUNCTION [dbo].[advtf_ncc_tv_fusion_hutput_book_weeks](
...
BEGIN
	...
	FROM (
		...
		GROUP BY a.NIELSEN_TV_BOOK_ID, a.[week], a.ID, AdjustedHour, MINUTEPART
		HAVING AVG(a.HOUSEHOLD_HUT) IS NOT NULL

		) suma
	GROUP BY [nielsen_tv_book_id], [week], [media_spot_tv_research_daytime_id]

	INSERT INTO @RETURN_TABLE
	SELECT
			u.nielsen_demo_code, u.avg_hutput, u.nielsen_tv_book_id, u.media_spot_tv_research_daytime_id, u.[week]
	FROM @tt
	UNPIVOT
	(
		avg_hutput
...
GRANT SELECT ON [advtf_ncc_tv_fusion_hutput_book_weeks] TO PUBLIC
GO
