...
BEGIN
	...
	FROM
		(SELECT 
			...
			d.ID,
			[week] = CASE
						WHEN a.INTAB_DATE BETWEEN b.START_DATETIME AND DATEADD(wk,-3,b.END_DATETIME) THEN 1
						WHEN a.INTAB_DATE BETWEEN DATEADD(wk,1,b.START_DATETIME) AND DATEADD(wk,-2,b.END_DATETIME) THEN 2
						WHEN a.INTAB_DATE BETWEEN DATEADD(wk,2,b.START_DATETIME) AND DATEADD(wk,-1,b.END_DATETIME) THEN 3
						WHEN a.INTAB_DATE BETWEEN DATEADD(wk,3,b.START_DATETIME) AND b.END_DATETIME THEN 4
					 END
...
GRANT SELECT ON [advtf_nielsen_intab_tv_book_weeks] TO PUBLIC
GO
