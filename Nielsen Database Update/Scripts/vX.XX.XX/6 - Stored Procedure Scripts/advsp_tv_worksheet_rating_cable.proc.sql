...
FROM (
	SELECT 
		...
		--NielsenHHAudience = nielsendatahh.audience,
		--NielsenDemoAudience = nielsendata.audience,
		--DMARating =
		--		CASE WHEN media_demo_id = 1 THEN 
		--			CASE WHEN nz = 0 THEN 0
		--			ELSE CAST(Impressions as decimal)/ CAST(nielsendatahh.audience as decimal) * 100
		--			END
		--		ELSE
		--			CASE WHEN nielsendata.audience = 0 THEN 0
		--		    ELSE CAST(Impressions as decimal) / CAST(nielsendata.audience as decimal) * 100
		--			END
		--		END
	FROM (
		SELECT
			...
			ProgramName = dbo.advfn_nielsen_program_get(@NIELSEN_MARKET_NUM, d.station_code, b.START_DATETIME, b.END_DATETIME, dt.StartHour, dt.EndHour,
										dt.Sunday, dt.Monday, dt.Tuesday, dt.Wednesday, dt.Thursday, dt.Friday, dt.Saturday, @ADJUST_MINUTES, 1),
...
GRANT EXEC ON [advsp_tv_worksheet_rating_cable] TO PUBLIC
GO