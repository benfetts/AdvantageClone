...
CREATE PROCEDURE [dbo].[advsp_tv_worksheet_rating_cable_program_drilldown]
...
IF (SELECT COUNT(1) FROM dbo.NCC_TV_LPM_MARKET WHERE NIELSEN_MARKET_NUM = @NIELSEN_MARKET_NUM) = 0 BEGIN
	...
	SELECT md.ID, SUM(aud.audience), aud.station_code, aud.nielsen_tv_book_id, aud.[week], aud.media_spot_tv_research_daytime_id, aud.number_days, 0, 0, 0, 0, 0, 0, 0, 0, 0, CASE WHEN @hutbookid = 0 THEN aud.nielsen_tv_book_id ELSE @hutbookid END
	FROM 
		@MEDIA_DEMO_TYPE md
		...
		INNER JOIN (
					SELECT * 
					FROM dbo.advtf_ncc_tv_fusion_audience_book_weeks(@MEDIA_SPOT_TV_RESEARCH_DAYTIME_TYPE, @NIELSEN_MARKET_NUM, @STATION_CODE, @SHARE_BOOK_ID)
	...
	FROM 
		...
		INNER JOIN (
					SELECT * 
					FROM dbo.advtf_ncc_tv_fusion_hutput_book_weeks(@MEDIA_SPOT_TV_RESEARCH_DAYTIME_TYPE, @NIELSEN_MARKET_NUM, @hutbookid)
	...
	FROM 
		...
		INNER JOIN (
					SELECT * 
					FROM dbo.advtf_ncc_tv_fusion_hutput_book_weeks(@MEDIA_SPOT_TV_RESEARCH_DAYTIME_TYPE, @NIELSEN_MARKET_NUM, @SHARE_BOOK_ID)
					) hutput ON nd.CODE = hutput.nielsen_demo_code 
	GROUP BY md.ID, hutput.nielsen_tv_book_id, hutput.media_spot_tv_research_daytime_id, hutput.[week]
...
SELECT
	...
	FROM @AUDIENCE a
		...
		INNER JOIN dbo.NIELSEN_TV_PROGRAM_BOOK pb ON pb.NIELSEN_MARKET_NUM = b.NIELSEN_MARKET_NUM AND pb.[YEAR] = b.[YEAR] AND pb.[MONTH] = b.[MONTH] AND pb.VALIDATED = 1
...
GRANT EXEC ON [advsp_tv_worksheet_rating_cable_program_drilldown] TO PUBLIC
GO