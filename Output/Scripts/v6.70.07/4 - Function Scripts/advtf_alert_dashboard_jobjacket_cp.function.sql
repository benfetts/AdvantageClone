IF EXISTS ( SELECT * FROM sysobjects WHERE id = OBJECT_ID( N'[dbo].[advtf_alert_dashboard_jobjacket_cp]' ) AND xtype IN ( N'FN', N'IF', N'TF' ))
BEGIN
	DROP FUNCTION [dbo].[advtf_alert_dashboard_jobjacket_cp]
END
GO
CREATE FUNCTION [dbo].[advtf_alert_dashboard_jobjacket_cp] (
...
BEGIN	
	...
	BEGIN
		...
		BEGIN
			...
			WHERE 
				AR.CURRENT_NOTIFY = 1
			    AND (COALESCE(A.IS_CS_REVIEW, 0) = 1 OR A.ALERT_CAT_ID = 79)
				AND COALESCE(A.IS_DRAFT, 0) = 0
				AND COALESCE(A.ASSIGN_COMPLETED, 0) = 0
				AND A.IS_WORK_ITEM = 1
			;
	...
	BEGIN -- DISMISSED!
		...
		BEGIN
			...
			WHERE 
				...
				AND COALESCE(A.IS_DRAFT, 0) = 0
				AND A.ALRT_NOTIFY_HDR_ID IS NULL
				AND 1 = CASE WHEN @IncludeCompleted = 0 AND COALESCE(A.ASSIGN_COMPLETED, 0) = 1 THEN 0
				             WHEN @IncludeCompleted = 1 AND COALESCE(A.ASSIGN_COMPLETED, 0) = 1 THEN 1
							 ELSE 1 END
				AND A.IS_WORK_ITEM = 1 AND
					1 =	CASE
							WHEN ISNULL(JC.JOB_NUMBER, 0) = 0 AND ISNULL(JC.JOB_COMPONENT_NBR, 0) = 0 THEN 1  -- NO NEED TO CHECK PROC CTRL IF NO JOB/COMP
