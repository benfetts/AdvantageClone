IF EXISTS ( SELECT * FROM sysobjects WHERE id = OBJECT_ID( N'[dbo].[advtf_alert_dashboard_original]' ) AND xtype IN ( N'FN', N'IF', N'TF' ))
BEGIN
	DROP FUNCTION [dbo].[advtf_alert_dashboard_original]
END
GO
CREATE FUNCTION [dbo].[advtf_alert_dashboard_original] (
...
BEGIN
	...
	BEGIN
		SELECT
			@START_DATE_AS_OF_TODAY = 0,
			@START_AND_DUE_DATE_NOT_NULL = 0,
			@TASK_STATUS = ''
			;
			SELECT @USER_CODE = (SELECT TOP 1 SU.USER_CODE FROM SEC_USER SU WITH(NOLOCK) WHERE SU.EMP_CODE = @EmployeeCode ORDER BY SU.SEC_USER_ID DESC);
	END
	-----	DATA  -----
	...
	BEGIN
			...
			BEGIN
				...
				WHERE 
					...
					1 =	CASE
							...
							WHEN @InboxType = 'sent' AND A.ALERT_USER = @USER_CODE THEN 1
		...
		BEGIN
			...
			WHERE 
				...
				AND A.IS_WORK_ITEM = 1
				AND 1 = CASE 
							WHEN @START_AND_DUE_DATE_NOT_NULL = 1 THEN
								CASE 
									WHEN A.[START_DATE] IS NOT NULL AND A.DUE_DATE IS NOT NULL THEN 1 
									ELSE 0 
								END
						ELSE 1 
						END
				AND 1 = CASE 
							WHEN @START_DATE_AS_OF_TODAY = 1 THEN
								CASE 
									WHEN A.[START_DATE] <= GETDATE() OR A.[START_DATE] IS NULL THEN 1 
									ELSE 0 
								END
							ELSE 1 
						END;
		...
		BEGIN
			...
			WHERE 
				...
				AND COALESCE(A.IS_DRAFT, 0) = 0
				AND 1 = CASE 
							WHEN @TASK_STATUS = 'P' THEN
								CASE 
									WHEN COALESCE(JTD.TASK_STATUS,'') IN ('P','','0') THEN 1 
									ELSE 0 
								END
							WHEN @TASK_STATUS = 'A' THEN
								CASE 
									WHEN COALESCE(JTD.TASK_STATUS,'') NOT IN ('P','','0') THEN 1 
									ELSE 0 
								END
							ELSE 1 
						END
				AND 1 = CASE 
							WHEN @START_AND_DUE_DATE_NOT_NULL = 1 THEN
								CASE 
									WHEN JTD.TASK_START_DATE IS NOT NULL AND JTD.JOB_REVISED_DATE IS NOT NULL THEN 1 
									ELSE 0 
								END
							ELSE 1 
						END
				AND 1 = CASE 
							WHEN @START_DATE_AS_OF_TODAY = 1 THEN
								CASE 
									WHEN JTD.TASK_START_DATE <= GETDATE() OR JTD.TASK_START_DATE IS NULL THEN 1 
									ELSE 0 
								END
							ELSE 1 
						END;
