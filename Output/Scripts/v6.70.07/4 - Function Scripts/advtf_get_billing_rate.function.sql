IF ( OBJECT_ID( N'[dbo].[advtf_get_billing_rate]', 'TF' ) IS NOT NULL )
	DROP FUNCTION [dbo].[advtf_get_billing_rate]
GO
...
CREATE FUNCTION [dbo].[advtf_get_billing_rate] ( 
												@emp_code VARCHAR(6), 
...
BEGIN
	...
	WHERE 
		...
		AND (( BRP.FNC_INCL = 1 AND BR.FNC_CODE = @fnc_code ) OR ( BRP.FNC_INCL = 0 AND BR.FNC_CODE IS NULL AND @fnc_type = 'E' ))
		AND (( BRP.EMP_INCL = 1 AND @fnc_type = 'E' AND BR.EMP_CODE = @emp_code ) OR ( BRP.EMP_INCL = 0 AND BR.EMP_CODE IS NULL ))
	...
	BEGIN
		...
		END	
		--estimate may have the function with no employee code
		IF ( @est_bill_rate IS NULL )
		BEGIN
	...
	BEGIN
		...
		WHERE 
			BR.BILL_RATE_PREC_ID = BRP.BILL_RATE_PREC_ID 
			AND (( BRP.FNC_INCL = 1 AND BR.FNC_CODE = @fnc_code ) OR ( BRP.FNC_INCL = 0 AND BR.FNC_CODE IS NULL AND @fnc_type = 'E' ))
			AND (( BRP.EMP_INCL = 1 AND BR.EMP_CODE = @emp_code AND @fnc_type = 'E' ) OR ( BRP.EMP_INCL = 0 AND BR.EMP_CODE IS NULL ))
			AND (( BRP.TITLE_INCL = 1 AND @fnc_type = 'E' AND BR.EMPLOYEE_TITLE_ID = @emp_title_id ) OR ( BRP.TITLE_INCL = 0 AND BR.EMPLOYEE_TITLE_ID IS NULL ))
			AND (( BRP.CL_INCL = 1 AND BR.CL_CODE = @cl_code ) OR ( BRP.CL_INCL = 0 AND BR.CL_CODE IS NULL ))
			AND (( BRP.DIV_INCL = 1 AND BR.DIV_CODE = @div_code ) OR ( BRP.DIV_INCL = 0 AND BR.DIV_CODE IS NULL ))
			AND (( BRP.PRD_INCL = 1 AND BR.PRD_CODE = @prd_code ) OR ( BRP.PRD_INCL = 0 AND BR.PRD_CODE IS NULL ))
