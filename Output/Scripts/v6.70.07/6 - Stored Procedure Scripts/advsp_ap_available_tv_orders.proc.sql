--force update
CREATE PROCEDURE [dbo].[advsp_ap_available_tv_orders] @vn_code varchar(6) = NULL, @office_code varchar(4) = NULL,
...
-- 20161021 MJC eliminated temp table for speed enhancement
BEGIN
	SET NOCOUNT ON;
	...
	FROM
		...
		SELECT
			...
			[NetworkID] = O.CABLE_NETWORK_STATION_CODE,
		...
		FROM
			[dbo].TV_HDR A
			INNER JOIN [dbo].VENDOR B ON A.VN_CODE = B.VN_CODE
			INNER JOIN [dbo].TV_DETAIL O ON A.ORDER_NBR = O.ORDER_NBR 
			INNER JOIN [dbo].PRODUCT P ON A.CL_CODE = P.CL_CODE AND A.DIV_CODE = P.DIV_CODE AND A.PRD_CODE = P.PRD_CODE
		WHERE	
	...
	WHERE
		(
			(@order_id IS NOT NULL AND @source_code IS NOT NULL)
			AND EXISTS (SELECT 1
						FROM dbo.BRD_IMPORT_XREF BIX
						WHERE O.OrderNumber = BIX.ORDER_NBR
						AND BIX.IMPORT_ORDER_NBR = @order_id
						AND O.LineNumber = BIX.LINE_NBR 
						AND BIX.IMPORTED_FROM = @source_code
						AND BIX.MEDIA_TYPE IN ('TV','T','CA')
						)
		OR
			(@order_id IS NULL OR @source_code IS NULL)
		)
	ORDER BY
