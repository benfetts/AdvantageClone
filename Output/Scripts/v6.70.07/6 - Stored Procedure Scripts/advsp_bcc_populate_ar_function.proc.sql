...
BEGIN
	...
	-- BJL 05/24/2013 - Validate co-op pcts sum to 100%
	-- PJH 07/27/2018 - Added @unit_split check
	IF ( @ret_val = 0 ) --AND ( @unit_split = 0 )
	BEGIN
		IF EXISTS(SELECT 1 FROM #AR_ORDER_LIST WHERE NULLIF(COOP_CODE,'') IS NOT NULL AND COOP_CODE NOT IN (select BILL_COOP_CODE from dbo.BILLING_COOP))
			SET @ret_val = -5
