...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[advsp_job_detail_analysis_qva_load_report]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[advsp_job_detail_analysis_qva_load_report]
GO


CREATE PROCEDURE [dbo].[advsp_job_detail_analysis_qva_load_report](
...
BEGIN
	...
	(
		...
		[EstimateComponentDescription] varchar(60),
	...
	BEGIN
		...
		FROM 
			...
			[dbo].[ESTIMATE_COMPONENT] AS EC ON EC.ESTIMATE_NUMBER = JC.ESTIMATE_NUMBER AND EC.EST_COMPONENT_NBR = JC.EST_COMPONENT_NBR LEFT OUTER JOIN 
...
--	SELECT GETDATE() 'BEFORE MAIN'
 ...
 --   WHERE J.JobNumber = 89653
	...
	SELECT
		...
		[FunctionHeading] = JDA.[FunctionHeading],
		...
		[SumOfNonBillableMarkup] = SUM(JDA.[SumOfNonBillableMarkup]),
		...
		[EstimateDescription] = JDA.[EstimateDescription],
		[EstimateComponentNumber] = JDA.[EstimateComponentNumber],
		[EstimateComponentDescription] = JDA.[EstimateComponentDescription],
		[EstimateQuoteNumber] = JDA.[EstimateQuoteNumber],
		[EstimateRevisionNumber] = JDA.[EstimateRevisionNumber],
		[DueDate],
		[StartDate],
		[SumOfEstimateNetEmployeeTime] = CASE JDA.[FunctionType] WHEN 'E' THEN SUM(JDA.[SumOfEstimate]) - SUM(JDA.[SumOfEstimateCont]) - SUM(JDA.[SumOfEstimateResaleTax]) - SUM(JDA.[SumOfEstimateExtMarkup]) ELSE 0 END,
		[SumOfActualNetEmployeeTime] = CASE JDA.[FunctionType] WHEN 'E' THEN SUM(JDA.[SumOfLineTotal]) -  SUM(JDA.[SumOfExtMarkupAmount]) - SUM(JDA.[SumOfResaleTax]) + SUM(JDA.[SumOfNonBillableAmount]) - SUM(JDA.[SumOfNonBillableMarkup]) ELSE 0 END,
		[SumOfEstimateNet] = SUM(JDA.[SumOfEstimate]) - SUM(JDA.[SumOfEstimateCont]) - SUM(JDA.[SumOfEstimateResaleTax]) - SUM(JDA.[SumOfEstimateExtMarkup]),
		[SumOfActualNet] = SUM(JDA.[SumOfLineTotal]) -  SUM(JDA.[SumOfExtMarkupAmount]) - SUM(JDA.[SumOfResaleTax]) + SUM(JDA.[SumOfNonBillableAmount]) - SUM(JDA.[SumOfNonBillableMarkup])
	FROM
		(SELECT 
		[ID] = ISNULL(NEWID(), NEWID()),
		...
		[FunctionHeading] = JD.[FunctionHeading],
		...
		[ItemDate] = CASE JD.[FunctionType] WHEN 'E' THEN JD.[ItemDate] ELSE NULL END,	
		...
		[SumOfNonBillableAmount] = CASE WHEN JD.[ResourceType] IN('E','V') AND JD.[IsNonBillable] = 1 THEN JD.[Total] - JD.[ResaleTaxAmount] ELSE 0 END,
		[SumOfNonBillableMarkup] = CASE WHEN JD.[ResourceType] IN('E','I','V') AND JD.[IsNonBillable] = 1 THEN ISNULL(JD.[ExtMarkupAmount],0) ELSE 0 END,
		...
		[EstimateDescription] = J.[EstimateDescription],
		[EstimateComponentNumber] = J.[EstimateComponentNumber],
		[EstimateComponentDescription] = J.[EstimateComponentDescription],
		[EstimateQuoteNumber] = JD.[EstimateQuoteNumber],
		[EstimateRevisionNumber] = JD.[EstimateRevisionNumber],
		[DueDate],
		[StartDate]
	FROM 
		#JOB AS J INNER JOIN 
	...
	GROUP BY 
			...
			JDA.[JobTypeCode],
			JDA.[EstimateNumber],
			JDA.[EstimateDescription],
			JDA.[EstimateComponentNumber],
			JDA.[EstimateComponentDescription],
			JDA.[EstimateQuoteNumber],
			JDA.[EstimateRevisionNumber],
			JDA.[JobComponent],
			...
			JDA.[IsAdvanceBill],
			JDA.[DueDate],
			JDA.[StartDate] OPTION (RECOMPILE)--) AS J
