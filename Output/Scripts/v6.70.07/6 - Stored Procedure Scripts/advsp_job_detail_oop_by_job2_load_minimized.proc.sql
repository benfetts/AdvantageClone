if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[advsp_job_detail_oop_by_job2_load_minimized]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[advsp_job_detail_oop_by_job2_load_minimized]
GO

CREATE PROCEDURE [dbo].[advsp_job_detail_oop_by_job2_load_minimized](
	@DATE_TYPE AS int,
...
IF @debug = 1 SELECT getdate() 'before JOB'		
	...
	FROM 
		...
		[dbo].[TRAFFIC] AS T ON T.TRF_CODE = JOBT.TRF_CODE 
	WHERE
...
IF @debug = 1 SELECT getdate() 'after ND'
	...
	--						 J.[JobComponentNumber] = JD.[JobComponentNumber] WHERE J.[JobNumber] = 241
	...
	SELECT 
		...
		--J.ProductUDF3,
		--J.ProductUDF4,
		--[JobBudget] = (SELECT SUM(JOB_COMP_BUDGET_AM) FROM JOB_COMPONENT JC WHERE JC.JOB_NUMBER = J.JobNumber GROUP BY JC.JOB_NUMBER),
