CREATE PROCEDURE [dbo].[advsp_load_drpt_sales_journal]
...
FROM (
	...
	FROM
		(SELECT
			...
			--[InvoiceFooterComment] = C.CL_MFOOTER, -- ISNULL(dbo.advfn_invoice_printing_comment('Invoices', 'Standard Footer', ARS.OFFICE_CODE, ARS.CL_CODE, ARS.DIV_CODE, ARS.PRD_CODE), C.CL_MFOOTER),
		    ...
		    [dbo].[FNC_HEADING] AS FH ON FH.FNC_HEADING_ID = F.FNC_HEADING_ID LEFT OUTER JOIN
				(SELECT 
					...
					GROUP BY 
						...
						ERD.ESTIMATE_NUMBER,
						ERD.FNC_CODE) AS ESTC ON ESTC.JOB_NUMBER = ARS.JOB_NUMBER AND
