CREATE PROCEDURE [dbo].[advsp_media_broadcast_schedule_report_summary_station_system] 
...
BEGIN
	...
	(
		...
		,BudgetGross						DECIMAL (18,3)
		,MarketGross						DECIMAL (18,3)
		,Group1TotalGRP						FLOAT
		,Group2TotalGRP						FLOAT
	)

	
	DECLARE @MediaBroadcastWorksheetID INT
	DECLARE @MediaTypeCode VARCHAR(2) = 'T'
	DECLARE @MarketCode VARCHAR(20)
	DECLARE @BudgetGross	FLOAT
	...
	DECLARE @SPOTS_STATIONGROSS AS TABLE (MEDIA_BROADCAST_WORKSHEET_MARKET_ID INT, VN_CODE VARCHAR(100), RATE DECIMAL, SPOTS INT, STATIONGROSS DECIMAL)

	INSERT INTO @SPOTS_STATIONGROSS
	SELECT
		MBW_MD.MEDIA_BROADCAST_WORKSHEET_MARKET_ID
		,MBW_MD.VN_CODE
		,CASE WHEN SUM(SPOTS) = 0 then 0.00 ELSE SUM(RATE*SPOTS)/SUM(SPOTS) END
		,SUM(SPOTS) SPOTS
		,SUM(SPOTS * RATE) STATIONGROSS
	FROM
		MEDIA_BROADCAST_WORKSHEET MBW
	...
	WHERE (1=1)
		AND (MBW_MDD.DATE BETWEEN @START_DATE AND @END_DATE)
	GROUP BY
		MBW_MD.MEDIA_BROADCAST_WORKSHEET_MARKET_ID
		,MBW_MD.VN_CODE
	...
	UPDATE @TotalsTable
		SET MarketGross = TTX.MarketGross
	FROM
		@TotalsTable TT
			INNER JOIN 
				(SELECT
						X.MEDIA_BROADCAST_WORKSHEET_MARKET_ID
						,X.MarketCode
						,SUM(X.StationGross) MarketGross
					FROM @TotalsTable X
					GROUP BY
						X.MEDIA_BROADCAST_WORKSHEET_MARKET_ID
						,X.MarketCode) TTX
				ON TT.MEDIA_BROADCAST_WORKSHEET_MARKET_ID = TTX.MEDIA_BROADCAST_WORKSHEET_MARKET_ID

	UPDATE @TotalsTable
		SET 
			Group1TotalGRP = TTX.Group1TotalGRP
			,Group2TotalGRP = TTX.Group2TotalGRP
	FROM
		@TotalsTable TT
			INNER JOIN 
				(SELECT
						X.MEDIA_BROADCAST_WORKSHEET_MARKET_ID
						,X.MarketCode
						,SUM(X.PrimaryGRP) Group1TotalGRP
						,SUM(X.SecondaryGRP) Group2TotalGRP
					FROM @TotalsTable X
					GROUP BY
						X.MEDIA_BROADCAST_WORKSHEET_MARKET_ID
						,X.MarketCode) TTX
	...
	UPDATE @TotalsTable
		SET BudgetGross = ISNULL(MBW_MG.BudgetAmount,0.0)
	FROM
		@TotalsTable TT
		LEFT JOIN 
			(SELECT 
				MBW_MG.MEDIA_BROADCAST_WORKSHEET_MARKET_ID,
				BudgetAmount = SUM(CASE WHEN MBW_MG.WAS_RATE_IMPORTED = 1 THEN MBW_MGD.RATE * MBW_MGD.GRP
									ELSE MBW_MG.CPP * MBW_MGD.GRP END)
			FROM 
				dbo.MEDIA_BROADCAST_WORKSHEET_MARKET_GOAL MBW_MG
				INNER JOIN dbo.MEDIA_BROADCAST_WORKSHEET_MARKET_GOAL_DATE MBW_MGD ON MBW_MGD.MEDIA_BROADCAST_WORKSHEET_MARKET_GOAL_ID = MBW_MG.MEDIA_BROADCAST_WORKSHEET_MARKET_GOAL_ID
				INNER JOIN #HeadLineTable HLT ON HLT.MEDIA_BROADCAST_WORKSHEET_MARKET_ID = MBW_MG.MEDIA_BROADCAST_WORKSHEET_MARKET_ID
			WHERE
				(MBW_MGD.DATE BETWEEN HLT.START_DATE AND HLT.END_DATE)
			GROUP BY
				MBW_MG.MEDIA_BROADCAST_WORKSHEET_MARKET_ID
			 )MBW_MG
				ON TT.MEDIA_BROADCAST_WORKSHEET_MARKET_ID = MBW_MG.MEDIA_BROADCAST_WORKSHEET_MARKET_ID
	...
	IF (@Debug = 1)
		BEGIN
			...
			--SELECT * FROM @SPOTS_STATIONGROSS
		END
	...
	SELECT 
		...
		,CASE
		 ...
		 ,TT.Group1TotalGRP		TotalGroup1GRP
		 ,TT.Group2TotalGRP		TotalGroup2GRP
		 ,TT.Group2TotalGRP
		 ,@SumTotalStationGross		TotalStationGross
		 ,TT.BudgetGross		
		 ,CASE	
