if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[advsp_project_hours_used_report]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[advsp_project_hours_used_report]
GO

CREATE PROCEDURE [dbo].[advsp_project_hours_used_report]
...
AND (@JobTypeCodes IS NULL OR (@JobTypeCodes IS NOT NULL AND JC.JT_CODE IN (SELECT * FROM dbo.udf_split_list(@JobTypeCodes, ','))))
	GROUP BY
		...
		JUDV4.UDV_DESC,
		JUDV5.UDV_DESC,
		CASE WHEN jobs.IS_OPEN = 1 THEN 'Yes' ELSE 'No' END,
...
GRANT EXECUTE ON [advsp_project_hours_used_report] TO PUBLIC
GO