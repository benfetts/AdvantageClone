IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[advsp_project_schedule_report]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[advsp_project_schedule_report]
GO

CREATE PROCEDURE [dbo].[advsp_project_schedule_report]
...
EXEC [dbo].[advsp_project_schedule_maint_complete_schedules]
...
SELECT
	...
	[Employee] = STUFF((SELECT ', ' + JTDE.EMP_CODE
		  FROM
		...
		FOR XML PATH('')),1 ,2 ,''),
	[Employees] = STUFF((SELECT ', ' + E.EMP_LNAME + COALESCE( ' ' + LEFT( E.EMP_FNAME, 1 ) + '.', '' )
		  FROM
		dbo.JOB_TRAFFIC_DET_EMPS JTDE
		INNER JOIN
		dbo.EMPLOYEE_CLOAK E ON JTDE.EMP_CODE = E.EMP_CODE
		...
		JTDE.SEQ_NBR = JOBTD.SEQ_NBR 
		FOR XML PATH('')),1 ,2 ,''),
	--[Employee] = [dbo].[fn_task_employees_ds]( JOBTD.JOB_NUMBER, JOBTD.JOB_COMPONENT_NBR, JOBTD.SEQ_NBR, 1 ),
	--[Employees] = [dbo].[fn_task_employees_ds]( JOBTD.JOB_NUMBER, JOBTD.JOB_COMPONENT_NBR, JOBTD.SEQ_NBR, 3 ),
	...
	--[IsScheduleTemplate] = JOBT.IS_TEMPLATE,
...
FROM
	...
	--	FROM
	--	dbo.JOB_TRAFFIC_DET) JOBTDEMP ON JOBTDEMP.JOB_NUMBER = JOBTD.JOB_NUMBER ANDl
	...
	GROUP BY JOB_NUMBER, 		
				JOB_COMPONENT_NBR,  		
				SEQ_NBR ) HA ON HA.JOB_NUMBER = JOBTD.JOB_NUMBER AND
	...
	GROUP BY JOB_NUMBER, 		
				JOB_COMPONENT_NBR,  		
				SEQ_NBR  ) TD ON TD.JOB_NUMBER = JOBTD.JOB_NUMBER AND
...
--ONLY FOR CLIENT SAFE SCRIPTS
--GRANT EXECUTE ON [advsp_project_schedule_report] TO PUBLIC
--GO