if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_Dataset_AR_Aging]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_Dataset_AR_Aging]
GO


CREATE PROCEDURE [dbo].[usp_wv_Dataset_AR_Aging] 
	@UserID varchar(100),
...
(
	...
	SELECT 
		...
		[ClientPOProduction] = CASE WHEN amt.REC_TYPE = 'P' OR amt.REC_TYPE = 'S' THEN CASE WHEN AR.PRD_PO_NBR IS NULL THEN jc.JOB_CL_PO_NBR ELSE AR.PRD_PO_NBR END END,		
		[JobType] = jc.JT_CODE,	
		[JobContact] = ISNULL(ch.CONT_FNAME,'') + ' ' + ISNULL(ch.CONT_LNAME,''),--#01	
		[OrderNumber] = m.ORDER_NBR,	
		[OrderDescription] = m.ORDER_DESC,	
		[ClientPOMedia] = CASE WHEN amt.REC_TYPE <> 'P' AND amt.REC_TYPE <> 'S' THEN CASE WHEN AR.PRD_PO_NBR IS NULL THEN m.CLIENT_PO ELSE AR.PRD_PO_NBR END END,	
  ...
  --  LEFT JOIN 
	...
	WHERE 
		UPPER(amt.[USER_ID]) = UPPER(@UserID)
