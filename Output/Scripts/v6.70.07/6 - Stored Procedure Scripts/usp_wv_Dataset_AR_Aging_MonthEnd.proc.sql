if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_Dataset_AR_Aging_MonthEnd]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_Dataset_AR_Aging_MonthEnd]
GO


CREATE PROCEDURE [dbo].[usp_wv_Dataset_AR_Aging_MonthEnd] 
	@UserID varchar(100),
...
(
	...
	SELECT 
		...
		[InvoiceSequence] = RIGHT('00' + CAST(amt.AR_INV_SEQ AS varchar), 2),	
  ...
  --  LEFT JOIN 
    ...
    LEFT OUTER JOIN GLOXREF gx ON gl.GLAOFFICE = gx.GLOXGLOFFICE 
	LEFT JOIN 
		dbo.OFFICE AS o2 ON gx.GLOXOFFICE = o2.OFFICE_CODE	
	LEFT JOIN 
	...
	WHERE 
		UPPER(amt.[USER_ID]) = UPPER(@UserID) AND
		(o2.OFFICE_CODE IN (SELECT * FROM dbo.udf_split_list(@OFFICE_LIST, ',')) OR @OFFICE_LIST IS NULL)
   ...
   INSERT INTO #temp
	SELECT CASE WHEN ACCT_REC.REC_TYPE = 'P' THEN
				CASE WHEN ACCT_REC.DUE_DATE IS NOT NULL THEN DATEDIFF([Day],MAX(ACCT_REC.DUE_DATE),MAX(CR_CLIENT.CR_CHECK_DATE)) ELSE
					CASE WHEN CL_P_PAYDAYS IS NOT NULL THEN DATEDIFF([Day],DateAdd(Day, CL_P_PAYDAYS, MAX(ACCT_REC.AR_INV_DATE)),MAX(CR_CLIENT.CR_CHECK_DATE))
							ELSE DATEDIFF([Day],MAX(ACCT_REC.AR_INV_DATE),MAX(CR_CLIENT.CR_CHECK_DATE)) END 
				END
				ELSE
				CASE WHEN ACCT_REC.DUE_DATE IS NOT NULL THEN DATEDIFF([Day],MAX(ACCT_REC.DUE_DATE),MAX(CR_CLIENT.CR_CHECK_DATE)) ELSE 
					CASE WHEN CL_M_PAYDAYS IS NOT NULL THEN DATEDIFF([Day],DateAdd(Day, CL_M_PAYDAYS, MAX(ACCT_REC.AR_INV_DATE)),MAX(CR_CLIENT.CR_CHECK_DATE))
							ELSE DATEDIFF([Day],MAX(ACCT_REC.AR_INV_DATE),MAX(CR_CLIENT.CR_CHECK_DATE)) END
				END	
			 END AS Days,	
