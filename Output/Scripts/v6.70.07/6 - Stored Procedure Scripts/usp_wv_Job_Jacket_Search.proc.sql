/****** Object:  StoredProcedure [dbo].[usp_wv_Job_Jacket_Search]    Script Date: 7/15/2019 1:29:18 PM ******/
SET ANSI_NULLS ON
GO
...
/*
	...
	EXEC [dbo].[usp_wv_Job_Jacket_Search] 'SYSADM','','','','','',0,0,'',0,'nosales',1,null,0,50,@TotalRows=@totalRows OUTPUT,@SumBudget=@sumBudget OUTPUT
	print @totalRows
	print @sumBudget
*/
	...
	SET	@sql = ';WITH DataCTE AS (
					SELECT JOB_LOG.JOB_NUMBER JobNumber,
					isnull(JOB_LOG.JOB_DESC, '''') AS JobDescription, 
					JOB_COMPONENT.JOB_COMPONENT_NBR JobComponentNumber,
					...
					SALES_CLASS.SC_DESCRIPTION SalesClass,
					OFFICE.OFFICE_NAME OfficeName,
					JOB_COMPONENT.JOB_COMP_BUDGET_AM Budget,
					JOB_TYPE.JT_CODE,
					JOB_TYPE.JT_DESC JobType,
					JOB_LOG.COMP_OPEN,
					EstimateStatus = CASE WHEN (NOT (JOB_COMPONENT.ESTIMATE_NUMBER IS NULL)) AND (NOT (JOB_COMPONENT.EST_COMPONENT_NBR IS NULL)) AND
										  ...
										  WHEN (NOT (JOB_COMPONENT.ESTIMATE_NUMBER IS NULL)) AND (NOT (JOB_COMPONENT.EST_COMPONENT_NBR IS NULL)) AND
											   (SELECT COUNT(*) FROM ESTIMATE_APPROVAL EA WITH(NOLOCK) WHERE EA.JOB_NUMBER = JOB_COMPONENT.JOB_NUMBER AND EA.JOB_COMPONENT_NBR = JOB_COMPONENT.JOB_COMPONENT_NBR) > 0 THEN ''Client Approved''
										  WHEN (NOT (JOB_COMPONENT.ESTIMATE_NUMBER IS NULL)) AND (NOT (JOB_COMPONENT.EST_COMPONENT_NBR IS NULL)) AND
											   (SELECT COUNT(*) FROM ESTIMATE_INT_APPR EA WITH(NOLOCK) WHERE EA.JOB_NUMBER = JOB_COMPONENT.JOB_NUMBER AND EA.JOB_COMPONENT_NBR = JOB_COMPONENT.JOB_COMPONENT_NBR) > 0 THEN ''Internally Approved''
										  WHEN JOB_COMPONENT.ESTIMATE_NUMBER IS NULL AND JOB_COMPONENT.EST_COMPONENT_NBR IS NULL THEN ''No Estimate'' ELSE ''Pending Approval'' END,
					ROW_NUMBER() OVER (ORDER BY JOB_LOG.ROWID DESC) AS RowNumber
				FROM JOB_LOG 
				...
				LEFT JOIN JOB_TYPE ON JOB_TYPE.JT_CODE = JOB_COMPONENT.JT_CODE
						'
	...
	SET @sql = @sql + ' )					SELECT *, (Select count(RowNumber) from DataCTE) as Totalrows FROM DataCTE 
					WHERE 
						(RowNumber BETWEEN @START_REC AND @END_REC) or (@END_REC = 0)
					'
