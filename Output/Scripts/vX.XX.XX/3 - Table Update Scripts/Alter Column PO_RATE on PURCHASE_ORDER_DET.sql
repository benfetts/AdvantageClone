...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[V_DRPT_JOB_PO]') and OBJECTPROPERTY(id, N'IsView') = 1)
	drop view [dbo].[V_DRPT_JOB_PO]
GO

exec advsp_sql_column_prepare 'PURCHASE_ORDER_DET', 'PO_RATE'
GO

IF EXISTS(SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'PURCHASE_ORDER_DET' AND COLUMN_NAME = 'PO_RATE') BEGIN

	ALTER TABLE [dbo].[PURCHASE_ORDER_DET] ALTER COLUMN [PO_RATE] Decimal(17,4)
	
END
GO

CREATE VIEW [dbo].[V_DRPT_JOB_PO]
...
AS
	...
	SELECT
		...
		[VendorCode] = PURCHASE_ORDER.VN_CODE, 
		[VendorName] = VENDOR.VN_NAME, 
		[EmployeeCode] = PURCHASE_ORDER.EMP_CODE, 
		[EmployeeName] = COALESCE((RTRIM(EMPLOYEE_CLOAK.EMP_FNAME) + ' '),'') + COALESCE((EMPLOYEE_CLOAK.EMP_MI + '. '),'') + COALESCE(EMPLOYEE_CLOAK.EMP_LNAME,''),
		[PODate] = PURCHASE_ORDER.PO_DATE, 
		[PODueDate] = PURCHASE_ORDER.PO_DUE_DATE,
        ...
        [POQuantity] = PURCHASE_ORDER_DET.PO_QTY,
        [PORate] = PURCHASE_ORDER_DET.PO_RATE,
		[POLineAmount] = PURCHASE_ORDER_DET.PO_EXT_AMOUNT,
		[POMarkupAmount] = PURCHASE_ORDER_DET.EXT_MARKUP_AMT, 		
		[POLineTotal] = PURCHASE_ORDER_DET.PO_EXT_AMOUNT + PURCHASE_ORDER_DET.EXT_MARKUP_AMT,
		[POInstructions] = PURCHASE_ORDER.PO_MAIN_INSTRUCT,
		[PODetailInstructions] = PURCHASE_ORDER_DET.DET_INSTRUCT,
		[POComplete] = CASE WHEN PURCHASE_ORDER_DET.PO_COMPLETE = 1 THEN 'Yes' ELSE 'No' END,
		[POCreateDate] = PURCHASE_ORDER.PO_CREATE_DATE
	FROM
		...
		[dbo].PURCHASE_ORDER_DET ON JC.JOB_NUMBER = dbo.PURCHASE_ORDER_DET.JOB_NUMBER AND 
									JC.JOB_COMPONENT_NBR = dbo.PURCHASE_ORDER_DET.JOB_COMPONENT_NBR LEFT OUTER JOIN
		[dbo].PURCHASE_ORDER ON dbo.PURCHASE_ORDER.PO_NUMBER = dbo.PURCHASE_ORDER_DET.PO_NUMBER INNER JOIN
		[dbo].FUNCTIONS ON dbo.PURCHASE_ORDER_DET.FNC_CODE = dbo.FUNCTIONS.FNC_CODE INNER JOIN
        [dbo].VENDOR ON dbo.PURCHASE_ORDER.VN_CODE = dbo.VENDOR.VN_CODE INNER JOIN
        [dbo].EMPLOYEE_CLOAK ON dbo.PURCHASE_ORDER.EMP_CODE = dbo.EMPLOYEE_CLOAK.EMP_CODE
	WHERE (PURCHASE_ORDER.VOID_FLAG IS NULL OR PURCHASE_ORDER.VOID_FLAG = 0)
GO
