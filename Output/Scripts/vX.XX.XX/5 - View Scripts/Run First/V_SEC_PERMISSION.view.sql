CREATE VIEW [dbo].[V_SEC_PERMISSION]
									   ...
									   ELSE
											...
											ELSE 
												CASE 
													...
													ELSE 
														CASE 
															WHEN MAX(MA.GroupID) > 0 AND MAX(CAST(MA.UserCheckUserAccess AS int)) = 1 THEN MIN(CAST(MA.UserIsBlocked AS int)) 
															WHEN MAX(MA.GroupID) = 0 AND MAX(CAST(MA.UserCheckUserAccess AS int)) = 1 THEN MIN(CAST(MA.UserIsBlocked AS int)) 
															WHEN MAX(MA.GroupID) = 0 AND MAX(CAST(MA.UserCheckUserAccess AS int)) = 0 THEN 1
															ELSE 1 
															END 
													END
											END 
	...
	FROM
		...
		FROM
			...
			[dbo].[SEC_USER_SETTING] US ON US.SEC_USER_ID = U.SEC_USER_ID AND
										   US.SETTING_CODE = 'TIME_ENTRY_ONLY') AS MA
	GROUP BY
		MA.ApplicationID,
		MA.ModuleID,
		MA.ModuleCode,
