...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[advsp_client_ar_stmt_contact_add_update_api]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[advsp_client_ar_stmt_contact_add_update_api]
GO
...
CREATE PROCEDURE [dbo].[advsp_client_ar_stmt_contact_add_update_api] 
    @cl_code varchar(6),
    @cdp_contact_id int,
...
SELECT @found = 1 FROM [dbo].[CDP_CONTACT_HDR] WHERE [CDP_CONTACT_ID] = @cdp_contact_id AND CL_CODE = @cl_code

IF @found = 0 BEGIN
	IF @error_msg_w > '' BEGIN
		SET @error_msg_w = @error_msg_w + ' | '
	END
	SET @error_msg_w = @error_msg_w + 'Contact ID is Invalid.'
END
...
SELECT @found = 1 FROM CLIENT_AR_STMT WHERE [CDP_CONTACT_ID] = @cdp_contact_id AND CL_CODE = @cl_code
...
IF @action = 'INSERT' BEGIN
	...
	INSERT INTO [dbo].[CLIENT_AR_STMT]
			   ([CL_CODE]
			   ,[CDP_CONTACT_ID]
			   ,[DIST_VIA_EMAIL]
			   ,[DIST_VIA_PRINT]
...
GRANT EXECUTE ON [advsp_client_ar_stmt_contact_add_update_api] TO PUBLIC AS dbo
GO