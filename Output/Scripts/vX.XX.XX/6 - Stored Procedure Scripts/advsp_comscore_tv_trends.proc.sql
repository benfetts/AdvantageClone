CREATE PROCEDURE [dbo].[advsp_comscore_tv_trends]
	@MARKET_NUMBER int,
	@SHARE_BOOK_ID int,
	@STATION_NUMBER int,
...
--insert into @MEDIA_SPOT_TV_DAYTIME_TYPE values(2,1,1,1,1,1,0,0,N'05:00 AM',N'06:00 AM',500,600,N'M-F',NULL)
 ...
 --   WeekDate
	...
	FROM (
		...
		FROM (
				SELECT
					...
					h.DEMO_NUMBER,
					[Week] = CASE 
								WHEN QTR_HR BETWEEN @WEEK1_START AND @WEEK1_END THEN 1
								WHEN QTR_HR BETWEEN @WEEK2_START AND @WEEK2_END THEN 2
								WHEN QTR_HR BETWEEN @WEEK3_START AND @WEEK3_END THEN 3
								WHEN QTR_HR BETWEEN @WEEK4_START AND @WEEK4_END THEN 4
							 END
	...
	GROUP BY a.STATION_NUMBER, a.STREAM, a.COMSCORE_TV_BOOK_ID, a.ID, a.DEMO_NUMBER, a.[Week]
	...
	UPDATE r SET [Program] = dbo.[advfn_comscore_program_trend_get](@MARKET_NUMBER, r.BookID, r.StationCode, r.ComscoreDemoNumber,
																	 StartHour, EndHour, Sunday, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, @WEEK1_START, @WEEK1_END) 
	FROM @tt r
		INNER JOIN @MEDIA_SPOT_TV_DAYTIME_TYPE d ON r.MediaBroadcastWorksheetMarketDetailID = d.ID 
	WHERE r.Book = 'Week 1'
	
	UPDATE r SET [Program] = dbo.[advfn_comscore_program_trend_get](@MARKET_NUMBER, r.BookID, r.StationCode, r.ComscoreDemoNumber,
																	 StartHour, EndHour, Sunday, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, @WEEK2_START, @WEEK2_END) 
	FROM @tt r
		INNER JOIN @MEDIA_SPOT_TV_DAYTIME_TYPE d ON r.MediaBroadcastWorksheetMarketDetailID = d.ID 
	WHERE r.Book = 'Week 2'
	
	UPDATE r SET [Program] = dbo.[advfn_comscore_program_trend_get](@MARKET_NUMBER, r.BookID, r.StationCode, r.ComscoreDemoNumber,
																	 StartHour, EndHour, Sunday, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, @WEEK3_START, @WEEK3_END) 
	FROM @tt r
		INNER JOIN @MEDIA_SPOT_TV_DAYTIME_TYPE d ON r.MediaBroadcastWorksheetMarketDetailID = d.ID 
	WHERE r.Book = 'Week 3'
	
	UPDATE r SET [Program] = dbo.[advfn_comscore_program_trend_get](@MARKET_NUMBER, r.BookID, r.StationCode, r.ComscoreDemoNumber,
																	 StartHour, EndHour, Sunday, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, @WEEK4_START, @WEEK4_END) 
	FROM @tt r
		INNER JOIN @MEDIA_SPOT_TV_DAYTIME_TYPE d ON r.MediaBroadcastWorksheetMarketDetailID = d.ID 
	WHERE r.Book = 'Week 4'
