IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[advsp_employee_time_util_with_indirect_report]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[advsp_employee_time_util_with_indirect_report]
GO

CREATE PROCEDURE [dbo].[advsp_employee_time_util_with_indirect_report]
(
	@StartDate datetime,
	@EndDate datetime,
	@UserID varchar(100),
...
BEGIN
	...
	BEGIN
		...
		Begin			
			SELECT @alter_sql = 'ALTER TABLE #emp_util_time_data_total ADD [' + @label_text + 'Hours_' + CAST(@ct as varchar(50)) + '] decimal(15,2)'		
			SELECT @select_sql = @select_sql + 'ISNULL([' + @label_text + 'Hours_' + CAST(@ct as varchar(50)) + '],0) AS [' + @label_text + 'Hours_' + CAST(@ct as varchar(50)) + '],'									
		End
		Else
		Begin
		...
		If exists (Select * from tempdb.information_schema.columns where table_name like '#emp_util_time_data_total%' and column_name like @label_text) --AND (@pos >= @pos2)
		Begin			
			SELECT @update_sql = 'UPDATE #emp_util_time_data_total SET [' + @label_text + 'Hours_' + CAST(@ct as varchar(50)) + '] = '
		End
		Else
		Begin
	...
	SELECT @sql = 'SELECT 
			...
			CAST(CASE WHEN DirectHours = 0 THEN 0 ELSE ISNULL((BilledAmount/DirectHours),0) END AS DECIMAL(15,2)) as AverageHourlyRateAchieved,E.EMP_TERM_DATE AS TerminatedDate, E.VAC_FROM_DATE AS VacationDateFrom, E.VAC_TO_DATE AS VacationDateTo, E.VAC_HRS AS VacationHoursAllowed,
			E.SICK_FROM_DATE AS SickDateFrom, E.SICK_TO_DATE AS SickDateTo, E.SICK_HRS AS SickHoursAllowed, E.PERS_FROM_DATE AS PersonalDateFrom, E.PERS_TO_DATE AS PersonalDateTo, E.PERS_HRS AS PersonalHoursAllowed'
			if @select_sql <> ''
			Begin
				SELECT @sql = + @sql + ', ' + @select_sql
			End

			SELECT @sql = @sql + ', ((E.VAC_HRS + E.SICK_HRS + E.PERS_HRS) - NonDirectHours) AS TotalVariance
	...
	END	
		...
		SELECT @sql = 'SELECT 
			...
			CASE WHEN DirectHours = 0 THEN 0 ELSE ISNULL((BilledAmount/DirectHours),0) END as AverageHourlyRateAchieved,E.EMP_TERM_DATE AS TerminatedDate, E.VAC_FROM_DATE AS VacationDateFrom, E.VAC_TO_DATE AS VacationDateTo, E.VAC_HRS AS VacationHoursAllowed,
			E.SICK_FROM_DATE AS SickDateFrom, E.SICK_TO_DATE AS SickDateTo, E.SICK_HRS AS SickHoursAllowed, E.PERS_FROM_DATE AS PersonalDateFrom, E.PERS_TO_DATE AS PersonalDateTo, E.PERS_HRS AS PersonalHoursAllowed'
			if @select_sql <> ''
			Begin
				SELECT @sql = + @sql + ', ' + @select_sql
