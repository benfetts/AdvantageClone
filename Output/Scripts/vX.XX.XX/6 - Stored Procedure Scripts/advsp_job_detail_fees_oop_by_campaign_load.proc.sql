if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[advsp_job_detail_fees_oop_by_campaign_load]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[advsp_job_detail_fees_oop_by_campaign_load]
GO

CREATE PROCEDURE [dbo].[advsp_job_detail_fees_oop_by_campaign_load](
	@DATE_TYPE AS int,
...
IF @debug = 1 SELECT getdate() 'after ND'
	...
	SELECT 
		...
		[OfficeDescription] = OFFICE.OFFICE_NAME,
		...
		J.CampaignCode,
		J.CampaignName,
		[BillingBudget] = CD.CMP_BILL_BUDGET,
		[IncomeBudget] = CD.CMP_INC_BUDGET,
		SUM(JD.FeeEstimate) AS FeeEstimate,
		SUM(JD.OOPEstimate) AS OOPEstimate,
		[EstimateTotal] = SUM(JD.FeeEstimate) + SUM(JD.OOPEstimate),
		SUM(JD.FeeTotal) AS BillableFeeTotal,
		SUM(JD.BillableOOPTotal) AS BillableOOPTotal,
		[BillableTotal] = SUM(JD.FeeTotal) + SUM(JD.BillableOOPTotal),
		[BillableFeeVariance] = SUM(JD.FeeEstimate) - SUM(JD.FeeTotal),
		[BillableOOPVariance] = SUM(JD.OOPEstimate) - SUM(JD.BillableOOPTotal),
		[BillableVariance] = (SUM(JD.FeeEstimate) + SUM(JD.OOPEstimate)) - (SUM(JD.FeeTotal) + SUM(JD.BillableOOPTotal)),
		[PercentOfTotalEstimate] = CASE WHEN (SUM(JD.FeeEstimate) + SUM(JD.OOPEstimate)) > 0 THEN ((SUM(JD.FeeTotal) + SUM(JD.BillableOOPTotal)) / (SUM(JD.FeeEstimate) + SUM(JD.OOPEstimate))) * 100 ELSE 0 END,
		SUM(JD.FeeBilled) AS FeeBilled,
		SUM(JD.OOPBilled) AS OOPBilled,
		[BilledTotal] = SUM(JD.FeeBilled) + SUM(JD.OOPBilled),
		SUM(JD.TotalAdvanceBilled) AS AdvanceBilledBalance,
		SUM(JD.FeeUnbilled) AS FeeUnbilled,
		SUM(JD.OOPUnbilled) AS OOPUnbilled,
		[UnbilledTotal] = SUM(JD.FeeUnbilled) + SUM(JD.OOPUnbilled),
		SUM(JD.NonBillableFee) AS NonBillableFee,
		SUM(JD.NonBillableOOP) AS NonBillableOOP,
		[BillableAndNonBillableFeeTotal] = SUM(JD.FeeTotal) + SUM(JD.NonBillableFee),
		[BillableAndNonBillableVariance] = SUM(JD.FeeEstimate) - (SUM(JD.FeeTotal) + SUM(JD.NonBillableFee))			
	FROM 
		#JOB AS J INNER JOIN 
		#JOB_DETAIL AS JD ON J.[JobNumber] = JD.[JobNumber] AND
							 J.[JobComponentNumber] = JD.[JobComponentNumber] LEFT OUTER JOIN
		CAMPAIGN C ON C.CMP_IDENTIFIER = J.CampaignID LEFT OUTER JOIN 
		CLIENT ON C.CL_CODE = CLIENT.CL_CODE LEFT OUTER JOIN 
		DIVISION ON C.CL_CODE = DIVISION.CL_CODE AND C.DIV_CODE = DIVISION.DIV_CODE LEFT OUTER JOIN 
		PRODUCT ON C.CL_CODE = PRODUCT.CL_CODE AND C.DIV_CODE = PRODUCT.DIV_CODE AND C.PRD_CODE = PRODUCT.PRD_CODE LEFT OUTER JOIN
		OFFICE ON OFFICE.OFFICE_CODE = C.OFFICE_CODE LEFT OUTER JOIN
	    CAMPAIGN_DETAIL CD ON C.CMP_IDENTIFIER = CD.CMP_IDENTIFIER
	WHERE J.CampaignID IS NOT NULL
	GROUP BY 
		J.CampaignID,
		J.CampaignCode,
		J.CampaignName,
