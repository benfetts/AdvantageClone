...
FROM #INVOICE I 
	INNER JOIN (SELECT 
					...
					AP_GROSS_AMT = CASE WHEN NET_GROSS = 1 THEN CAST(NET_AMT / (1 - AP.COMM_PCT/100) as decimal(16,2)) ELSE CAST(NET_AMT + (NET_AMT * AP.MARKUP_PCT / 100) as decimal(16,2)) END
...
FROM #INVOICE I
	INNER JOIN (SELECT 
					...
					AP_GROSS_AMT = CASE WHEN NET_GROSS = 1 THEN CAST(NET_AMT / (1 - AP.COMM_PCT/100) as decimal(16,2)) ELSE CAST(NET_PLUS + (NET_PLUS * AP.MARKUP_PCT / 100) as decimal(16,2)) END
...
FROM #INVOICE I
	INNER JOIN (SELECT 
					...
					AP_GROSS_AMT = CASE WHEN NET_GROSS = 1 THEN CAST(EXT_NET_AMT / (1 - AP.COMM_PCT/100) as decimal(16,2)) ELSE CAST(EXT_NET_AMT + (EXT_NET_AMT * AP.MARKUP_PCT / 100) as decimal(16,2)) END
