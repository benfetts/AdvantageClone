IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[usp_cp_dto_dashboard_alert]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
    DROP PROCEDURE [dbo].[usp_cp_dto_dashboard_alert]
GO
CREATE PROCEDURE [dbo].[usp_cp_dto_dashboard_alert]
@CP_ID varchar(6),
...
SELECT
	...
	X.IsProof
FROM
	[dbo].[advtf_alert_dashboard_cp] ('', 'inbox', @GROUP_BY, @INCLUDE_BACKLOG, @OFFSET, @CP_ID) AS X
