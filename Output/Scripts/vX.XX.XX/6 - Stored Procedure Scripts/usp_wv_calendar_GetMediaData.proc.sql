...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_wv_calendar_GetMediaData]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_wv_calendar_GetMediaData]
GO




CREATE PROCEDURE [dbo].[usp_wv_calendar_GetMediaData]
@MagazineInc Char(1),
@NewspaperInc Char(1),
@InternetInc Char(1),
@OutOfHomeInc Char(1),
...
SELECT @sql = 
	'INSERT INTO #media_rows
     SELECT ''M'' AS TYPE, ''A'' AS EST_ACT, MH.OFFICE_CODE, MH.ORDER_NBR,
		 ...
		 MD.LINE_TOTAL, 0 AS COST_PER, 0 AS IMPRESSIONS, MD.LINE_CANCELLED, CL.CL_NAME, D.DIV_NAME, P.PRD_DESCRIPTION, MD.REV_NBR, MD.MAT_COMP, '''' AS SIZE_DESC, 0
	 FROM MAGAZINE_HEADER MH INNER JOIN MAGAZINE_DETAIL MD ON MH.ORDER_NBR = MD.ORDER_NBR
		...
		If @OfficeRestrictions > 0
			SELECT @sql = @sql + ' INNER JOIN EMP_OFFICE ON V.OFFICE_CODE = EMP_OFFICE.OFFICE_CODE AND EMP_OFFICE.EMP_CODE = ''' + @EMP_CODE + ''''
		if @Rescrictions > 0 
			SELECT @sql = @sql + ' LEFT OUTER JOIN SEC_CLIENT SC ON MH.CL_CODE = SC.CL_CODE AND 
						          MH.DIV_CODE = SC.DIV_CODE AND MH.PRD_CODE = SC.PRD_CODE 
								  WHERE UPPER(SC.USER_ID) = UPPER(@UserID) AND (SC.TIME_ENTRY = 0 OR SC.TIME_ENTRY IS NULL)'
		else
		...
		if @RunInsertionDate = 'Y' and @LineCancelled = 'N' and @Rescrictions > 0
			SELECT @sql = @sql + ' AND MH.ORD_PROCESS_CONTRL <> 6 AND MD.ACTIVE_REV=1 AND MD.START_DATE >= @StartDate AND MD.START_DATE <= @EndDate OR UPPER(SC.USER_ID) = UPPER(@UserID) AND MD.LINE_CANCELLED IS NULL AND MH.ORD_PROCESS_CONTRL <> 6 AND MD.ACTIVE_REV=1 AND MD.END_DATE >= @StartDate AND MD.END_DATE <= @EndDate OR UPPER(SC.USER_ID) = UPPER(@UserID) AND MD.LINE_CANCELLED IS NULL AND MH.ORD_PROCESS_CONTRL <> 6 AND MD.ACTIVE_REV=1 AND @StartDate >= MD.START_DATE AND @StartDate <= MD.END_DATE OR UPPER(SC.USER_ID) = UPPER(@UserID) AND MD.LINE_CANCELLED IS NULL AND MH.ORD_PROCESS_CONTRL <> 6 AND MD.ACTIVE_REV=1 AND @EndDate >= MD.START_DATE AND @EndDate <= MD.END_DATE					 '
		if @RunInsertionDate = 'Y' and @LineCancelled = 'Y' and @Rescrictions > 0
			SELECT @sql = @sql + ' AND MH.ORD_PROCESS_CONTRL <> 6 AND MD.ACTIVE_REV=1 AND MD.START_DATE >= @StartDate AND MD.START_DATE <= @EndDate OR UPPER(SC.USER_ID) = UPPER(@UserID) AND MH.ORD_PROCESS_CONTRL <> 6 AND MD.ACTIVE_REV=1 AND MD.END_DATE >= @StartDate AND MD.END_DATE <= @EndDate OR UPPER(SC.USER_ID) = UPPER(@UserID) AND MH.ORD_PROCESS_CONTRL <> 6 AND MD.ACTIVE_REV=1 AND @StartDate >= MD.START_DATE AND @StartDate <= MD.END_DATE OR UPPER(SC.USER_ID) = UPPER(@UserID) AND MH.ORD_PROCESS_CONTRL <> 6 AND MD.ACTIVE_REV=1 AND @EndDate >= MD.START_DATE AND @EndDate <= MD.END_DATE					 '
		if @RunInsertionDate = 'Y' and @LineCancelled = 'N' and @Rescrictions = 0
		...
		SELECT @paramlist = '@StartDate smalldatetime, @EndDate smalldatetime, @UserID varchar(20)'
...
If @NewspaperInc = 'Y'
	...
	'INSERT INTO #media_rows
     SELECT ''N'' AS TYPE, ''A'' AS EST_ACT, NH.OFFICE_CODE, NH.ORDER_NBR, 
		...
		CL.CL_NAME, D.DIV_NAME, P.PRD_DESCRIPTION, ND.REV_NBR, ND.MAT_COMP, '''' AS SIZE_DESC, 0
	FROM NEWSPAPER_HEADER NH INNER JOIN NEWSPAPER_DETAIL ND ON NH.ORDER_NBR = ND.ORDER_NBR
		...
		If @OfficeRestrictions > 0
			SELECT @sql2 = @sql2 + ' INNER JOIN EMP_OFFICE ON V.OFFICE_CODE = EMP_OFFICE.OFFICE_CODE AND EMP_OFFICE.EMP_CODE = ''' + @EMP_CODE + ''''
		if @Rescrictions > 0 
			SELECT @sql2 = @sql2 + ' LEFT OUTER JOIN SEC_CLIENT SC ON NH.CL_CODE = SC.CL_CODE AND 
		...
		if @MatDueDate = 'Y'
			SELECT @sql2 = @sql2 + ' AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND MATL_CLOSE_DATE >= ''' + CAST(@StartDate AS VARCHAR) + ''' AND MATL_CLOSE_DATE <= ''' + CAST(@EndDate AS VARCHAR) + ''''
		if @ExtMatDueDate = 'Y' 
			SELECT @sql2 = @sql2 + ' AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND EXT_MATL_DATE >= ''' + CAST(@StartDate AS VARCHAR) + ''' AND EXT_MATL_DATE <= ''' + CAST(@EndDate AS VARCHAR) + ''''
		if @CloseDate = 'Y'
			SELECT @sql2 = @sql2 + ' AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND CLOSE_DATE >= ''' + CAST(@StartDate AS VARCHAR) + ''' AND CLOSE_DATE <= ''' + CAST(@EndDate AS VARCHAR) + ''''
		if @ExtCloseDate = 'Y'
			SELECT @sql2 = @sql2 + ' AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND EXT_CLOSE_DATE >= ''' + CAST(@StartDate AS VARCHAR) + ''' AND EXT_CLOSE_DATE <= ''' + CAST(@EndDate AS VARCHAR) + ''''
		if @RunInsertionDate = 'Y' and @LineCancelled = 'N' and @Rescrictions > 0
			SELECT @sql2 = @sql2 + ' AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ND.START_DATE >= ''' + CAST(@StartDate AS VARCHAR) + ''' AND ND.START_DATE <= ''' + CAST(@EndDate AS VARCHAR) + ''' OR UPPER(SC.USER_ID) = UPPER(@UserID) AND ND.LINE_CANCELLED IS NULL AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ND.END_DATE >= ''' + CAST(@StartDate AS VARCHAR) + ''' AND ND.END_DATE <= ''' + CAST(@EndDate AS VARCHAR) + ''' OR UPPER(SC.USER_ID) = UPPER(@UserID) AND ND.LINE_CANCELLED IS NULL AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ''' + CAST(@StartDate AS VARCHAR) + ''' >= ND.START_DATE AND ''' + CAST(@StartDate AS VARCHAR) + ''' <= ND.END_DATE OR UPPER(SC.USER_ID) = UPPER(@UserID) AND ND.LINE_CANCELLED IS NULL AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ''' + CAST(@EndDate AS VARCHAR) + ''' >= ND.START_DATE AND ''' + CAST(@EndDate AS VARCHAR) + ''' <= ND.END_DATE					 '
		if @RunInsertionDate = 'Y' and @LineCancelled = 'Y' and @Rescrictions > 0
			SELECT @sql2 = @sql2 + ' AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ND.START_DATE >= ''' + CAST(@StartDate AS VARCHAR) + ''' AND ND.START_DATE <= ''' + CAST(@EndDate AS VARCHAR) + ''' OR UPPER(SC.USER_ID) = UPPER(@UserID) AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ND.END_DATE >= ''' + CAST(@StartDate AS VARCHAR) + ''' AND ND.END_DATE <= ''' + CAST(@EndDate AS VARCHAR) + ''' OR UPPER(SC.USER_ID) = UPPER(@UserID) AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ''' + CAST(@StartDate AS VARCHAR) + ''' >= ND.START_DATE AND ''' + CAST(@StartDate AS VARCHAR) + ''' <= ND.END_DATE OR UPPER(SC.USER_ID) = UPPER(@UserID) AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ''' + CAST(@EndDate AS VARCHAR) + ''' >= ND.START_DATE AND ''' + CAST(@EndDate AS VARCHAR) + ''' <= ND.END_DATE					 '
		if @RunInsertionDate = 'Y' and @LineCancelled = 'N' and @Rescrictions = 0
			SELECT @sql2 = @sql2 + ' AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ND.START_DATE >= ''' + CAST(@StartDate AS VARCHAR) + ''' AND ND.START_DATE <= ''' + CAST(@EndDate AS VARCHAR) + ''' OR ND.LINE_CANCELLED IS NULL AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ND.END_DATE >= ''' + CAST(@StartDate AS VARCHAR) + ''' AND ND.END_DATE <= ''' + CAST(@EndDate AS VARCHAR) + ''' OR ND.LINE_CANCELLED IS NULL AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ''' + CAST(@StartDate AS VARCHAR) + ''' >= ND.START_DATE AND ''' + CAST(@StartDate AS VARCHAR) + ''' <= ND.END_DATE OR ND.LINE_CANCELLED IS NULL AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ''' + CAST(@EndDate AS VARCHAR) + ''' >= ND.START_DATE AND ''' + CAST(@EndDate AS VARCHAR) + ''' <= ND.END_DATE					 '
		if @RunInsertionDate = 'Y' and @LineCancelled = 'Y' and @Rescrictions = 0
			SELECT @sql2 = @sql2 + ' AND NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ND.START_DATE >= ''' + CAST(@StartDate AS VARCHAR) + ''' AND ND.START_DATE <= ''' + CAST(@EndDate AS VARCHAR) + ''' OR NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ND.END_DATE >= ''' + CAST(@StartDate AS VARCHAR) + ''' AND ND.END_DATE <= ''' + CAST(@EndDate AS VARCHAR) + ''' OR NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ''' + CAST(@StartDate AS VARCHAR) + ''' >= ND.START_DATE AND ''' + CAST(@StartDate AS VARCHAR) + ''' <= ND.END_DATE OR NH.ORD_PROCESS_CONTRL <> 6 AND ND.ACTIVE_REV=1 AND ''' + CAST(@EndDate AS VARCHAR) + ''' >= ND.START_DATE AND ''' + CAST(@EndDate AS VARCHAR) + ''' <= ND.END_DATE					 '

		IF @debug = 1                                                      
		   PRINT @sql2    

		SELECT @paramlist2 = '@UserID varchar(20)'

		EXEC sp_executesql @sql2, @paramlist2, @UserID
...
If @InternetInc = 'Y'
	...
	'INSERT INTO #media_rows
	 SELECT ''I'' AS TYPE, ''A'' AS EST_ACT, IH.OFFICE_CODE, IH.ORDER_NBR, 
		...
		ID.GUARANTEED_IMPRESS AS IMPRESSIONS, ID.LINE_CANCELLED, CL.CL_NAME, D.DIV_NAME, P.PRD_DESCRIPTION, ID.REV_NBR, ID.MAT_COMP, ID.SIZE AS SIZE_DESC, 0
	FROM INTERNET_HEADER IH INNER JOIN INTERNET_DETAIL ID ON IH.ORDER_NBR = ID.ORDER_NBR
		...
		If @OfficeRestrictions > 0
			SELECT @sql3 = @sql3 + ' INNER JOIN EMP_OFFICE ON V.OFFICE_CODE = EMP_OFFICE.OFFICE_CODE AND EMP_OFFICE.EMP_CODE = ''' + @EMP_CODE + ''''
		if @Rescrictions > 0 
			SELECT @sql3 = @sql3 + ' LEFT OUTER JOIN SEC_CLIENT SC ON IH.CL_CODE = SC.CL_CODE AND 
		...
		if @RunInsertionDate = 'Y' and @LineCancelled = 'N' and @Rescrictions > 0
			SELECT @sql3 = @sql3 + ' AND IH.ORD_PROCESS_CONTRL <> 6 AND ID.ACTIVE_REV=1 AND ID.START_DATE >= @StartDate AND ID.START_DATE <= @EndDate OR UPPER(SC.USER_ID) = UPPER(@UserID) AND ID.LINE_CANCELLED IS NULL AND IH.ORD_PROCESS_CONTRL <> 6 AND ID.ACTIVE_REV=1 AND ID.END_DATE >= @StartDate AND ID.END_DATE <= @EndDate OR UPPER(SC.USER_ID) = UPPER(@UserID) AND ID.LINE_CANCELLED IS NULL AND IH.ORD_PROCESS_CONTRL <> 6 AND ID.ACTIVE_REV=1 AND @StartDate >= ID.START_DATE AND @StartDate <= ID.END_DATE OR UPPER(SC.USER_ID) = UPPER(@UserID) AND ID.LINE_CANCELLED IS NULL AND IH.ORD_PROCESS_CONTRL <> 6 AND ID.ACTIVE_REV=1 AND @EndDate >= ID.START_DATE AND @EndDate <= ID.END_DATE					 '
		if @RunInsertionDate = 'Y' and @LineCancelled = 'Y' and @Rescrictions > 0
			SELECT @sql3 = @sql3 + ' AND IH.ORD_PROCESS_CONTRL <> 6 AND ID.ACTIVE_REV=1 AND ID.START_DATE >= @StartDate AND ID.START_DATE <= @EndDate OR UPPER(SC.USER_ID) = UPPER(@UserID) AND IH.ORD_PROCESS_CONTRL <> 6 AND ID.ACTIVE_REV=1 AND ID.END_DATE >= @StartDate AND ID.END_DATE <= @EndDate OR UPPER(SC.USER_ID) = UPPER(@UserID) AND IH.ORD_PROCESS_CONTRL <> 6 AND ID.ACTIVE_REV=1 AND @StartDate >= ID.START_DATE AND @StartDate <= ID.END_DATE OR UPPER(SC.USER_ID) = UPPER(@UserID) AND IH.ORD_PROCESS_CONTRL <> 6 AND ID.ACTIVE_REV=1 AND @EndDate >= ID.START_DATE AND @EndDate <= ID.END_DATE					 '
		if @RunInsertionDate = 'Y' and @LineCancelled = 'N' and @Rescrictions = 0
		...
		SELECT @paramlist3 = '@StartDate smalldatetime, @EndDate smalldatetime, @UserID varchar(20)'

		EXEC sp_executesql @sql3, @paramlist3, @StartDate, @EndDate, @UserID
...
If @OutOfHomeInc = 'Y'
	...
	'INSERT INTO #media_rows
	 SELECT ''O'' AS TYPE, ''A'' AS EST_ACT, OH.OFFICE_CODE, OH.ORDER_NBR, 
		...
		OD.EST_REV_NBR, OD.LINE_TOTAL, 0 AS COST_PER, 0 AS IMPRESSIONS, OD.LINE_CANCELLED, CL.CL_NAME, D.DIV_NAME, P.PRD_DESCRIPTION, OD.REV_NBR, OD.MAT_COMP, AD_SIZE.SIZE_DESC, 0
	FROM OUTDOOR_HEADER OH INNER JOIN OUTDOOR_DETAIL OD ON OH.ORDER_NBR = OD.ORDER_NBR
		...
		If @OfficeRestrictions > 0
			SELECT @sql4 = @sql4 + ' INNER JOIN EMP_OFFICE ON V.OFFICE_CODE = EMP_OFFICE.OFFICE_CODE AND EMP_OFFICE.EMP_CODE = ''' + @EMP_CODE + ''''
		if @Rescrictions > 0 
			SELECT @sql4 = @sql4 + ' LEFT OUTER JOIN SEC_CLIENT SC ON OH.CL_CODE = SC.CL_CODE AND 
		...
		if @RunInsertionDate = 'Y' and @LineCancelled = 'N' and @Rescrictions > 0
			SELECT @sql4 = @sql4 + ' AND OH.ORD_PROCESS_CONTRL <> 6 AND OD.ACTIVE_REV=1 AND OD.POST_DATE >= @StartDate AND OD.POST_DATE <= @EndDate OR UPPER(SC.USER_ID) = UPPER(@UserID) AND OD.LINE_CANCELLED IS NULL AND OH.ORD_PROCESS_CONTRL <> 6 AND OD.ACTIVE_REV=1 AND OD.END_DATE >= @StartDate AND OD.END_DATE <= @EndDate OR UPPER(SC.USER_ID) = UPPER(@UserID) AND OD.LINE_CANCELLED IS NULL AND OH.ORD_PROCESS_CONTRL <> 6 AND OD.ACTIVE_REV=1 AND @StartDate >= OD.POST_DATE AND @StartDate <= OD.END_DATE OR UPPER(SC.USER_ID) = UPPER(@UserID) AND OD.LINE_CANCELLED IS NULL AND OH.ORD_PROCESS_CONTRL <> 6 AND OD.ACTIVE_REV=1 AND @EndDate >= OD.POST_DATE AND @EndDate <= OD.END_DATE'		
		if @RunInsertionDate = 'Y' and @LineCancelled = 'Y' and @Rescrictions > 0
			SELECT @sql4 = @sql4 + ' AND OH.ORD_PROCESS_CONTRL <> 6 AND OD.ACTIVE_REV=1 AND OD.POST_DATE >= @StartDate AND OD.POST_DATE <= @EndDate OR UPPER(SC.USER_ID) = UPPER(@UserID) AND OH.ORD_PROCESS_CONTRL <> 6 AND OD.ACTIVE_REV=1 AND OD.END_DATE >= @StartDate AND OD.END_DATE <= @EndDate OR UPPER(SC.USER_ID) = UPPER(@UserID) AND OH.ORD_PROCESS_CONTRL <> 6 AND OD.ACTIVE_REV=1 AND @StartDate >= OD.POST_DATE AND @StartDate <= OD.END_DATE OR UPPER(SC.USER_ID) = UPPER(@UserID) AND OH.ORD_PROCESS_CONTRL <> 6 AND OD.ACTIVE_REV=1 AND @EndDate >= OD.POST_DATE AND @EndDate <= OD.END_DATE'
		if @RunInsertionDate = 'Y' and @LineCancelled = 'N' and @Rescrictions = 0
		...
		SELECT @paramlist4 = '@StartDate smalldatetime, @EndDate smalldatetime, @UserID varchar(20)'

		EXEC sp_executesql @sql4, @paramlist4, @StartDate, @EndDate, @UserID
...
BEGIN
			...
			INSERT INTO @Radio_Header_Data
				...
				FROM (RADIO_HEADER INNER JOIN @Radio_Header_Max_Rev RHMR ON (RADIO_HEADER.REV_NBR = RHMR.REV_NBR) 
						...
						INNER JOIN CLIENT ON RADIO_HEADER.CL_CODE = CLIENT.CL_CODE INNER JOIN
							  ...
							  CLIENT.CL_CODE = DIVISION.CL_CODE LEFT OUTER JOIN SEC_CLIENT SC ON RADIO_HEADER.CL_CODE = SC.CL_CODE AND 
				...
				WHERE UPPER(SC.USER_ID) = UPPER(@UserID) AND (SC.TIME_ENTRY = 0 OR SC.TIME_ENTRY IS NULL) AND RADIO_HEADER.ORD_PROCESS_CONTRL <> 6
		End
		Else
		Begin
			INSERT INTO @Radio_Header_Data
	...
	if @LineCancelled = 'Y' 
		INSERT INTO @Radio_Detail_Data_No_Amounts
			SELECT RADIO_DETAIL1.ORDER_NBR, RADIO_DETAIL1.LINE_NBR, RADIO_DETAIL1.REV_NBR, RADIO_DETAIL1.BRDCAST_YEAR, 
	...
	Else
		...
		if @MatDueDate = 'Y' 
			...
			SELECT RHD.TYPE, 'A' AS EST_ACT, RHD.OFFICE_CODE, 
				...
				0 AS IMPRESSIONS, RDDNA.LINE_CANCELLED, RHD.CL_NAME AS CL_NAME, RHD.DIV_NAME AS DIV_NAME, RHD.PRD_DESCRIPTION AS PRD_DESCRIPTION, RDDNA.REV_NBR AS REV_NBR, RDDNA.MAT_COMP AS MAT_COMP, '' AS SIZE_DESC,0		
		...
		SELECT @sqlradio = 'INSERT INTO #media_rows
					SELECT ''R'', ''A'' AS EST_ACT, RH.OFFICE_CODE,
							 ...
							 RD.LINE_NBR, RD.ACTIVE_REV,
							 (CONVERT(varchar(30), RD.START_DATE, 101) + '' - '' + CONVERT(varchar(30), RD.END_DATE, 101)) AS FLIGHT_DATES, RD.START_DATE, RD.END_DATE,
							 ...
							 '''' AS PRODUCTION_SIZE, 0  AS EST_NBR, 0 AS EST_LINE_NBR, 0 AS EST_REV_NBR, 0 AS LINE_TOTAL, 0 AS COST_PER,
							 0 AS IMPRESSIONS, RD.LINE_CANCELLED, CLIENT.CL_NAME, DIVISION.DIV_NAME, PRODUCT.PRD_DESCRIPTION, RD.REV_NBR, RD.MAT_COMP, '''' AS SIZE_DESC, 0
			...
			If @OfficeRestrictions > 0
				Begin
					SELECT @sqlradio = @sqlradio + ' INNER JOIN EMP_OFFICE ON VENDOR.OFFICE_CODE = EMP_OFFICE.OFFICE_CODE AND EMP_OFFICE.EMP_CODE = ''' + @EMP_CODE + ''''
				End				
			if @Rescrictions > 0 
				Begin
					SELECT @sqlradio = @sqlradio + ' INNER JOIN SEC_CLIENT SC ON RH.CL_CODE = SC.CL_CODE AND RH.DIV_CODE = SC.DIV_CODE AND RH.PRD_CODE = SC.PRD_CODE
										  WHERE UPPER(SC.USER_ID) = UPPER(@UserID) AND (SC.TIME_ENTRY = 0 OR SC.TIME_ENTRY IS NULL) AND RH.ORD_PROCESS_CONTRL <> 6 AND RD.ACTIVE_REV = 1'
				End
			else
				Begin
					SELECT @sqlradio = @sqlradio + ' WHERE RH.ORD_PROCESS_CONTRL <> 6 AND RD.ACTIVE_REV = 1'	 
				End
			if @MatDueDate = 'Y'
				Begin
			...
			if @ExtMatDueDate = 'Y'
				Begin
					SELECT @sqlradio = @sqlradio + ' AND RD.EXT_MATL_DATE >= @StartDate AND RD.EXT_MATL_DATE <= @EndDate'	 
				End
			if @CloseDate = 'Y'
				Begin
					SELECT @sqlradio = @sqlradio + ' AND RD.CLOSE_DATE >= @StartDate AND RD.CLOSE_DATE <= @EndDate'	 
				End
			if @ExtCloseDate = 'Y'
				Begin
					SELECT @sqlradio = @sqlradio + ' AND RD.EXT_CLOSE_DATE >= @StartDate AND RD.EXT_CLOSE_DATE <= @EndDate'	 
				End
			if @RunInsertionDate = 'Y'
				Begin
			...
			if @AcceptedOrdersOnly = 'Y'
				Begin
					SELECT @sqlradio = @sqlradio + '  AND RH.ORDER_ACCEPTED = 1'	 
				End
			IF @debug = 1                                                      
				PRINT @sqlradio     

			SELECT @paramlist7 = '@StartDate smalldatetime, @EndDate smalldatetime, @UserID varchar(20)'

			EXEC sp_executesql @sqlradio, @paramlist7, @StartDate, @EndDate, @UserID
END		
 --TV Media Data
If @TelevisionInc = 'Y'
BEGIN
		...
		Begin
			...
			FROM (TV_HEADER INNER JOIN @TV_Header_Max_Rev THMR ON (TV_HEADER.REV_NBR = THMR.REV_NBR) 
					...
					INNER JOIN CLIENT ON TV_HEADER.CL_CODE = CLIENT.CL_CODE INNER JOIN
							DIVISION ON PRODUCT.CL_CODE = DIVISION.CL_CODE AND PRODUCT.DIV_CODE = DIVISION.DIV_CODE AND 
							CLIENT.CL_CODE = DIVISION.CL_CODE LEFT OUTER JOIN SEC_CLIENT SC ON TV_HEADER.CL_CODE = SC.CL_CODE AND 
			...
			WHERE UPPER(SC.USER_ID) = UPPER(@UserID) AND (SC.TIME_ENTRY = 0 OR SC.TIME_ENTRY IS NULL) AND TV_HEADER.ORD_PROCESS_CONTRL <> 6
		End
		Else
		Begin
			INSERT INTO @TV_Header_Data
	...
	if @LineCancelled = 'Y'
		INSERT INTO @TV_Detail_Data_No_Amounts
			SELECT TV_DETAIL1.ORDER_NBR, TV_DETAIL1.LINE_NBR, TV_DETAIL1.REV_NBR, TV_DETAIL1.BRDCAST_YEAR, 
	...
	Else
		...
		if @MatDueDate = 'Y'
			...
			SELECT THD.TYPE, 'A' AS EST_ACT, THD.OFFICE_CODE, 
				...
				'' AS PRODUCTION_SIZE, 0 AS EST_NBR, 0 AS EST_LINE_NBR, 0 AS EST_REV_NBR, 0 AS LINE_TOTAL, 0 AS COST_PER,
				0 AS IMPRESSIONS, TDDNA.LINE_CANCELLED, THD.CL_NAME AS CL_NAME, THD.DIV_NAME AS DIV_NAME, THD.PRD_DESCRIPTION AS PRD_DESCRIPTION, TDDNA.REV_NBR AS REV_NBR, TDDNA.MAT_COMP AS MAT_COMP, '' AS SIZE_DESC,0				
	...
	SELECT @sqltv = 'INSERT INTO #media_rows
					SELECT ''T'', ''A'' AS EST_ACT, TH.OFFICE_CODE,
						...
						TD.LINE_NBR, TD.ACTIVE_REV,  
						(CONVERT(varchar(30), TD.START_DATE, 101) + '' - '' + CONVERT(varchar(30), TD.END_DATE, 101)) AS FLIGHT_DATES, TD.START_DATE, TD.END_DATE,
						...
						'''' AS PRODUCTION_SIZE, 0 AS EST_NBR, 0 AS EST_LINE_NBR, 0 AS EST_REV_NBR, 0 AS LINE_TOTAL, 0 AS COST_PER, 
						0 AS IMPRESSIONS, TD.LINE_CANCELLED, CLIENT.CL_NAME, DIVISION.DIV_NAME, PRODUCT.PRD_DESCRIPTION, TD.REV_NBR, TD.MAT_COMP, '''' AS SIZE_DESC, 0
			...
			if @Rescrictions > 0 
				Begin
					SELECT @sqltv = @sqltv + ' INNER JOIN SEC_CLIENT SC ON TH.CL_CODE = SC.CL_CODE AND TH.DIV_CODE = SC.DIV_CODE AND TH.PRD_CODE = SC.PRD_CODE
										  WHERE UPPER(SC.USER_ID) = UPPER(@UserID) AND (SC.TIME_ENTRY = 0 OR SC.TIME_ENTRY IS NULL) AND TH.ORD_PROCESS_CONTRL <> 6 AND TD.ACTIVE_REV = 1'
				End
			else
				Begin
					SELECT @sqltv = @sqltv + ' WHERE TH.ORD_PROCESS_CONTRL <> 6 AND TD.ACTIVE_REV = 1'	 
				End
			if @MatDueDate = 'Y'
				Begin
			...
			if @ExtMatDueDate = 'Y'
				Begin
					SELECT @sqltv = @sqltv + ' AND TD.EXT_MATL_DATE >= @StartDate AND TD.EXT_MATL_DATE <= @EndDate'	 
				End
			if @CloseDate = 'Y'
				Begin
					SELECT @sqltv = @sqltv + ' AND TD.CLOSE_DATE >= @StartDate AND TD.CLOSE_DATE <= @EndDate'	 
				End
			if @ExtCloseDate = 'Y'
				Begin
					SELECT @sqltv = @sqltv + ' AND TD.EXT_CLOSE_DATE >= @StartDate AND TD.EXT_CLOSE_DATE <= @EndDate'	 
				End
			if @RunInsertionDate = 'Y'
				Begin
			...
			if @AcceptedOrdersOnly = 'Y'
				Begin
					SELECT @sqltv = @sqltv + '  AND TH.ORDER_ACCEPTED = 1'	 
				End
			if @LineCancelled <> 'Y'
				Begin
			...
			SELECT @paramlist8 = '@StartDate varchar(20), @EndDate varchar(20), @UserID varchar(20)'

			EXEC sp_executesql @sqltv, @paramlist8, @StartDate, @EndDate, @UserID
END		

UPDATE #media_rows
SET DOCUMENT_COUNT = (SELECT COUNT(*) FROM MAGAZINE_DOCUMENT D WHERE #media_rows.ORDER_NBR = D.ORDER_NBR)
WHERE [TYPE] = 'M'

UPDATE #media_rows
SET DOCUMENT_COUNT = (SELECT COUNT(*) FROM NEWSPAPER_DOCUMENT D WHERE #media_rows.ORDER_NBR = D.ORDER_NBR)
WHERE [TYPE] = 'N'

UPDATE #media_rows
SET DOCUMENT_COUNT = (SELECT COUNT(*) FROM INTERNET_DOCUMENT D WHERE #media_rows.ORDER_NBR = D.ORDER_NBR)
WHERE [TYPE] = 'I'

UPDATE #media_rows
SET DOCUMENT_COUNT = (SELECT COUNT(*) FROM OUTDOOR_DOCUMENT D WHERE #media_rows.ORDER_NBR = D.ORDER_NBR)
WHERE [TYPE] = 'O'

UPDATE #media_rows
SET DOCUMENT_COUNT = (SELECT COUNT(*) FROM RADIO_DOCUMENT D WHERE #media_rows.ORDER_NBR = D.ORDER_NBR)
WHERE [TYPE] = 'R'

UPDATE #media_rows
SET DOCUMENT_COUNT = (SELECT COUNT(*) FROM TV_DOCUMENT D WHERE #media_rows.ORDER_NBR = D.ORDER_NBR)
WHERE [TYPE] = 'T'
...
SELECT 
        ISNULL(TYPE,'') AS TYPE, 
        ISNULL(EST_ACT,'') AS EST_ACT, 
        ISNULL(OFFICE_CODE,'') AS OFFICE_CODE, 
        ISNULL(ORDER_NBR,'') AS ORDER_NBR, 
        ISNULL(ORDER_DESC,'') AS ORDER_DESC, 
        ISNULL(MEDIA_TYPE,'') AS MEDIA_TYPE, 
        ISNULL(CL_CODE,'') AS CL_CODE, 
        ISNULL(DIV_CODE,'') AS DIV_CODE,
        ISNULL(PRD_CODE,'') AS PRD_CODE, 
        ISNULL(CMP_IDENTIFIER,-1) AS CMP_IDENTIFIER, 
        ISNULL(CMP_CODE,'') AS CMP_CODE, 
        ISNULL(CMP_NAME,'') AS CMP_NAME, 
        ISNULL(VN_CODE,'') AS VN_CODE, 
        ISNULL(VN_NAME,'') AS VN_NAME, 
        ISNULL(BUYER,'') AS BUYER,
        ISNULL(CLIENT_PO,'') AS CLIENT_PO, 
        ISNULL(MARKET_CODE,'') AS MARKET_CODE, 
        ISNULL(MARKET_DESC,'') AS MARKET_DESC, 
        ISNULL(ORDER_ACCEPTED,'') AS ORDER_ACCEPTED, 
        ISNULL(LINE_NBR,'') AS LINE_NBR,
        ISNULL(ACTIVE_REV,'') AS ACTIVE_REV, 
        ISNULL(FLIGHT_DATES,'') AS FLIGHT_DATES, 
        ISNULL(START_DATE,'') AS START_DATE, 
        ISNULL(END_DATE,'') AS END_DATE,
        ISNULL(MEDIA_MONTH,'') AS MEDIA_MONTH, 
        ISNULL(MEDIA_YEAR,'') AS MEDIA_YEAR, 
        ISNULL(CLOSE_DATE,'') AS CLOSE_DATE, 
        ISNULL(MATL_CLOSE_DATE,'') AS MATL_CLOSE_DATE,
        ISNULL(EXT_CLOSE_DATE,'') AS EXT_CLOSE_DATE, 
        ISNULL(EXT_MATL_DATE,'') AS EXT_MATL_DATE, 
        ISNULL(SIZE,'') AS SIZE, 
        ISNULL(PRINT_COLUMNS,0) AS PRINT_COLUMNS,
        ISNULL(PRINT_INCHES,0) AS PRINT_INCHES, 
        ISNULL(PRINT_LINES,0) AS PRINT_LINES, 
        ISNULL(AD_NUMBER,'') AS AD_NUMBER, 
        ISNULL(HEADLINE,'') AS HEADLINE,
        ISNULL(INTERNET_TYPE,'') AS INTERNET_TYPE, 
        ISNULL(OUTDOOR_TYPE,'') AS OUTDOOR_TYPE, 
        ISNULL(MATERIAL,'') AS MATERIAL, 
        ISNULL(EDITION_ISSUE,'') AS EDITION_ISSUE,
        ISNULL(SECTION,'') AS SECTION, 
        ISNULL(START_TIME,'') AS START_TIMR, 
        ISNULL(END_TIME,'') AS END_TIME, 
        ISNULL(LENGTH,'') AS LENGTH,
        ISNULL(TAG,'') AS TAG, 
        ISNULL(PROGRAMMING,'') AS PROGRAMMING, 
        ISNULL(MATL_NOTES,'') AS MATL_NOTES, 
        ISNULL(REMARKS,'') AS REMARKS,
        ISNULL(JOB_NUMBER,-1) AS JOB_NUMBER, 
        ISNULL(JOB_COMPONENT_NBR,-1) AS JOB_COMPONENT_NBR, 
        ISNULL(PRODUCTION_SIZE,'') AS PRODUCTION_SIZE,
        ISNULL(EST_NBR,0) AS EST_NBR, 
        ISNULL(EST_LINE_NBR,0) AS EST_LINE_NBR, 
        ISNULL(EST_REV_NBR,0) AS EST_REV_NBR, 
        ISNULL(LINE_TOTAL,0) AS LINE_TOTAL,
        ISNULL(COST_PER,0) AS COST_PER, 
        ISNULL(IMPRESSIONS,0) AS IMPRESSIONS, 
        ISNULL(LINE_CANCELLED,'') AS LINE_CANCELLED, 
        ISNULL(CL_NAME,'') AS CL_NAME, 
        ISNULL(DIV_NAME,'') AS DIV_NAME, 
        ISNULL(PRD_DESCRIPTION,'') AS PRD_DESCRIPTION, 
        ISNULL(REV_NBR,'') AS REV_NBR, 
        ISNULL(MAT_COMP,'') AS MAT_COMP, 
        ISNULL(SIZE_DESC,'') AS SIZE_DESC,
        ISNULL(DOCUMENT_COUNT,0) AS DOCUMENT_COUNT
from #media_rows
WHERE ISNULL(CL_CODE,'') LIKE '%' + @ClientCode AND ISNULL(DIV_CODE,'') LIKE '%' + @DivCode AND ISNULL(PRD_CODE,'') LIKE '%' + @ProdCode
	  ...
	  AND ISNULL(VN_CODE,'') LIKE '%' + @VendorID AND ISNULL(BUYER,'') LIKE '%' + @BuyerID AND ISNULL(OFFICE_CODE, '') LIKE '%' + @OfficeCode
ORDER BY TYPE,ORDER_NBR ASC, LINE_NBR
