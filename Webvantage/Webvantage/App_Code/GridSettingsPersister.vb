...
Public Class GridSettingsPersister
    ...
    'won't read to cookie, serialization problem??
    Public Sub SaveToCookie()
        Dim ValueString As String = Me.SaveSettings()
        HttpContext.Current.Response.Cookies(Me.Grid.ID & "Settings").Value = ValueString
        HttpContext.Current.Response.Cookies(Me.Grid.ID & "Settings").Expires = DateTime.Now.AddDays(30)

    End Sub
    Public Sub LoadFromCookie()
        Try
            If HttpContext.Current.Request.Cookies(Me.Grid.ID & "Settings") IsNot Nothing _
                AndAlso HttpContext.Current.Request.Cookies(Me.Grid.ID & "Settings").Value IsNot Nothing Then

                Me.LoadSettings(HttpContext.Current.Request.Cookies(Me.Grid.ID & "Settings").Value.ToString())
    ...
    Public Sub SaveToSession()
        Dim ValueString As String = Me.SaveSettings()
        HttpContext.Current.Session(Me.Grid.ID & "Settings") = ValueString
    End Sub
    Public Sub LoadFromSession()
        If Not HttpContext.Current.Session(Me.Grid.ID & "Settings") Is Nothing Then
            Me.LoadSettings(HttpContext.Current.Session(Me.Grid.ID & "Settings").ToString())
        End If
    End Sub

    Public Sub SaveToDatabase()
        Dim ValueString As String = Me.SaveSettings()
        Dim a As New cAppVars(cAppVars.Application.GRID_SETTINGS)
        a.setAppVarDB(Me.Grid.ID, ValueString)
    End Sub
    Public Sub LoadFromDatabase()
        Dim ValueString As String = ""
        Try
            Dim a As New cAppVars(cAppVars.Application.GRID_SETTINGS)
            ValueString = a.getAppVarDB(Me.Grid.ID, ValueString)
        Catch ex As Exception
            ValueString = ""
        End Try
        If ValueString <> "" Then
            Me.LoadSettings(ValueString)
        End If
    End Sub
