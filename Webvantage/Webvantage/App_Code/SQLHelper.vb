...
#Region " ExecuteDataTable "
    ...
    Public Overloads Shared Function ExecuteDataTable(ByVal connectionString As String, _
        ...
        Return ExecuteDataTable(connectionString, commandType, commandText, TableName, CType(Nothing, SqlParameter()))
    ...
    Public Overloads Shared Function ExecuteDataTable(ByVal connectionString As String, _
        ...
        If Not (parameterValues Is Nothing) AndAlso parameterValues.Length > 0 Then
            commandParameters = SqlHelperParameterCache.GetSpParameterSet(connectionString, spName)
            AssignParameterValues(commandParameters, parameterValues)
            Return ExecuteDataTable(connectionString, CommandType.StoredProcedure, spName, TableName, commandParameters)
        Else
            Return ExecuteDataTable(connectionString, CommandType.StoredProcedure, spName, TableName)
        End If
    ...
    Public Overloads Shared Function ExecuteDataTable(ByVal connection As SqlConnection, _
                                                     ...
                                                     ByVal TableName As String) As DataTable
        Try
            Return ExecuteDataTable(connection, commandType, commandText, TableName, CType(Nothing, SqlParameter()))
        Finally
            If Not connection Is Nothing Then
                connection.Close()
                connection.Dispose()
    ...
    Public Overloads Shared Function ExecuteDataTable(ByVal connection As SqlConnection, _
        ...
        If (connection Is Nothing) Then Throw New ArgumentNullException("connection")
        If (TableName Is Nothing) Then Throw New ArgumentNullException("TableName")
        If (spName Is Nothing OrElse spName.Length = 0) Then Throw New ArgumentNullException("spName")
        Try
            Dim commandParameters As SqlParameter()
            ...
            If Not (parameterValues Is Nothing) AndAlso parameterValues.Length > 0 Then
                ...
                ' Call the overload that takes an array of SqlParameters
                Return ExecuteDataTable(connection, CommandType.StoredProcedure, spName, TableName, commandParameters)
            Else ' Otherwise we can just call the SP without params
                Return ExecuteDataTable(connection, CommandType.StoredProcedure, spName, TableName)
            End If
        Finally
            If Not connection Is Nothing Then
                connection.Close()
    ...
    Public Overloads Shared Function ExecuteDataTable(ByVal transaction As SqlTransaction, _
        ...
        ' Pass through the call providing null for the set of SqlParameters
        Try
            Return ExecuteDataTable(transaction, commandType, commandText, TableName, CType(Nothing, SqlParameter()))
        Finally
            If Not transaction.Connection Is Nothing Then
                transaction.Connection.Close()
                transaction.Connection.Dispose()
    ...
    Public Overloads Shared Function ExecuteDataTable(ByVal transaction As SqlTransaction, _
        ...
        If (transaction Is Nothing) Then Throw New ArgumentNullException("transaction")
        If Not (transaction Is Nothing) AndAlso (transaction.Connection Is Nothing) Then Throw New ArgumentException("The transaction was rollbacked or commited, please provide an open transaction.", "transaction")
        If (TableName Is Nothing) Then Throw New ArgumentNullException("You must enter a name for the table")
        Try
            Dim cmd As New SqlCommand
            Dim ds As New DataSet
            Dim dataAdapter As SqlDataAdapter
    ...
    Public Overloads Shared Function ExecuteDataTable(ByVal transaction As SqlTransaction, _
        ...
        Try
            ...
            If Not (parameterValues Is Nothing) AndAlso parameterValues.Length > 0 Then
                ...
                ' Call the overload that takes an array of SqlParameters
                Return ExecuteDataTable(transaction, CommandType.StoredProcedure, spName, TableName, commandParameters)
            Else ' Otherwise we can just call the SP without params
                Return ExecuteDataTable(transaction, CommandType.StoredProcedure, spName, TableName)
            End If
        Finally
            If Not transaction.Connection Is Nothing Then
                transaction.Connection.Close()
...
#Region "ExecuteScalar"
    ...
    Public Overloads Shared Function ExecuteScalar(ByVal connectionString As String, _
        ...
        '''''Finally
        '''''    If Not connection Is Nothing Then
        '''''        '-- CTB:  2006/06/05:  Added the Close() method prior to calling the Dispose() method.
        '''''        connection.Close()
        '''''        connection.Dispose()
        '''''    End If
