...
#Region " Razor Views "
        Public Function Index(ByVal AAFilter As AdvantageFramework.AlertSystem.Classes.AlertsAndAssignmentsManagerFilter) As ActionResult
...
#Region " Get "
        ...
        Public Function GetUserViewSettings(ApplicationName As String) As JsonResult
            ...
            Using DbContext = New AdvantageFramework.Database.DbContext(Me.SecuritySession.ConnectionString, Me.SecuritySession.UserCode)
                ...
                ViewSettings = AdvantageFramework.AlertSystem.GetUserViewSettings(DbContext, Me.SecuritySession.UserCode, ApplicationName)
        ...
        <HttpGet>
        Public Function GetAAManagerData(ByVal AAFilter As AdvantageFramework.AlertSystem.Classes.AlertsAndAssignmentsManagerFilter) As JsonResult
            ...
            If String.IsNullOrWhiteSpace(Me.CurrentQueryString.UserCode) = False Then
                ...
                AAFilter.UserCode = Me.CurrentQueryString.UserCode

            Else

                AAFilter.UserCode = Me.SecuritySession.UserCode

            End If

            If AAFilter.GroupBy = Nothing Then
            ...
            Using DbContext = New AdvantageFramework.Database.DbContext(Me.SecuritySession.ConnectionString, Me.SecuritySession.UserCode)
                ...
                '12/21/20 - NS is this necessary now, test after you've added the JJ user settings
                'If AAFilter.InitialLoadFlag Then
                '    'Get the AAM data based on the users view settings for the initial load
                '    UserViewSettings(AAFilter)
                ...
                If MiscFN.IsClientPortal = True Then
                    ...
                    AAFilter.IsClientPortal = True
                    AAFilter.ClientContactID = Me.SecuritySession.ClientPortalUser.ClientContactID

                    AAManager.Items = AdvantageFramework.AlertSystem.LoadAlertsAndAssignmentsManagerCP(DbContext, AAFilter, ErrorMessage)
                ...
                Else
                    ...
                    AAManager.Items = AdvantageFramework.AlertSystem.LoadAlertsAndAssignmentsManager(DbContext, AAFilter, ErrorMessage)
                    Dim test As Date

                    For Each item In AAManager.Items
                        If item.DueDate IsNot Nothing Then
                            item.DueDate = item.DueDate.Value.AddHours(-Offset)
                        End If
                        If item.SprintStartDate IsNot Nothing Then
                            item.SprintStartDate = item.SprintStartDate.Value.AddHours(-Offset)
                        End If
                        If item.StartDate IsNot Nothing Then
                            item.StartDate = item.StartDate.Value.AddHours(-Offset)
                        End If
                        If item.TempCompleteDate IsNot Nothing Then
                            item.TempCompleteDate = item.TempCompleteDate.Value.AddHours(-Offset)
                        End If
                    Next
            ...
            SetAssignToLinkAddress(AAManager.Items)

            Return MaxJson(AAManager.Items, JsonRequestBehavior.AllowGet)
        ...
        Public Function GetGridColumnSettings(GridName As String) As JsonResult
            ...
            Using DbContext = New AdvantageFramework.Database.DbContext(Me.SecuritySession.ConnectionString, Me.SecuritySession.UserCode)
                ...
                UserColumn = AdvantageFramework.AlertSystem.GetAlertsAndAssignmentsManagerUserColumnSettings(DbContext, GridName, Me.SecuritySession.UserCode)
...
#Region " Post "
        Public Function SaveColumnSetting(ByVal GridName As String, ByVal Column As String, ShowHide As Boolean) As JsonResult
            ...
            Try
                Select Case Column
                    ...
                    Case "Subject"
                        colName = "Subject"
                    Case "GeneratedNoTime"
                        colName = "Generated"
                    Case "LastUpdatedNoTime"
                        colName = "LastUpdated"
                    Case "StartDate"
                        colName = "StartDate"
                    Case "DueDate"
                    ...
                    Case "JobNumber"
                        colName = "JobNumber"
                    Case "JobComponent"
                        colName = "JobComponent"
                    Case "ComponentNumber"
                        colName = "ComponentNumber"
                    Case "JobComponentDetailed"
                        colName = "JobComponentDetailed"
                    Case "JobAndComponentNumber"
                        colName = "JobAndComponentNumber"
                    Case "JobDescription"
                        colName = "JobDescription"
                    Case "ComponentDescription"
                        colName = "ComponentDescription"
                    Case "ID"
                        colName = "ID"
                    Case "SoftwareVersion"
                        colName = "SoftwareVersion"
                    Case "SoftwareBuild"
                        colName = "SoftwareBuild"
                    Case "Type"
                        colName = "Type"
                    Case "Level"
                        colName = "Level"
                    ...
                    Case "Template"
                        colName = "Template"
                    Case "TaskComments"
                        colName = "TaskComments"
                    Case "HoursAllowed"
                        colName = "HoursAllowed"
                    Case "UserName"
                        colName = "UserName"
                    Case "TempCompleteDateNoTime"
                        colName = "TempCompleteDate"
                    Case "CCEmployeeCodes"
                        colName = "CCEmployeeCodes"
                    Case "CCEmployeeNames"
                        colName = "CCEmployeeNames"
                    Case "Board"
                        colName = "Board"
                    Case "BoardState"
                        colName = "BoardState"
                    Case "SprintDescription"
                        colName = "SprintDescription"
                    Case "SprintStartDate"
                        colName = "SprintStartDate"
                    Case "TaskStatusDescription"
                        colName = "TaskStatusDescription"
                End Select
        ...
        End Function
        <HttpPost>
        Public Function ProcessGridColumnWidthChange(ByVal GridName As String, ByVal GridColumns As List(Of AdvantageFramework.AlertSystem.Classes.AlertsAndAssignmentsManagerGridColumnUpdates)) _
                                                 As JsonResult
            ...
            If GridColumns IsNot Nothing Then
                ...
                For Each Item As AdvantageFramework.AlertSystem.Classes.AlertsAndAssignmentsManagerGridColumnUpdates In GridColumns
                    ...
                    Try
                        ...
                        Using myconn As New SqlConnection(Me.SecuritySession.ConnectionString)
                            ...
                            With mycommand
                                ...
                                .CommandText = "usp_wv_AAMUpdateColumnWidthSettings"
                                .Connection = myconn
                            End With
        ...
        End Function
        <HttpPost>
        Public Function ProcessGridColumnReorder(ByVal GridName As String, ByVal GridColumns As List(Of AdvantageFramework.AlertSystem.Classes.AlertsAndAssignmentsManagerGridColumnUpdates)) _
                                                 As JsonResult
                ...
                For Each Item As AdvantageFramework.AlertSystem.Classes.AlertsAndAssignmentsManagerGridColumnUpdates In GridColumns
                    ...
                    Try
                        ...
                        Using myconn As New SqlConnection(Me.SecuritySession.ConnectionString)
                            ...
                            With mycommand
                                .CommandType = CommandType.StoredProcedure
                                .CommandText = "usp_wv_AAMUpdateColumnOrderSettings"
                                .Connection = myconn
                            End With
        ...
        End Function
        <HttpPost>
        Public Function BookmarkAlertInbox(SearchCriteria As String, InboxType As String, ShowAssignmentType As String, StartDate As String,
                                      EndDate As String, EmployeeCode As String, GridSize As Integer, GroupBy As String,
                                      ShowOnlyTempCompletedTasks As Boolean) As JsonResult
            ...
            QueryString.Page = "Desktop/Inbox"
            QueryString.Add("bm", "1")

            If SearchCriteria.Length > 0 Then

                QueryString.Add("st", SearchCriteria)

            End If

            QueryString.Add("aamit", InboxType)
            QueryString.Add("aamsat", ShowAssignmentType)
            ...
            Else
                ...
                StartDateLogic = CDate(StartDate).ToShortDateString
                QueryString.Add("sd", StartDateLogic)
            ...
            Else
                ...
                EndDateLogic = CDate(EndDate).ToShortDateString
                QueryString.Add("ed", EndDateLogic)

            End If

            'If EmployeeCode.Length > 0 Then

            '    QueryString.Add("emp", EmployeeCode)
            ...
            If GroupBy.Length > 0 Then
                ...
                QueryString.Add("aamgb", GroupBy)

            End If

            If ShowOnlyTempCompletedTasks Then
                QueryString.Add("aamstct", 1)
            Else
                QueryString.Add("aamstct", 0)
            End If


            Description = GetInboxDescription(InboxType) + " (" + GetAssignmentTypeDescription(ShowAssignmentType) + ")"

            Bookmark.Description = Description
            Bookmark.BookmarkType = AdvantageFramework.Web.Presentation.Bookmarks.BookmarkType.Desktop_AlertInbox
            Bookmark.UserCode = Me.SecuritySession.UserCode

            Bookmark.Name = String.Format("{0}", "Assignments and Alerts Manager")
        ...
        End Function
        <HttpPost>
        Public Function ReOpenAlert(AlertId As Integer) As JsonResult
            Dim _Controller As AdvantageFramework.Controller.Desktop.AlertController = Nothing
            Dim Alert As AdvantageFramework.DTO.Desktop.Alert = Nothing
            ...
            Try
                Reopened = _Controller.ReopenAlert(AlertId)
            Catch ex As Exception
                ErrorMessage = ex.Message
            End Try

            Return MaxJson(New With {.Success = Reopened, .Message = ErrorMessage})
            ...
            '        With mycommand
            '            .CommandType = CommandType.StoredProcedure
            '            .CommandText = "usp_wv_AAMReOpenAssignment"
            '            .Connection = myconn
            '        End With
            ...
            '        mycommand.Parameters.Add(AlertIDSqlParameter)
