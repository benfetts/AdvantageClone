...
Public Class DesktopDirectTime
    ...
    Private Property CurrentPageNumber() As Integer
        Get
            ...
            Try
                ...
                If ViewState("RadGridDirectTimeDOCurrentPageNumber") Is Nothing Then ViewState("RadGridDirectTimeDOCurrentPageNumber") = 0
                Return CInt(ViewState("RadGridDirectTimeDOCurrentPageNumber"))
        ...
        Set(ByVal value As Integer)
            ViewState("RadGridDirectTimeDOCurrentPageNumber") = value
        End Set
    End Property
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            If Me.IsFloating = False Then
                Me.butExport.Attributes.Add("onclick", "window.open('dtp_directtime.aspx?export=1', 'PopLookup','screenX=150,left=150,screenY=150,top=150,width=100,height=100,scrollbars=yes,resizable=no,menubar=no,maximazable=no');return false;")
            End If
            ...
            If Not Page.IsPostBack Then
                ...
                If Session("DirectTime_StartDate") IsNot Nothing Then
                    Me.RadDatePickerStart.SelectedDate = Session("DirectTime_StartDate")
                End If

                If Session("DirectTime_EndDate") IsNot Nothing Then
                    Me.RadDatePickerDue.SelectedDate = Session("DirectTime_EndDate")
                End If
                ...
                'HashtableGroup = Nothing
                Session("HashtableGroupDirectTIme") = Nothing
    ...
    'Private Sub ClearExpandedChildren(ByVal parentHierarchicalIndex)
    '    Dim indexes As String()
    '    indexes = New String(HashTableGroupStates.Keys.Count)
    ...
    Private Sub RadGridDirectTimeDO_NeedDataSource(ByVal source As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles RadGridDirectTimeDO.NeedDataSource
        ...
        Try
            ...
            Using DbContext = New AdvantageFramework.Database.DbContext(_Session.ConnectionString, _Session.UserCode)
                ...
                RadGridDirectTimeDO.DataSource = AdvantageFramework.Database.Procedures.DirectTime.LoadByEmployee(DbContext, Me.RadDatePickerStart.SelectedDate.Value, Me.RadDatePickerDue.SelectedDate.Value, Me.RadComboBoxEmployee.SelectedValue, Me.RadComboBoxClient.SelectedValue, job, comp, _Session.UserCode)
    ...
    Private Sub RadGridDirectTimeDO_PreRender(sender As Object, e As EventArgs) Handles RadGridDirectTimeDO.PreRender
        Try
    ...
    Private Sub LoadClientDropdownlist()
        Try
            ...
            taskVar = otask.getAppVars(Session("UserCode"), "DirectTime", "DTClient")
            If taskVar <> "" Then
    ...
    Private Sub RadComboBoxClient_SelectedIndexChanged(sender As Object, e As RadComboBoxSelectedIndexChangedEventArgs) Handles RadComboBoxClient.SelectedIndexChanged
        Dim otask As cTasks = New cTasks(Session("ConnString"))
        otask.setAppVars(Session("UserCode"), "DirectTime", "DTClient", "", Me.RadComboBoxClient.SelectedValue)
        LoadJobCompList()
        LoadRequests()
    End Sub
    ...
    Private Sub LoadJobCompList()
        Try
            ...
            taskVar = otask.getAppVars(Session("UserCode"), "DirectTime", "DTJob")
            If taskVar <> "" Then
    ...
    Private Sub RadComboBoxJob_SelectedIndexChanged(sender As Object, e As RadComboBoxSelectedIndexChangedEventArgs) Handles RadComboBoxJob.SelectedIndexChanged
        ...
        Session("DirectTime_Job") = Me.RadComboBoxJob.SelectedValue
        LoadRequests()
    End Sub
    ...
    Private Sub LoadEmployees()
        ...
        RadComboBoxEmployee.DataSource = EmployeeList.Select(Function(Employee) New With {.Code = Employee.Code,
                                                                                          .Name = Employee.ToString}).OrderBy(Function(Employee) Employee.Name)
        RadComboBoxEmployee.DataTextField = "Name"
        RadComboBoxEmployee.DataValueField = "Code"
        RadComboBoxEmployee.DataBind()
        RadComboBoxEmployee.Items.Insert(0, New Telerik.Web.UI.RadComboBoxItem("All Employees", ""))

        Dim otask As cTasks = New cTasks(Session("ConnString"))
        Dim taskVar As String

        taskVar = otask.getAppVars(Session("UserCode"), "MyDirectTime", "DTEmployee")
        If taskVar <> "" Then
            Me.RadComboBoxEmployee.SelectedValue = taskVar
        Else
            Me.RadComboBoxEmployee.SelectedValue = ""
        End If

    End Sub

    Private Sub RadComboBoxEmployee_SelectedIndexChanged(sender As Object, e As RadComboBoxSelectedIndexChangedEventArgs) Handles RadComboBoxEmployee.SelectedIndexChanged
        Dim otask As cTasks = New cTasks(Session("ConnString"))
        'otask.setAppVars(Session("UserCode"), "MyDirectTime", "DTEmployee", "", Me.RadComboBoxEmployee.SelectedValue)
        Session("DirectTime_Employee") = Me.RadComboBoxEmployee.SelectedValue
        LoadRequests()
    End Sub
    ...
    Private Sub RadDatePickerDue_SelectedDateChanged(ByVal sender As Object, ByVal e As Telerik.Web.UI.Calendar.SelectedDateChangedEventArgs) Handles RadDatePickerDue.SelectedDateChanged
        ...
        Else
            Session("DirectTime_EndDate") = LoGlo.FirstOfMonth()
            'Me.RadDatePickerDue.SelectedDate = LoGlo.LastOfMonth()
        End If
    ...
    Private Sub RadDatePickerStart_SelectedDateChanged(ByVal sender As Object, ByVal e As Telerik.Web.UI.Calendar.SelectedDateChangedEventArgs) Handles RadDatePickerStart.SelectedDateChanged
        ...
        If Me.RadDatePickerStart.SelectedDate IsNot Nothing Then
            ...
            End If
        Else
            Session("DirectTime_StartDate") = LoGlo.LastOfMonth
            'Me.RadDatePickerStart.SelectedDate = LoGlo.FirstOfMonth()
        End If
    ...
    Private Sub butPrint_Click(sender As Object, e As ImageClickEventArgs) Handles butPrint.Click
        Try
            ...
            Session("DirectTime_EmployeeCode") = Me.RadComboBoxEmployee.SelectedValue
    ...
    Private Sub ImageButtonBookmark_Click(sender As Object, e As ImageClickEventArgs) Handles ImageButtonBookmark.Click
        Try
            ...
            With b
                ...
                .BookmarkType = AdvantageFramework.Web.Presentation.Bookmarks.BookmarkType.Employee_DirectTime
                .UserCode = Session("UserCode")
                .Name = "Direct Time (All)"
                .Description = "Direct Time (All)"
