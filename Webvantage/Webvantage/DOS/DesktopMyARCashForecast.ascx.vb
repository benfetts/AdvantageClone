Partial Public Class DesktopMyARCashForecast
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        'Me.ARForcatsRadGrid.Skin = MiscFN.SetRadGridSkin()
        ...
        Dim printJS As String = "Javascript:window.open('dtp_arcashforecastproduct.aspx?my=1&client=" & StrClient & "&office=" & StrOffice & "&division=" & strDivision & "&product=" & strProduct & "&From=mar', 'PopLookup','screenX=150,left=150,screenY=150,top=150,width=785,height=400,scrollbars=yes,resizable=no,menubar=no,maximazable=no');return false;"
        Me.PrintImageButton.Attributes.Add("onclick", printJS)
        Me.butExport.Attributes.Add("onclick", "window.open('dtp_arcashforecastproduct.aspx?export=1&client=" & StrClient & "&office=" & StrOffice & "&division=" & strDivision & "&product=" & strProduct & "&From=mar', 'PopLookup','screenX=150,left=150,screenY=150,top=150,width=785,height=400,scrollbars=yes,resizable=no,menubar=no,maximazable=no');return false;")
    End Sub
    ...
    Private Sub LoadClientDropdownlist()
        ...
        With Me.ClientDropDownList
            .DataSource = oDD.GetClientListAE(Session("UserCode"), AdvantageFramework.EmployeeUtilities.MyObjectsDefinition.Objects.MyARCashForecast)
            .DataValueField = "Code"
    ...
    Private Sub LoadDivisionDropdownlist()
        Try
            ...
            With Me.DivisionDropDownList
                .DataSource = oDD.GetDivisionListAE(Session("UserCode"), cli, AdvantageFramework.EmployeeUtilities.MyObjectsDefinition.Objects.MyARCashForecast)
                .DataValueField = "Code"
                .DataTextField = "Description"
                .DataBind()
    ...
    Private Sub LoadProductDropdownlist()
        Try
            ...
            With Me.ProductDropDownList
                .DataSource = oDD.GetProductListAE(Session("UserCode"), cli, div, AdvantageFramework.EmployeeUtilities.MyObjectsDefinition.Objects.MyARCashForecast)
                .DataValueField = "Code"
                .DataTextField = "Description"
                .DataBind()
    ...
    'Private Sub ARForcatsRadGrid_NeedDataSource(ByVal source As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles ARForcatsRadGrid.NeedDataSource
    '    Dim oDO As cDesktopObjects = New cDesktopObjects(CStr(Session("ConnString")))
    '    Dim myForecasts As New Webvantage.ARFocasts
    '    Me.ARForcatsRadGrid.Columns(5).HeaderText = Date.Now.ToString("MMMM")
    '    Me.ARForcatsRadGrid.Columns(6).HeaderText = Date.Now.AddMonths(1).ToString("MMMM")
    '    Me.ARForcatsRadGrid.Columns(7).HeaderText = Date.Now.AddMonths(2).ToString("MMMM")
    '    Me.ARForcatsRadGrid.Columns(8).HeaderText = Date.Now.AddMonths(3).ToString("MMMM")
    '    myForecasts = oDO.GetARForecastProduct(CStr(Session("UserCode")), StrClient, StrOffice, strDivision, strProduct, "mar")
    '    Me.ARForcatsRadGrid.DataSource = myForecasts
    'End Sub
    ...
    Private Sub ImageButtonBookmark_Click(sender As Object, e As ImageClickEventArgs) Handles ImageButtonBookmark.Click
        Try
            ...
            With b
                ...
                .BookmarkType = AdvantageFramework.Web.Presentation.Bookmarks.BookmarkType.FinanceAccounting_MyARCashForecast
                .UserCode = Session("UserCode")
                .Name = "AR Cash Forecast (My)"
                .Description = "AR Cash Forecast (My)"
