...
Public Class DesktopMyJobRequests
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            Me.butPrint.Attributes.Add("onclick", "window.open('dtp_jobrequests.aspx?DesktopObject=My', 'PopLookup','screenX=150,left=150,screenY=150,top=150,width=1100,height=800,scrollbars=yes,resizable=no,menubar=no,maximazable=no');return false;")
            Me.butExport.Attributes.Add("onclick", "window.open('dtp_jobrequests.aspx?export=1&DesktopObject=My', 'PopLookup','screenX=150,left=150,screenY=150,top=150,width=100,height=100,scrollbars=yes,resizable=no,menubar=no,maximazable=no');return false;")
            If Not Page.IsPostBack Then
                ...
                If otask.getAppVars(Session("UserCode"), "MyJobRequests", "dDateStart") <> "" Then
                    Me.RadDatePickerStart.SelectedDate = otask.getAppVars(Session("UserCode"), "MyJobRequests", "dDateStart")
                Else
                    otask.setAppVars(Session("UserCode"), "MyJobRequests", "dDateStart", "", CStr(RadDatePickerStart.SelectedDate))
                End If
                If otask.getAppVars(Session("UserCode"), "MyJobRequests", "dDateDue") <> "" Then
                    Me.RadDatePickerDue.SelectedDate = otask.getAppVars(Session("UserCode"), "MyJobRequests", "dDateDue")
                Else
                    otask.setAppVars(Session("UserCode"), "MyJobRequests", "dDateDue", "", CStr(RadDatePickerDue.SelectedDate))
                End If

                taskVar = otask.getAppVars(Session("UserCode"), "MyJobRequests", "sSearch")
                If taskVar <> "" Then
                ...
                If otask.getAppVars(Session("UserCode"), "MyJobRequests", "CheckboxExclude") <> "" Then
                    Me.CheckboxExclude.Checked = CType(otask.getAppVars(Session("UserCode"), "MyJobRequests", "CheckboxExclude"), Boolean)
                End If
            ...
            Else
                ...
                Select Case Me.EventTarget
                    Case "RebindGrid", "UpdatePanelRadDock"
                        Me.RadGridJobRequestsDO.Rebind()
        ...
        Me.MyUnityContextMenu.SetRadGrid(Me.RadGridJobRequestsDO)

        'This has to be called on every load
        AdvantageFramework.Web.Presentation.Controls.LoadDataGridViewColumnUserSettings(RadGridJobRequestsDO)
        If Me.IsClientPortal = True Then
            AddHandler RadGridJobRequestsDO.HeaderContextMenu.ItemCreated, AddressOf HeaderContextMenuItemCreated
        ...
        Try
            ...
            Try
                If Not Me.RadGridJobRequestsDO.MasterTableView.GetItems(GridItemType.Header) Is Nothing Then
                    head = TryCast(Me.RadGridJobRequestsDO.MasterTableView.GetItems(GridItemType.Header)(0), GridHeaderItem)
                End If
            Catch ex As Exception
                head = Nothing
            End Try
    ...
    Private Sub butRefresh_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles butRefresh.Click
        ...
        otask.setAppVars(Session("UserCode"), "MyJobRequests", "sSearch", "", Me.txtSearch.Text)
        LoadRequests()
    End Sub
    ...
    Private Sub Search(ByVal Search As String)
        ...
        Dim otask As cTasks = New cTasks(Session("ConnString"))
        otask.setAppVars(Session("UserCode"), "MyJobRequests", "sSearch", "", Search)
        Me.RadGridJobRequestsDO.MasterTableView.CurrentPageIndex = 0

        LoadRequests()

    End Sub

    Private Sub TaskRadGrid_ItemCommand(ByVal source As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles RadGridJobRequestsDO.ItemCommand
        Try
            ...
            Select Case e.CommandName
                Case "Detail"
                    If e.Item IsNot Nothing AndAlso e.Item.ItemIndex > -1 Then
                        ...
                        CurrentGridDataItem = RadGridJobRequestsDO.Items(e.Item.ItemIndex)
        ...
        End Try
    End Sub
    Private Sub TaskRadGrid_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles RadGridJobRequestsDO.ItemDataBound
    ...
    End Sub
    Private Sub TaskRadGrid_NeedDataSource(ByVal source As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles RadGridJobRequestsDO.NeedDataSource
        Try
            Dim oDO As cDesktopObjects = New cDesktopObjects(CStr(Session("ConnString")))
            Dim jv As New JobVersion(Session("ConnString"))

            If IsClientPortal = True Then
                RadGridJobRequestsDO.DataSource = jv.GetJobRequestsDO(Session("CL_CODE"), "", "", Me.RadDatePickerStart.SelectedDate, Me.RadDatePickerDue.SelectedDate, Me.txtSearch.Text, Me.CheckboxExclude.Checked, "My", True, Session("UserID"))
            Else
                RadGridJobRequestsDO.DataSource = jv.GetJobRequestsDO("", "", "", Me.RadDatePickerStart.SelectedDate, Me.RadDatePickerDue.SelectedDate, Me.txtSearch.Text, Me.CheckboxExclude.Checked, "My", False)
            End If
        ...
        End Try
    End Sub
    Private Sub RadGridJobRequestsDO_PageIndexChanged(ByVal source As Object, ByVal e As Telerik.Web.UI.GridPageChangedEventArgs) Handles RadGridJobRequestsDO.PageIndexChanged
        LoadRequests()
    End Sub
    Private Sub RadGridJobRequestsDO_PageSizeChanged(sender As Object, e As GridPageSizeChangedEventArgs) Handles RadGridJobRequestsDO.PageSizeChanged
        MiscFN.SavePageSize(Me.RadGridJobRequestsDO.ID, e.NewPageSize)
    End Sub
    Private Sub RadGridJobRequestsDO_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles RadGridJobRequestsDO.PreRender

    End Sub
    Private Sub RadGridJobRequestsDO_SortCommand(ByVal source As Object, ByVal e As Telerik.Web.UI.GridSortCommandEventArgs) Handles RadGridJobRequestsDO.SortCommand
    ...
    Private Sub MyUnityContextMenu_RadContextMenuUnityItemClick(sender As Object, e As RadMenuEventArgs) Handles MyUnityContextMenu.RadContextMenuUnityItemClick
        ...
        Me.MyUnityContextMenu.JobNumber = RadGridJobRequestsDO.Items(Me.MyUnityContextMenu.GridIndex).GetDataKeyValue("JOB_NUMBER")
        Me.MyUnityContextMenu.JobComponentNumber = RadGridJobRequestsDO.Items(Me.MyUnityContextMenu.GridIndex).GetDataKeyValue("JOB_COMPONENT_NBR")
    ...
    Private Sub RadDatePickerDue_SelectedDateChanged(ByVal sender As Object, ByVal e As Telerik.Web.UI.Calendar.SelectedDateChangedEventArgs) Handles RadDatePickerDue.SelectedDateChanged
        ...
        otask.setAppVars(Session("UserCode"), "MyJobRequests", "dDateDue", "", CStr(dDateEnd))
    ...
    Private Sub RadDatePickerStart_SelectedDateChanged(ByVal sender As Object, ByVal e As Telerik.Web.UI.Calendar.SelectedDateChangedEventArgs) Handles RadDatePickerStart.SelectedDateChanged
        ...
        otask.setAppVars(Session("UserCode"), "MyJobRequests", "dDateStart", "", CStr(dDateStart))
    ...
    Private Sub CheckboxExclude_CheckedChanged(sender As Object, e As EventArgs) Handles CheckboxExclude.CheckedChanged
        ...
        otask.setAppVars(Session("UserCode"), "MyJobRequests", "CheckboxExclude", "Boolean", Me.CheckboxExclude.Checked)
    ...
    Private Sub ImageButtonBookmark_Click(sender As Object, e As ImageClickEventArgs) Handles ImageButtonBookmark.Click
        Try
            ...
            With b
                ...
                .BookmarkType = AdvantageFramework.Web.Presentation.Bookmarks.BookmarkType.ProjectManagement_MyJobRequests
                .UserCode = Session("UserCode")
                .Name = "Job Requests (My)"
                .Description = "Job Requests (My)"
