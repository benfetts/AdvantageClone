...
Partial Public Class DesktopOfficeStatistics
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            Me.butExport.Attributes.Add("onclick", "window.open('dtp_statistics.aspx?export=1&DO=office', 'PopLookup','screenX=150,left=150,screenY=150,top=150,width=100,height=100,scrollbars=yes,resizable=no,menubar=no,maximazable=no');return false;")
    ...
    Private Sub LoadGrid()
        ...
        Me.JobStatsRadGrid.DataSource = Me.CreateOfficeDT
    ...
    Private Sub butrefresh_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles butrefresh.Click
        Try
            Dim otask As cTasks = New cTasks(Session("ConnString"))
            otask.setAppVars(Session("UserCode"), "OfficeStatistics", "sDropTrafficFunctions", "", Me.DropTrafficFunctions.SelectedValue)
            otask.setAppVars(Session("UserCode"), "OfficeStatistics", "sChkClosedStatus", "", Me.ChkIsClosedStatus.Checked)
            otask.setAppVars(Session("UserCode"), "OfficeStatistics", "sEmployee", "", Me.ddEmployee.SelectedValue)
            otask.setAppVars(Session("UserCode"), "OfficeStatistics", "sManager", "", Me.ddManager.SelectedValue)
            otask.setAppVars(Session("UserCode"), "OfficeStatistics", "sDateStart", "", Me.RadDatePickerStartDate.SelectedDate)
            otask.setAppVars(Session("UserCode"), "OfficeStatistics", "sDateEnd", "", Me.RadDatePickerEndDate.SelectedDate)
            LoadGrid()
        Catch ex As Exception
            ...
            For k As Integer = 2 To ds.Columns.Count - 1
                ...
                If ds.Columns(k).ColumnName = "JOBS_CANCELLED" Then
                    If Me.DropTrafficFunctions.SelectedIndex > 0 And Me.ChkIsClosedStatus.Checked = False Then
                        row.Item("Type") = "Status<br />(" & Me.DropTrafficFunctions.SelectedValue.ToString() & ")"
