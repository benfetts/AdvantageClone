...
#Region " Page Functions "
    ...
    Private Sub DashboardProduction_Init(sender As Object, e As EventArgs) Handles Me.Init

        Dim Dashboard As Webvantage.cDashboard = Nothing

        Dashboard = New cDashboard(_Session.ConnectionString, _Session.UserCode)

        Dashboard.InitializeRadialGauge(RadRadialGaugeRequiredHoursVsTotalHoursGauge, "")
        Dashboard.InitializeRadialGauge(RadRadialGaugeDirectHoursGoalVsDirectHoursGauge, "")
        Dashboard.InitializeRadialGauge(RadRadialGaugeBillableHoursGoalVsBillableHoursGauge, "")
        Dashboard.InitializeRadialGauge(RadRadialGaugeDirectHoursVsBillableHoursGauge, "")

        Dashboard.InitializeTotalsColumnChart(RadHtmlChartRequiredHoursVsTotalHoursChart, "")
        Dashboard.InitializeTotalsColumnChart(RadHtmlChartDirectHoursGoalVsDirectHoursChart, "")
        Dashboard.InitializeTotalsColumnChart(RadHtmlChartBillableHoursGoalVsBillableHoursChart, "")
        Dashboard.InitializeTotalsColumnChart(RadHtmlChartDirectHoursVsBillableHoursChart, "")
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Me.IsPostBack Then
            ...
            If Me.OfficeCode = "" Then
                Me.Title = "Productivity Summary - Goal vs. Actual for All Offices\"
            Else
                Me.Title = "Productivity Summary - Goal vs. Actual for " & Request.QueryString("OName") & "\"
            End If
            If Me.DeptCode = "" Then
        ...
        Try
            ...
            Select Case e.Item.Value
                ...
                Case "Print"
                    ...
                    Me.OpenWindow("Employee Utilization Print", "DashboardPrint.aspx?From=dashprod&EmpCode=" & EmpCode & "&EmpName=" & empname & "&month=" & month & "&year=" & year & "&include=" & include & "&Office=" & Me.OfficeCode & "&Dept=" & Me.DeptCode & "&billed=" & period & "&OName=" & officename & "&DName=" & deptname & "&Data=" & dash & "&yValue=" & yearValue & "&monthCount=" & count & "&month2=" & month2)
                Case Else
                    Dim y As String
                    Dim m As String
                    ...
                    End If
                    If e.Item.Value.Length = 6 And e.Item.Text <> "Detail" Then
                        For Each rtb As RadToolBarButton In Me.RadToolbarDash.Items
                            If rtb.Value.Length = 6 And rtb.Text <> "Detail" Then
                                If rtb.Checked = True Then
                                    If count = 0 Then
...
#Region " Functions "
    ...
    Private Sub LoadData()
        ...
        Try
            ...
            'ds = dash.GetDirectHoursWithTotal(EmpCode, 1, month, year, Me.OfficeCode, Me.DeptCode)
            ...
            'ds = dash.GetDirectHoursWithBillable(EmpCode, 1, month, year, Me.OfficeCode, Me.DeptCode)
            ...
            'ds = dash.GetRequiredHoursWithTotal(EmpCode, 1, month, year, Session("UserCode"), Me.OfficeCode, Me.DeptCode, include, yearValue)
            Session("ds_DASH_RequiredvsTotalGauge") = ds
            ds = dash.GetGoalHoursWithBillable(EmpCode, 0, month, year, include, Me.OfficeCode, Me.DeptCode, yearValue, Session("UserCode"), month2, count)
            Session("ds_DASH_GoalvsBillable") = ds
            'ds = dash.GetGoalHoursWithBillable(EmpCode, 1, month, year, include, Me.OfficeCode, Me.DeptCode, yearValue)
    ...
    End Sub
    Private Sub CreateGauges()

        CreateGauge(RadRadialGaugeRequiredHoursVsTotalHoursGauge, Session("ds_DASH_RequiredvsTotalGauge"), "", "required")
        CreateGauge(RadRadialGaugeDirectHoursGoalVsDirectHoursGauge, Session("ds_DASH_DirectWithTotalGauge"), "", "direct")
        CreateGauge(RadRadialGaugeBillableHoursGoalVsBillableHoursGauge, Session("ds_DASH_DirectWithTotalGaugeGoal"), "", "goal")
        CreateGauge(RadRadialGaugeDirectHoursVsBillableHoursGauge, Session("ds_DASH_DirectvsBillableGauge"), "", "billable")

    End Sub
    Private Sub CreateGauge(ByVal RadRadialGauge As Telerik.Web.UI.RadRadialGauge, ByVal DataSet As System.Data.DataSet, ByVal Caption As String, ByVal Type As String)

        'objects
        Dim Dashboard As Webvantage.cDashboard = Nothing

        If RadRadialGauge IsNot Nothing Then

            Dashboard = New Webvantage.cDashboard(_Session.ConnectionString, _Session.UserCode)

            Dashboard.GetRadialGauge(RadRadialGauge, DataSet, Caption, Type)
    ...
    Private Sub CreateCharts()
        ...
        CreateChart(RadHtmlChartRequiredHoursVsTotalHoursChart, Session("ds_DASH_RequiredvsTotal"), "", "required")
        CreateChart(RadHtmlChartDirectHoursGoalVsDirectHoursChart, Session("ds_DASH_DirectWithTotal"), "", "direct")
        CreateChart(RadHtmlChartBillableHoursGoalVsBillableHoursChart, Session("ds_DASH_GoalvsBillable"), "", "Goal")
        CreateChart(RadHtmlChartDirectHoursVsBillableHoursChart, Session("ds_DASH_DirectvsBillable"), "", "bill")

    End Sub
    Private Sub CreateChart(ByVal RadHtmlChart As Telerik.Web.UI.RadHtmlChart, ByVal DataSet As System.Data.DataSet, ByVal Caption As String, ByVal Type As String)
        ...
        If RadHtmlChart IsNot Nothing Then
            ...
            Dashboard.GetTotalsColumnChart(RadHtmlChart, DataSet, Caption, Type)
