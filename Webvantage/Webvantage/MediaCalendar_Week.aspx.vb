'st: This page is an update of the old media calendar.
'st: I want it noted that the code was updated and not completely re-written!
...
Partial Public Class MediaCalendar_Week
    ...
    Private Sub LoadTasks()
        ...
        StartDate = ThisDate.AddDays(-ThisDate.DayOfWeek)

        Try
            If Not MonthDT Is Nothing Then
                If MonthDT.Rows.Count > 0 Then
                    Dim DvSunday As DataView = New DataView(MonthDT)
                    Dim DvMonday As DataView = New DataView(MonthDT)
                    Dim DvTuesday As DataView = New DataView(MonthDT)
                    Dim DvWednesday As DataView = New DataView(MonthDT)
                    Dim DvThursday As DataView = New DataView(MonthDT)
                    Dim DvFriday As DataView = New DataView(MonthDT)
                    Dim DvSaturday As DataView = New DataView(MonthDT)

                    DvSunday.RowFilter = "DISPLAY_START_DATE = '" & StartDate.ToString() & "'"
                    DvMonday.RowFilter = "DISPLAY_START_DATE = '" & StartDate.AddDays(1).ToString() & "'"
                    DvTuesday.RowFilter = "DISPLAY_START_DATE = '" & StartDate.AddDays(2).ToString() & "'"
                    DvWednesday.RowFilter = "DISPLAY_START_DATE = '" & StartDate.AddDays(3).ToString() & "'"
                    DvThursday.RowFilter = "DISPLAY_START_DATE = '" & StartDate.AddDays(4).ToString() & "'"
                    DvFriday.RowFilter = "DISPLAY_START_DATE = '" & StartDate.AddDays(5).ToString() & "'"
                    DvSaturday.RowFilter = "DISPLAY_START_DATE = '" & StartDate.AddDays(6).ToString() & "'"
                    ...
                    End With
                    With Me.repTuesday
                        .DataSource = DvTuesday
                        .DataBind()
                    End With
                    With Me.repWednesday
                        .DataSource = DvWednesday
                        .DataBind()
                    End With
                    With Me.repThursday
                        .DataSource = DvThursday
                        .DataBind()
                    End With
                    With Me.repFriday
                        .DataSource = DvFriday
        ...
        Catch ex As Exception
        End Try
        Me.lblTitle.Text = StartDate.ToLongDateString() & " - " & StartDate.AddDays(6).ToLongDateString()
