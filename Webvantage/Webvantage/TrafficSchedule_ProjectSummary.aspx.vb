...
Public Class TrafficSchedule_ProjectSummary
    Inherits Webvantage.BaseChildPage

    Public job, comp As String

    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load

        job = Request.QueryString("JobNo")
        comp = Request.QueryString("JobComp")

        'Clean up old querystring names by letting clean qs class override
        Dim qs As New AdvantageFramework.Web.QueryString()
        qs = qs.FromCurrent()

        If qs.JobNumber > 0 Then Me.job = qs.JobNumber
        If qs.JobComponentNumber > 0 Then Me.comp = qs.JobComponentNumber

        Me.MyUnityContextMenu.JobNumber = Me.job
        Me.MyUnityContextMenu.JobComponentNumber = Me.comp
        Me.MyUnityContextMenu.HideItems = New UnityUC.UnityItem() {UnityUC.UnityItem.Snapshot}

        If Not Me.IsPostBack And Not Me.Page.IsCallback Then

            Me.jobNbr.Text = job.PadLeft(6, "0")
            Me.compNbr.Text = comp.PadLeft(3, "0")

            Me.jobDesc.Text = Me.getJobDesc(job)
            Me.compDesc.Text = Me.getCompDesc(job, comp)

            Me.PageTitle = "Snapshot" '"Snapshot for " & job.PadLeft(6, "0") & "/" & comp.PadLeft(2, "0") & " - " & Me.getCompDesc(job, comp)
    ...
    Private Function getEstTooltipInfo() As String
        ...
        If dr.HasRows Then
            ...
            tooltip = "Estimate: " & CStr(estNbr.ToString.PadLeft(6, "0")) & "  Component: " & CStr(estComp.ToString.PadLeft(3, "0")) & "<br/>"
    ...
    Private Sub btnBack_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnBack.Click
        ...
        End With
        Try
            If Not Page.IsStartupScriptRegistered("PVSummaryClose") Then
                Dim str As String = sbScript.ToString
                Page.RegisterStartupScript("PVSummaryClose", str)
            End If
    ...
    Private Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        With sbScript
            .Append("<script type=""text/javascript"">")
            .Append("window.print();</script>")
        End With
        Try
            If Not Page.IsStartupScriptRegistered("PVSummaryPrint") Then
                Dim str As String = sbScript.ToString
                Page.RegisterStartupScript("PVSummaryPrint", str)
            End If
        Catch ex As Exception
            Me.ShowMessage("Error! " & ex.Message.ToString())
        End Try
