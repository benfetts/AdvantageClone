...
Partial Public Class VendorQuote_Vendors
    ...
    Private Sub RadGridVendor_NeedDataSource(ByVal source As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles RadGridVendor.NeedDataSource
        Try
            Dim est As New cEstimating(Session("ConnString"), Session("UserCode"))
            ...
            Dim curr As String = ""
            If Me.cbDefault.Checked = True Then
                ds = est.GetRequestQuotes(Me.EstimateNum, Me.EstimateComp, Me.VendorQuoteNbr, 0, Session("UserCode"))
                If ds.Tables(1).Rows.Count > 0 Then
                    For i As Integer = 0 To ds.Tables(1).Rows.Count - 1
                        If curr <> ds.Tables(1).Rows(i)("EST_FNC_CODE").ToString() Then
                            functions &= ds.Tables(1).Rows(i)("EST_FNC_CODE").ToString() & ","
                            curr = ds.Tables(1).Rows(i)("EST_FNC_CODE").ToString
                        Else

                        End If
                    Next
                End If
                functions = MiscFN.RemoveTrailingDelimiter(functions, ",")
                Me.RadGridVendor.DataSource = od.GetVendorsByFunctions(functions)
            Else
                Me.RadGridVendor.DataSource = od.GetVendorsAll
            End If
        Catch ex As Exception
    ...
    Private Sub BtnCopy_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnCopy.Click
        ...
        Dim dsFunc As DataSet
        Dim vendorcode As String
        Dim vendorcont As String
        ...
        For Each gridDataItem As Telerik.Web.UI.GridDataItem In Me.RadGridVendor.MasterTableView.Items
            ...
            If chk.Checked = True Then
                'Set variables:
                vendorcode = gridDataItem("colVN_CODE").Text.Replace("&nbsp;", "")
                po.Get_PO_Vendor_Primary_Contact(vendorcode, vendorcont, vendorname, vendoremail)
                'Save:
                Try
                    Dim oEstimating As cEstimating = New cEstimating(Session("ConnString"), CStr(Session("UserCode")))
                    oEstimating.AddNewRequestVendor(EstimateNum, EstimateComp, VendorQuoteNbr, vendorcode, vendorcont, "", "", "", "", "")

                    ds = oEstimating.GetRequestQuotes(EstimateNum, EstimateComp, VendorQuoteNbr, 0, Session("UserCode"))
                    If ds.Tables(0).Rows.Count > 0 Then
                        For x As Integer = 0 To ds.Tables(0).Rows.Count - 1
                            If IsDBNull(ds.Tables(0).Rows(x)("EST_QUOTE_NBR")) = False Then
                                QuoteNum = ds.Tables(0).Rows(x)("EST_QUOTE_NBR")
                            End If
                            If IsDBNull(ds.Tables(0).Rows(x)("EST_REV_NBR")) = False Then
                                RevNum = ds.Tables(0).Rows(x)("EST_REV_NBR")
                            End If
                            dsFunc = oEstimating.GetRequestQuotes(EstimateNum, EstimateComp, VendorQuoteNbr, QuoteNum, Session("UserCode"))
                            If dsFunc.Tables(1).Rows.Count > 0 Then
                                For i As Integer = 0 To dsFunc.Tables(1).Rows.Count - 1
                                    If IsDBNull(dsFunc.Tables(1).Rows(i)("EST_FNC_CODE")) = False Then
                                        functioncode = dsFunc.Tables(1).Rows(i)("EST_FNC_CODE")
                                    End If
    ...
    Private Sub CloseAndRefresh()
        Me.CloseThisWindowAndRefreshCaller("VendorQuote.aspx?tab=0&PageMode=edit&EstNum=" & EstimateNum & "&EstComp=" & EstimateComp & "&vendorreq=" & VendorQuoteNbr, True)
    End Sub
