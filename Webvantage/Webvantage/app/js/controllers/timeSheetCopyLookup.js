app.controller('timeSheetCopyLookupController', function ($scope, $rootScope, $http, $modal, $timeout, dataService) {
    ...
    $scope.getFunctionCategoryCode = function () {
        return $scope.getInputValue('FunctionCategory');
    };
    $scope.getFunctionCategoryDescription = function () {
        return $scope.getDescriptionInputValue('FunctionCategory');
    };
    ...
    $scope.getProductCategoryCode = function () {
        return $scope.getInputValue('ProductCategory');
    };
    // common
    ...
    $scope.setFunctionCategoryCode = function (value) {
        $scope.setInputValue('FunctionCategory', value);
    };
    $scope.setFunctionCategoryDescription = function (value) {
        $scope.setDescriptionInputValue('FunctionCategory', value);
    };
    ...
    $scope.setProductCategoryCode = function (value) {
        $scope.setInputValue('ProductCategory', value);
    };
    // common
    ...
    $scope.getSearchCriteria = function (lookupType) {
        ...
        $scope.searchCriteria = {
            ...
            FunctionCategory: {
                FunctionCategoryCode: $scope.getFunctionCode(),
                FunctionCategoryDescription: $scope.getFunctionDescription(),
                JobCode: $scope.getJobNumber(),
            ...
            ProductCategory: {
                ProductCategoryCode: $scope.getProductCategoryCode()
            },
    ...
    $scope.processLookupSelection = function (selectedItem) {
        ...
        if (selectedItem.ObjectName === 'ProductCategory') {
            $scope.productCategoryValuesChanged(selectedItem, true);
        }
        if (selectedItem.ObjectName === 'FunctionCategory') {
            $scope.functionCatgoryValuesChanged(selectedItem, true);
        }
    };
    ...
    $scope.open = function (lookupType) {
        ...
        if (lookupType === 'FunctionCategory') {
            currentCriteria.FunctionCategory.FunctionCategoryCode = '';
        }
        ...
        if (lookupType === 'ProductCategory') {
            currentCriteria.JobComponent.ProductCategoryCode = '';
        }
            ...
            if (!$scope.getInputValue(level)) {
                ...
                if (input) {
                    if ($($scope.parentRow).is('tr') === true) {
                        ...
                        nextInput = $('input[type=text]:enabled').filter(function () {
    ...
    $scope.functionCatgoryValuesChanged = function (newFunction, newVal) {
        var currentCriteria = $scope.getSearchCriteria();
        currentCriteria.LookupType = 'FunctionCategory';
        currentCriteria.FunctionCategory = newFunction;
        var selectedFunction;
        dataService.searchLookup(currentCriteria).then(function (result) {            
            selectedFunction = newFunction;            
            $scope.fillFunctionCategoryValues(selectedFunction);
            if (newVal) {
                $scope.focusNextControl('FunctionCategory');
            }
        });
    };
    ...
    $scope.productCategoryValuesChanged = function (productCategory, fetch) {
        $scope.fillProductCategoryValues(productCategory);
        if (fetch) {
            $scope.searchCriteria.LookupType = 'fillProductCategoryValues';
            $scope.searchCriteria.ProductCategory = productCategory;
            dataService.searchLookup($scope.searchCriteria).then(function (result) {
                if (result.data.Results.length == 1) {
                    $scope.fillProductCategoryValues(result.data.Results[0]);
                } else {
                    $scope.fillProductCategoryValues(productCategory);
                }
            });
    ...
    $scope.fillProductCategoryValues = function (productCategory) {
        var code, name;
        if (productCategory) {
            code = productCategory.ProductCategoryCode;
        }
        if (!code) {
            name = null;
        }
        $scope.setProductCategoryCode(code);
    };
    ...
    $scope.fillJobComponentValues = function (JobComponent) {
        var clCode, clName, divCode, divName, prdCode, prdName, jobNum, jobDesc, compNum, compDesc, prodcat;
    ...
    $scope.fillFunctionCategoryValues = function (newFunction) {
        var code, desc, cpm, display;
        if (newFunction) {
            code = newFunction.FunctionCode;
            desc = newFunction.FunctionDescription;
        ...
        $scope.setFunctionCategoryCode(code);
        $scope.setFunctionCategoryDescription(desc);
    };
