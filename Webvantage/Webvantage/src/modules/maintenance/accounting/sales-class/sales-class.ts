...
import { SalesClassService } from 'services/maintenance/accounting/sales-class-service';
import { inject, bindable } from 'aurelia-framework';
import { SalesClassModel } from 'models/maintenance/sales-class-model';

@inject(SalesClassService)
export class SalesClass extends ModuleBase {

    // properties
    service: SalesClassService;
    model: any;
    salesclasses: Array<SalesClassModel>;
    ...
    export() {
        ...
        this.grid.options.excel.fileName = "SalesClasses.xlsx";
    ...
    delete(uid) {
        ...
        this.confirm('Are you sure you want to delete?').then(() => {
            ...
            if (item.isNew() == false) {
                ...
                //delete the item
                this.service.deleteSalesClass(item).then(response => {
                    ...
                    if (success) {
                        ...
                        this.showNotification("This sales class is in use and cannot be deleted!", "error");
                    }
                });
            }
            else {
    ...
    cancelRowChanges(uid) {
        ...
        if (item.isNew() === false) {
            ...
            this.service.getSalesClass(dataItem.Code).then(response => {
    ...
    //    //get all of the function codes
    //    this.service.getSalesClassTypes().then(response => {
    //        this.salesclasstypes = new kendo.data.DataSource({
    ...
    //    //get all of the status codes
    //    this.service.getStatusCodes().then(response => {
    //        this.statusCodes = new kendo.data.DataSource({
    //            data: response.content
    //        });

    //        //console.log('getStatusCodes:response', response);
    //    });
    //}
    ...
    gridOnEdit(e) {
        ...
        e.container.find("input[name=Code]").attr("maxlength", 6);
    ...
    salesclassTypesEditor(container, options) {
        ...
        input.kendoDropDownList({
            ...
            dataSource: this.salesclasstypes
        });
    ...
    attached() {
        ...
        this.service.initSalesClassMaintenance().then(response => {

            this.salesclasstypes = response.content.SalesClassTypes;
    ...
    constructor(Service: SalesClassService) {
        super();
        this.service = Service;
        this.salesclasses = new Array<SalesClassModel>();
        ...
        this.datasource = new kendo.data.DataSource({
            transport: {
                read: {
                    url: 'Maintenance/Accounting/SalesClass/GetSalesClasses'
                },
                update: {
                    url: 'Maintenance/Accounting/SalesClass/UpdateSalesClasses',
                    type: 'POST'
                },
                create: {
                    url: 'Maintenance/Accounting/SalesClass/CreateSalesClasses',
                    type: 'POST'
                },
                destroy: {
                    url: 'Maintenance/Accounting/SalesClass/DeleteSalesClasses',
                    type: 'POST'
                },
                parameterMap: function (data, type) {
                    if (type !== 'read') {
                        return {
                            SalesClasses: data.models
                        }
                    }
                }
            },
            ...
            requestEnd: function (e) {
                ...
                switch (e.type) {
                    ...
                    case "destroy":
                        {
                            if (e.response == false) {
                                me.showNotification("This sales class is in use and cannot be deleted!","error");
                            }
                            else if (e.response == true) {
                        ...
                        Code: {
                            ...
                            validation: {
                                ...
                                CodeValidation: function (input) {
                                    ...
                                    $.ajax({
                                        url: "Maintenance/Accounting/SalesClass/SalesClassExists",
                                        async: false,
                                        data: {
                                            Code: input.val()
                                        },
