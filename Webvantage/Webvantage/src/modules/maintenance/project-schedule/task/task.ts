...
import { TaskService } from 'services/maintenance/project-schedule/task-service';
import { inject, bindable } from 'aurelia-framework';
import { TaskModel } from 'models/maintenance/task-model';

@inject(TaskService)
export class Task extends ModuleBase {
    ...
    export() {
        ...
        this.grid.options.excel.fileName = "Tasks.xlsx";
    ...
    delete(uid) {
        ...
        this.confirm('Are you sure you want to delete?').then(() => {
            ...
            if (item.isNew() == false) {
                ...
                //delete the item
                this.service.deleteTask(item).then(response => {
                    ...
                    if (success) {
                        ...
                        this.showNotification("This task is in use and cannot be deleted!", "error");
                    }
                });
            }
            else {
    ...
    cancelRowChanges(uid) {
        ...
        if (item.isNew() === false) {
            ...
            this.service.getTask(dataItem.Code).then(response => {
    ...
    //    this.service.getFunctionCodess().then(response => {
    //        this.functionCodes = new kendo.data.DataSource({
    ...
    processOrderNumberEditor(container, options) {
    ...
    daysToCompleteEditor(container, options) {
    ...
    hoursAllowedEditor(container, options) {
    ...
    statusCodeEditor(container, options) {
    ...
    activate(model: any) {
        ...
        for (var i = 0; i < model.length; i++) {
            var maintTaskModel = new TaskModel;
            Object.assign(maintTaskModel, model[i]);
            this.tasks.push(maintTaskModel);
        }
    ...
    attached() {
        ...
        this.service.initTaskMaintenance().then(response => {
    ...
    constructor(Service: TaskService) {
        ...
        this.datasource = new kendo.data.DataSource({
            transport: {
                read: {
                    url: 'Maintenance/ProjectSchedule/Task/GetTasks'
                },
                update: {
                    url: 'Maintenance/ProjectSchedule/Task/UpdateTasks',
                    type: 'POST'
                },
                create: {
                    url: 'Maintenance/ProjectSchedule/Task/CreateTasks',
                    type: 'POST'
                },
                destroy: {
                    url: 'Maintenance/ProjectSchedule/Task/DeleteTasks',
                    type: 'POST'
                },
            ...
            requestEnd: function (e) {
                ...
                switch (e.type) {
                    ...
                    case "destroy":
                        {
                            if (e.response == false) {
                                me.showNotification("This task is in use and cannot be deleted!","error");
                            }
                            else if (e.response == true) {
                        ...
                        Code: {
                            ...
                            validation: {
                                ...
                                CodeValidation: function (input) {
                                    ...
                                    $.ajax({
                                        url: "Maintenance/ProjectSchedule/Task/TaskExists",
                                        async: false,
                                        data: {
                                            Code: input.val()
                                        },
